...
class Orders {
    ...
    public function __construct() {
        ...
        if ( $this->enabled_buyers || $this->enable_super_admin_sms || $this->enable_product_admin_sms ) {
            ...
            if ( is_admin() ) {
                ...
                add_action( 'wp_ajax_nopriv_change_sms_text', array( $this, 'change_sms_text_callback' ) );
            }
        }
    }
    ...
    public function mobile_label( $fields ) {
        ...
        if ( ! empty( $fields[ $mobile_meta ]['label'] ) ) {
            $fields[ $mobile_meta ]['label'] = PWSMS()->get_option( 'buyer_phone_label',
                $fields[ $mobile_meta ]['label'] );
        }

        if ( ! empty( $fields['billing'][ $mobile_meta ]['label'] ) ) {
            $fields['billing'][ $mobile_meta ]['label'] = PWSMS()->get_option( 'buyer_phone_label',
                $fields['billing'][ $mobile_meta ]['label'] );
        }
    ...
    public function checkout_script() {
        ...
        if ( PWSMS()->get_option( 'allow_buyer_select_status' ) && is_checkout() ) {
            ...
            wp_register_script( 'pwoosms-multiselect', PWSMS_URL . '/assets/js/multi-select.js', [ 'jquery' ],
                PWSMS_VERSION, true );
    ...
    public function checkout_fields( $checkout ) {
        ...
        if ( PWSMS()->get_option( 'allow_buyer_select_status' ) ) {
            ...
            $mode                    = PWSMS()->get_option( 'buyer_status_mode',
                'selector' ) == 'selector' ? 'pwoosms_multiselect' : 'pwoosms_multicheckbox';
    ...
    public function save_sms_order_meta( $order_id ) {
        ...
        $order->update_meta_data( '_allow_buyer_select_status',
            PWSMS()->get_option( 'allow_buyer_select_status', '__' ) );
        ...
        if ( ! empty( $_POST['buyer_sms_status'] ) ) {
            $statuses = is_array( $_POST['buyer_sms_status'] ) ? array_map( 'sanitize_text_field',
                $_POST['buyer_sms_status'] ) : sanitize_text_field( $_POST['buyer_sms_status'] );
    ...
    public function send_order_sms( int $order_id, $old_status = '', $new_status = 'created' ) {
        ...
        if ( ( $order_page && ! empty( $_POST['sms_order_send'] ) ) || ( ! $order_page && $this->buyer_can_get_sms( $order_id,
                    $new_status ) ) ) {
            ...
            if ( ( $result = PWSMS()->send_sms( $data ) ) === true ) {
                ...
                $order->add_order_note( sprintf( 'پیامک بخاطر خطا به مشتری با شماره %s ارسال نشد.<br>پاسخ وبسرویس: %s',
                    $mobile, $result ) );
            }
        }
        ...
        //superAdmin
        if ( $this->enable_super_admin_sms && in_array( $new_status,
                (array) PWSMS()->get_option( 'super_admin_order_status' ) ) ) {
            ...
            if ( ( $result = PWSMS()->send_sms( $data ) ) === true ) {
                ...
                $order->add_order_note( sprintf( 'پیامک بخاطر خطا به مدیر کل با شماره %s ارسال نشد.<br>پاسخ وبسرویس: %s',
                    $mobile, $result ) );
            }
        }
        ...
        if ( $this->enable_product_admin_sms ) {
            ...
            foreach ( (array) $mobiles as $mobile => $product_ids ) {
                ...
                if ( ( $result = PWSMS()->send_sms( $data ) ) === true ) {
                    $order->add_order_note( sprintf( 'پیامک با موفقیت به مدیر محصول با شماره %s ارسال گردید.',
                        $mobile ) );
                } else {
                    $order->add_order_note( sprintf( 'پیامک بخاطر خطا به مدیر محصول با شماره %s ارسال نشد.<br>پاسخ وبسرویس: %s',
                        $mobile, $result ) );
                }
            }
        }
    ...
    public function buyer_can_get_sms( int $order_id, string $new_status ): bool {
        ...
        if ( in_array( $new_status,
                $allowed_status ) && PWSMS()->maybe_bool( $order->get_meta( '_buyer_sms_notify' ) ) ) {
    ...
    public function change_sms_text_callback() {
        ...
        if ( isset( $_POST['order_status'] ) ) {
            $_order_status = is_array( $_POST['order_status'] ) ? array_map( 'sanitize_text_field',
                $_POST['order_status'] ) : sanitize_text_field( $_POST['order_status'] );
        ...
        echo '<input type="checkbox" id="sms_order_send" class="sms_order_send" name="sms_order_send" value="true" style="margin-top:2px;width:20px; float:right" ' . wp_kses( $sms_checked,
                $this->SafePWOOCSS ) . '/>
