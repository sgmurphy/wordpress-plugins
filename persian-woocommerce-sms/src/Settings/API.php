...
class API {
        ...
        foreach ( $this->settings_sections as $section ) {
            ...
            if ( isset( $section['desc'] ) && ! empty( $section['desc'] ) ) {
                ...
                $callback        = create_function( '',
                    'echo "' . str_replace( '"', '\"', esc_attr( $section['desc'] ) ) . '";' );
            } else {
    ...
    public function callback_text( $args ) {
        ...
        $html = sprintf( '<input type="text" class="%1$s-text" id="%2$s[%3$s]" name="%2$s[%3$s]" value="%4$s" style="%5$s"/>',
            $size, $args['section'], $args['id'], $value, $style );
    ...
    public function callback_checkbox( $args ) {
        ...
        $value             = esc_attr( $this->get_option( $args['id'], $args['section'], $args['std'] ) );
        $html              = sprintf( '<input type="hidden" name="%1$s[%2$s]" value="off" />', $args['section'],
            $args['id'] );
        $html              .= sprintf( '<input type="checkbox" class="checkbox" id="wpuf-%1$s[%2$s]" name="%1$s[%2$s]" value="on" %4$s />',
            $args['section'], $args['id'], $value, checked( $value, 'on', false ) );
        $html              .= sprintf( '<label for="wpuf-%1$s[%2$s]"> %3$s</label>', $args['section'], $args['id'],
            $args['desc'] );
    ...
    public function callback_multicheck( $args ) {
        ...
        foreach ( $args['options'] as $key => $label ) {
            ...
            $html    .= sprintf( '<label for="wpuf-%1$s[%2$s][%4$s]"> %3$s</label><br>', $args['section'], $args['id'],
                $label, $key );
        }
        $html .= sprintf( '<div class="description" ' . $this->desc_style . '> %s</div>', $args['desc'] );
    ...
    public function callback_radio( $args ) {
        ...
        foreach ( $args['options'] as $key => $label ) {
            $html .= sprintf( '<label style="' . $style . '" for="wpuf-%s[%s][%s]">', $args['section'], $args['id'],
                $key );
            $html .= sprintf( '<input type="radio" class="radio inline" id="wpuf-%1$s[%2$s][%3$s]" name="%1$s[%2$s]" value="%3$s"%4$s />',
                    $args['section'], $args['id'], $key, checked( $value, $key, false ) ) . $label;
            $html .= '</label>';
    ...
    public function callback_select( $args ) {
        ...
        $html = sprintf( '<div style="max-width:350px"><select class="%1$s" name="%2$s[%3$s]" id="%2$s[%3$s]" %4$s>',
            $size, $args['section'], $args['id'], $ltr );
        foreach ( $args['options'] as $key => $label ) {
    ...
    public function callback_textarea( $args ) {
        ...
        $style = 'width: 50%';
        $html  = sprintf( '<textarea rows="%6$s" style="%5$s" class="%1$s-text" id="%2$s[%3$s]" name="%2$s[%3$s]">%4$s</textarea>',
            $size, $args['section'], $args['id'], $value, $style, $row );
    ...
    public function callback_html( $args ) {
        ...
        echo sprintf( '<div class="description" ' . esc_attr( $this->desc_style ) . '> %s</div>',
    ...
    public function callback_file( $args ) {
        ...
        $html = sprintf( '<input type="text" class="%1$s-text pwoosms-url" id="%2$s[%3$s]" name="%2$s[%3$s]" value="%4$s"/>',
    ...
    public function callback_color( $args ) {
        ...
        $html       .= sprintf( '<div class="description" ' . $this->desc_style . ' style="display:block;"> %s</div>',
            $args['desc'] );
    ...
    public function show_navigation() {
        ...
        foreach ( $this->settings_sections as $form ) {
            ...
            $html .= sprintf( '<a href="%1$s" class="nav-tab %3$s" id="%1$s-tab">%2$s</a>', esc_url( $url ),
                $form['title'], $class );
        }
        $html .= '</nav>';
