	...
	class BWF_JSON_Cache {
		...
		public static function maybe_clear_fb_endpoints( $endpoints, $plugin_slug = '' ) {
			...
			if ( class_exists( 'BWF_Admin_General_Settings' ) ) {
				$bwf_obj = BWF_Admin_General_Settings::get_instance();
				if ( ! empty( $bwf_obj->get_option( 'checkout_page_base' ) ) ) {
					$endpoints[] = '/' . $bwf_obj->get_option( 'checkout_page_base' ) . '/';
				}
				if ( ! empty( $bwf_obj->get_option( 'wfocu_page_base' ) ) ) {
					$endpoints[] = '/' . $bwf_obj->get_option( 'wfocu_page_base' ) . '/';
				}
			}
