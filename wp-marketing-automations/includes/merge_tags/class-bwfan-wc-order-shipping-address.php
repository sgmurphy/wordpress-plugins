...
class BWFAN_WC_Order_Shipping_Address extends BWFAN_Merge_Tag {

	private static $instance = null;

	public function __construct() {
		$this->tag_name        = 'order_shipping_address';
		$this->tag_description = __( 'Order Shipping Address', 'wp-marketing-automations' );
		add_shortcode( 'bwfan_order_shipping_address', array( $this, 'parse_shortcode' ) );
	}
	...
	public function parse_shortcode( $attr ) {
		...
		if ( isset( $attr['format'] ) && 'address_1' === $attr['format'] ) {
			...
			return $this->parse_shortcode_output( $shipping_address_1, $attr );
		}
		if ( isset( $attr['format'] ) && 'address_2' === $attr['format'] ) {
			$shipping_address_2 = $order->get_shipping_address_2();

			return $this->parse_shortcode_output( $shipping_address_2, $attr );
		}
