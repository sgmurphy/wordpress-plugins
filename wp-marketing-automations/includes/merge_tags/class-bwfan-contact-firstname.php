...
if ( ! bwfan_is_autonami_pro_active() || version_compare( BWFAN_PRO_VERSION, '2.0.3', '>=' ) ) {
	class BWFAN_Contact_FirstName extends BWFAN_Merge_Tag {
		...
		public function __construct() {
			...
			$this->tag_description = __( 'Contact First Name', 'wp-marketing-automations' );
		...
		public function get_dummy_preview( $attr = [] ) {
			...
			$first_name = ! empty( $attr['fallback'] ) ? $attr['fallback'] : 'John';
	 ...
	 */
	BWFAN_Merge_Tag_Loader::register( 'bwf_contact', 'BWFAN_Contact_FirstName', null, __( 'Contact', 'wp-marketing-automations' ) );
}