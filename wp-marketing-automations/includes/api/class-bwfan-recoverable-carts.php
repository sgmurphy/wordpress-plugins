	...
	public static function get_recovered_carts( $search = '', $offset = '', $limit = '', $only_count = false ) {
		...
		if ( isset( $search ) && ! empty( $search ) && $only_count === false ) { //phpcs:ignore WordPress.Security.NonceVerification
			...
			if ( BWF_WC_Compatibility::is_hpos_enabled() ) {
				...
				$hpos_where     = " AND p.billing_email LIKE '%$search%' "; //phpcs:ignore WordPress.Security.NonceVerification
				$hpos_where     .= " AND p.billing_email > 0 ";
			}
		}
		...
		if ( $only_count === false ) {
			if ( BWF_WC_Compatibility::is_hpos_enabled() ) {
				$query = $wpdb->prepare( "SELECT p.id as id FROM {$wpdb->prefix}wc_orders as p LEFT JOIN {$wpdb->prefix}wc_orders_meta as m ON p.id = m.order_id $hpos_left_join WHERE p.type = %s AND p.status NOT IN $post_status AND m.meta_key = %s $hpos_where ORDER BY p.date_created_gmt DESC LIMIT $offset,$limit", 'shop_order', '_bwfan_ab_cart_recovered_a_id' );
			} else {
		...
		if ( BWF_WC_Compatibility::is_hpos_enabled() ) {
			$count_query                = $wpdb->prepare( "SELECT COUNT(p.id) FROM {$wpdb->prefix}wc_orders as p LEFT JOIN {$wpdb->prefix}wc_orders_meta as m ON p.id = m.order_id $hpos_left_join WHERE p.type = %s AND p.status NOT IN $post_status AND m.meta_key = %s $hpos_where ", 'shop_order', '_bwfan_ab_cart_recovered_a_id' );
