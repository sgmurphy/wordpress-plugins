...
class Lgx_Carousel_Settings_API {
    ...
    function admin_init() {
        ...
        foreach ( $this->settings_sections as $section ) {
            ...
            if ( isset($section['desc']) && !empty($section['desc']) ) {
                ...
                $secDesc  = $section['desc'];

                $callback = function() use(&$secDesc)  {
                    echo str_replace('"', '\"', $secDesc);
                };
	...
	function callback_textimg($args) {
		...
		//$lgxwp_image_name = $this->get_option('lgxwp_image_name', 'logosliderwp_basic');
		...
		$html  = sprintf('<input type="text" class="logosliderwpbg %1$s-text '.(($lgxwp_image_name != 'custom') ? ' hidden' : '').'" id="%2$s[%3$s]" name="%2$s[%3$s]" value="%4$s" placeholder="%5$s" style="float:left; "/>', $size, $args['section'], $args['id'], $value, $placeholder);
