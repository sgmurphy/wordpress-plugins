 ...
 */
class TFWCTOOL_WC_Widget_Layered_Nav extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = __( 'Display a list of attributes to filter products in your store.', 'woo-tools' );
		$this->widget_id          = 'tfwctool_woocommerce_layered_nav';
		$this->widget_name        = __( 'Filter Products by Attribute', 'woo-tools' );
		parent::__construct();
	}
	...
	public function init_settings() {
		...
		$this->settings = array(
			'title'        => array(
				'type'  => 'text',
				'std'   => __( 'Filter by', 'woo-tools' ),
				'label' => __( 'Title', 'woo-tools' ),
			),
			'attribute'    => array(
				'type'    => 'select',
				'std'     => $std_attribute,
				'label'   => __( 'Attribute', 'woo-tools' ),
				'options' => $attribute_array,
			),
			'display_type' => array(
				'type'    => 'select',
				'std'     => 'list',
				'label'   => __( 'Display type', 'woo-tools' ),
				'options' => array(
					'list'     => __( 'List', 'woo-tools' ),
					'dropdown' => __( 'Dropdown', 'woo-tools' ),
				),
			),
			'query_type'   => array(
				'type'    => 'select',
				'std'     => 'and',
				'label'   => __( 'Query type', 'woo-tools' ),
				'options' => array(
					'and' => __( 'AND', 'woo-tools' ),
					'or'  => __( 'OR', 'woo-tools' ),
				),
			),
		);
	...
	protected function layered_nav_dropdown( $terms, $taxonomy, $query_type ) {
		...
		if ( $taxonomy !== $this->get_current_taxonomy() ) {
			...
			$any_label      = apply_filters( 'woocommerce_layered_nav_any_label', sprintf( __( 'Any %s', 'woo-tools' ), $taxonomy_label ), $taxonomy_label, $taxonomy );
			...
			if ( $multiple ) {
				echo '<button class="woocommerce-widget-layered-nav-dropdown__submit" type="submit" value="' . esc_attr__( 'Apply', 'woo-tools' ) . '">' . esc_html__( 'Apply', 'woo-tools' ) . '</button>';
			}
				...
				if ( jQuery().selectWoo ) {
					var wc_layered_nav_select = function() {
						jQuery( '.dropdown_layered_nav_" . esc_js( $taxonomy_filter_name ) . "' ).selectWoo( {
							...
							language: {
								noResults: function() {
									return '" . esc_js( _x( 'No matches found', 'enhanced select', 'woo-tools' ) ) . "';
								}
							}
						} );
					};
...
function tfwctool_wc_widget_filter_by_attributes_register_widgets() {
	register_widget('TFWCTOOL_WC_Widget_Layered_Nav');
}
add_action('widgets_init', 'tfwctool_wc_widget_filter_by_attributes_register_widgets');