		...
		if(function_exists('WC') && WC()->cart){
			...
			if ($product_ids) {
				...
				foreach ($product_ids as $key => $product_id) {
					...
					if($_product){
						$product  = $_product;
						if ($_product->exists() && $_product->is_visible()) {

							$product_permalink = $_product->is_visible() ? $_product->get_permalink($product_id) : '';
							$products_data[$key] = array(
								'id' => $product_id,
								'remove' => sprintf('<a href="%s" data-product_id="%s" class="tfwctool-remove-quick-compare"><span>%s</span> <i class="fa fa-times"></i></a>', add_query_arg('remove-from-compare', $product_id, esc_url(site_url())), absint($product_id), esc_html__('Remove', 'woocommerce-tools')),
