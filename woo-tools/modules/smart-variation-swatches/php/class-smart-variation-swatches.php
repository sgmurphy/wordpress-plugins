...
class TFWC_Tool_Variation_Swatches_Admin {
	...
	function __construct() {
		...
		if(isset($_GET['post_type']) && $_GET['post_type'] == 'product' && isset($_GET['page']) && $_GET['page'] == 'product_attributes'){
			add_filter( 'product_attributes_type_selector', function($type){
				$type['color'] = esc_html__( 'Color', 'woocommerce-tools' );
				$type['image'] = esc_html__( 'Image', 'woocommerce-tools' );
				$type['label'] = esc_html__( 'Label', 'woocommerce-tools' );
				$type['radio'] = esc_html__( 'Radio', 'woocommerce-tools' );
				return $type;
			});
		}
	...
	public function add_attribute_columns($columns){
		...
		$new_columns['preview'] = esc_html__( 'Preview', 'woocommerce-tools' );;
		unset( $columns['cb'] );
...
function tfwctool_awesome_woocommerce_dropdown_variation_attribute_options_html($html, $args){
 	...
 	$args = wp_parse_args(
			...
			array(
				...
				'id'               => '',
				'class'            => '',
				'show_option_none' => __( 'Choose an option', 'woocommerce-tools' ),
			)
		);
		...
		$show_option_none_text = $args['show_option_none'] ? $args['show_option_none'] : __( 'Choose an option', 'woocommerce-tools' ); // We'll do our best to hide the placeholder, but we'll need to show something when resetting options.
