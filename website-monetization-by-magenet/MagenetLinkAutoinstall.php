    ...
    class MagenetLinkAutoinstall
    {
        ...
        public function add_magenet_links($content, $isFooter = false, $isObStart = false)
        {
            try {
                ...
                if ($mads_block == '' && strpos($content, '</body>')) {
                    $mads_block = '<div class="mads-block"></div>';
                    $content = str_replace('</body>', $mads_block . '</body>', $content);
                } else {
                    $content .= $mads_block;
                }
        ...
        public function admin_magenet_settings()
        {
            ...
            if (isset($_POST['key'])) {
                if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'magenet_key' ) ) {
                    wp_die( __( 'Error 1: Page is expired. Reload it and try again.' ), 403 );
                }
                if(!empty($_POST['key'])) {
            ...
            if (isset($_POST['update_data']) && $_POST['update_data'] == 1) {
                if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'magenet_update_data' ) ) {
                    wp_die( __( 'Error 2: Page is expired. Reload it and try again.' ), 403 );
                }
            ...
            if (isset($_POST['links_show_by'])) {
                if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'magenet_links_show_by' ) ) {
                    wp_die( __( 'Error 3: Page is expired. Reload it and try again.' ), 403 );
                }
