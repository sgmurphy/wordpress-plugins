jQuery(document).ready(function ($) {
	...
	jQuery(".attachment_post_title").change(function () {
		...
		var data = {
			...
			pid: pid,
			ptitle: ptitle,
			wah_nonce: jQuery(this).attr("data-nonce"),
		};
	...
	jQuery(".attachment_post_alt").change(function () {
		...
		var data = {
			...
			pid: pid,
			palt: palt,
			acc_nonce: jQuery('input[name="acc_nonce"]').val(),
		};
			...
			if (neworder) {
				jQuery.ajax({
					...
					data: {
						...
						alldata: neworder,
						nonce: jQuery("#sortable-wah-widget").attr("data-nonce"),
					},
