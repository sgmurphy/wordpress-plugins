	...
	function cntctfrm_admin_init() {
		...
		if ( empty( $bws_plugin_info ) ) {
			$bws_plugin_info = array(
				'id'      => '77',
				'version' => $cntctfrm_plugin_info['Version'],
			);
		}
		...
		$bws_shortcode_list['cntctfrm'] = array(
			'name'        => 'Contact Form',
			'js_function' => 'cntctfrm_shortcode_init',
		);
	}
}
...
if ( ! function_exists( 'cntctfrm_settings' ) ) {
	 ...
	 *
	 * @param mixed $form_id (Optional) Form id.
	 */
		...
		if ( $contact_form_multi_active ) {
			...
			if ( isset( $GLOBALS['cntctfrmmlt_id_form'] ) || $form_id ) {
				...
				if ( ! empty( $cntctfrm_options ) ) {
					...
					if ( 'pro' === $contact_form_multi_active || $contact_form_multi_active ) {
						$options_main = get_option( 'cntctfrmmltpr_options_main' );
					}

					if ( 1 === $id && ! empty( $options_main['name_id_form'] ) && 1 === count( $options_main['name_id_form'] ) ) {
...
if ( ! function_exists( 'cntctfrm_get_option_defaults' ) ) {
	...
	function cntctfrm_get_option_defaults() {
		...
		$option_defaults = array(
			...
			'display_date_time'       => 1,
			'message_limit'           => 0,
			'message_limit_min'       => 50,
			'message_limit_max'       => 300,
			'mail_method'             => 'wp-mail',
			...
			'message_limit_error'     => array( 'default' => __( 'Message length does not meet requirements.', 'contact-form-plugin' ) ),
...
/**
 * Changing for cntctfrm_related_plugins
 *
 * @since 4.0.1
...
if ( ! function_exists( 'cntctfrm_get_ordered_fields' ) ) {
	 ...
	 *
	 * @param number $id (Optional) Form id.
	 */
	function cntctfrm_get_ordered_fields( $id = false ) {
		...
		if ( ! $contact_form_multi_active ) {
			...
			$display_captcha        = ( isset( $cntctfrm_options['display_captcha'] ) && 1 === absint( $cntctfrm_options['display_captcha'] ) ) ? true : false;
			$display_google_captcha = ( isset( $cntctfrm_options['display_google_captcha'] ) && 1 === absint( $cntctfrm_options['display_google_captcha'] ) ) ? true : false;
			$display_subscriber     = ( isset( $cntctfrm_options['display_subscribe'] ) && 1 === absint( $cntctfrm_options['display_subscribe'] ) ) ? true : false;
		}

		$default_order_fields = array(
			'cntctfrm_contact_name'       => ( 1 === absint( $cntctfrm_options['display_name_field'] ) ) ? true : false,
			'cntctfrm_contact_address'    => ( 1 === absint( $cntctfrm_options['display_address_field'] ) ) ? true : false,
			'cntctfrm_contact_email'      => true,
			'cntctfrm_contact_phone'      => ( 1 === absint( $cntctfrm_options['display_phone_field'] ) ) ? true : false,
...
if ( ! function_exists( 'cntctfrm_settings_page' ) ) {
	...
	function cntctfrm_settings_page() {
		...
		<div class="wrap">
			...
			<?php
			if ( isset( $_POST['cntctfrm_hide_note'] ) && ! isset( $cntctfrm_options['hide_captcha_note'] ) && isset( $_POST['cntctfrm_nonce_hide_note_field'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['cntctfrm_nonce_hide_note_field'] ) ), 'cntctfrm_hide_note_action' ) ) {
			...
			if ( ! array_key_exists( 'captcha', $cntctfrm_related_plugins ) && ! array_key_exists( 'google-captcha', $cntctfrm_related_plugins ) && ! isset( $cntctfrm_options['hide_captcha_note'] ) ) {
				...
				<div class="notice notice-info inline is-dismissible">
					<form method="post" action="">
						...
						<?php wp_nonce_field( 'cntctfrm_hide_note_action', 'cntctfrm_nonce_hide_note_field' ); ?>
					</form>
					<p><?php echo sprintf( esc_html__( '%1$sNote:%2$s The contact form is not protected from spam and bots. Use %3$s Captcha by BestWebSoft plugin %5$s or %4$s reCaptcha by BestWebSoft plugin %5$s to protect your contact form.', 'contact-form-plugin' ), '<strong>', '</strong>', '<a href="https://bestwebsoft.com/products/wordpress/plugins/captcha/?k=8820a3dc45ab5d7d493999becb13a0b8" target="_blank">', '<a href="https://bestwebsoft.com/products/wordpress/plugins/google-captcha/?k=119dd1c656f672aab67aaf405dd6f08e" target="_blank">', '</a>' ); ?></p>
				</div>
				<?php
			}
			...
			if ( ! $contact_form_multi_active ) {
				...
				<h3 class="nav-tab-wrapper cntctfrm_noborder">
					...
					<a id="cntctfrm_show_multi_notice" class="nav-tab" target="_new" href="https://bestwebsoft.com/products/wordpress/plugins/contact-form-multi/?k=57d8351b1c6b67d3e0600bd9a680c283&amp;pn=3&amp;v=<?php echo esc_attr( $cntctfrm_plugin_info['Version'] ); ?>&amp;wp_v=<?php echo esc_attr( $wp_version ); ?>" title="<?php esc_html_e( 'If you want to create multiple contact forms, please install the Contact Form Multi plugin.', 'contact-form-plugin' ); ?>">+</a>
				</h3>
			<?php } else { ?>
...
if ( ! function_exists( 'cntctfrm_display_form' ) ) {
	 ...
	 *
	 * @param array $atts Attributes for form.
	 */
	function cntctfrm_display_form( $atts = array( 'lang' => 'default' ) ) {
		...
		$default_value = '';
		$form_submited = isset( $_POST['cntctfrm_form_submited'] ) ? absint( $_POST['cntctfrm_form_submited'] ) : 0;
		$name          = ( isset( $_POST['cntctfrm_contact_name'] ) && $cntctfrm_form_count === $form_submited ) ? sanitize_text_field( wp_unslash( $_POST['cntctfrm_contact_name'] ) ) : apply_filters( 'cntctfrm_default_name_check', $default_value );
		$address       = ( isset( $_POST['cntctfrm_contact_address'] ) && $cntctfrm_form_count === $form_submited ) ? sanitize_text_field( wp_unslash( $_POST['cntctfrm_contact_address'] ) ) : '';
		$email         = ( isset( $_POST['cntctfrm_contact_email'] ) && $cntctfrm_form_count === $form_submited ) ? sanitize_email( wp_unslash( $_POST['cntctfrm_contact_email'] ) ) : apply_filters( 'cntctfrm_default_email_check', $default_value );
		...
		if ( true === $cntctfrm_result && $cntctfrm_form_count === $form_submited ) {
			if ( 1 === absint( $cntctfrm_options['action_after_send'] ) ) {
		...
		if ( true !== $cntctfrm_result || $cntctfrm_form_count !== $form_submited ) {
			...
			$classes  = ( 1 === absint( $cntctfrm_options['layout'] ) ) ? ' cntctfrm_one_column' : ' cntctfrm_two_columns';
			...
			for ( $i = 1; $i <= $cntctfrm_options['layout']; $i++ ) {
				...
				$column        = ( 1 === $i ) ? 'first_column' : 'second_column';
					...
					switch ( $cntctfrm_field ) {
						case 'cntctfrm_contact_name':
							if ( 1 === absint( $cntctfrm_options['display_name_field'] ) ) {
								...
								$content .= '<div class="cntctfrm_label cntctfrm_label_name">
									<label for="cntctfrm_contact_name' . $form_countid . '">' . $cntctfrm_options['name_label'][ $lang ] . ( 1 === absint( $cntctfrm_options['required_name_field'] ) ? ' <span class="required">' . $cntctfrm_options['required_symbol'] . '</span></label>' : '</label>' );
								$content .= '</div>';
						...
						case 'cntctfrm_contact_address':
							if ( 1 === absint( $cntctfrm_options['display_address_field'] ) ) {
								...
								$content .= '<div class="cntctfrm_label cntctfrm_label_address">
										<label for="cntctfrm_contact_address' . $form_countid . '">' . $cntctfrm_options['address_label'][ $lang ] . ( 1 === absint( $cntctfrm_options['required_address_field'] ) ? ' <span class="required">' . $cntctfrm_options['required_symbol'] . '</span></label>' : '</label>' ) . '</div>';
						...
						case 'cntctfrm_contact_email':
							...
							$content .= '<div class="cntctfrm_label cntctfrm_label_email">
									<label for="cntctfrm_contact_email' . $form_countid . '">' . $cntctfrm_options['email_label'][ $lang ] . ( 1 === absint( $cntctfrm_options['required_email_field'] ) ? ' <span class="required">' . $cntctfrm_options['required_symbol'] . '</span></label>' : '</label>' ) . '
								</div>';
							...
							$content .= '<div class="cntctfrm_input cntctfrm_input_email">
									<input ' . apply_filters( 'cntctfrm_readonly', 'email' ) . ' class="text" type="email" size="40" value="' . esc_html( $email ) . '" name="cntctfrm_contact_email" id="cntctfrm_contact_email' . $form_countid . '" />';
							$content .= '</div>';
							$content .= '</div>';
							break;
						case 'cntctfrm_contact_phone':
							if ( 1 === absint( $cntctfrm_options['display_phone_field'] ) ) {
								...
								$content .= '<div class="cntctfrm_label cntctfrm_label_phone">
										<label for="cntctfrm_contact_phone' . $form_countid . '">' . $cntctfrm_options['phone_label'][ $lang ] . ( 1 === absint( $cntctfrm_options['required_phone_field'] ) ? ' <span class="required">' . $cntctfrm_options['required_symbol'] . '</span></label>' : '</label>' ) . '
									</div>';
								...
								$content .= '<div class="cntctfrm_input cntctfrm_input_phone">
									<input class="text" type="text" size="40" value="' . esc_html( $phone ) . '" pattern="[0-9]{6,}" title="' . __( 'Please enter only numbers at least 6 characters', 'contact-form-plugin' ) . '" name="cntctfrm_contact_phone" id="cntctfrm_contact_phone' . $form_countid . '" />';
								$content .= '</div>';
								$content .= '</div>';
							}
							break;
						case 'cntctfrm_contact_subject':
							...
							$content .= '<div class="cntctfrm_label cntctfrm_label_subject">
									<label for="cntctfrm_contact_subject' . $form_countid . '">' . $cntctfrm_options['subject_label'][ $lang ] . ( 1 === absint( $cntctfrm_options['required_subject_field'] ) ? ' <span class="required">' . $cntctfrm_options['required_symbol'] . '</span></label>' : '</label>' ) . '
								</div>';
						...
						case 'cntctfrm_contact_message':
							...
							$content .= '<div class="cntctfrm_label cntctfrm_label_message">
								<label for="cntctfrm_contact_message' . $form_countid . '">' . $cntctfrm_options['message_label'][ $lang ] . ( 1 === absint( $cntctfrm_options['required_message_field'] ) ? ' <span class="required">' . $cntctfrm_options['required_symbol'] . '</span></label>' : '</label>' ) . '
							</div>';
							...
							if ( isset( $cntctfrm_error_message['error_message_limit'] ) && $cntctfrm_form_count == $form_submited ) {
								$content .= '<div class="cntctfrm_error_text">' . $cntctfrm_error_message['error_message_limit'] . '</div>';
							}
							$min_length = isset( $cntctfrm_options['message_limit_min'] ) && ! empty( $cntctfrm_options['message_limit_min'] ) ? absint( $cntctfrm_options['message_limit_min'] ) : null;
							$max_length = isset( $cntctfrm_options['message_limit_max'] ) && ! empty( $cntctfrm_options['message_limit_max'] ) ? absint( $cntctfrm_options['message_limit_max'] ) : null;
							if ( ! empty( $min_length ) && ! empty( $max_length ) ) {
								$message_limit_string = 'minlength="' . $min_length . '" maxlength="' . $max_length . '"';
							} elseif ( ! empty( $min_length ) ) {
								$message_limit_string = 'minlength="' . $min_length . '"';
							} elseif ( ! empty( $max_length ) ) {
								$message_limit_string = 'maxlength="' . $max_length . '"';
							}
							$content .= '<div class="cntctfrm_input cntctfrm_input_message">
								<textarea ' . apply_filters( 'cntctfrm_readonly', 'message' ) . ' rows="5" cols="30"' . ( 1 === $cntctfrm_options['message_limit'] ? $message_limit_string : '' ) . 'name="cntctfrm_contact_message" id="cntctfrm_contact_message' . $form_countid . '">' . wp_kses_post( $message ) . '</textarea>';
							$content .= '</div>';
							$content .= '</div>';
							break;
						case 'cntctfrm_contact_attachment':
							if ( 1 === absint( $cntctfrm_options['attachment'] ) ) {
								...
								if ( 1 === absint( $cntctfrm_options['active_multi_attachment'] ) ) {
									...
									$content .= '</div>';
								if ( 1 === absint( $cntctfrm_options['attachment_explanations'] ) ) {
						...
						case 'cntctfrm_contact_send_copy':
							if ( 1 === absint( $cntctfrm_options['send_copy'] ) ) {
								$content .= '<div class="cntctfrm_field_wrap cntctfrm_field_attachment_wrap">';
								$content .= '<div class="cntctfrm_checkbox cntctfrm_checkbox_send_copy">
									<input type="checkbox" value="1" name="cntctfrm_contact_send_copy" id="cntctfrm_contact_send_copy"' . ( 1 === absint( $send_copy ) ? ' checked="checked" ' : '' ) . ' />
						...
						case 'cntctfrm_contact_gdpr':
							if ( 1 === absint( $cntctfrm_options['gdpr'] ) ) {
								$content .= '<div class="cntctfrm_field_wrap cntctfrm_field_attachment_wrap">';
								$content .= '<div class="cntctfrm_checkbox cntctfrm_checkbox_gdpr">
									<input type="checkbox" value="" required name="cntctfrm_contact_gdpr" id="cntctfrm_contact_gdpr"' . ( 1 === absint( $gdpr ) ? ' checked="checked" ' : '' ) . ' />
						...
						case 'cntctfrm_subscribe':
							if ( has_filter( 'sbscrbr_cntctfrm_checkbox_add' ) && ( ( ! $contact_form_multi_active && ! empty( $cntctfrm_related_plugins['subscriber']['options']['contact_form'] ) ) || ! empty( $cntctfrm_options['display_subscribe'] ) ) ) {
								...
								$cntctfrm_sbscrbr_checkbox = apply_filters(
									'sbscrbr_cntctfrm_checkbox_add',
									array(
...
if ( ! function_exists( 'cntctfrm_handle_captcha_filters' ) ) {
	 ...
	 *
	 * @param string $action          Can be 'remove_filters' or 'add_filters'.
	 * @param array  $removed_filters Array of existed filters (returned from this function when 'remove_filters' action).
...
if ( ! function_exists( 'cntctfrm_check_and_send' ) ) {
	...
	function cntctfrm_check_and_send() {
		...
		if ( ( isset( $_POST['cntctfrm_contact_action'] ) && isset( $_POST['cntctfrm_language'] ) ) || true === $cntctfrm_result ) {
			$cntctfrm_options = isset( $_POST['cntctfrm_options_name'] ) ? get_option( sanitize_text_field( wp_unslash( $_POST['cntctfrm_options_name'] ) ) ) : array();
			...
			if ( true === $cntctfrm_result ) {
				setcookie( 'cntctfrm_send_mail', true, time() + 60, COOKIEPATH, COOKIE_DOMAIN );
				if ( 0 === absint( $cntctfrm_options['action_after_send'] ) ) {
...
if ( ! function_exists( 'cntctfrm_check_form' ) ) {
	...
	function cntctfrm_check_form() {
		...
		$language                       = isset( $_POST['cntctfrm_language'] ) ? sanitize_text_field( wp_unslash( $_POST['cntctfrm_language'] ) ) : 'default';
		$cntctfrm_path_of_uploaded_file = '';
		$cntctfrm_result                = '';
		/* Error messages array */
		$cntctfrm_error_message = array();
		$name                   = isset( $_POST['cntctfrm_contact_name'] ) ? sanitize_text_field( wp_unslash( $_POST['cntctfrm_contact_name'] ) ) : '';
		...
		if ( 1 === absint( $cntctfrm_options['required_name_field'] ) && 1 === absint( $cntctfrm_options['display_name_field'] ) ) {
			$cntctfrm_error_message['error_name'] = $cntctfrm_options['name_error'][ $language ];
		}
		if ( 1 === absint( $cntctfrm_options['required_address_field'] ) && 1 === absint( $cntctfrm_options['display_address_field'] ) ) {
			$cntctfrm_error_message['error_address'] = $cntctfrm_options['address_error'][ $language ];
		}
		if ( 1 === absint( $cntctfrm_options['required_email_field'] ) ) {
			$cntctfrm_error_message['error_email'] = $cntctfrm_options['email_error'][ $language ];
		}
		if ( 1 === absint( $cntctfrm_options['required_subject_field'] ) ) {
			$cntctfrm_error_message['error_subject'] = $cntctfrm_options['subject_error'][ $language ];
		}
		if ( 1 === absint( $cntctfrm_options['required_message_field'] ) ) {
			$cntctfrm_error_message['error_message'] = $cntctfrm_options['message_error'][ $language ];
		}
		if ( 1 === $cntctfrm_options['message_limit'] ) {
			$min_length     = isset( $cntctfrm_options['message_limit_min'] ) && ! empty( $cntctfrm_options['message_limit_min'] ) ? absint( $cntctfrm_options['message_limit_min'] ) : null;
			$max_length     = isset( $cntctfrm_options['message_limit_max'] ) && ! empty( $cntctfrm_options['message_limit_max'] ) ? absint( $cntctfrm_options['message_limit_max'] ) : null;
			if ( ! empty( $min_length ) && ! empty( $max_length ) ) {
				$message_limit_error =
					sprintf(
						esc_html__( 'between %1$d and %2$d characters', 'contact-form-plugin' ),
						$min_length,
						$max_length
					);
			} elseif ( ! empty( $min_length ) ) {
				$message_limit_error =
					sprintf(
						esc_html__( 'more than %1$d characters', 'contact-form-plugin' ),
						$min_length
					);
			} elseif ( ! empty( $max_length ) ) {
				$message_limit_error =
					sprintf(
						esc_html__( 'less than %1$d characters', 'contact-form-plugin' ),
						$max_length
					);
			}
			$cntctfrm_error_message['error_message_limit'] = trim( $cntctfrm_options['message_limit_error'][ $language ], '.' ) . ': ' . $message_limit_error;
		}
		...
		if ( 1 === absint( $cntctfrm_options['display_phone_field'] ) && 1 === absint( $cntctfrm_options['required_phone_field'] ) && '' !== $phone ) {
		...
		if ( 1 === $cntctfrm_options['message_limit'] && '' !== $message ) {
			...
			if ( ! empty( $min_length ) && ! empty( $max_length ) && $message_length >= $min_length && $message_length <= $max_length ) {
				unset( $cntctfrm_error_message['error_message_limit'] );
			} elseif ( ( ! empty( $min_length ) && empty( $max_length ) && $message_length >= $min_length ) || ( ! empty( $max_length ) && empty( $min_length ) && $message_length <= $max_length ) ) {
				unset( $cntctfrm_error_message['error_message_limit'] );
			}
		}
		...
		if ( has_filter( 'cntctfrm_check_form' ) ) {
			$result = apply_filters( 'cntctfrm_check_form', array( true, 'string', 'bws_contact' ) );
		}
		...
		if (
			...
			} elseif ( ! empty( $result ) && ( is_string( $result ) || is_wp_error( $result ) ) ) {
		...
		if ( ! empty( $_FILES['cntctfrm_contact_attachment']['tmp_name'] ) && array( '' ) !== $_FILES['cntctfrm_contact_attachment']['tmp_name'] ) {
			...
			for ( $i = 0; $i < $num_files;$i++ ) {
				...
				if ( $cntctfrm_options['active_multi_attachment'] && isset( $_FILES['cntctfrm_contact_attachment']['name'][ $i ] ) ) {
					$contact_attachment_name = sanitize_file_name( wp_unslash( $_FILES['cntctfrm_contact_attachment']['name'][ $i ] ) );
					$new_filename = 'cntctfrm_' . md5( $contact_attachment_name . time() . $email . mt_rand() ) . '_' . $contact_attachment_name;
				} else {
					$contact_attachment_name = sanitize_file_name( wp_unslash( $_FILES['cntctfrm_contact_attachment']['name'] ) );
					$new_filename = 'cntctfrm_' . md5( $contact_attachment_name . time() . $email . mt_rand() ) . '_' . $contact_attachment_name;
				}
				if ( is_multisite() ) {
					if ( defined( 'UPLOADS' ) ) {
				...
				if ( $cntctfrm_options['active_multi_attachment'] && isset( $_FILES['cntctfrm_contact_attachment']['tmp_name'][ $i ] ) ) {
					$tmp_path = sanitize_text_field( $_FILES['cntctfrm_contact_attachment']['tmp_name'][ $i ] );
					$size     = sanitize_text_field( wp_unslash( $_FILES['cntctfrm_contact_attachment']['size'][ $i ] ) );
					$type     = sanitize_text_field( wp_unslash( $_FILES['cntctfrm_contact_attachment']['type'][ $i ] ) );
				} else {
					$tmp_path = sanitize_text_field( $_FILES['cntctfrm_contact_attachment']['tmp_name'] );
					$size     = sanitize_text_field( wp_unslash( $_FILES['cntctfrm_contact_attachment']['size'] ) );
					$type     = sanitize_text_field( wp_unslash( $_FILES['cntctfrm_contact_attachment']['type'] ) );
				}
				...
				if ( isset( $path_info['extension'] ) && array_key_exists( strtolower( $path_info['extension'] ), $cntctfrm_mime_type ) ) {
					if ( is_uploaded_file( $tmp_path ) ) {
						...
						if ( $move_file && ! isset( $move_file['error'] ) ) {
							do_action( 'cntctfrm_get_attachment_data', $move_file['file'] );
							/* Adding to array */
							$cntctfrm_path_of_uploaded_files[] = $move_file['file'];
							...
							if ( $cntctfrm_options['active_multi_attachment'] && isset( $_FILES['cntctfrm_contact_attachment']['size'][ $i ] ) ) {
								$cntctfrm_contact_attachment_size = absint( $_FILES['cntctfrm_contact_attachment']['size'][ $i ] );
							} else {
								$cntctfrm_contact_attachment_size = absint( $_FILES['cntctfrm_contact_attachment']['size'] );
							}
							...
							} elseif ( isset( $move_file['error'] ) ) {
								$cntctfrm_error_message['error_attachment'] = $move_file['error'];
							} else {
		...
		if ( 1 === count( $cntctfrm_error_message ) ) {
			if ( has_filter( 'sbscrbr_cntctfrm_checkbox_check' ) ) {
				$cntctfrm_sbscrbr_check = apply_filters(
					'sbscrbr_cntctfrm_checkbox_check',
					array(
...
if ( ! function_exists( 'cntctfrm_send_mail' ) ) {
	...
	function cntctfrm_send_mail() {
		...
		} elseif ( 'custom' === $cntctfrm_options['select_email'] ) {
		...
		if ( '' !== $to ) {
			...
			if ( 'on' === strtolower( getenv( 'HTTPS' ) ) ) {
				$form_action_url = esc_url( 'https://' . sanitize_text_field( wp_unslash( $_SERVER['HTTP_HOST'] ) ) . sanitize_text_field( wp_unslash( $_SERVER['REQUEST_URI'] ) ) );
			} else {
				$form_action_url = esc_url( 'http://' . sanitize_text_field( wp_unslash( $_SERVER['HTTP_HOST'] ) ) . sanitize_text_field( wp_unslash( $_SERVER['REQUEST_URI'] ) ) );
			}

			if ( 1 === absint( $cntctfrm_options['display_add_info'] ) ) {
				$cntctfrm_remote_addr = filter_var( $_SERVER['REMOTE_ADDR'], FILTER_VALIDATE_IP );
				$userdomain           = @gethostbyaddr( $cntctfrm_remote_addr );
				if ( 1 === absint( $cntctfrm_options['display_add_info'] ) ||
						1 === absint( $cntctfrm_options['display_sent_from'] ) ||
						1 === absint( $cntctfrm_options['display_coming_from'] ) ||
						1 === absint( $cntctfrm_options['display_user_agent'] ) ) {
					if ( 1 === absint( $cntctfrm_options['html_email'] ) ) {
						$user_info_string .= '<tr><td><br /></td><td><br /></td></tr>';
					}
				}
				if ( 1 === absint( $cntctfrm_options['display_sent_from'] ) ) {
					if ( 1 === absint( $cntctfrm_options['html_email'] ) ) {
			...
			/* 'from' name */
			$from_field_name = ( 'custom' === $cntctfrm_options['select_from_field'] ) ? wp_unslash( $cntctfrm_options['from_field'] ) : $name;
			/* 'from' email */
			$from_email = ( 'custom' === $cntctfrm_options['from_email'] ) ? wp_unslash( $cntctfrm_options['custom_from_email'] ) : wp_unslash( $email );
			...
			if ( 'wp-mail' === $cntctfrm_options['mail_method'] ) {
				...
				if ( 1 === absint( $cntctfrm_options['attachment'] ) && ! empty( $_FILES['cntctfrm_contact_attachment']['tmp_name'] ) && array( '' ) !== $_FILES['cntctfrm_contact_attachment']['tmp_name'] ) {
					if ( empty( $cntctfrm_path_of_uploaded_files ) || ! is_array( $cntctfrm_path_of_uploaded_files ) || count( $cntctfrm_path_of_uploaded_files ) <= 1 ) {
				...
				if ( 1 === absint( $cntctfrm_options['attachment'] ) && ! empty( $_FILES['cntctfrm_contact_attachment']['tmp_name'] ) && array( '' ) !== $_FILES['cntctfrm_contact_attachment']['tmp_name'] && 1 === absint( $cntctfrm_options['delete_attached_file'] ) ) {
...
if ( ! function_exists( 'cntctfrm_clean_input' ) ) {
	 ...
	 * @param number $preserve_space (Optional) Flag for preserve space.
	 */
	function cntctfrm_clean_input( $string, $preserve_space = 0 ) {
		if ( is_string( $string ) ) {
			if ( $preserve_space ) {
				return cntctfrm_sanitize_string( wp_strip_all_tags( wp_unslash( $string ) ), $preserve_space );
			}
			return trim( cntctfrm_sanitize_string( wp_strip_all_tags( wp_unslash( $string ) ) ) );
...
if ( ! function_exists( 'cntctfrm_sanitize_string' ) ) {
	 ...
	 *
	 * @param string $string         String for sanitize.
	 * @param number $preserve_space (Optional) Flag for preserve space.
	 */
...
if ( ! function_exists( 'cntctfrm_admin_head' ) ) {
	/**
	 * Add styles and scripts for dashboard
	 */
	...
	function cntctfrm_wp_footer() {
		...
		for ( $count = 1; $count <= $cntctfrm_form_count; $count++ ) {
			...
			if ( 'custom' === $options['width']['type'] ) {
				?>
				<style id="cntctfrm_custom_styles__<?php echo esc_attr( $count ); ?>" type="text/css">
					<?php echo esc_attr( $form_countid ); ?>.cntctfrm_width_custom {
						max-width: <?php printf( '%d%s', esc_attr( $options['width']['input_value'] ), esc_attr( $options['width']['input_unit'] ) ); ?>;
					}
				</style>
				<?php
			}
...
if ( ! function_exists( 'cntctfrm_add_language' ) ) {
	...
	function cntctfrm_add_language() {
		...
		if ( $is_ajax ) {
			...
			$_POST['cntctfrm_change_tab'] = isset( $_REQUEST['cntctfrm_languages'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['cntctfrm_languages'] ) ) : '';
		}

		$lang = $is_ajax ? sanitize_text_field( wp_unslash( $_REQUEST['lang'] ) ) : sanitize_text_field( wp_unslash( $_REQUEST['cntctfrm_languages'] ) );
		...
		if ( $contact_form_multi_active ) {
			...
			if ( isset( $multi_options_main['id_form'] ) && $cntctfrm_options = get_option( 'cntctfrmmlt_options_' . absint( $multi_options_main['id_form'] ) ) ) {
				$cntctfrm_options = get_option( 'cntctfrmmlt_options_' . absint( $multi_options_main['id_form'] ) );
			} else {
				$cntctfrm_options = get_option( 'cntctfrmmlt_options' );
			}
		} else {
...
if ( ! function_exists( 'cntctfrm_remove_language' ) ) {
	...
	function cntctfrm_remove_language() {
		...
		$lang = $is_ajax && isset( $_REQUEST['lang'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['lang'] ) ) : sanitize_text_field( wp_unslash( $_REQUEST['cntctfrm_delete_button'] ) );

		$key = array_search( $lang, $cntctfrm_options['language'] );
		if ( false !== $key ) {
		...
		if ( isset( $cntctfrm_options['message_limit_error'][ $lang ] ) ) {
			unset( $cntctfrm_options['message_limit_error'][ $lang ] );
		}
...
if ( ! function_exists( 'cntctfrm_plugin_banner' ) ) {
	...
	function cntctfrm_plugin_banner() {
		global $hook_suffix;
		if ( 'plugins.php' === $hook_suffix || ( isset( $_REQUEST['page'] ) && 'contact_form.php' === sanitize_text_field( wp_unslash( $_REQUEST['page'] ) ) ) ) {
			...
			foreach ( $bstwbsftwppdtplgns_banner_array as $key => $value ) {
				if ( $this_banner === $value[0] ) {
					if ( ! isset( $bstwbsftwppdtplgns_cookie_add ) ) {
						wp_enqueue_script( 'bstwbsftwppdtplgns_cookie_add', plugins_url( 'bws_menu/js/c_o_o_k_i_e.js', __FILE__ ), false, null, false );
					}
					...
					wp_register_script( 'cntctfrm_bws_script_banner', '//', false, null, false );
					...
					if ( ! array_key_exists( 'contact-form-to-db/contact_form_to_db.php', $all_plugins ) && ! array_key_exists( 'contact-form-to-db-pro/contact_form_to_db_pro.php', $all_plugins ) ) {
						...
						<div class="updated" style="padding: 0; margin: 0; border: none; background: none;">
							<div class="cntctfrm_for_ctfrmtdb_message bws_banner_on_plugin_page" style="display: none;">
								<button class="<?php echo esc_attr( $this_banner_prefix ); ?>_close_icon close_icon notice-dismiss bws_hide_settings_notice" title="<?php esc_html_e( 'Close notice', 'contact-form-plugin' ); ?>"></button>
								<div class="icon">
								...
								<div class="button_div">
									<a class="button" target="_blank" href="https://bestwebsoft.com/products/wordpress/plugins/contact-form-to-db/?k=6ebf0743736411607343ad391dc3b436&amp;pn=77&amp;v=<?php echo esc_attr( $cntctfrm_plugin_info['Version'] ); ?>&amp;wp_v=<?php echo esc_attr( $wp_version ); ?>"><?php esc_html_e( 'Learn More', 'contact-form-plugin' ); ?></a>
								</div>
							</div>
						</div>
						<?php
...
if ( ! function_exists( 'cntctfrm_shortcode_button_content' ) ) {
	 ...
	 * Shortcode function
	 *
	 * @param string $conettn Shortcode content.
	 */
	function cntctfrm_shortcode_button_content( $content ) {
		...
		<div id="cntctfrm" style="display:none;">
				...
				<?php if ( $contact_form_multi_active ) { ?>
					<label>
						<select name="cntctfrm_forms_list" id="cntctfrm_forms_list">
							<?php
							foreach ( $multi_forms as $id => $title ) {
									printf( '<option value="%1$s">%2$s</option>', esc_attr( $id ), esc_html( $title ) );
							}
							?>
						</select>
						...
						<select name="cntctfrm_multi_languages_list" id="cntctfrm_multi_languages_list">
							...
							foreach ( $multi_forms_languages as $id => $languages ) {
								foreach ( $languages as $language ) {
									printf( '<option value="%1$s" data-form-id="%2$s" %3$s>%4$s</option>', esc_attr( strtolower( $language ) ), esc_attr( $id ), ( $i > 1 ) ? 'style="display: none;"' : '', esc_html( 'default' === $language ? $lang_default : $cntctfrm_lang_codes[ $language ] ) );
								}
								$i++;
							}
							?>
					...
					<input class="bws_default_shortcode" type="hidden" name="default" value="[bestwebsoft_contact_form id=<?php echo absint( array_shift( $multi_ids ) ); ?>]" />
				<?php } else { ?>
					<label>
						<select name="cntctfrm_languages_list" id="cntctfrm_languages_list">
							<?php
							foreach ( $options['language'] as $language ) {
								printf( '<option value="%1$s">%2$s</option>', esc_attr( strtolower( $language ) ), esc_html( 'default' === $language ? $lang_default : $cntctfrm_lang_codes[ $language ] ) );
							}
							?>
						</select>
						<span class="title"><?php esc_html_e( 'Language', 'contact-form-plugin' ); ?></span>
			...
			<?php
			$script = "function cntctfrm_shortcode_init() {
					( function( $ ) {
					'use strict'
			...
			wp_register_script( 'cntctfrm_bws_shortcode_button_script', '//', false, null, false );
...
/* Ccustom filter for bws button in tinyMCE */
