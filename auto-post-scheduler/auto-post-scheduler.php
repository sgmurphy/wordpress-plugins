...
define('AUTOPOSTSCHEDULER_VERSION', '1.83');
...
<a target='_blank' href='https://codecanyon.net/item/wp-auto-poster/23805990?ref=johnh10' class='button-primary'><?php _e( 'WP Auto Poster', 'auto-post-sheduler' );?></a> <?php _e( 'is the premium version, completely rewritten with more features, unlimited rulesets, and full support.', 'auto-post-scheduler' );?>
	...
	<h3><?php _e('Auto Post Scheduler Options', 'auto-post-scheduler' );?></h3>
        ...
        <tr valign="top"><td width="25%" align="right">
                ...
                <input type="checkbox" name="aps_restart" value="checkbox" <?php if (get_option('aps_restart')) echo "checked='checked'"; ?>/> <?php _e('If checked, posts published manually or by other plugins will also reset the next scheduler check time.', 'auto-post-scheduler' );?> 
        </td></tr>
...
function aps_auto_post() {
	...
	if ($aps_debug) aps_write_log( sprintf( __("DEBUG: aps_num_day stats are %s and today is day %d.", 'auto-post-scheduler'), print_r($aps_num_day,true), $today) );

	if ($aps_num_day[0] && $aps_num_day[0] != $today) # if new day, reset num
		$day_num = (int) 0;
	else
		$day_num = isset($aps_num_day[1]) ? (int) $aps_num_day[1] : (int) 0;

	if ($aps_max_per_day && $day_num >= $aps_max_per_day) {
		if ($aps_debug) aps_write_log( sprintf( __("DEBUG: Reached maximum number of published/recycled posts (%s) for day %s, no post check will occur.", 'auto-post-scheduler'), $aps_max_per_day, $today) );
		return;
	}
	...
	if ($results->have_posts()) {
		...
		while ($results->have_posts() && $cnt < $aps_batch) {
			...
			if ($aps_max_per_day && $day_num >= $aps_max_per_day) {
				if ($aps_debug) aps_write_log( sprintf( __("DEBUG: Reached maximum number of published/recycled posts (%s) for day %s.", 'auto-post-scheduler'), $aps_max_per_day, $today) );
				break;
			}

		}
		if ($cnt < $aps_batch && $day_num < $aps_max_per_day ) // only happens for special case check
