...
//
// This file is part of DSGVOTCPDF software library.
//
// DSGVOTCPDF is free software: you can redistribute it and/or modify it
// under the terms of the GNU Lesser General Public License as
// published by the Free Software Foundation, either version 3 of the
// License, or (at your option) any later version.
//
// DSGVOTCPDF is distributed in the hope that it will be useful, but
// WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
// See the GNU Lesser General Public License for more details.
//
// You should have received a copy of the License
// along with DSGVOTCPDF. If not, see
 ...
 * DSGVOTCPDF project (http://www.tcpdf.org) was originally derived in 2002 from the Public Domain FPDF class by Olivier Plathey (http://www.fpdf.org), but now is almost entirely rewritten.<br>
 * <h3>DSGVOTCPDF main features are:</h3>
 * <ul>
 ...
 * DSGVOTCPDF project (http://www.tcpdf.org) has been originally derived in 2002 from the Public Domain FPDF class by Olivier Plathey (http://www.fpdf.org), but now is almost entirely rewritten.<br>
 * @package com.tecnick.tcpdf
...
class DSGVOTCPDF {
	...
	/**
	 * If true print DSGVOTCPDF meta link.
	 * @protected
	 * @since 5.9.152 (2012-03-23)
	 */
	 ...
	 *
	 * IMPORTANT: Please note that this method sets the mb_internal_encoding to ASCII, so if you are using the mbstring module functions with DSGVOTCPDF you need to correctly set/unset the mb_internal_encoding when needed.
	 *
	...
	public function __construct($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false, $pdfa=false) {
		...
		$this->file_id = md5(DSGVOTCPDF_STATIC::getRandomSeed('DSGVOTCPDF'.$orientation.$unit.$serformat.$encoding));
	...
	/**
	 * Throw an exception or print an error message and die if the K_DSGVOTCPDF_PARSER_THROW_EXCEPTION_ERROR constant is set to true.
	...
	public function Error($msg) {
		...
		$this->_destroy(true);
		if (defined('K_DSGVOTCPDF_THROW_EXCEPTION_ERROR') AND !K_DSGVOTCPDF_THROW_EXCEPTION_ERROR) {
			die('<strong>DSGVOTCPDF ERROR: </strong>'.$msg);
		} else {
			throw new Exception('DSGVOTCPDF ERROR: '.$msg);
		}
	}
	...
	public function AddFont($family, $style='', $fontfile='', $subset='default') {
		...
		if (DSGVOTCPDF_STATIC::empty_string($fontfile) OR (!@DSGVOTCPDF_STATIC::file_exists($fontfile))) {
		...
		// include font file
		if (!DSGVOTCPDF_STATIC::empty_string($fontfile) AND (@DSGVOTCPDF_STATIC::file_exists($fontfile))) {
			include($fontfile);
		} else {
	...
	public function Annotation($x, $y, $w, $h, $text, $opt=array('Subtype'=>'Text'), $spaces=0) {
		...
		if (!$this->pdfa_mode) {
			if ((($opt['Subtype'] == 'FileAttachment') OR ($opt['Subtype'] == 'Sound')) AND (!DSGVOTCPDF_STATIC::empty_string($opt['FS']))
				AND (@DSGVOTCPDF_STATIC::file_exists($opt['FS']) OR DSGVOTCPDF_STATIC::isValidURL($opt['FS']))
				AND (!isset($this->embeddedfiles[basename($opt['FS'])]))) {
				$this->embeddedfiles[basename($opt['FS'])] = array('f' => ++$this->n, 'n' => ++$this->n, 'file' => $opt['FS']);
			}
		}
		// Add widgets annotation's icons
		if (isset($opt['mk']['i']) AND @DSGVOTCPDF_STATIC::file_exists($opt['mk']['i'])) {
			$this->Image($opt['mk']['i'], '', '', 10, 10, '', '', '', false, 300, '', false, false, 0, false, true);
		}
		if (isset($opt['mk']['ri']) AND @DSGVOTCPDF_STATIC::file_exists($opt['mk']['ri'])) {
			$this->Image($opt['mk']['ri'], '', '', 0, 0, '', '', '', false, 300, '', false, false, 0, false, true);
		}
		if (isset($opt['mk']['ix']) AND @DSGVOTCPDF_STATIC::file_exists($opt['mk']['ix'])) {
	...
	public function Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		...
		if ($file[0] === '@') {
			...
			if (!@DSGVOTCPDF_STATIC::file_exists($file)) {
				return false;
			}
		...
		if ($newimage) {
			...
			// Specific image handlers (defined on DSGVOTCPDF_IMAGES CLASS)
			$mtd = '_parse'.$type;
			// GD image handler function
			$gdfunction = 'imagecreatefrom'.$type;
			$info = false;
			if ((method_exists('DSGVOTCPDF_IMAGES', $mtd)) AND (!($resize AND (function_exists($gdfunction) OR extension_loaded('imagick'))))) {
				// DSGVOTCPDF image functions
	...
	protected function ImagePngAlpha($file, $x, $y, $wpx, $hpx, $w, $h, $type, $link, $align, $resize, $dpi, $palign, $filehash='') {
		...
		if ($parsed === false) {
			if (empty($parse_error)) {
				$this->Error('DSGVOTCPDF requires the Imagick or GD extension to handle PNG images with alpha channel.');
			} else {
	...
	protected function _putinfo() {
		...
		// default producer
		$out .= ' /Producer '.$this->_textstring(DSGVOTCPDF_STATIC::getDSGVOTCPDFProducer(), $oid);
	...
	protected function _putXMP() {
		...
		$xmp .= "\t\t\t".'<pdf:Producer>'.DSGVOTCPDF_STATIC::_escapeXML(DSGVOTCPDF_STATIC::getDSGVOTCPDFProducer()).'</pdf:Producer>'."\n";
		$xmp .= "\t\t".'</rdf:Description>'."\n";
	...
	protected function _putcatalog() {
		...
		if (!empty($this->pdflayers)) {
			...
			$out .= ' /Creator '.$this->_textstring('DSGVOTCPDF', $oid);
			$out .= ' /BaseState /ON';
			$out .= ' /OFF ['.$lyrobjs_off.']';
	...
	/**
	 * Return an hash code used to ensure that the serialized data has been generated by this DSGVOTCPDF instance.
	 * @param $data (string) serialized data
	 * @return string
	 * @public static
	 */
	protected function getHashForDSGVOTCPDFtagParams($data) {
		return md5(strlen($data).$this->file_id.$data);
	}

	/**
	 * Serialize an array of parameters to be used with DSGVOTCPDF tag in HTML code.
	 * @param $data (array) parameters array
	 * @return string containing serialized data
	 * @public static
	 */
	public function serializeDSGVOTCPDFtagParameters($data) {
		$encoded = urlencode(json_encode($data));
		return $this->getHashForDSGVOTCPDFtagParams($encoded).$encoded;
	}

	/**
	 * Unserialize parameters to be used with DSGVOTCPDF tag in HTML code.
	 * @param $data (string) serialized data
	 * @return array containing unserialized data
	 * @protected static
	 */
	protected function unserializeDSGVOTCPDFtagParameters($data) {
		$hash = substr($data, 0, 32);
		$encoded = substr($data, 32);
		if ($hash != $this->getHashForDSGVOTCPDFtagParams($encoded)) {
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='') {
		...
		while ($key < $maxel) {
			...
			if ($dom[$key]['tag']) {
				...
				// text
				$this->htmlvspace = 0;
				$isRTLString = preg_match(DSGVOTCPDF_FONT_DATA::$uni_RE_PATTERN_RTL, $dom[$key]['value']) || preg_match(DSGVOTCPDF_FONT_DATA::$uni_RE_PATTERN_ARABIC, $dom[$key]['value']);
		...
		switch($tag['value']) {
			...
			case 'img': {
				...
				if ($imgsrc[0] === '@') {
					...
					// get image type
					$type = DSGVOTCPDF_IMAGES::getImageFileType($imgsrc);
				}
				if (!isset($tag['width'])) {
					$tag['width'] = 0;
				}
			...
			case 'tcpdf': {
				if (defined('K_DSGVOTCPDF_CALLS_IN_HTML') AND (K_DSGVOTCPDF_CALLS_IN_HTML === true)) {
					// Special tag used to call DSGVOTCPDF methods
					if (isset($tag['attribute']['method'])) {
						$tcpdf_method = $tag['attribute']['method'];
						if (method_exists($this, $tcpdf_method)) {
							if (isset($tag['attribute']['params']) AND (!empty($tag['attribute']['params']))) {
								$params = $this->unserializeDSGVOTCPDFtagParameters($tag['attribute']['params']);
	...
	/**
	 * Stores a copy of the current DSGVOTCPDF object used for undo operation.
	 * @public
	 * @since 4.5.029 (2009-03-19)
	 */
	...
	/**
	 * Delete the copy of the current DSGVOTCPDF object used for undo operation.
	 * @public
	 * @since 4.5.029 (2009-03-19)
	 */
	...
	/**
	 * This method allows to undo the latest transaction by returning the latest saved DSGVOTCPDF object with startTransaction().
