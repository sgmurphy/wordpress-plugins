...
class CMB2_Type_Wysiwyg extends CMB2_Type_Textarea {
	...
	public function add_wysiwyg_template_for_group() {
		...
		echo '<div class="cmb2-wysiwyg-inner-wrap" data-iterator="{{ data.iterator }}" data-groupid="' . esc_attr( $group_id ) . '" data-id="' . esc_attr( $field_id ) . '" data-hash="' . esc_attr( $hash ) . '">' . wp_kses_post( $editor ) . '</div>';
		echo '</script>';
	}
