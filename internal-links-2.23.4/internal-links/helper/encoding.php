...
class Encoding
{
    ...
    public static function maskPattern($pattern)
    {
	    ...
	    if ($boundary_start != '' && !preg_match('/^[a-z0-9àâçéèêëîïôûùüÿñæœ]/', strtolower($pattern))) $boundary_start = '(?<=^|\s|\"|\'|\{|\[|\<|\(|\,)';
	    if ($boundary_end != '' && !preg_match('/[a-z0-9àâçéèêëîïôûùüÿñæœ]$/', strtolower($pattern))) $boundary_end = '(?=$|\s|\"|\.|\?|\!|\,|\)|\}|\]|\>|\;|\:)';
     ...
     * @return string
     */
    public static function escapeAscii($pattern)
    {
