...
function theme_back_enqueue_script()
{
	...
	if (isset($_GET['page'])) {
		if ($_GET['page'] == 'sfsi-options') {
			...
			wp_localize_script('SFSICustomJs', 'sfsi_icon_ajax_object', array(
				'ajax_url' => admin_url('admin-ajax.php'),
				'plugin_url' => SFSI_PLUGURL,
				'nonceTIFM' => wp_create_nonce('tifm_notice_nonce'),
        'unonce' => wp_create_nonce('usm_universal_nonce_icons')
			));
		}
	}
}
...
function theme_front_enqueue_script() {
	...
	/* initilaize the ajax url in javascript */
	wp_localize_script('SFSICustomJs', 'sfsi_icon_ajax_object', array('nonce' => wp_create_nonce('usm_universal_nonce_icons'),'ajax_url' => admin_url('admin-ajax.php'), 'plugin_url' => SFSI_PLUGURL));
