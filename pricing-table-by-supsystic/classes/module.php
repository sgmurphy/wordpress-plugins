...
abstract class modulePts extends baseObjectPts {
   ...
   public function __construct($d) {
      ...
      if (isset($d['id'])) $this->_setID($d['id']);
      if (isset($d['ex_plug_dir']) && !empty($d['ex_plug_dir'])) {
         $this->isExternal(true);
         $this->setExternalDir(utilsPts::getExtModDir($d['ex_plug_dir']));
         $this->setExternalPath(utilsPts::getExtModPath($d['ex_plug_dir']));
      }
   }
   public function isExternal($newVal = NULL) {
      if (is_null($newVal)) return $this->_isExternal;
      $this->_isExternal = $newVal;
   }
   public function getModDir() {
      if (empty($this->_externalDir)) {
         return PTS_MODULES_DIR . $this->getCode() . DS;
      }
      else {
   ...
   public function getModPath() {
      if (empty($this->_externalPath)) {
         return PTS_MODULES_PATH . $this->getCode() . '/';
      }
      else {
   ...
   protected function _createController() {
      ...
      if (file_exists($this->getModDir() . 'controller.php')) {
         $className = '';
         if (importPts($this->getModDir() . 'controller.php')) {
            $className = toeGetClassNamePts($this->getCode() . 'Controller');
         }
         if (!empty($className)) {
   ...
   public function getHelper() {
      if (!$this->_helper) $this->_createHelper();
      return $this->_helper;
   }
   protected function _createHelper() {
      ...
      if (file_exists($this->getModDir() . 'helper.php')) {
         ...
         importClassPts($helper, $this->getModDir() . 'helper.php');
