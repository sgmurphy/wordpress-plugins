...
class validatorPts {
   ...
   static public function validate($field, $label = '', $validate = array()) {
      self::$errors = array();
      if (is_object($field) && get_class($field) != 'fieldPts') {
         $value = $field;
         $field = new fieldPtsPts('noMatter');
         $field->label = $label;
      ...
      if (!empty($field->validate)) {
         foreach ($field->validate as $v) {
            if (method_exists('validatorPts', $v)) self::$v($field);
         }
      }
      if (method_exists('validatorPts', $field->type)) {
         $validate = $field->type;
         self::$validate($field);
      }
      if ($field->maxlen) {
   ...
   static public function addError($error, $key = '') {
      if ($key) self::$errors[$key] = $error;
      else self::$errors[] = $error;
   }
   ...
   static public function getProductValidationMethods() {
      $res = array();
      $all = get_class_methods('validatorPts');
      foreach ($all as $m) {
         if (in_array($m, array(
            'int',
            'none',
   ...
   static public function prepareInput($input) {
      ...
      else return $wpdb->_real_escape($input);
   }
}
