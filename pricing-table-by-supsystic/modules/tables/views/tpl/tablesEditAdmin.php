			...
			<div class="ptsTableSettingsShell row">
                ...
                <div class="ptsSettingsContent">
                    <div id="#main" class="active main">
                        ...
                        <div class="ptsTableSetting supMd3">
                                <label>
                                    <?php _e('AJAX Loading', PTS_LANG_CODE)?>
                                    <?php echo htmlPts::selectbox('params[ajax]', array(
                                        'options' => array('disable' => 'disable', 'toggle_by_header' => 'toggle by header'),
                                        'value' => (isset($this->table['params']['ajax']['val']) ? $this->table['params']['ajax']['val'] : 'disable'),
                                        'attrs' => ($isPro) ? 'class="chosen"' : 'disabled class="chosen"'
                                    ))?>
                                    <i class="fa fa-question supsystic-tooltip" title="<?php echo esc_html(__('Suitable if you need to load the table by action, but not when the page is initiated.<br> "toggle by header" - you can also use " title=\'custom label\' " param in shortcode to enter custom label.', PTS_LANG_CODE))?>"></i>
                                </label>
                                <?php if(!$isPro){?>
