...
function wp_vid_lightbox_vimeo5_handler($atts)
{
    ...
    if(!is_numeric($width) || $width < 1){
        return '<p>'.__('Error! You must specify a valid width to use this shortcode!', 'wp-video-lightbox').'</p>';
    }
    if(!is_numeric($height) || $height < 1){
        return '<p>'.__('Error! You must specify a valid height to use this shortcode!', 'wp-video-lightbox').'</p>';
    }
    ...
    if(!is_numeric($aspect_ratio) || $aspect_ratio <= 0 ){
        return '<p>'.__('Error! You must specify a valid width and height to use this shortcode!', 'wp-video-lightbox').'</p>';
    }
    $esc_js = 'esc_js';
    $output = "";
