...
class WPPopups_Builder_Panel_Content extends WPPopups_Builder_Panel {
	...
	public function panel_sidebar() {
		...
		echo '<h3>'. __('WP Popups templates', 'wp-popups-lite' ) .'</h3>';?>
		<p><?php esc_html_e( 'Select your template to insert the shortcode to the editor', 'wp-popups-lite' ); ?>:</p>
		<select id="wppopups-templates" >
			<option value=""><?php _e('Choose a template', 'wp-popups-lite');?></option>
			<?php
			$templates = get_posts(['post_type' => 'wppopups-templates', 'posts_per_page'=> -1]);
			if($templates) {
		...
		<?php
		echo '<h3>'. __('Available shortcodes', 'wp-popups-lite' ) .'</h3>';
		?>
		<div class="inside">
