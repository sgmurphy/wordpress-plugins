...
class WPPopups_Builder {
	...
	public function enqueues() {
		...
		$contentTemplatesPosts = get_posts([
			'post_type'   => 'wppopups-templates',
			'post_status' => [ 'publish' ],
			'numberposts' => -1,
			'fields'      => 'all',
		]);

		foreach ( $contentTemplatesPosts as $contentTemplate ) {
			$contentTemplates[ $contentTemplate->ID ] = do_blocks( $contentTemplate->post_content );
		}
