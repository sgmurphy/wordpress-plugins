...
(function ($) {
  ...
  var WPPopupsBuilder = {
    ...
    ready: function ready() {
      ...
      $('.spu-content').html(WPPopupsBuilder.applyContentTemplates($('.spu-content').html()));
    ...
    initChoicesJS: function initChoicesJS() {
      ...
      $('#wppopups-templates').each(function () {
        // if already initiated continue
        if (typeof $(this)[0].choices != 'undefined') return true;
        var $this = $(this),
            args = {
        ...
        $(this)[0].choices = new Choices($this[0], args);
        $(this)[0].choices.passedElement.addEventListener('choice', function (choice) {
          window.parent.send_to_editor('[wppopup-template id="' + choice.detail.choice.value + '"]');
    ...
    updateEditorContent: function updateEditorContent(editor) {
      ...
      if (html) {
        $('.spu-content').html(WPPopupsBuilder.applyContentTemplates(html));
      }
    },
    // --------------------------------------------------------------------//
    // Settings Panel
     ...
     * @return string
     */
    applyContentTemplates: function applyContentTemplates(html) {
      if (wppopups_builder_vars.content_templates.length === 0) {
        return html;
      }

      var templates = wppopups_builder_vars.content_templates;
      Object.keys(templates).forEach(function (id) {
        html = html.replace('[wppopup-template id="' + id + '"]', templates[id]);
      });
      return html;
    },
    ...
    fieldChoiceSortable: function fieldChoiceSortable(type, selector) {
      ...
      $('#wppopups-templates').sortable({
        ...
        stop: function stop(e, ui) {
          var id = ui.item.parent().data('field-id');
          var shortcode = '[wppopup-template id="' + id + '"]';
          alert(shortcode);
        },
        update: function update(e, ui) {}
      });
