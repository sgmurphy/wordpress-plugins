    ...
    public static function field_checkbox_row_static( $field_name, $field, $echo = true ) {
	    ...
	    if ( 1 ) {
			...
			$params_checkbox = array(
									...
									, 'name'     => $field_name
									, 'label'    => array( 'title' => wp_kses_post( $field['label'] ) , 'position' => 'right' )           //FixIn: 9.6.1.5
									, 'toggle_style' => $field['css'] 											// CSS of select element
									, 'class'    => $field['class'] 										// CSS Class of select element
									, 'disabled' => $field['disabled']
									, 'attr'     => $field['attr'] 											// Any  additional attributes, if this radio | checkbox element
									, 'legend'   => wp_kses_post( $field['title'] )							// aria-label parameter
									, 'value'    => $field['value'] 										// Some Value from optins array that selected by default
									, 'selected' => ( ( 'On' == $field['value'] ) ? true : false )			// Selected or not
				...
				?><legend class="screen-reader-text"><span><?php echo wp_kses_post( $field['title'] ); ?></span></legend>
					<label  class="wpbc-form-checkbox" for="<?php echo esc_attr( $field_name ); ?>">
						<input  type="checkbox"
							...
							<?php echo self::description_static( $field, $field['description_tag'] ); ?><?php
		}
