...
trait BladeOneCache
{
    ...
    public function writeCacheLog($txt, $nivel) : void
    {
        if (!$this->cacheLog) {
            return;
            // if there is not a file assigned then it skips saving.
        }
     ...
     * @return string|null
     */
    private function cacheUniqueGUID($serialize = \true) : ?string
    {
    ...
    public function cacheExpired($templateName, $id, $cacheDuration) : int
    {
        if ($this->getMode() & 1) {
            return 2;
            // forced mode, hence it always expires. (fast mode is ignored).
        }
