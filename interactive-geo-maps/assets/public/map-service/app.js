...
geocluster.prototype._cluster = function (
  elements,
  bias,
  defaults,
  tooltipTemplate
...
iMapsRouter.getGeoFiles = function (regionClicked) {
  ...
  geoFiles.src =
    "https://cdn.amcharts.com/lib/4/geodata/" + urlappend + regionID + "Low.js";
    ...
    if (
      ...
      fullData[index].overlay.forEach(function (odata, ind) {
        ...
        if (
          ...
          fullData[index].overlay[ind].include =
            iMapsModel.prepareOverlayInclude(odata);
        }
...
iMapsModel.prepareMultiGeoLine = function (data) {
  if (Array.isArray(data.lines) && data.lines.length) {
    ...
    data.lines.map(function (line) {
      ...
      line.arrow =
        typeof line.arrow !== "undefined"
          ? iMapsManager.bool(line.arrow)
          : false;
...
iMapsModel.prepareEntriesData = function (data) {
  ...
  if (Array.isArray(data.regions) && data.regions.length) {
    data.regions.map(function (region) {
      ...
      if (
        typeof data.regionActiveState !== "undefined" &&
        data.regionActiveState.enabled === "1"
      ) {
      ...
      if (
        ...
        if (data.regionLabels.customSource.includes(".")) {
          region.autoLabel = data.regionLabels.customSource
            .split(".")
      ...
      if (data.heatMapRegions && data.heatMapRegions.enabled === "1") {
        ...
        if (data.heatMapRegions.source.includes(".")) {
          region.heatMapRegionRef = data.heatMapRegions.source
            .split(".")
            .reduce(function (obj, i) {
              if (obj[i]) {
                return obj[i];
      ...
      region.tooltipContent = iMapsModel.prepareTooltip(
      ...
      if (
        region.value &&
        _typeof(region.value) !== undefined &&
        region.value !== ""
      ) {
  ...
  if (Array.isArray(data.roundMarkers) && data.roundMarkers.length) {
    data.roundMarkers.map(function (marker) {
      ...
      if (
        data.roundMarkersMobileSize &&
        parseInt(data.roundMarkersMobileSize) !== 100
      ) {
        if (window.innerWidth <= 780) {
          marker.radius =
            (parseFloat(marker.radius) *
              parseFloat(data.roundMarkersMobileSize)) /
            100;
        }
      }
      ...
      if (data.heatMapMarkers && data.heatMapMarkers.enabled === "1") {
        ...
        if (data.heatMapMarkers.source.includes(".")) {
          marker.heatMapMarkerRef = data.heatMapMarkers.source
            .split(".")
            .reduce(function (obj, i) {
              if (obj[i]) {
                return obj[i];
      ...
      if (
        marker.value &&
        _typeof(marker.value) !== undefined &&
        marker.value !== ""
      ) {
      ...
      marker.tooltipContent = iMapsModel.prepareTooltip(
    ...
    data.roundMarkers = data.roundMarkers.filter(function (marker) {
      return (
        typeof marker.latitude !== "undefined" &&
        marker.latitude !== "" &&
        marker.name !== ""
      );
    }); // heatmap with dot notation
  ...
  if (Array.isArray(data.iconMarkers) && data.iconMarkers.length) {
    data.iconMarkers.map(function (marker) {
      ...
      if (
        data.iconMarkersMobileSize &&
        parseInt(data.iconMarkersMobileSize) !== 100
      ) {
        if (window.innerWidth <= 780) {
          marker.scale =
            (parseFloat(marker.scale) *
              parseFloat(data.iconMarkersMobileSize)) /
            100;
        }
      }
  ...
  if (Array.isArray(data.imageMarkers) && data.imageMarkers.length) {
    data.imageMarkers.map(function (marker) {
      ...
      if (
        data.imageMarkersMobileSize &&
        parseInt(data.imageMarkersMobileSize) !== 100
      ) {
        if (window.innerWidth <= 780) {
          marker.size =
            (parseFloat(marker.size) *
              parseFloat(data.imageMarkersMobileSize)) /
            100;
        }
      }
  ...
  if (Array.isArray(data.labels) && data.labels.length) {
    data.labels.map(function (label) {
      ...
      if (data.labelsMobileSize && parseInt(data.labelsMobileSize) !== 100) {
        if (window.innerWidth <= 780) {
          label.fontSize =
            (parseInt(label.fontSize) * parseInt(data.labelsMobileSize)) / 100;
          label.size = label.fontSize;
        }
      }

      label.tooltipContent = iMapsModel.prepareTooltip(
...
iMapsModel.prepareExcludeIncludes = function (data) {
  if (
    typeof data.onlyIncludeActive !== "undefined" &&
    parseInt(data.onlyIncludeActive) === 1
  ) {
    data.include = [];
    data.exclude = [];
...
iMapsModel.prepareColor = function (data) {
  ...
  var colorFields = [
    ...
    createGradient = function createGradient(data) {
      ...
      gradientType =
        typeof igmGradientType !== "undefined"
          ? igmGradientType
          : "LinearGradient";
      gradientOffset =
        typeof igmGradientOffset !== "undefined" &&
      ...
      //rotation
      gradient.rotation =
        typeof igmGradientRotation !== "undefined" ? igmGradientRotation : 0;
      return gradient;
    },
    ...
    checkColor = function checkColor(key, obj) {
      if (colorFields.includes(key)) {
        ...
        // by setting an almost transparent colour
          ...
          key === "hover" &&
          (obj[key] === "transparent" || obj[key].endsWith(",0)"))
        ) {
...
iMapsManager.addMap = function (index) {
  ...
  if (
    ...
    aspRatioContainer.style.paddingTop =
      String(data.visual.paddingTopMobile) + "%";
  } else {
  ...
  if (data.exportMenu && im.bool(data.exportMenu.enable)) {
    ...
    map.exporting.menu.align = data.exportMenu.align
      ? data.exportMenu.align
      : "right";
  ...
  if (data.legend && im.bool(data.legend.enabled)) {
    ...
    if (
      typeof data.legend.style !== "undefined" &&
      data.legend.style !== "default"
    ) {
  ...
  if (data.customLegend && im.bool(data.customLegend.enabled)) {
    var customLegendType =
      typeof data.customLegend.type !== "undefined"
        ? data.customLegend.type
        : "internal";
    ...
    if (
      ...
      if (
        typeof data.customLegend.style !== "undefined" &&
        data.customLegend.style !== "default"
      ) {
      ...
      customLegend.itemContainers.template.cursorOverStyle =
        am4core.MouseCursorStyle["default"];
      customLegend.labels.template.truncate = false;
      customLegend.labels.template.wrap = true; // positioing

      if (
        data.customLegend.position === "top" ||
        data.customLegend.position === "bottom"
      ) {
  ...
  if (Array.isArray(data.roundMarkers) && data.roundMarkers.length) {
    ...
    if (data.clusterMarkers && im.bool(data.clusterMarkers.enabled)) {
      ...
      clusterSeries[id].zoomLevels[data.clusterMarkers.zoomLevel] =
        markerSeries;
...
iMapsManager.handleZoom = function (id) {
  ...
  if (
    ...
    parseFloat(data.viewport.zoomLevel) >= 1
  ) {
  ...
  if (typeof data.viewport !== "undefined" && data.viewport.offset) {
    ...
    if (
      data.viewport.offset.longitude !== "" &&
      data.viewport.offset.longitude !== "0"
    ) {
      defaultOffset = false;
    ...
    if (
      data.viewport.offset.latitude !== "" &&
      data.viewport.offset.latitude !== "0"
    ) {
      defaultOffset = false;
  ...
  if (data.projection !== "Orthographic") {
    map.seriesContainer.draggable = data.zoom
      ? im.bool(data.zoom.draggable)
      : false;
    map.seriesContainer.resizable = data.zoom
      ? im.bool(data.zoom.draggable)
      : false; // don't zoom out to center
  ...
  map.events.on("zoomlevelchanged", function (ev) {
    ...
    if (
      isDrill &&
      typeof data.alwaysKeepBase !== "undefined" &&
      im.bool(data.alwaysKeepBase)
    ) {
      keepBase = true;
    }
      ...
      Object.keys(clusterSeries).forEach(function (key) {
        ...
        if (isDrill || hasLiveFilter) {
          if (
            ...
            (hasLiveFilter &&
              keepBase &&
              !im.bool(clusterSeries[key].overlay)) ||
...
iMapsManager.latlongOffsetFix = function (data, mapObj, defaultZoom) {
  ...
  if (
    mapsFixInclude.includes(mapSelected) &&
    data.exclude &&
    !data.exclude.includes("RU")
  ) {
...
iMapsManager.pushSeries = function (id, data) {
  ...
  if (Array.isArray(data.roundMarkers) && data.roundMarkers.length) {
    ...
    if (data.clusterMarkers && im.bool(data.clusterMarkers.enabled)) {
      ...
      clusterSeries[data.id].zoomLevels[data.clusterMarkers.zoomLevel] =
        markerSeries; // we setup the main index series (zoom=1) to be visible
      // when doing it inside setupClusters function, there was a bug
...
iMapsManager.pushRegionSeries = function (id, data, groupHover) {
  ...
  regionSeries.name =
    data.regionLegend && data.regionLegend.title !== ""
      ? data.regionLegend.title
      : data.title;
  regionSeries.hiddenInLegend = data.regionLegend
    ? !im.bool(data.regionLegend.enabled)
    : true; // if it's a base series

  // override hide in legend for scenarios where there are grouped entries for example
  if (
    data.regionLegend &&
    data.regionLegend.enabled === "onlyGroups" &&
    groupHover
  ) {
  ...
  if (
    data.regionLegend &&
    data.regionLegend.enabled === "ignoreGroups" &&
    groupHover
  ) {
    regionSeries.hiddenInLegend = true;
  }
  ...
  if (data.regionLabels && im.bool(data.regionLabels.source)) {
    ...
    var background =
      typeof igmLabelsBackground !== "undefined" ? igmLabelsBackground : false;
    ...
    if (
      data.regionLabels.mobileFontSize &&
      parseInt(data.regionLabels.mobileFontSize) !== 100
    ) {
      if (window.innerWidth <= 780) {
        labelTemplate.fontSize =
          (parseInt(labelTemplate.fontSize) *
            parseInt(data.regionLabels.mobileFontSize)) /
          100;
    ...
    var regionLabelCustomCoordinates = im.isJSON(
      data.regionLabels.regionLabelCustomCoordinates
    )
    ...
    regionSeries.events.on(event, function () {
      regionSeries.mapPolygons.each(function (polygon) {
        ...
        if (
          data.map !== "custom" &&
          typeof polygon.visualLatitude === "undefined"
        ) {
          return;
        }
        ...
        label.tooltipPosition = im.bool(data.tooltip.fixed)
          ? "fixed"
  ...
  if (
    typeof data.externalDropdown !== "undefined" &&
    im.bool(data.externalDropdown.enabled)
  ) {
    regionSeries.calculateVisualCenter = true;
...
iMapsManager.pushRoundMarkerSeries = function (id, data) {
  ...
  if (Array.isArray(data.roundMarkers) && data.roundMarkers.length) {
    ...
    circle.nonScaling =
      typeof igmRoundMarkersNonScaling !== "undefined"
    ...
    if (data.heatMapMarkers && im.bool(data.heatMapMarkers.enabled)) {
      ...
      if (
        typeof data.heatMapMarkers.type !== "undefined" &&
        data.heatMapMarkers.type === "range"
      ) {
    ...
    if (data.roundMarkerLabels && im.bool(data.roundMarkerLabels.enabled)) {
      ...
      var markerLabelPosition =
        ...
        data.roundMarkerLabels.position !== ""
          ...
          : "bottom";
      markerLabel.text =
        typeof data.roundMarkerLabels.source !== "undefined" &&
...
iMapsManager.pushImageMarkerSeries = function (id, data) {
  ...
  if (Array.isArray(data.imageMarkers) && data.imageMarkers.length) {
    ...
    if (
      ...
      imageMarker.tooltipText = data.tooltip.template
        ? data.tooltip.template
        : "{tooltipContent}";
      imageMarker.tooltipHTML = data.tooltip.template
        ? data.tooltip.template
        : "{tooltipContent}";
    } // Set property fields
...
iMapsManager.pushIconMarkerSeries = function (id, data) {
  ...
  if (Array.isArray(data.iconMarkers) && data.iconMarkers.length) {
    ...
    if (data.iconMarkerLabels && im.bool(data.iconMarkerLabels.enabled)) {
      var markerLabel = markerSeriesTemplate.createChild(am4core.Label);
      markerLabel.text =
        typeof data.iconMarkerLabels.source !== "undefined" &&
...
iMapsManager.pushLineSeries = function (id, data) {
  ...
  if (Array.isArray(data.lines) && data.lines.length) {
    ...
    lineSeries.name =
      data.linesLegend && data.linesLegend.title !== ""
        ? data.linesLegend.title
        : data.title;
    lineSeries.hiddenInLegend = data.linesLegend
      ? !im.bool(data.linesLegend.enabled)
      : false;
    ...
    lineSeries.mapLines.template.arrow.propertyFields.disabled =
      "arrowDisabled";
...
iMapsManager.pushLabelSeries = function (id, data) {
  ...
  if (Array.isArray(data.labels) && data.labels.length) {
    ...
    labelSeries.name =
      data.labelsLegend && data.labelsLegend.title !== ""
        ? data.labelsLegend.title
        : data.title;
    labelSeries.hiddenInLegend = data.labelsLegend
      ? !im.bool(data.labelsLegend.enabled)
      : false;
    ...
    label.nonScaling =
      typeof igmLabelsNonScaling !== "undefined" ? igmLabelsNonScaling : true;
...
iMapsManager.setupTooltip = function (id, series, data, marker) {
  ...
  // box-shadow
  if (
    typeof tooltip.customShadow !== "undefined" &&
    im.bool(tooltip.customShadow)
  ) {
...
iMapsManager.setupHitEvents = function (id, ev) {
  var im = this,
    ...
    zoomCluster = data.clusterMarkers
      ? parseFloat(data.clusterMarkers.zoomLevel)
      : 1,
    currentRegion,
    mapFunction,
    container = document.getElementById(data.container),
  ...
  if (
    ...
    if (
      Array.isArray(
        iMapsManager.maps[id].seriesById[parseInt(dataContext.content)]
      )
    ) {
      currentRegion =
        iMapsManager.maps[id].seriesById[parseInt(dataContext.content)];
...
iMapsManager.zoomToRegion = function (ev, id) {
  ...
  if (seriesType == "MapImage") {
    ...
    if (dataContext.id === "asia" && !data.map.startsWith("continents")) {
      ev.target.series.chart.deltaLongitudeOriginal =
        ev.target.series.chart.deltaLongitude;
      ...
      dataContext.id === "northAmerica" &&
      data.map.startsWith("continents")
    ) {
      ev.target.series.chart.zoomToGeoPoint(
        {
          latitude: 55.5,
      ...
      if (
        typeof ev.target.series.chart.deltaLongitudeOriginal !== "undefined"
      ) {
        ev.target.series.chart.deltaLongitude =
          ev.target.series.chart.deltaLongitude;
      }
...
iMapsManager.setupHoverEvents = function (id, ev) {
  ...
  if (
    ...
    (typeof iMaps.maps[id].mapClicked === "undefined" ||
      iMaps.maps[id].mapClicked === false)
  ) {
    iMapsManager.select(
      id,
      dataContext.id,
  ...
  if (
    im.bool(dataContext.triggerRegionHover) &&
    dataContext.val &&
    dataContext.val !== ""
  ) {
...
iMapsManager.groupHit = function (id, ev) {
  ...
  if (
    ...
    ev.target.dataItem.dataContext.activeState
  ) {
    iMaps.maps[id].mapClicked = true;
  }
...
iMapsManager.groupHover = function (id, ev) {
  ...
  if (
    ev.target.dataItem.dataContext.action &&
    ev.target.dataItem.dataContext.action != "none"
  ) {
    ev.target.cursorOverStyle = am4core.MouseCursorStyle.pointer;
        ...
        if (elID.includes(",")) {
          ...
          group.forEach(function (rxid, indx) {
            ...
            if (typeof select !== "undefined" && select) {
              ...
              if (
                select.dataItem.dataContext.originalID !== elID &&
                !customRegionGroup
              ) {
                return;
              }
              ...
              if (indx === 0 && showTooltip) {
                ...
                if (forceFixedTooltip) {
                  defaultTooltipPosition =
                    ...
                    typeof select.tooltipPosition !== "undefined"
              ...
              if (
                data.regionActiveState &&
                im.bool(data.regionActiveState.enabled)
              ) {
...
iMapsManager.setupRangeHeatMap = function (series, id, data) {
  ...
  if (seriesType === "MapImageSeries") {
    if (
      !Array.isArray(data.heatMapMarkers.range) ||
      data.heatMapMarkers.range.length === 0
    ) {
      return;
    }
    ...
    if (
      !Array.isArray(data.heatMapRegions.range) ||
      data.heatMapRegions.range.length === 0
    ) {
      return;
    } // reorder ranges by value
...
iMapsManager.setupHeatMap = function (series, id, data) {
  ...
  if (seriesType === "MapImageSeries") {
    ...
    if (
      typeof data.heatMapRegions.type !== "undefined" &&
      data.heatMapRegions.type === "range"
    ) {
      im.setupRangeHeatMap(series, data.id, data);
      return;
    }
  ...
  if (im.bool(dataSource.legend)) {
    ...
    legendContainer.align =
      typeof dataSource.legendAlign !== "undefined"
        ? dataSource.legendAlign
        : "right";
    legendContainer.valign =
      typeof dataSource.legendValign !== "undefined"
        ? dataSource.legendValign
        : "bottom";
...
iMapsManager.drillTo = function (id, ev, currentRegion, customMap) {
  ...
  (opacity =
    typeof igmDrilldownBaseMapOpacity !== "undefined"
...
iMapsManager.drillDown = function (id, ev) {
  ...
  if (!mapName || !isNaN(targetID)) {
    ...
    if (
      _typeof(clicked) === undefined ||
      clicked.action !== "igm_display_map"
    ) {
      return false;
    }
  }
  ...
  if (ev.target.polygon) {
    ...
    checkSeries.forEach(function (ser) {
      if (
        ...
        if (
          Array.isArray(
            iMapsManager.maps[id].seriesById[parseInt(clicked.content)]
          )
        ) {
          seriesExists = true;
          currentRegion =
        ...
        if (eID.includes(",")) {
          ...
          group.forEach(function (rxid, indx) {
            ...
            if (hover) {
              if (forceFixedTooltip) {
                defaultTooltipPosition =
                  typeof hover.tooltipPosition !== "undefined"
                    ? hover.tooltipPosition
                    : "fixed";
                defaultShowTooltipOn =
                  typeof hover.showTooltipOn !== "undefined"
                    ? hover.showTooltipOn
                    : "hover";
...
iMapsManager.setupClusters = function (data, id, overlay) {
  ...
  if (Array.isArray(data.roundMarkers)) {
    biasLevels.forEach(function (item, index) {
      ...
      map.clusterSeries[currentMap].zoomLevels[zoomLevels[index]] =
        markerSeries;

      if (index === 0 && !overlay) {
        // this somehow interfeers with the clusters after home button is clicked
        // im.maps[id].allBaseSeries.push(markerSeries);
      }
...
iMapsManager.bool = function (string) {
  var bool =
    Number(string) === 0 || string === "false" || typeof string === "undefined"
      ? false
      : true;
  return bool;
};
...
iMaps.init = function (hold) {
  ...
  if (typeof am4core === "undefined") {
    ...
    let oxygen = document.querySelector(
  ...
  am4core.ready(function () {
    ...
    if (
      ...
      (iMapsData.options.animations === "1" ||
        iMapsData.options.animations === true)
    ) {
