...
if ( function_exists( __NAMESPACE__ . '\igmfreemiusinit' ) ) {
	...
	if ( ! function_exists( __NAMESPACE__ . '\igmfreemiusinit' ) ) {
			...
			if ( ! isset( $igmfreemiusinit ) ) {
				...
				$igmfreemiusinit = fs_dynamic_init(
					array(
						...
						'secret_key'          => 'sk_#*ncIu!5g.noRn=qM1##FFh;6+.r@',
					)
				);
			}
		...
		igmfreemiusinit()->add_filter( 'show_admin_notice', 'Saltus\WP\Plugin\Saltus\InteractiveMaps\igm_remove_trial_notice', 10, 2 );
		...
		*/
		...
		add_action(
			...
			function () use ( $framework ) {
				$plugin = new Core( 'interactive-geo-maps', '1.6.18', __FILE__, $framework );
				$plugin->init();
			}
		);
	}
