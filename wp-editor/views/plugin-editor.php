...
<div class="wrap">
				...
				<?php
					if ( is_plugin_active( $data['plugin'] ) ) {
						if ( is_writable( $data['real_file'] ) ) {
							echo __( 'Editing <span class="current_file">', 'wp-editor' ) . esc_html( $data['file'] ) . __( '</span> (active)', 'wp-editor' );
						}
						else {
							echo __( 'Browsing <span class="current_file">', 'wp-editor' ) . esc_html( $data['file'] ) . __( '</span> (active)', 'wp-editor' );
						}
					} else {
						if ( is_writable( $data['real_file'] ) ) {
							echo __( 'Editing <span class="current_file">', 'wp-editor' ) . esc_html( $data['file'] ) . __( '</span> (inactive)', 'wp-editor' );
						}
						else {
							echo __( 'Browsing <span class="current_file">', 'wp-editor' ) . esc_html( $data['file'] ) . __( '</span> (inactive)', 'wp-editor' );
						}
					}
				?>
			</h3>
			...
			<div id="plugin-upload-files">
				<?php if ( is_writable( $data['real_file'] ) ): ?>
					<form enctype="multipart/form-data" id="plugin_upload_form" method="POST">
							...
							<input type="hidden" name="current_plugin_root" value="<?php echo esc_html( $data['current_plugin_root'] ); ?>" id="current_plugin_root" />
	...
	<form name="template" id="template_form" action="" method="post" class="ajax-editor-update" style="float:left width:auto;overflow:hidden;position:relative;">
		...
		<div>
			...
			<input type="hidden" name="scroll_to" id="scroll_to" value="<?php echo esc_attr( $data['scroll_to'] ); ?>" />
			<input type="hidden" name="content-type" id="content-type" value="<?php echo esc_attr( $data['content-type'] ); ?>" />
			<?php
				$pathinfo = pathinfo( $data['plugin'] );
			?>
			<input type="hidden" name="extension" id="extension" value="<?php echo esc_attr( $pathinfo['extension'] ); ?>" />
		</div>
		<p class="submit">
	...
	<script type="text/javascript">
		(function( $){
			$(document).ready(function(){
				...
				runCodeMirror( '<?php echo esc_js( $pathinfo["extension"] ); ?>' );
				$( '.ajax-loader' ).hide();
