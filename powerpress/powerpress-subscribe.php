...
function powerpresssubscribe_get_settings($ExtraData, $detect_category=true)
{
	...
	if( !empty($category_id) && $ExtraData['subscribe_type'] == 'category' )
	{
		...
		if( !empty($Settings) )
		{
            ...
            $Settings['subscribe_feature_youtube'] = (isset($GeneralSettings['subscribe_feature_youtube']) ? $GeneralSettings['subscribe_feature_youtube'] : false );
            ...
            $Settings['subscribe_feature_youtube_shortcode'] = (isset($GeneralSettings['subscribe_feature_youtube_shortcode']) ? $GeneralSettings['subscribe_feature_youtube_shortcode'] : true );
            ...
            $Settings['subscribe_feature_youtube_sidebar'] = (isset($GeneralSettings['subscribe_feature_youtube_sidebar']) ? $GeneralSettings['subscribe_feature_youtube_sidebar'] : false );
	...
	if( $ExtraData['subscribe_type'] == 'post_type' )
	{
		if( !empty($GeneralSettings['posttype_podcasting']) )
		{
			...
			switch( $post_type )
			{
				...
				default: {
					$SettingsArray = get_option('powerpress_posttype_'.$post_type, array());
					$Settings = false;
	...
	if( !empty($Settings) )
	{
        ...
        if( !empty($FeedSettings['premium']) ) {
            ...
            $Settings['subscribe_feature_youtube'] = false;
            ...
            $Settings['subscribe_feature_youtube_shortcode'] = false;
            ...
            $Settings['subscribe_feature_youtube_sidebar'] = false;
        }
...
function powerpressplayer_link_subscribe_pre($content, $media_url, $ExtraData = array() )
{
    ...
    if( !empty($SubscribeSettings['subscribe_feature_youtube']) && !empty($SubscribeSettings['youtube_url']) )
    {
        $SubscribeSettings['youtube_url'] = trim($SubscribeSettings['youtube_url']);
        $links_array[] = "<a href=\"".  esc_attr($SubscribeSettings['youtube_url'] ) ."\" class=\"powerpress_link_subscribe powerpress_link_subscribe_youtube\" target=\"_blank\" title=\"". __('Subscribe on Youtube Music', 'powerpress') ."\" rel=\"nofollow\">". __('Youtube Music','powerpress') ."</a>".PHP_EOL_WEB;
    }
...
function powerpress_do_subscribe_widget($settings, $PowerPressSettings)
{
	...
	$link_counter = 0;
			if( !empty($settings['itunes_url']) &&  !empty($settings['subscribe_feature_apple_shortcode']) ) {
			    $link_counter++;
				$html .= '<a href="'.  esc_url( $settings['itunes_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-itunes" target="_blank" title="'.  esc_attr( __('Subscribe on Apple Podcasts', 'powerpress') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('Apple Podcasts', 'powerpress') ) .'</a>';
			}

            if(  !empty($settings['subscribe_feature_spotify_shortcode']) && !empty($settings['spotify_url']) )
            {
                $link_counter++;
                $html .= '<a href="'.  esc_url( $settings['spotify_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-spotify" target="_blank" title="'.  esc_attr( __('Subscribe on Spotify', 'powerpress') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('Spotify', 'powerpress') ) .'</a>';
            }

            if( !empty($settings['subscribe_feature_amazon_shortcode']) &&  !empty($settings['amazon_url']) ) {
                $link_counter++;
                $html .= '<a href="'.  esc_url( $settings['amazon_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-amazon" target="_blank" title="'.  esc_attr( __('Subscribe on Amazon Music', 'powerpress') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('Amazon Music', 'powerpress') ) .'</a>';
            }

            if( !empty($settings['subscribe_feature_android_shortcode']) &&  preg_match('/^(https?:\/\/)(.*)$/i', $settings['feed_url'], $matches ) ) {
                $android_url =  $matches[1] . 'subscribeonandroid.com/' . $matches[2];
                $link_counter++;
                $html .= '<a href="'.  esc_url( $android_url ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-android" target="_blank" title="'.  esc_attr( __('Subscribe on Android', 'powerpress') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('Android', 'powerpress') ) .'</a>';
            }

            if( !empty($settings['subscribe_feature_pandora_shortcode']) &&  !empty($settings['pandora_url']) ) {
                $link_counter++;
                $html .= '<a href="'.  esc_url( $settings['pandora_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-pandora" target="_blank" title="'.  esc_attr( __('Subscribe on Pandora', 'powerpress') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('Pandora', 'powerpress') ) .'</a>';
            }
            ...
            if( !empty($settings['subscribe_feature_podchaser_shortcode']) &&  !empty($settings['podchaser_url']) ) {
                $link_counter++;
                $html .= '<a href="'.  esc_url( $settings['podchaser_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-podchaser" target="_blank" title="'.  esc_attr( __('Subscribe on Podchaser', 'powerpress') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('Podchaser', 'powerpress') ) .'</a>';
            }

            if( !empty($settings['subscribe_feature_gaana_shortcode']) &&  !empty($settings['gaana_url']) ) {
                $link_counter++;
                $html .= '<a href="'.  esc_url( $settings['gaana_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-gaana" target="_blank" title="'.  esc_attr( __('Subscribe on Gaana', 'powerpress') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('Gaana', 'powerpress') ) .'</a>';
            }

            if( !empty($settings['subscribe_feature_pcindex_shortcode']) &&  !empty($settings['pcindex_url']) ) {
                $link_counter++;
                $html .= '<a href="'.  esc_url( $settings['pcindex_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-pcindex" target="_blank" title="'.  esc_attr( __('Subscribe on Podcast Index', 'powerpress') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('Podcast Index', 'powerpress') ) .'</a>';
            }

            if( !empty($settings['subscribe_feature_email_shortcode']) &&  preg_match('/^(https?:\/\/)(.*)$/i', $settings['feed_url'], $matches ) ) {
                $link_counter++;
                $email_url =  $matches[1] . 'subscribebyemail.com/' . $matches[2];
                $html .= '<a href="'.  esc_url( $email_url ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-email" target="_blank" title="'.  esc_attr( __('Subscribe by Email', 'powerpress') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('by Email', 'powerpress') ) .'</a>';
            }

            if( !empty($settings['subscribe_feature_tunein_shortcode']) &&  !empty($settings['tunein_url']) )
            {
                $link_counter++;
                $html .= '<a href="'.  esc_url( $settings['tunein_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-tunein" target="_blank" title="'.  esc_attr( __('Subscribe on TuneIn', 'powerpress') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('TuneIn', 'powerpress') ) .'</a>';
            }

            if( !empty($settings['subscribe_feature_deezer_shortcode']) &&  !empty($settings['deezer_url']) ) {
                $link_counter++;
                $html .= '<a href="'.  esc_url( $settings['deezer_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-deezer" target="_blank" title="'.  esc_attr( __('Subscribe on Deezer', 'powerpress') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('Deezer', 'powerpress') ) .'</a>';
            }

            if( !empty($settings['subscribe_feature_anghami_shortcode']) &&  !empty($settings['anghami_url']) ) {
                $link_counter++;
                $html .= '<a href="'.  esc_url( $settings['anghami_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-anghami" target="_blank" title="'.  esc_attr( __('Subscribe on Anghami', 'powerpress') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('Anghami', 'powerpress') ) .'</a>';
            }

            if( !empty($settings['subscribe_feature_youtube_shortcode']) &&  !empty($settings['youtube_url']) ) {
                $link_counter++;
                $html .= '<a href="'.  esc_url( $settings['youtube_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-youtube" target="_blank" title="'.  esc_attr( __('Subscribe on Youtube Music', 'powerpress') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('Youtube Music', 'powerpress') ) .'</a>';
            }
            ...
            if (!empty($settings['subscribe_feature_rss_shortcode'])) {
                $link_counter++;
                $html .= '<a href="'.  esc_url( $settings['feed_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-rss" target="_blank" title="'.  esc_attr( __('Subscribe via RSS', 'powerpress') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('RSS', 'powerpress') ) .'</a>';
            }
...
*/
	...
	$html .= '<div class="pp-ssb-widget pp-ssb-widget-modern' . $important_tags_class .'">';
        ...
        if(  !empty($settings['subscribe_feature_youtube_sidebar']) && !empty($settings['youtube_url']) ) {
            $settings['youtube_url'] = trim($settings['youtube_url']);
            $html .= '<a href="'.  esc_url( $settings['youtube_url'] ) .'" class="pp-ssb-btn'.$settings['modern_style'].' '.$settings['modern_direction'].' pp-ssb-youtube" target="_blank" title="'.  esc_attr( __('Subscribe on Youtube Music', 'powerpress') ) .'"><span class="pp-ssb-ic"></span><span class="pp-ssb-text">'.  esc_html( __('Youtube Music', 'powerpress') ) .'</span></a>';
        }
