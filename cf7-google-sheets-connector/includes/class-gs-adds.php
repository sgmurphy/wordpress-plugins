...
class GS_Connector_Adds {
   ...
   public function display_auth_expired_adds_block() {
      ...
      if($auth_expired == "true"){
         if ( empty( $get_display_interval ) || current_time( 'timestamp' ) > $adds_interval_timestamp ) {
          add_action( 'admin_notices', array( $this, 'show_gs_auth_expired_adds' ) );
        }   
     }
  ...
  public function set_auth_expired_adds_interval() {
   ...
   public function show_gs_auth_expired_adds() {
      ...
      $review_text = '<div class="gs-auth-expired-adds-notice ">';
      $review_text .= 'CF7 Google Sheet Connector FREE is installed but it is not connected to your Google account, so you are missing out the submission entries.
         <a href="admin.php?page=wpcf7-google-sheet-config&tab=integration" target="_blank">Connect now</a>. It only takes 30 seconds!. 
          ';
      $review_text .= '<ul class="review-rating-list">';
      $review_text .= '<li><a href="javascript:void(0);" class="set-auth-expired-adds-interval" title="Nope, may be later">Nope, may be later.</a></li>';
      $review_text .= '<li><a href="javascript:void(0);" class="close-auth-expired-adds-interval" title="Dismiss">Dismiss</a></li>';
      $review_text .= '</ul>';
      $review_text .= '<input type="hidden" name="gs_auth_expired_adds_ajax_nonce" id="gs_auth_expired_adds_ajax_nonce" value="' . $ajax_nonce . '" /></div>';
