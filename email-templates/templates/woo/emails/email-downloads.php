							...
							case 'download-file':
								...
								if ( true == $responsive_check ) {
									if ( ! empty( $download['access_expires'] ) ) {
										?>
										<p style="margin-bottom:0;"><small><?php esc_html_e( 'Expires:', 'woocommerce' ); ?> <time datetime="<?php echo esc_attr( date( 'Y-m-d', strtotime( $download['access_expires'] ) ) ); ?>" title="<?php echo esc_attr( strtotime( $download['access_expires'] ) ); ?>"><?php echo esc_html( date_i18n( get_option( 'date_format' ), strtotime( $download['access_expires'] ) ) ); ?></time></small></p>
										<?php
									} else {
										echo '<p style="margin-bottom:0;"><small>' . esc_html__( 'Expires: Never', 'woocommerce' ) . '</small></p>';
									}
								}
								break;
