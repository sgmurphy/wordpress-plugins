...
class IRP_Form {
    ...
    public function help($name) {
        ...
        $label=$irp->Lang->L($k);
        ?>
        <img src="<?php echo esc_url(IRP_PLUGIN_IMAGES).'question-mark.png'?>" class="irp-help" id="<?php echo esc_attr($name)?>-help" alt="<?php echo esc_attr($label)?>" />
    <?php
    }
    public function label($name, $args='') {
        ...
        $aClass='';
        ?>
        <label for="<?php echo esc_attr($for)?>" <?php echo wp_kses_post($otherText)?> >
    ...
    public function textarea($name, $value='', $args=NULL) {
        ...
        ?>
            <textarea dir="ltr" dirname="ltr" id="<?php echo esc_attr($name) ?>" name="<?php echo esc_attr($name)?>" <?php echo wp_kses_post($other)?> ><?php echo esc_html($value) ?></textarea>
        <?php
        $this->newline=FALSE;
    ...
    public function text($name, $value='', $options=NULL) {
        ...
        $this->leftInput($name, $args);
        ?>
            <input type="<?php echo esc_attr($type)?>" id="<?php echo esc_attr($name) ?>" name="<?php echo esc_attr($name) ?>" value="<?php echo esc_attr($value) ?>" <?php echo wp_kses_post($other)?> />
        <?php
        $this->rightInput($name, $args);
    }
    ...
    public function hidden($name, $value='', $args=NULL) {
        ...
        $other=$this->getTextArgs($args, $defaults);
        ?>
        <input type="hidden" id="<?php echo esc_attr($name) ?>" name="<?php echo esc_attr($name) ?>" value="<?php echo esc_attr($value) ?>" <?php echo wp_kses_post($other)?> />
    <?php }
    ...
    public function select($name, $value, $options, $multiple=FALSE, $args=NULL) {
        ...
        $this->leftInput($name, $args);
        ?>
            <select id="<?php echo esc_attr($name) ?>" name="<?php echo esc_attr($name)?><?php echo (esc_attr($multiple) ? '[]' : '')?>" <?php echo (esc_attr($multiple) ? 'multiple' : '')?> <?php echo wp_kses_post($other)?> >
                <?php
                foreach($options as $v) {
                    $style='';
                    if(isset($v['style'])) {
                    ...
                    ?>
                    <option value="<?php echo esc_attr($v[$this->selectIdField])?>" <?php echo wp_kses_post($selected)?> style="<?php echo wp_kses_post($style)?>"><?php echo esc_html($v['name'])?></option>
                <?php } ?>
            </select>
        <?php
        $this->rightInput($name, $args);
    ...
    public function submit($value='', $args=NULL) {
        ...
        $this->newline();
        ?>
            <input type="submit" class="button-primary irp-button irp-submit" value="<?php $irp->Lang->P($value)?>" <?php echo wp_kses_post($other)?>/>
    <?php }
    ...
    public function delete($id, $action='delete', $args=NULL) {
        ...
        $other=$this->getTextArgs($args, $defaults);
        ?>
            <input type="button" class="button irp-button" value="<?php $irp->Lang->P('Delete?')?>" onclick="if (confirm('<?php $irp->Lang->P('Are you sure you want to delete?')?>') ) window.location='<?php echo esc_url(IRP_TAB_BUILDER_URI)?>&action=<?php echo esc_attr($action)?>&id=<?php echo esc_attr($id) ?>&amp;irp_nonce=<?php echo esc_attr(wp_create_nonce('irp_delete')); ?>';" <?php echo wp_kses_post($other)?> />
            &nbsp;
        <?php
    }
    ...
    public function checkbox($name, $current=1, $value=1, $args=NULL) {
        ...
        */
        ...
        $this->leftInput($name, $args);
        ?>
        <input type="<?php echo esc_attr($type) ?>" name="<?php echo esc_attr($name)?>" value="<?php echo esc_attr($value)?>" <?php echo($current==$value ? 'checked="checked"' : '') ?> <?php echo wp_kses_post($other)?> >
        <?php
        $this->rightInput($name, $args);
    ...
    public function checkText($nameActive, $nameText, $value) {
        ...
        ?>
        <div id="<?php echo esc_attr($nameText)?>Text" style="float:left;">
            <?php
            $prev=$this->labels;
            $this->labels=FALSE;
            $args=array();
    ...
    public function checkSelect($nameActive, $nameArray, $value, $values) {
        global $irp;
        ?>
        <div id="<?php echo esc_attr($nameArray)?>Box" style="float:left;">
            <?php
            $args=array('class'=>'irp-hideShow irp-checkbox'
                , 'irp-hideIfTrue'=>'false'
            ...
            $this->checkbox($nameActive, $value, 1, $args);
            if(TRUE) { ?>
                <div id="<?php echo esc_attr($nameArray)?>Tags" style="float:left;">
                    <?php
                    $prev=$this->labels;
                    $this->labels=FALSE;
    ...
    public function tag($overridePremium=FALSE) {
        ...
        */
        ...
        $tagText='NEW!';
        ?>
        <div style="float:left;" class="irp-tag <?php echo esc_attr($tagClass)?>"><?php echo wp_kses_post($tagText)?></div>
    <?php
    }
}