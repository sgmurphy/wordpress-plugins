...
if ( ! class_exists( 'WP_Analytify_Settings' ) ) {
	...
	class WP_Analytify_Settings {
		 ...
		 * @param array $args settings field args
		 */
		function callback_email_receivers( $args ) {
			$current_values = $this->get_option( $args['id'], $args['section'], $args['std'] );
			

			// Backward compatibility added when emails where added in a single fields separated by commas.
			...
			// To remove the serialized array's keys.
			$current_values = array_values( $current_values );

			$count = 0;
			
			$html = '<table id="wp-analytify-email-table">
			<thead>
				<tr>
				<th>' . __( 'Name', 'wp-analytify' ) . '</th>
				<th>' . __( 'Email Address', 'wp-analytify' ) . '</th>
				</tr>
			</thead>
			<tbody>';
			...
			if ( empty( $current_values ) ) {
				...
				foreach( $current_values as $current_value => $vals ) {
					...
					$html .= sprintf( '<td><input type="text" name="%1$s[%2$s]['.$count.'][name]" id="%1$s[%2$s]"" value="'.trim( esc_html( $vals['name'] ) ).'" placeholder="Name"></td>', $args['section'], $args['id'] );

					$html .= sprintf( '<td><input type="email" name="%1$s[%2$s]['.$count.'][email]" id="%1$s[%2$s]" value="'.sanitize_email( $vals['email'] ).'" placeholder="Email Address" required></td>', $args['section'], $args['id'] );
			...
			$html .= '<div class="inside">'.$args['desc'].'</div>';
			$html .= '</tbody></table><button type="button" class="button wp-analytify-add-email">Add Receiver</button>';
		...
		function callback_affiliates_repeater( $args ) {
			...
			if ( empty( $current_values ) ) {
				...
				$html .= sprintf( '<td><input type="text" class="affiliates-path" name="%1$s[%2$s]['.$count.'][path]" id="%1$s[%2$s]" placeholder="/refer/" value="" ></td>', $args['section'], $args['id'] );
				$html .= sprintf( '<td><input type="text" class="affiliates-label" name="%1$s[%2$s]['.$count.'][label]" id="%1$s[%2$s]" placeholder="loginpress link" value="" ></td>', $args['section'], $args['id'] );
				...
				foreach( $current_values as $current_value => $vals ) {
					$html .= '<tr class="single_affiliates">';
					$html .= sprintf(
						'<td><input type="text" class="affiliates-path" placeholder="/refer/" name="%1$s[%2$s][%3$s][path]" id="%1$s[%2$s]" value="%4$s"></td>',
						$args['section'],
						$args['id'],
						$count,
						htmlspecialchars( $vals['path'] )
					);

					$html .= sprintf(
						'<td><input type="text" class="affiliates-label" placeholder="loginpress link" name="%1$s[%2$s][%3$s][label]" id="%1$s[%2$s]" value="%4$s"></td>',
						$args['section'],
						$args['id'],
						$count,
						htmlspecialchars( $vals['label'] )
					);
...
<?php	} else { ?>
		...
		<div class="wrap">
			...
			<div class="wp-analytify-debug">
				...
				<div class="wp-analytify-view-error-log">
					<button id="analytify-download-diagnostic" class="button"><?php _e( 'Download Diagnostic Log', 'wp-analytify' ); ?></button>
					<a class="button" href="<?php echo admin_url( 'admin.php?page=analytify-logs' ); ?>"><?php _e( 'View Error Logs', 'wp-analytify' ); ?></a>
				</div>
			</div>

			<div class="wp-analytify-debug">
				<h3><?php esc_html_e( 'Import / Export Analytify Settings', 'wp-analytify' ); ?></h3>
				<div class="wp-analytify-import-export">
					<div class="analytify-export-container">
						<h4>Export Settings</h4>
						<p><?php _e( 'Export the Analytify settings file in JSON format.', 'wp-analytify' ); ?></p>
						<a id="analytify-export-settings" class="button" href="javascript:void(0)"><?php _e( 'Download Export File', 'wp-analytify' ); ?></a>
					</div>
					<div class="analytify-import-container">
						<h4>Import Settings</h4>
						<p><?php _e( 'Select the export file and click on Upload Import File button.', 'wp-analytify' ); ?></p>
						<p><?php _e( 'This will not include your Google authentication, you might need to authenticate again after import.', 'wp-analytify' ); ?></p>
						<form id="analytify-import-form">
							<input id="analytify-import-settings" type="file" title="Upload Import File">
							<button id="analytify-import-submit" style="submit" class="button"><?php _e( 'Upload Import File', 'wp-analytify' ); ?></button>
						</form>
						<p class="analytify-import-notice"><?php _e( 'Import completed successfully!', 'wp-analytify' ); ?></p>
						<p class="analytify-import-notice"><?php _e( 'Reloading settings page to reflect settings..', 'wp-analytify' ); ?></p>
					</div>
				</div>
			</div>
...
<?php } elseif ( 'wp-analytify-tracking' === $form['id'] AND isset( $form['accordion'] ) ) {
		...
		public function analytify_delete_cache() {
			...
			//Check for delete cache request coming from wp admin bar.
			$delete_cache_from_bar      = isset( $_GET['analytify_delete_cache_nonce'] ) && wp_verify_nonce( $_GET['analytify_delete_cache_nonce'], 'analytify_delete_cache' );
			//Check for delete cache request coming from Analytify settings.
			$delete_cache_from_settings = isset( $_POST['analytify_delete_cache_nonce'] ) && wp_verify_nonce( $_POST['analytify_delete_cache_nonce'], 'analytify_delete_cache' );
