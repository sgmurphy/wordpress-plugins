...
function rafflepress_lite_save_giveaway() {
	if ( check_ajax_referer( 'rafflepress_lite_save_giveaway' ) ) {
		...
		if ( current_user_can('unfiltered_html') && current_user_can('administrator') ) {
			// Allow unfiltered HTML for admins with unfiltered_html capability.
			$settings = wp_json_encode( $settings );
		} else {
			// Sanitize HTML for all other users.
			$settings     = wp_json_encode( $settings );
			$settings_obj = json_decode( $settings );

			if ( ! empty( $settings_obj->prizes ) ) {
				foreach ( $settings_obj->prizes as $k => $v ) {
					if ( ! empty( $v->name ) ) {
						$settings_obj->prizes[ $k ]->name = wp_strip_all_tags( $v->name );
					}
					if ( ! empty( $v->description ) ) {
						$settings_obj->prizes[ $k ]->description = wp_kses_post( $v->description );
					}
					if ( ! empty( $v->image ) ) {
						$settings_obj->prizes[ $k ]->image = esc_url( $v->image );
					}
				}
			}
