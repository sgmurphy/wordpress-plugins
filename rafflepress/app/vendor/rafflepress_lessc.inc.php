 ...
 *
 * LESS CSS compiler, adapted from http://rafflepress_lesscss.org
 *
 ...
 * by `rafflepress_lessc_parser` into a syntax tree, then it is compiled into another tree
 * representing the CSS structure by `rafflepress_lessc`. The CSS tree is fed into a
 * formatter, like `rafflepress_lessc_formatter` which then outputs CSS as a string.
 *
 * During the first compile, all values are *reduced*, which means that their
 * types are brought to the lowest form before being dump as strings. This
 * handles math equations, variable dereferences, and the like.
 *
 * The `parse` function of `rafflepress_lessc` is the entry point.
 *
 * In summary:
 *
 * The `rafflepress_lessc` class creates an instance of the parser, feeds it LESS code,
 * then transforms the resulting tree to a CSS tree. This class also holds the
 * evaluation context, such as all available mixins and variables at any given
 * time.
 *
 * The `rafflepress_lessc_parser` class is only concerned with parsing its input.
 *
 * The `rafflepress_lessc_formatter` takes a CSS tree, and dumps it to a formatted string,
 * handling things like indentation.
 */
class rafflepress_lessc {
     ...
     *
     * See rafflepress_lessc::compileProp()
     *
     */
     ...
     *
     * http://rafflepress_lesscss.org/functions/#color-operations-tint
     *
     * @return array Color
     */
     ...
     *
     * http://rafflepress_lesscss.org/functions/#color-operations-shade
     *
     * @return array Color
     */
    ...
    protected function injectVariables($args) {
        $this->pushEnv();
        $parser = new rafflepress_lessc_parser($this, __METHOD__);
    ...
    protected function makeParser($name) {
        $parser = new rafflepress_lessc_parser($this, $name);
    ...
    protected function newFormatter() {
        $className = "rafflepress_lessc_formatter_lessjs";
        if (!empty($this->formatterName)) {
            if (!is_string($this->formatterName))
                return $this->formatterName;
            $className = "rafflepress_lessc_formatter_$this->formatterName";
        }
...
// syntax tree
class rafflepress_lessc_parser {
    ...
    public function __construct($rafflepress_lessc, $sourceName = null) {
        ...
        $this->rafflepress_lessc = $rafflepress_lessc;
        ...
        if (!self::$operatorString) {
            self::$operatorString =
                '('.implode('|', array_map(array('rafflepress_lessc', 'preg_quote'),
                    array_keys(self::$precedence))).')';

            $commentSingle = rafflepress_lessc::preg_quote(self::$commentSingle);
            $commentMultiLeft = rafflepress_lessc::preg_quote(self::$commentMultiLeft);
            $commentMultiRight = rafflepress_lessc::preg_quote(self::$commentMultiRight);
     ...
     *
     * Consider the function rafflepress_lessc::keyword(). (all parse functions are
     * structured the same)
     *
     ...
     *
     * All of these parse functions are powered by rafflepress_lessc::match(), which behaves
     ...
     * avoid this, rafflepress_lessc::seek() is used to remember and set buffer positions.
     *
            ...
            if (is_null($block->type)) {
                ...
                if (!isset($block->args)) {
                    foreach ($block->tags as $tag) {
                        if (!is_string($tag) || $tag[0] != $this->rafflepress_lessc->mPrefix) {
                            $hidden = false;
                            break;
                        }
                    }
    ...
    protected function isDirective($dirname, $directives) {
        ...
        $pattern = implode("|",
            array_map(array("rafflepress_lessc", "preg_quote"), $directives));
    ...
    protected function fixTags($tags) {
        ...
        foreach ($tags as &$tag) {
            if ($tag[0] == $this->rafflepress_lessc->vPrefix)
                $tag[0] = $this->rafflepress_lessc->mPrefix;
        }
        return $tags;
    }
    ...
    protected function expressionList(&$exps) {
        ...
        $exps = rafflepress_lessc::compressList($values, ' ');
        return true;
    }
    ...
    public function propertyValue(&$value, $keyName = null) {
        ...
        $value = rafflepress_lessc::compressList($values, ', ');
        return true;
    }
    ...
    protected function openString($end, &$out, $nestingOpen=null, $rejectStrs = null) {
        ...
        $stop = array("'", '"', "@{", $end);
        $stop = array_map(array("rafflepress_lessc", "preg_quote"), $stop);
    ...
    protected function string(&$out) {
        ...
        $patt = '([^\n]*?)(@\{|\\\\|' .
            rafflepress_lessc::preg_quote($delim).')';
            ...
            while (true) {
                ...
                if ($this->string($str)) {
                    ...
                    foreach ($str[2] as &$chunk) {
                        $chunk = str_replace($this->rafflepress_lessc->parentSelector, "$&$", $chunk);
                    }
    ...
    protected function variable(&$name) {
        $s = $this->seek();
        if ($this->literal($this->rafflepress_lessc->vPrefix, false) &&
            ($this->variable($sub) || $this->keyword($name))
        ) {
            if (!empty($sub)) {
                $name = array('variable', $sub);
            } else {
                $name = $this->rafflepress_lessc->vPrefix.$name;
            }
            return true;
        }
    ...
    protected function literal($what, $eatWhitespace = null) {
        ...
        if (!isset(self::$literalCache[$what])) {
            self::$literalCache[$what] = rafflepress_lessc::preg_quote($what);
        }
    ...
    protected function to($what, &$out, $until = false, $allowNewline = false) {
        ...
        if (!$this->match('('.$validChars.'*?)'.rafflepress_lessc::preg_quote($what), $m, !$until)) return false;
...
class rafflepress_lessc_formatter_classic {
    public $indentChar = "  ";
...
class rafflepress_lessc_formatter_compressed extends rafflepress_lessc_formatter_classic {
...
class rafflepress_lessc_formatter_lessjs extends rafflepress_lessc_formatter_classic {
    public $disableSingle = true;
