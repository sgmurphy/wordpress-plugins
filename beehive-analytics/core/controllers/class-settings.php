...
class Settings extends Base {
	 ...
	 * @var string
	 */
	private $masked_settings = array(
		'access_token' => 'google_login',
	);
	...
	private $default_settings = array(
		...
		'google_login' => array(
			...
			'accounts_count' => 0, // Whether accounts limit reached.
		),
		'misc'         => array(
	...
	public function init( $network = false ) {
		// Get network options.
		$options = $this->get_options( false, $network, true, false );
	 ...
	 * @return array
	 */
	public function get_settings_with_default( $network = false, $mask = true ) {
		// Get the settings.
		$settings = $this->get_options( false, $network, false, $mask );
	 ...
	 * @return mixed
	 */
	public function get( $key, $group, $network = false, $default = false, $force = false, $mask = true ) {
		...
		// Get group values.
		$options = $this->get_options( $group, $network, $force, $mask );
	 ...
	 * @return mixed
	 */
	public function get_options( $group = false, $network = false, $force = false, $mask = true ) {
		if ( $force ) {
			// Get option from WP.
		 ...
		 * @param bool  $network Network flag.
		 */
		$options = apply_filters( 'beehive_get_options', $options, $network, $mask );

		// Mask required data.
		if ( $mask ) {
			$options = $this->mask_data( $options );
		}
	...
	public function update_options( $values, $network = false ) {
		...
		// Unmask if required.
		$values = $this->maybe_unmask_data( $values, $network );
	...
	/**
	 * Make sure we don't expose sensitive data.
	 *
	 * @since 3.4.5
	 *
	...
	private function mask_data( $data ) {
		foreach ( $this->masked_settings as $key => $group ) {
			// Mask item.
			if ( isset( $data[ $group ] ) ) {
				$data[ $group ][ $key ] = '****';
			}
		...
		/**
		 * Filter to perform additional masking for settings.
		 *
		 * @since 3.4.5
		 *
		 * @param array $data Data.
		 */
		return apply_filters( 'beehive_settings_mask_data', $data );
	}

	/**
	 * Make sure we add masked data back before update.
	 *
	 * @since 3.4.5
	 *
	 ...
	 * @return array
	 */
	private function maybe_unmask_data( array $data, $network = false ) {
		foreach ( $this->masked_settings as $key => $group ) {
			// Add item back.
			if ( isset( $data[ $group ][ $key ] ) && '****' === $data[ $group ][ $key ] ) {
				$data[ $group ][ $key ] = $this->get( $key, $group, $network, false, false, false );
			}
		}

		/**
		 * Filter to perform additional unmasking for settings.
		 *
		 * @since 3.2.4
		 *
		 * @param array $data    Data.
		 * @param bool  $network Network flag.
		 */
		return apply_filters( 'beehive_settings_unmask_data', $data, $network );
	}
}
