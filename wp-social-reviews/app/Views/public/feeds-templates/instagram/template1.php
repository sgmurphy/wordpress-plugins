...
if (!empty($feeds) && is_array($feeds)) {
    ...
    foreach ($feeds as $index => $feed) {
        if ($index >= $sinceId && $index <= $maxId && (isset($feed['media_url']) || isset($feed['default_media']) || isset($feed['children']))) {
                ...
                $displayMode = Arr::get($template_meta, 'post_settings.display_mode', '' );
            ...
            <div role="group" class="wpsr-ig-post <?php echo ($layout_type === 'carousel' && defined('WPSOCIALREVIEWS_PRO')) ? 'swiper-slide' : ''; ?>" data-user_name="<?php echo esc_attr(Arr::get($feed, 'username', ''));?>" data-post_id="<?php echo esc_attr(Arr::get($feed, 'id', ''));?>" data-image_size="<?php echo esc_attr($imageSize);?>">
                ...
                <a class="wpsr-ig-playmode" <?php echo ($displayMode === 'instagram' && $feedLink ) ? 'href=' . esc_url($feedLink) . '' : ''; ?>
                   ...
                   data-user_name="<?php echo esc_attr(Arr::get($feed, 'username', ''));?>" data-post_id="<?php echo esc_attr($feedId);?>" 
