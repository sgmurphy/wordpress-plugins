...
class ThumbGallery extends EAE_Widget_Base {
	...
	protected function render() {
		...
		if ( ! empty( $slides_per_view ) ) {
			$this->add_render_attribute( 'outer-wrapper', 'data-slides-per-view', wp_json_encode( $slides_per_view, JSON_NUMERIC_CHECK ) );
		}

		if ( ! empty( $space_between ) ) {
			$this->add_render_attribute( 'outer-wrapper', 'data-space', wp_json_encode( $space_between, JSON_NUMERIC_CHECK ) );
		}
