...
class TestimonialSlider extends EAE_Widget_Base {
	...
	protected function render() {
		...
		if ( $settings['layout'] === 'carousel' ) {
			...
			$this->add_render_attribute( 'outer-wrapper', 'data-space', wp_json_encode( $space, JSON_NUMERIC_CHECK ) );
			if ( $loop === 'yes' ) {
				$this->add_render_attribute( 'outer-wrapper', 'data-loop', $loop );
			}

			if ( ! empty( $slide_per_view ) ) {
				$this->add_render_attribute( 'outer-wrapper', 'data-slides-per-view', wp_json_encode( $slide_per_view, JSON_NUMERIC_CHECK ) );
			}

			if ( ! empty( $slides_per_group ) ) {
				$this->add_render_attribute( 'outer-wrapper', 'data-slides-per-group', wp_json_encode( $slides_per_group, JSON_NUMERIC_CHECK ) );
			}
			...
			<div <?php echo $this->get_render_attribute_string( 'outer-wrapper' ); ?>>
				<div class="swiper-container">
					...
					<?php if ( $navigation_button === 'yes' ) { ?>
						<?php
						if ( $settings['horizontal_position_arrow'] !== 'center' ) {
							;
							?>
