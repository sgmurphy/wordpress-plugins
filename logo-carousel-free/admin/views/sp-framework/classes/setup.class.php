...
if ( ! class_exists( 'SPLC' ) ) {
	...
	class SPLC {
		 ...
		 * @var string
		 */
		public static $version = SP_LC_VERSION;
		...
		public static function include_plugin_url( $file ) {
			return esc_url( SP_LC_URL . '/admin/views/sp-framework' ) . '/' . ltrim( $file, '/' );
		}
		...
		public static function add_admin_enqueue_scripts() {
			...
			wp_localize_script(
				...
				array(
					...
					'i18n'          => array(
						...
						'typing_text'     => esc_html__( 'Please enter or more characters', 'logo-carousel-free' ),
			...
			if ( 'lc_help' === $page ) {
				...
				wp_enqueue_style( 'sp-lc-admin-help-fontello', SP_LC_URL . 'admin/assets/help-page/css/fontello.min.css', array(), SP_LC_VERSION );

				wp_enqueue_script( 'sp-lc-admin-help', SP_LC_URL . 'admin/assets/help-page/js/help_page.min.js', array( 'jquery' ), SP_LC_VERSION, true );
			}
		...
		public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
			...
			if ( ! empty( $field_type ) ) {
				...
				if ( ! empty( $field['title'] ) ) {
					$title_info = ( ! empty( $field['title_info'] ) ) ? '<span class="splogocarousel-help title-info"><div class="splogocarousel-help-text">' . wp_kses_post( $field['title_info'] ) . '</div><img src="' . self::include_plugin_url( 'assets/images/info.svg' ) . '"></span>' : '';
