...
if (defined('WOOBE_PATH')) {
    add_action('admin_notices', function () {
        ...
        <div class="notice notice-error is-dismissible">
            <p><?php esc_html_e("Hello, looks like you bought and installed premium version of BEAR - WooCommerce Bulk Editor and Products Manager Professional, please deactivate free version before, then uninstall it. Any way: 2 versions of the same plugin cannot be activated on the same time! Use most new one version.", 'woo-bulk-editor'); ?></p>
        </div>
        <?php
    });
...
//***
            ...
            add_action('admin_notices', function () {
                ...
                if (!get_user_meta($user_id, 'woobe_notice_dismissed')) {
                    echo '<div class="notice notice-warning"><p>' . sprintf(esc_html__('If you not familiar with the plugin, firstly %s please', 'woo-bulk-editor'), WOOBE_HELPER::draw_link(array(
                            ...
                            ))) . '</p><a href="edit.php?post_type=product&page=woobe&woobe-notice-dismissed=1&notice_nonce=' . wp_create_nonce('woobe_notice_nonce') . '" class="notice-dismiss"></a></div>';
                }
            });
            add_action('admin_init', function () {
                $user_id = get_current_user_id();
                if (isset($_GET['woobe-notice-dismissed']) && isset($_GET['notice_nonce']) && wp_verify_nonce($_GET['notice_nonce'], 'woobe_notice_nonce')) {
        ...
        add_action('admin_bar_menu', function ($wp_admin_bar) {
            $opt = get_option('woobe_options_' . apply_filters('woobe_settings_key_options', get_current_user_id())); //not beauty but we need it here
            $show = true;
        ...
        if (isset($_GET['page']) AND $_GET['page'] == 'woobe') {
            ...
            <script>
                ...
                var woobe_settings = {
                    ...
                    number_posts_processed: <?php echo intval($this->settings->number_posts_processed) ?>,
            ...
            if ($_REQUEST['value']) {
                if (is_array($_REQUEST['value'])) {
                    ...
                    if ($is_encoded) {
                        ...
                        if (apply_filters('woobe_use_kses_for_page_field', true)) {
                ...
                $value = "";
                if (isset($this->settings->get_fields()[sanitize_text_field($_REQUEST['field'])]) && 'taxonomy' == $this->settings->get_fields()[sanitize_text_field($_REQUEST['field'])]['field_type']) {
                    $value = array();
                }
            }
...
//***
            ...
            if (0 == $this->settings->autocomplet_txt_search) {
            ...
            } elseif (1 == $this->settings->autocomplet_txt_search) {
                $_REQUEST['woobe_autocomplete_search'] = $st;
                add_filter('posts_where', array($this, 'autocomplete_search'), 101);
            }
...
//http://easyautocomplete.com/guide
            if ($query->have_posts()) {
                ...
                foreach ($query->posts as $p) {
                    $data = array(
                        "name" => $p->post_title . ' (#' . $p->ID . ')' . ' : ' . get_post_meta($p->ID, '_sku', true),
                        "id" => $p->ID,
                        "type" => "product"
                    );
    ...
    public function autocomplete_search($where = '') {

        if (!isset($_REQUEST['woobe_autocomplete_search'])) {
            return $where;
        }
        global $wpdb;
        $search_text = sanitize_text_field($_REQUEST['woobe_autocomplete_search']);
        ...
        if (!empty($product_variations_ids)) {
            ...
            $sql[] = " ($wpdb->posts.ID IN($product_variations_ids_string))";
        }

        //id
        $id_search = (int) $search_text;
        if ($id_search) {
            $sql[] = " ($wpdb->posts.ID = $id_search )";
        }


        //title
        $like = '%' . $wpdb->esc_like($search_text) . '%';
        $sql[] = $wpdb->prepare("($wpdb->posts.post_title LIKE %s)", $like);
...
//ajax
    ...
    public function woobe_create_new_term() {
        ...
        $slugs = wc_clean($_REQUEST['slugs']); //sanitized in cycle
        $description = sanitize_textarea_field($_REQUEST['description']);
        $taxonomy = sanitize_text_field(trim($_REQUEST['tax_key']));
    ...
    public function clone_custom_taxonomies($origin, $clone) {
        ...
        $taxonomies = array();
        $exclude_tax = array('product_type', 'product_visibility', 'product_shipping_class', 'product_cat', 'product_tag', 'language', 'post_translations');
...
//do not onit it exept of one woobe page and its ajax requests
        ...
        if (defined('DOING_AJAX')) {
            if (isset($_REQUEST['action']) && strpos($_REQUEST['action'], 'woobe_') !== FALSE) {
                $init = true;
            }
        }
