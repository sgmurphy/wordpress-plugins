...
class Envira_Gallery_Shortcode {
	...
	public function __construct() {
		...
		$this->is_mobile = envira_lite_mobile_detect()->isMobile();

		// Register main gallery style.
		wp_register_style( $this->base->plugin_slug . '-style', plugins_url( 'assets/css/envira.css', $this->base->file ), [], $this->base->version );
		wp_register_style( $this->base->plugin_slug . '-fancybox', plugins_url( 'assets/css/fancybox.css', $this->base->file ), [], $this->base->version );
		wp_register_style( $this->base->plugin_slug . '-lazyload', plugins_url( 'assets/css/responsivelyLazy.css', $this->base->file ), [], $this->base->version );
		wp_register_style( $this->base->plugin_slug . '-jgallery', plugins_url( 'assets/css/justifiedGallery.css', $this->base->file ), [], $this->base->version );

		// Register main gallery script.
		wp_register_script( $this->base->plugin_slug . '-script', plugins_url( 'assets/js/min/envira-min.js', $this->base->file ), [ 'jquery', 'jquery-masonry' ], $this->base->version, true );
	...
	public function generate_gallery_item_markup( $gallery, $data, $item, $id, $i ) {
		...
		if ( $create_link ) {
			$output .= '<a href="' . esc_url( $item['link'] ) . '" class="' . ( ( isset( $item['link_new_window'] ) && true === boolval( $item['link_new_window'] ) ) ? '' : 'envira-gallery-' . sanitize_html_class( $data['id'] ) . ' ' ) . 'envira-gallery-link" ' . $html5_attribute . '="enviragallery' . sanitize_html_class( $data['id'] ) . '" title="' . $title . '" data-envira-caption="' . $caption . '" data-envira-retina="' . ( isset( $item['lightbox_retina_image'] ) ? $item['lightbox_retina_image'] : '' ) . '" data-thumbnail="' . esc_url( $item['thumb'] ) . '"' . ( ( isset( $item['link_new_window'] ) && true === boolval( $item['link_new_window'] ) ) ? ' target="_blank"' : '' ) . ' ' . apply_filters( 'envira_gallery_output_link_attr', '', $id, $item, $data, $i ) . ' itemprop="contentUrl">';
		}

		$output = apply_filters( 'envira_gallery_output_before_image', $output, $id, $item, $data, $i );

		$gallery_theme = intval( $this->get_config( 'columns', $data ) ) === 0 ? ' envira-' . $this->get_config( 'justified_gallery_theme', $data ) : '';
		...
		$envira_lazy_load = intval( $this->get_config( 'lazy_loading', $data ) ) === 1 ? 'envira-lazy' : '';

		// Determine/confirm the width/height of the image.
		// $placeholder should hold it but not for instagram.
		if ( 'mobile' === $this->is_mobile && ! $this->get_config( 'mobile', $data ) ) { // if the user is viewing on mobile AND user unchecked 'Create Mobile Gallery Images?' in mobile tab.
			$output_src = $item['src'];
		} elseif ( $this->get_config( 'crop', $data ) ) { // the user has selected the image to be cropped.
			$output_src = $imagesrc;
		} elseif ( $this->get_config( 'image_size', $data ) && $imagesrc ) { // use the image being provided thanks to the user selecting a unique image size.
			$output_src = $imagesrc;
		...
		} elseif ( intval( $this->get_config( 'columns', $data ) ) !== 0 && $this->get_config( 'image_size', $data ) && $this->get_config( 'image_size', $data ) !== 'full' && $this->get_config( 'crop_width', $data ) && $this->get_config( 'crop_height', $data ) ) {
			$output_width = $this->get_config( 'crop_width', $data );
		} elseif ( ! empty( $item['width'] ) ) {
		...
		if ( intval( $this->get_config( 'columns', $data ) ) === 0 ) {
				...
				var responsivelyLazy = window.responsivelyLazy;
					...
					<?php if ( intval( $this->get_config( 'columns', $data ) ) === 0 ) : ?>
						...
						$('#envira-gallery-<?php echo intval( $data['id'] ); ?>').enviraJustifiedGallery({
							...
							lastRow: 'nojustify',
							border: 0,
							margins: <?php echo $this->get_config( 'justified_margins', $data ) ? intval( $this->get_config( 'justified_margins', $data ) ) : 1; ?>,
						...
						$('#envira-gallery-<?php echo intval( $data['id'] ); ?>').css('opacity', '1');

					<?php endif; ?>

					<?php
					// Isotope: Start.
					if ( intval( $this->get_config( 'columns', $data ) ) > 0 && $this->get_config( 'isotope', $data ) ) {
						...
						?>
						envira_isotopes['<?php echo intval( $data['id'] ); ?>'].enviraImagesLoaded()
							.done(function() {
								envira_isotopes['<?php echo intval( $data['id'] ); ?>'].masonry('layout');
							})
							.progress(function() {
								envira_isotopes['<?php echo intval( $data['id'] ); ?>'].masonry('layout');
							});
					...
					if ( $this->get_config( 'lightbox_enabled', $data ) ) {
						...
						envira_gallery_options = {
							...
							padding: <?php echo in_array( $this->get_config( 'lightbox_theme', $data ), [ 'base_dark' ], true ) ? '4' : '15'; ?>,
							cyclic: true,
						...
						if ( ! $lightbox_images ) {
							...
							?>
							envira_gallery_images[<?php echo intval( $data['id'] ); ?>] = [];
							<?php
	...
	public function get_image_src( $id, $item, $data, $mobile = false, $retina = false ) {
		...
		if ( 'full' !== $image_size && ( 'default' !== $image_size && $retina ) ) {
