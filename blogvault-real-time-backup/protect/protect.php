...
if (!class_exists('BVProtect_V553')) :
...
class BVProtect_V553 {
	...
	public static function init($mode) {
		if ($mode == BVProtect_V553::MODE_PREPEND) {
			$config_file = MCDATAPATH .  MCCONFKEY . '-' . 'mc.conf';
			$config = BVProtectUtils_V553::parseFile($config_file);

			if (empty($config['time']) || !($config['time'] > time() - (48*3600)) ||
					!isset($config['mc_conf_version']) ||
					(BVProtect_V553::CONF_VERSION !== $config['mc_conf_version'])) {
				return false;
			...
			$request = new BVProtectRequest_V553($request_ip_header, $req_config);
			$fw_config = array_key_exists('fw', $config) ? $config['fw'] : array();

			BVProtectFW_V553::getInstance($mode, $request, $fw_config, $brand_name)->init();
		} else {
			...
			BVProtectFW_V553::getInstance($mode, $request, $fw_config, $brand_name)->init();
			BVProtectLP_V553::getInstance($request, $lp_config, $brand_name)->init();
		}
	}
	...
	public static function uninstall() {
		...
		BVProtectIpstore_V553::uninstall();
		BVProtectFW_V553::uninstall();
		BVProtectLP_V553::uninstall();

		BVProtect_V553::removeWPPrepend();
		BVProtect_V553::removePHPPrepend();
		BVProtect_V553::removeMCData();
	...
	private static function removeWPPrepend() {
		...
		BVProtectUtils_V553::fileRemovePattern($fname, $pattern);
	}

	private static function removePHPPrepend() {
		BVProtect_V553::removeHtaccessPrepend();
		BVProtect_V553::removeUseriniPrepend();
	}

	private static function removeHtaccessPrepend() {
		$pattern = "/# MalCare WAF(.|\n)*# END MalCare WAF/i";

		BVProtectUtils_V553::fileRemovePattern(dirname(ABSPATH) . "/.htaccess", $pattern, true);
	}

	private static function removeUseriniPrepend() {
		$pattern = "/; MalCare WAF(.|\n)*; END MalCare WAF/i";

		BVProtectUtils_V553::fileRemovePattern(dirname(ABSPATH) . "/.user.ini", $pattern, true);
	}
	...
	private static function removeMCData() {
		...
		BVProtectUtils_V553::rrmdir($mc_data_dir);
	}
}
endif;