...
if (!class_exists('BVProtect_V545')) :
...
class BVProtect_V545 {
	...
	public static function init($mode) {
		if ($mode == BVProtect_V545::MODE_PREPEND) {
			$config_file = MCDATAPATH .  MCCONFKEY . '-' . 'mc.conf';
			$config = BVProtectUtils_V545::parseFile($config_file);

			if (empty($config['time']) || !($config['time'] > time() - (48*3600)) ||
					!isset($config['mc_conf_version']) ||
					(BVProtect_V545::CONF_VERSION !== $config['mc_conf_version'])) {
				return false;
			...
			$request = new BVProtectRequest_V545($request_ip_header);
			$fw_config = array_key_exists('fw', $config) ? $config['fw'] : array();

			BVProtectFW_V545::getInstance($mode, $request, $fw_config, $brand_name)->init();
		} else {
			...
			BVProtectFW_V545::getInstance($mode, $request, $fw_config, $brand_name)->init();
			BVProtectLP_V545::getInstance($request, $lp_config, $brand_name)->init();
		}
	}
	...
	public static function uninstall() {
		...
		BVProtectIpstore_V545::uninstall();
		BVProtectFW_V545::uninstall();
		BVProtectLP_V545::uninstall();

		BVProtect_V545::removeWPPrepend();
		BVProtect_V545::removePHPPrepend();
		BVProtect_V545::removeMCData();
	...
	private static function removeWPPrepend() {
		...
		BVProtectUtils_V545::fileRemovePattern($fname, $pattern);
	}

	private static function removePHPPrepend() {
		BVProtect_V545::removeHtaccessPrepend();
		BVProtect_V545::removeUseriniPrepend();
	}

	private static function removeHtaccessPrepend() {
		$pattern = "/# MalCare WAF(.|\n)*# END MalCare WAF/i";

		BVProtectUtils_V545::fileRemovePattern(dirname(ABSPATH) . "/.htaccess", $pattern, true);
	}

	private static function removeUseriniPrepend() {
		$pattern = "/; MalCare WAF(.|\n)*; END MalCare WAF/i";

		BVProtectUtils_V545::fileRemovePattern(dirname(ABSPATH) . "/.user.ini", $pattern, true);
	}
	...
	private static function removeMCData() {
		...
		BVProtectUtils_V545::rrmdir($mc_data_dir);
	}
}
endif;