...
class BVProtectIpstore_V545 {
	...
	function __construct($storage_type = BVProtectIpstore_V545::STORAGE_TYPE_DB) {
		$this->storage_type = $storage_type;
		if ($this->storage_type == BVProtectIpstore_V545::STORAGE_TYPE_FS) {
			$this->storage = new BVProtectIpstoreFS_V545();
		} else {
			$this->storage = new BVProtectIpstoreDB_V545();
		}
	}

	public static function uninstall() {
		BVProtectIpstoreDB_V545::uninstall();
	}

	public function isLPIPBlacklisted($ip) {
		if ($this->storage_type == BVProtectIpstore_V545::STORAGE_TYPE_DB) {
