...
class BVProtectIpstore_V553 {
	...
	function __construct($storage_type = BVProtectIpstore_V553::STORAGE_TYPE_DB) {
		$this->storage_type = $storage_type;
		if ($this->storage_type == BVProtectIpstore_V553::STORAGE_TYPE_FS) {
			$this->storage = new BVProtectIpstoreFS_V553();
		} else {
			$this->storage = new BVProtectIpstoreDB_V553();
		}
	}

	public static function uninstall() {
		BVProtectIpstoreDB_V553::uninstall();
	}

	public function isLPIPBlacklisted($ip) {
		if ($this->storage_type == BVProtectIpstore_V553::STORAGE_TYPE_DB) {
