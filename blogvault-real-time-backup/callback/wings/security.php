...
if (!class_exists('BVSecurityCallback')) :
	class BVSecurityCallback extends BVCallbackBase {
		...
		public function setupWP2FA($user_id, $secret, $to_encrypt = true, $cipher_algo = null, $enabled = null) {
			if ($to_encrypt === true) {
				...
				if (defined('SECURE_AUTH_KEY')) {
					$encryption_result = BVHelper::opensslEncrypt($secret, $cipher_algo, SECURE_AUTH_KEY);
		...
		public function verifyWP2FACode($user_id, $code, $cipher_algo = null) {
			$encoded_secret_info = get_user_meta($user_id, BVWP2FA::SECRET_META_KEY, true);

			$secret_info = BVWP2FAUtils::getSecretInfo($encoded_secret_info);
			...
			if ($is_secret_encrypted === true) {
				...
				if (defined('SECURE_AUTH_KEY')) {
					$decryption_result = BVHelper::opensslDecrypt($secret, $cipher_algo, SECURE_AUTH_KEY);
			...
			return array("status" => BVWP2FAAuthenticator::verifyCode($secret, $code, 2));
		}

		public function readWP2FAKeys($user_id) {
			$secret = get_user_meta($user_id, BVWP2FA::SECRET_META_KEY, true);
			$enabled = get_user_meta($user_id, BVWP2FA::FLAG_META_KEY, true);
			return array(
				"secret" => $secret,
				"enabled" => $enabled
			);
		...
		public function deleteWP2FAKeys($user_ids) {
			foreach ($user_ids as $user_id) {
				delete_user_meta($user_id, BVWP2FA::FLAG_META_KEY);
				delete_user_meta($user_id, BVWP2FA::SECRET_META_KEY);
			}
			return array("status" => true);
		}
