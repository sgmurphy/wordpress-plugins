...
if (!class_exists('BVWP2FA')) :

require_once dirname(__FILE__) . '/authenticator.php';
require_once dirname(__FILE__) . '/utils.php';

class BVWP2FA {
	const FLAG_META_KEY = 'bv_2fa_enabled';
	const SECRET_META_KEY = 'bv_2fa_secret';

	public static $cipher_algo = 'aes-256-cbc';
	public static $wp_2fa_option = 'bvWp2faConf';
	private $settings;
	private $config;
	...
	public function authenticate($user, $username, $password) {
		...
		$has_2fa = get_user_meta($user->ID, BVWP2FA::FLAG_META_KEY, true);
		...
		if ('1' === $has_2fa) {
			if (empty($_POST['twofa_code'])) {
				...
				$encoded_secret_info = get_user_meta($user->ID, BVWP2FA::SECRET_META_KEY, true);
