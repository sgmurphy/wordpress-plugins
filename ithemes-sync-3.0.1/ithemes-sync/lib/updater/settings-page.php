...
class Ithemes_Updater_Settings_Page {
	...
	public function add_styles() {
		wp_enqueue_style( 'ithemes-updater-settings-page-style', "{$this->path_url}/css/settings-page.css", array(), 3 );
	}
	...
	private function license_packages( $data ) {
		...
		if ( empty( $data['username'] ) && empty( $data['password'] ) )
			$this->errors[] = __( 'You must supply a SolidWP membership username and password in order to license products.', 'it-l10n-ithemes-sync' );
		else if ( empty( $data['username'] ) )
			$this->errors[] = __( 'You must supply a SolidWP membership username in order to license products.', 'it-l10n-ithemes-sync' );
		else if ( empty( $data['password'] ) )
			$this->errors[] = __( 'You must supply a SolidWP membership password in order to license products.', 'it-l10n-ithemes-sync' );
	...
	private function license_patchstack_site( $data ) {
		...
		if ( empty( $data['username'] ) && empty( $data['password'] ) )
			$this->errors[] = __( 'You must supply a SolidWP membership username and password in order to remove patchstack licenses.', 'it-l10n-ithemes-sync' );
		else if ( empty( $data['username'] ) )
			$this->errors[] = __( 'You must supply a SolidWP membership username in order to remove patchstack licenses.', 'it-l10n-ithemes-sync' );
		else if ( empty( $data['password'] ) )
			$this->errors[] = __( 'You must supply a SolidWP membership password in order to remove patchstack licenses.', 'it-l10n-ithemes-sync' );

		$keys = Ithemes_Updater_Keys::get();
		if ( empty( $keys['ithemes-security-pro'] ) )
			$this->errors[] = __( 'Unable to locate API key for Solid Security Pro.', 'it-l10n-ithemes-sync' );
		...
		if ( ! empty( $success ) )
			$this->messages[] = wp_sprintf( _n( 'Successfully added Patchstack license to %l.', 'Successfully removed Patchstack licenses from %l sites.', count( $success ), 'it-l10n-ithemes-sync' ), $success );

		if ( ! empty( $fail ) ) {
			foreach ( $fail as $name => $reason )
				$this->errors[] = sprintf( __( 'Unable to remove patchstack license from %1$s. Reason: %2$s', 'it-l10n-ithemes-sync' ), $name, $reason );
		}
	}
	...
	private function unlicense_patchstack_sites( $data ) {
		...
		if ( ! empty( $success ) )
			$this->messages[] = wp_sprintf( _n( 'Successfully removed Patchstack license from %l site.', 'Successfully removed Patchstack licenses from %l sites.', count( $success ), 'it-l10n-ithemes-sync' ), $success );
	...
	private function unlicense_packages( $data ) {
		...
		if ( empty( $data['username'] ) && empty( $data['password'] ) )
			$this->errors[] = __( 'You must supply a SolidWP membership username and password in order to remove licenses.', 'it-l10n-ithemes-sync' );
		else if ( empty( $data['username'] ) )
			$this->errors[] = __( 'You must supply a SolidWP membership username in order to remove licenses.', 'it-l10n-ithemes-sync' );
		else if ( empty( $data['password'] ) )
			$this->errors[] = __( 'You must supply a SolidWP membership password in order to remove licenses.', 'it-l10n-ithemes-sync' );
		else if ( empty( $data['packages'] ) )
			$this->errors[] = __( 'You must select at least one license to remove. Ensure that you select the licenses that you wish to remove in the listing below.', 'it-l10n-ithemes-sync' );
			...
			<table class="ithemes-updater-listing widefat">
				<thead>
					<tr>
						...
						<th scope="col" class="mobile-hidden"><?php _e( 'Member', 'it-l10n-ithemes-sync' ); ?></th>
						<th scope="col"><?php _e( 'Product Status', 'it-l10n-ithemes-sync' ); ?></th>
						<th scope="col" class="mobile-hidden"><?php _e( 'Expiration', 'it-l10n-ithemes-sync' ); ?></th>
						<th scope="col" class="mobile-hidden"><?php _e( 'Remaining Licenses', 'it-l10n-ithemes-sync' ); ?></th>
					</tr>
				</thead>
				<tbody>
					<?php $count = 0; ?>
...
?>
	<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo esc_attr( $this->self_url ); ?>" autocomplete="off">
		...
		<div class="ithemes-updater-products" id="ithemes-updater-licensed">
			<div class="solidwp-table-header">
				<div>
					<h3 class="subtitle"><?php _e( 'Patchstack Enabled Sites', 'it-l10n-ithemes-sync' ); ?></h3>
				</div>
			</div>
			...
			<table class="ithemes-updater-listing widefat">
				<thead>
					<tr>
						...
						<th scope="col">
							<label for="cb-select-all-patchstack"><?php printf( __( 'Site (%d Remaining Licenses)', 'it-l10n-ithemes-sync' ), $patchstack_available ); ?></label>
						</th>
					</tr>
				</thead>
				<tbody>
				...
				<tfoot>
					<tr>
						<td colspan="2">
							<input type="text" name="it-updater-username" placeholder="SolidWP Username" value="<?php echo esc_attr( $post_data['username'] ); ?>" autocomplete="off" />
							<input type="password" name="it-updater-password" placeholder="Password" value="<?php echo esc_attr( $post_data['password'] ); ?>" />
							<input class="button-primary" type="submit" name="submit" value="<?php _e( 'Remove Patchstack Licenses', 'it-l10n-ithemes-sync' ); ?>" />
...
?>
	<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo esc_attr( $this->self_url ); ?>" autocomplete="off">
		...
		<div class="ithemes-updater-products" id="ithemes-updater-unlicensed">
			<div class="solidwp-table-header">
				<div>
					<h3><?php _e( 'Unlicensed Products', 'it-l10n-ithemes-sync' ); ?></h3>
					...
					<p><?php _e( 'To license products, select the products you wish to license, enter your SolidWP membership username and password, and press the License Products button.', 'it-l10n-ithemes-sync' ); ?></p>
					<p><?php printf( __( 'Need help? <a href="%s">Click here for a quick video tutorial</a>.', 'it-l10n-ithemes-sync' ), 'https://go.solidwp.com/solidwp-updater' ); ?></p>
				</div>
			</div>
	...
	<div class="wrap" id="ithemes-updater-relicense">
		...
		<div class="solidwp-licensing-wrap">
			...
			<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo esc_attr( $this->self_url ); ?>">
				...
				<div id="ithemes-updater-settings">
					<div id="ithemes-updater-table-wrapper">
						<h3><?php _e( 'License Option', 'it-l10n-ithemes-sync' ); ?></h3>
						<p>
							<label>
						...
						<p>
							<label>
								...
								<?php printf( __( 'Change the existing licenses to be for <code>%s</code>.', 'it-l10n-ithemes-sync' ), esc_html( $data['site_url'] ) ); ?>
							</label>
						</p>
						<p class="description ithemes-updater-description-notice">
							<span class="dashicons dashicons-warning"></span>
							<?php printf( __( 'Note: If the <code>%s</code> site still exists and is different from this site, you will have to create new licenses on that site.', 'it-l10n-ithemes-sync' ), esc_html( $data['site_url'] ) ); ?>
						</p>
						<p class="description ithemes-updater-description-warning secondary-description"><?php printf( __( 'Use this option if this site\'s primary URL has changed from <code>%1$s</code> to <code>%2$s</code>.', 'it-l10n-ithemes-sync' ), esc_html( $site_url_from_server ), esc_html( $data['site_url'] ) ); ?></p>
						<div class="input-container">
							<label for="it-updater-username"><?php _e( 'SolidWP Username', 'it-l10n-ithemes-sync' ); ?></label>
							<input id="it-updater-username" type="text" name="it-updater-username" value="<?php echo esc_attr( $data['username'] ); ?>" />
							<label for="it-updater-password"><?php _e( 'SolidWP Password', 'it-l10n-ithemes-sync' ); ?></label>
...
<?php
	...
	private function relicense( $data ) {
		...
		if ( empty( $data['username'] ) && empty( $data['password'] ) ) {
			$this->errors[] = __( 'You must supply a SolidWP membership username and password in order to change the licensed URL.', 'it-l10n-ithemes-sync' );
		} else if ( empty( $data['username'] ) ) {
			$this->errors[] = __( 'You must supply a SolidWP membership username in order to change the licensed URL.', 'it-l10n-ithemes-sync' );
		} else if ( empty( $data['password'] ) ) {
			$this->errors[] = __( 'You must supply a SolidWP membership password in order to change the licensed URL.', 'it-l10n-ithemes-sync' );
