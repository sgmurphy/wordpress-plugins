...
if( !class_exists('SW_CLOUDFLARE_PAGECACHE') ) {
	...
	class SW_CLOUDFLARE_PAGECACHE {
		...
		function __construct() {
			add_action('admin_init', array( $this, 'maybe_deactivate_free' ) );

			//add_action( 'plugins_loaded', array($this, 'update_plugin') );
			register_deactivation_hook( SPC_PATH, array($this, 'deactivate_plugin') );
		...
		function actions() {
			...
			add_filter( 'plugin_action_links_' . plugin_basename(SPC_PATH), array($this, 'add_plugin_action_links') );
	...
	register_activation_hook( SPC_PATH, function() {
