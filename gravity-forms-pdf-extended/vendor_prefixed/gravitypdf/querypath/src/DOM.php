 ...
 */
abstract class DOM implements \GFPDF_Vendor\QueryPath\Query, \IteratorAggregate, \Countable
{
    /**
    ...
    protected $options = [
        ...
        'exception_level' => 771,
        // E_ERROR | E_USER_ERROR | E_USER_WARNING | E_WARNING
    ...
    {
        $string = \is_string($string) ? \trim($string) : '';
        $this->options = $options + \GFPDF_Vendor\QueryPath\Options::get() + $this->options;
        ...
        if (empty($document)) {
            ...
            if ($document instanceof \SplObjectStorage) {
                ...
                $this->setMatches($document->get(null, \true));
            ...
            } elseif ($document instanceof \GFPDF_Vendor\Masterminds\HTML5) {
                $this->document = $document;
                $this->setMatches($document->documentElement);
                ...
                throw new \GFPDF_Vendor\QueryPath\Exception('Unsupported class type: ' . \get_class($document));
            }
    ...
    private function parseXMLString($string, $flags = 0)
    {
        ...
        // <?xml
        try {
            \set_error_handler([\GFPDF_Vendor\QueryPath\ParseException::class, 'initializeFromError'], $this->errTypes);
            ...
            if ($useParser === 'html') {
                ...
                if ($this->options['replace_entities']) {
                    $string = \GFPDF_Vendor\QueryPath\Entities::replaceAllEntities($string);
                }
        ...
        \restore_error_handler();
        if (empty($document)) {
            throw new \GFPDF_Vendor\QueryPath\ParseException('Unknown parser exception.');
        }
        return $document;
    }
    /**
    ...
    public function setMatches($matches)
    {
        ...
        if ($matches instanceof \SplObjectStorage) {
            ...
            \trigger_error('Legacy array detected.');
    ...
    protected function prepareInsert($item)
    {
        ...
        if (\is_string($item)) {
            ...
            if ($this->options['replace_entities']) {
                $item = \GFPDF_Vendor\QueryPath\Entities::replaceAllEntities($item);
            }
        ...
        if ($item instanceof \DOMNode) {
            if ($item->ownerDocument !== $this->document) {
                ...
                $item = $this->document->importNode($item, \true);
            }
            return $item;
        }
        if ($item instanceof \SimpleXMLElement) {
            $element = \dom_import_simplexml($item);
            return $this->document->importNode($element, \true);
        }
        // What should we do here?
        //var_dump($item);
        throw new \GFPDF_Vendor\QueryPath\Exception('Cannot prepare item of unsupported type: ' . \gettype($item));
    }
    /**
    ...
    private function parseXMLFile($filename, $flags = 0, $context = null)
    {
        ...
        if (!empty($context)) {
            try {
                \set_error_handler(['\\GFPDF_Vendor\\QueryPath\\ParseException', 'initializeFromError'], $this->errTypes);
            ...
            \restore_error_handler();
            if ($contents == \false) {
                throw new \GFPDF_Vendor\QueryPath\ParseException(\sprintf('Contents of the file %s could not be retrieved.', $filename));
            }
            return $this->parseXMLString($contents, $flags);
        }
        $document = new \DOMDocument();
        $lastDot = \strrpos($filename, '.');
        $htmlExtensions = ['.html' => 1, '.htm' => 1];
        // Allow users to override parser settings.
        if (empty($this->options['use_parser'])) {
            $useParser = '';
        } else {
