...
namespace GFPDF_Vendor\Masterminds\HTML5\Parser;

use GFPDF_Vendor\Masterminds\HTML5\Elements;
use GFPDF_Vendor\Masterminds\HTML5\InstructionProcessor;
/**
 ...
 * parser.
 */
class DOMTreeBuilder implements \GFPDF_Vendor\Masterminds\HTML5\Parser\EventHandler
{
    /**
    ...
    public function __construct($isFragment = \false, array $options = array())
    {
        ...
        $this->rules = new \GFPDF_Vendor\Masterminds\HTML5\Parser\TreeBuildingRules();
        $implicitNS = array();
     ...
     */
    public function setInstructionProcessor(\GFPDF_Vendor\Masterminds\HTML5\InstructionProcessor $proc)
    {
        $this->processor = $proc;
    }
    ...
    public function startTag($name, $attributes = array(), $selfClosing = \false)
    {
        ...
        if ($this->insertMode >= static::IM_IN_BODY && \GFPDF_Vendor\Masterminds\HTML5\Elements::isA($name, \GFPDF_Vendor\Masterminds\HTML5\Elements::AUTOCLOSE_P)) {
            $this->autoclose('p');
        }
        // Set insert mode:
        switch ($name) {
        ...
        if ($this->insertMode === static::IM_IN_SVG) {
            $lname = \GFPDF_Vendor\Masterminds\HTML5\Elements::normalizeSvgElement($lname);
        }
        $pushes = 0;
        ...
        if ($this->onlyInline && \GFPDF_Vendor\Masterminds\HTML5\Elements::isA($lname, \GFPDF_Vendor\Masterminds\HTML5\Elements::BLOCK_TAG)) {
        ...
        if ($this->current instanceof \DOMElement && isset(\GFPDF_Vendor\Masterminds\HTML5\Elements::$optionalEndElementsParentsToClose[$lname])) {
            foreach (\GFPDF_Vendor\Masterminds\HTML5\Elements::$optionalEndElementsParentsToClose[$lname] as $parentElName) {
        ...
        if (\GFPDF_Vendor\Masterminds\HTML5\Elements::isA($lname, \GFPDF_Vendor\Masterminds\HTML5\Elements::BLOCK_ONLY_INLINE)) {
            $this->onlyInline = $lname;
        }
        // When we add some namespacess, we have to track them. Later, when "endElement" is invoked, we have to remove them.
        // When we are on a void tag, we do not need to care about namesapce nesting.
        if ($pushes > 0 && !\GFPDF_Vendor\Masterminds\HTML5\Elements::isA($name, \GFPDF_Vendor\Masterminds\HTML5\Elements::VOID_TAG)) {
        ...
        foreach ($attributes as $aName => $aVal) {
            ...
            if ($this->insertMode === static::IM_IN_SVG) {
                $aName = \GFPDF_Vendor\Masterminds\HTML5\Elements::normalizeSvgAttribute($aName);
        ...
        if ($this->frag !== $this->current && $this->rules->hasRules($name)) {
            ...
            if (!\GFPDF_Vendor\Masterminds\HTML5\Elements::isA($name, \GFPDF_Vendor\Masterminds\HTML5\Elements::VOID_TAG)) {
                $this->current = $ele;
            }
            ...
            if (\GFPDF_Vendor\Masterminds\HTML5\Elements::isHtml5Element($name)) {
                $selfClosing = \false;
            }
        }
        ...
        return \GFPDF_Vendor\Masterminds\HTML5\Elements::element($name);
    }
    public function endTag($name)
    {
        ...
        if ('br' === $name) {
            ...
            $this->startTag('br');
        } elseif (\GFPDF_Vendor\Masterminds\HTML5\Elements::isA($name, \GFPDF_Vendor\Masterminds\HTML5\Elements::VOID_TAG)) {
            return;
        }
    ...
    public function processingInstruction($name, $data = null)
    {
        ...
        if ($this->processor instanceof \GFPDF_Vendor\Masterminds\HTML5\InstructionProcessor) {
