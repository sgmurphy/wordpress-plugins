...
use GFPDF_Vendor\DeepCopy\Filter\ChainableFilter;
...
use GFPDF_Vendor\DeepCopy\TypeMatcher\TypeMatcher;
...
class DeepCopy
{
    ...
    public function addFilter(\GFPDF_Vendor\DeepCopy\Filter\Filter $filter, \GFPDF_Vendor\DeepCopy\Matcher\Matcher $matcher)
    {
        $this->filters[] = ['matcher' => $matcher, 'filter' => $filter];
    }
    public function prependFilter(\GFPDF_Vendor\DeepCopy\Filter\Filter $filter, \GFPDF_Vendor\DeepCopy\Matcher\Matcher $matcher)
    {
        \array_unshift($this->filters, ['matcher' => $matcher, 'filter' => $filter]);
    }
    public function addTypeFilter(\GFPDF_Vendor\DeepCopy\TypeFilter\TypeFilter $filter, \GFPDF_Vendor\DeepCopy\TypeMatcher\TypeMatcher $matcher)
    {
    ...
    private function copyObject($object)
    {
        ...
        if (\false === $isCloneable) {
            ...
            throw new \GFPDF_Vendor\DeepCopy\Exception\CloneException(\sprintf('The class "%s" is not cloneable.', $reflectedObject->getName()));
        }
        ...
        foreach (\GFPDF_Vendor\DeepCopy\Reflection\ReflectionHelper::getProperties($reflectedObject) as $property) {
    ...
    private function copyObjectProperty($object, \ReflectionProperty $property)
    {
        ...
        foreach ($this->filters as $item) {
            ...
            if ($matcher->matches($object, $property->getName())) {
                ...
                if ($filter instanceof \GFPDF_Vendor\DeepCopy\Filter\ChainableFilter) {
                    continue;
                }
