 ...
 */
class PdfDictionary extends \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfType
{
    /**
     ...
     */
    public static function parse(\GFPDF_Vendor\setasign\Fpdi\PdfParser\Tokenizer $tokenizer, \GFPDF_Vendor\setasign\Fpdi\PdfParser\StreamReader $streamReader, \GFPDF_Vendor\setasign\Fpdi\PdfParser\PdfParser $parser)
    {
        $entries = [];
        while (\true) {
            ...
            if (!$key instanceof \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfName) {
                $lastToken = null;
            ...
            if ($value instanceof \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfNull) {
                continue;
            }
            // catch missing value
            if ($value instanceof \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfToken && $value->value === '>' && $streamReader->getByte() === '>') {
                $streamReader->addOffset(1);
                break;
            }
     ...
     * @throws PdfTypeException
     */
    public static function get($dictionary, $key, \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfType $default = null)
    {
        $dictionary = self::ensure($dictionary);
        if (isset($dictionary->value[$key])) {
            return $dictionary->value[$key];
        }
        return $default === null ? new \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfNull() : $default;
    }
    /**
    ...
    {
        return \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfType::ensureType(self::class, $dictionary, 'Dictionary value expected.');
    }
}
