 ...
 */
class PdfArray extends \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfType
{
    /**
     ...
     */
    public static function parse(\GFPDF_Vendor\setasign\Fpdi\PdfParser\Tokenizer $tokenizer, \GFPDF_Vendor\setasign\Fpdi\PdfParser\PdfParser $parser)
    {
        $result = [];
    ...
    {
        $result = \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfType::ensureType(self::class, $array, 'Array value expected.');
        if ($size !== null && \count($array->value) !== $size) {
            throw new \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfTypeException(\sprintf('Array with %s entries expected.', $size), \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfTypeException::INVALID_DATA_SIZE);
        }
        return $result;
    }
}
