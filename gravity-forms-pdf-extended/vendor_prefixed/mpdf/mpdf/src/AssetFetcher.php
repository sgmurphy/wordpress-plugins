...
class AssetFetcher implements \GFPDF_Vendor\Psr\Log\LoggerAwareInterface
{
    use PsrLogAwareTrait;
    private $mpdf;
    private $contentLoader;
    private $http;
    public function __construct(\GFPDF_Vendor\Mpdf\Mpdf $mpdf, \GFPDF_Vendor\Mpdf\File\LocalContentLoaderInterface $contentLoader, \GFPDF_Vendor\Mpdf\Http\ClientInterface $http, \GFPDF_Vendor\Psr\Log\LoggerInterface $logger)
    {
        $this->mpdf = $mpdf;
    ...
    public function fetchRemoteContent($path)
    {
        ...
        try {
            ...
            $response = $this->http->sendRequest(new \GFPDF_Vendor\Mpdf\PsrHttpMessageShim\Request('GET', $path));
