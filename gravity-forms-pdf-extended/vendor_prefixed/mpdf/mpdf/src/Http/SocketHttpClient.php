...
class SocketHttpClient implements \GFPDF_Vendor\Mpdf\Http\ClientInterface, \GFPDF_Vendor\Psr\Log\LoggerAwareInterface
{
    use PsrLogAwareTrait;
    public function __construct(\GFPDF_Vendor\Psr\Log\LoggerInterface $logger)
    {
        $this->logger = $logger;
    }
    public function sendRequest(\GFPDF_Vendor\Psr\Http\Message\RequestInterface $request)
    {
        ...
        $url = $request->getUri();
        if (\is_string($url)) {
            $url = new \GFPDF_Vendor\Mpdf\Http\Uri($url);
        }
        $timeout = 1;
        $file = $url->getPath() ?: '/';
        ...
        $this->logger->debug(\sprintf('Opening socket on %s:%s of URL "%s"', $socketPath, $port, $request->getUri()), ['context' => \GFPDF_Vendor\Mpdf\Log\Context::REMOTE_CONTENT]);
        $response = new \GFPDF_Vendor\Mpdf\PsrHttpMessageShim\Response();
        if (!($fh = @\fsockopen($socketPath, $port, $errno, $errstr, $timeout))) {
            $this->logger->error(\sprintf('Socket error "%s": "%s"', $errno, $errstr), ['context' => \GFPDF_Vendor\Mpdf\Log\Context::REMOTE_CONTENT]);
            return $response;
        }
        ...
        while (!\feof($fh)) {
            ...
            $response = $response->withHeader($parsedHeader['headerName'], \trim($parsedHeader['headerValue']));
        }
        $body = '';
        while (!\feof($fh)) {
            $line = \fgets($fh, 1024);
        ...
        \fclose($fh);
        $stream = \GFPDF_Vendor\Mpdf\PsrHttpMessageShim\Stream::create($body);
        $stream->rewind();
