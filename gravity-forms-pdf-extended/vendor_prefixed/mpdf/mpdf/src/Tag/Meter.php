...
class Meter extends \GFPDF_Vendor\Mpdf\Tag\InlineTag
{
    public function open($attr, &$ahtml, &$ihtml)
    {
        $tag = $this->getTagName();
        $this->mpdf->inMeter = \true;
        $max = 1;
        if (!empty($attr['MAX'])) {
            $max = $attr['MAX'];
        }
        ...
        if ($w == 0 && $h == 0) {
            ...
            $w = $this->mpdf->FontSize / (10 / \GFPDF_Vendor\Mpdf\Mpdf::SCALE) * \abs($info['w']) / \GFPDF_Vendor\Mpdf\Mpdf::SCALE;
            $h = $this->mpdf->FontSize / (10 / \GFPDF_Vendor\Mpdf\Mpdf::SCALE) * \abs($info['h']) / \GFPDF_Vendor\Mpdf\Mpdf::SCALE;
        }
