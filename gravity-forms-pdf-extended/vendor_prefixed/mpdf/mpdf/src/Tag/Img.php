...
class Img extends \GFPDF_Vendor\Mpdf\Tag\Tag
{
    public function open($attr, &$ahtml, &$ihtml)
    {
        ...
        if (isset($attr['SRC'])) {
            ...
            if (isset($properties['MAX-WIDTH'])) {
                $maxw = $this->sizeConverter->convert($properties['MAX-WIDTH'], $this->mpdf->blk[$this->mpdf->blklvl]['inner_width'], $this->mpdf->FontSize, \false);
            } elseif (isset($attr['MAX-WIDTH'])) {
                $maxw = $this->sizeConverter->convert($attr['MAX-WIDTH'], $this->mpdf->blk[$this->mpdf->blklvl]['inner_width'], $this->mpdf->FontSize, \false);
            }
            if (isset($properties['MAX-HEIGHT'])) {
                $maxh = $this->sizeConverter->convert($properties['MAX-HEIGHT'], $this->mpdf->blk[$this->mpdf->blklvl]['inner_width'], $this->mpdf->FontSize, \false);
            } elseif (isset($attr['MAX-HEIGHT'])) {
                $maxh = $this->sizeConverter->convert($attr['MAX-HEIGHT'], $this->mpdf->blk[$this->mpdf->blklvl]['inner_width'], $this->mpdf->FontSize, \false);
            }
            if (isset($properties['MIN-WIDTH'])) {
                $minw = $this->sizeConverter->convert($properties['MIN-WIDTH'], $this->mpdf->blk[$this->mpdf->blklvl]['inner_width'], $this->mpdf->FontSize, \false);
            } elseif (isset($attr['MIN-WIDTH'])) {
                $minw = $this->sizeConverter->convert($attr['MIN-WIDTH'], $this->mpdf->blk[$this->mpdf->blklvl]['inner_width'], $this->mpdf->FontSize, \false);
            }
            if (isset($properties['MIN-HEIGHT'])) {
                $minh = $this->sizeConverter->convert($properties['MIN-HEIGHT'], $this->mpdf->blk[$this->mpdf->blklvl]['inner_width'], $this->mpdf->FontSize, \false);
            } elseif (isset($attr['MIN-HEIGHT'])) {
                $minh = $this->sizeConverter->convert($attr['MIN-HEIGHT'], $this->mpdf->blk[$this->mpdf->blklvl]['inner_width'], $this->mpdf->FontSize, \false);
            }
            ...
            // Image file
            $info = $this->imageProcessor->getImage($srcpath, \true, \true, $orig_srcpath, $interpolation);
            // mPDF 6
            if (!$info) {
            ...
            if ($w == 0 && $h == 0) {
                ...
                if ($info['type'] === 'wmf') {
                    ...
                    $w = \abs($info['w']) / (20 * \GFPDF_Vendor\Mpdf\Mpdf::SCALE);
                    ...
                    if ($info['type'] === 'svg') {
                        ...
                        $h = \abs($info['h']) / \GFPDF_Vendor\Mpdf\Mpdf::SCALE;
                    } else {
                        //Put image at default image dpi
                        $w = $info['w'] / \GFPDF_Vendor\Mpdf\Mpdf::SCALE * (72 / $this->mpdf->img_dpi);
                        $h = $info['h'] / \GFPDF_Vendor\Mpdf\Mpdf::SCALE * (72 / $this->mpdf->img_dpi);
                    }
                }
            ...
            $e = \GFPDF_Vendor\Mpdf\Mpdf::OBJECT_IDENTIFIER . "type=image,objattr=" . \serialize($objattr) . \GFPDF_Vendor\Mpdf\Mpdf::OBJECT_IDENTIFIER;
            /* -- TABLES -- */
            ...
            if ($this->mpdf->title2annots && isset($attr['TITLE'])) {
                ...
                $e = \GFPDF_Vendor\Mpdf\Mpdf::OBJECT_IDENTIFIER . "type=annot,objattr=" . \serialize($objattr) . \GFPDF_Vendor\Mpdf\Mpdf::OBJECT_IDENTIFIER;
                if ($this->mpdf->tableLevel) {
                    // *TABLES*
