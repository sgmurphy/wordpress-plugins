...
class BarCode extends \GFPDF_Vendor\Mpdf\Tag\Tag
{
    /**
     * @var \Mpdf\Barcode
     */
        ...
        if (!empty($attr['CODE'])) {
            ...
            if (\in_array($objattr['btype'], ['EAN13', 'ISBN', 'ISSN', 'UPCA', 'UPCE', 'EAN8'])) {
                ...
                if ($objattr['showtext'] && $objattr['btype'] === 'EAN13' || $objattr['btype'] === 'ISBN' || $objattr['btype'] === 'ISSN') {
                    $tisbnm = 1.5 * $objattr['bsize'];
                    // Top margin between TOP TEXT (isbn - if shown) & bars
                    $isbn_fontsize = 2.1 * $objattr['bsize'];
                    $h += $isbn_fontsize + $tisbnm;
                }
            } elseif ($objattr['btype'] === 'QR') {
                // QR-code
                $w = $h = $objattr['bsize'] * 25;
                // Factor of 25mm (default)
            ...
            /* -- END CSS-IMAGE-FLOAT -- */
            $e = \GFPDF_Vendor\Mpdf\Mpdf::OBJECT_IDENTIFIER . "type=barcode,objattr=" . \serialize($objattr) . \GFPDF_Vendor\Mpdf\Mpdf::OBJECT_IDENTIFIER;
            /* -- TABLES -- */
