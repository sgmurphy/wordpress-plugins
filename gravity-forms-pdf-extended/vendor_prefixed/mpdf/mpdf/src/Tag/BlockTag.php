...
use GFPDF_Vendor\Mpdf\Conversion\DecToAlpha;
use GFPDF_Vendor\Mpdf\Conversion\DecToRoman;
use GFPDF_Vendor\Mpdf\Mpdf;
use GFPDF_Vendor\Mpdf\Utils\Arrays;
use GFPDF_Vendor\Mpdf\Utils\UtfString;
abstract class BlockTag extends \GFPDF_Vendor\Mpdf\Tag\Tag
{
    public function open($attr, &$ahtml, &$ihtml)
    {
        ...
        if (isset($p['POSITION']) && (\strtolower($p['POSITION']) === 'fixed' || \strtolower($p['POSITION']) === 'absolute') && $this->mpdf->blklvl == 0) {
            if ($this->mpdf->inFixedPosBlock) {
                throw new \GFPDF_Vendor\Mpdf\MpdfException('Cannot nest block with position:fixed or position:absolute');
            }
        ...
        $this->mpdf->InlineBDF = [];
        // mPDF 6
        $this->mpdf->InlineBDFctr = 0;
        // mPDF 6
        $this->mpdf->InlineProperties = [];
        $this->mpdf->divbegin = \true;
        ...
        if ($this->mpdf->tableLevel) {
            ...
            if ($tag === 'LI') {
                ...
                $decToAlpha = new \GFPDF_Vendor\Mpdf\Conversion\DecToAlpha();
                $decToRoman = new \GFPDF_Vendor\Mpdf\Conversion\DecToRoman();
        ...
        // mPDF 6 (uses $p - preview of properties so blklvl can be incremented after page-break)
        if (!$this->mpdf->tableLevel && ($pagesel && (!$this->mpdf->page_box['current'] || $pagesel != $this->mpdf->page_box['current']) || isset($p['PAGE-BREAK-BEFORE']) && $p['PAGE-BREAK-BEFORE'])) {
        ...
        $this->mpdf->setCSS($properties, 'BLOCK', $tag);
        ...
        if (isset($properties['HEIGHT'])) {
            $currblk['css_set_height'] = $this->sizeConverter->convert($properties['HEIGHT'], $this->mpdf->h - $this->mpdf->tMargin - $this->mpdf->bMargin, $this->mpdf->FontSize, \false);
        ...
        if (isset($properties['FLOAT']) && \strtoupper($properties['FLOAT']) === 'RIGHT' && !$this->mpdf->ColActive) {
            ...
            if ($setwidth + $currblk['margin_right'] + $bdl + $pdl + $bdr + $pdr > $maxw || $maxw - ($currblk['margin_right'] + $bdl + $pdl + $bdr + $pdr) < 2 * $this->mpdf->GetCharWidth('W', \false)) {
                ...
                if ($l_max < $r_max && $setwidth + $currblk['margin_right'] + $bdl + $pdl + $bdr + $pdr <= $container_w - $r_width && $container_w - $r_width - ($currblk['margin_right'] + $bdl + $pdl + $bdr + $pdr) > $doubleCharWidth) {
                    $this->mpdf->ClearFloats('LEFT', $this->mpdf->blklvl - 1);
                } elseif ($r_max < $l_max && $setwidth + $currblk['margin_right'] + $bdl + $pdl + $bdr + $pdr <= $container_w - $l_width && $container_w - $l_width - ($currblk['margin_right'] + $bdl + $pdl + $bdr + $pdr) > $doubleCharWidth) {
            ...
            if ($setwidth + $currblk['margin_left'] + $currblk['margin_right'] + $bdl + $pdl + $bdr + $pdr > $maxw || $maxw - ($currblk['margin_right'] + $currblk['margin_left'] + $bdl + $pdl + $bdr + $pdr) < $doubleCharWidth) {
                // Too narrow to fit - try to move down past L or R float
                if ($l_max < $r_max && $setwidth + $currblk['margin_left'] + $currblk['margin_right'] + $bdl + $pdl + $bdr + $pdr <= $container_w - $r_width && $container_w - $r_width - ($currblk['margin_right'] + $currblk['margin_left'] + $bdl + $pdl + $bdr + $pdr) > $doubleCharWidth) {
                    $this->mpdf->ClearFloats('LEFT', $this->mpdf->blklvl - 1);
                } elseif ($r_max < $l_max && $setwidth + $currblk['margin_left'] + $currblk['margin_right'] + $bdl + $pdl + $bdr + $pdr <= $container_w - $l_width && $container_w - $l_width - ($currblk['margin_right'] + $currblk['margin_left'] + $bdl + $pdl + $bdr + $pdr) > $doubleCharWidth) {
                    $this->mpdf->ClearFloats('RIGHT', $this->mpdf->blklvl - 1);
                } else {
                    $this->mpdf->ClearFloats('BOTH', $this->mpdf->blklvl - 1);
                }
        ...
        if ($this->mpdf->autoPadding && !$this->mpdf->ColActive) {
            $currblk['border_radius_TL_H'] = \GFPDF_Vendor\Mpdf\Utils\Arrays::get($currblk, 'border_radius_TL_H', 0);
            $currblk['border_radius_TL_V'] = \GFPDF_Vendor\Mpdf\Utils\Arrays::get($currblk, 'border_radius_TL_V', 0);
            $currblk['border_radius_TR_H'] = \GFPDF_Vendor\Mpdf\Utils\Arrays::get($currblk, 'border_radius_TR_H', 0);
            $currblk['border_radius_TR_V'] = \GFPDF_Vendor\Mpdf\Utils\Arrays::get($currblk, 'border_radius_TR_V', 0);
            $currblk['border_radius_BL_H'] = \GFPDF_Vendor\Mpdf\Utils\Arrays::get($currblk, 'border_radius_BL_H', 0);
            $currblk['border_radius_BL_V'] = \GFPDF_Vendor\Mpdf\Utils\Arrays::get($currblk, 'border_radius_BL_V', 0);
            $currblk['border_radius_BR_H'] = \GFPDF_Vendor\Mpdf\Utils\Arrays::get($currblk, 'border_radius_BR_H', 0);
            $currblk['border_radius_BR_V'] = \GFPDF_Vendor\Mpdf\Utils\Arrays::get($currblk, 'border_radius_BR_V', 0);
        ...
        $cbti = $this->sizeConverter->convert($currblk['text_indent'], $this->mpdf->blk[$this->mpdf->blklvl]['inner_width'], $this->mpdf->FontSize, \false);
        if ($cbti < 0) {
            $hangind = -$cbti;
        ...
        if ($this->mpdf->use_kwt && isset($attr['KEEP-WITH-TABLE']) && !$this->mpdf->ColActive && !$this->mpdf->keep_block_together) {
            ...
            $this->mpdf->kwt_x0 = $this->mpdf->lMargin;
            // mPDF 6
        ...
        $currblk['initial_y0'] = $this->mpdf->y;
        // mPDF 6
        $currblk['x0'] = $this->mpdf->x;
        $currblk['initial_x0'] = $this->mpdf->x;
        // mPDF 6
        $currblk['initial_startpage'] = $this->mpdf->page;
        $currblk['startpage'] = $this->mpdf->page;
        // mPDF 6
        ...
        if ($bdf) {
            if ($bdf2) {
                $bdf2 = \GFPDF_Vendor\Mpdf\Utils\UtfString::code2utf($bdf);
            }
            $this->mpdf->OTLdata = [];
            if ($this->mpdf->tableLevel) {
                $this->mpdf->_saveCellTextBuffer(\GFPDF_Vendor\Mpdf\Utils\UtfString::code2utf($bdf) . $bdf2);
            } else {
                $this->mpdf->_saveTextBuffer(\GFPDF_Vendor\Mpdf\Utils\UtfString::code2utf($bdf) . $bdf2);
            }
    ...
    public function close(&$ahtml, &$ihtml)
    {
        ...
        $this->mpdf->blockjustfinished = \true;
            ...
            if (isset($this->mpdf->h2toc[$tag]) || isset($this->mpdf->h2bookmarks[$tag])) {
                ...
                if (\count($this->mpdf->textbuffer) == 1) {
                    ...
                    for ($i = 0; $i < \count($this->mpdf->textbuffer); $i++) {
                        if (0 !== \strpos($this->mpdf->textbuffer[$i][0], \GFPDF_Vendor\Mpdf\Mpdf::OBJECT_IDENTIFIER)) {
                            //inline object
        ...
        if (\count($this->mpdf->textbuffer) && $this->mpdf->textbuffer[\count($this->mpdf->textbuffer) - 1]) {
            if (0 !== \strpos($this->mpdf->textbuffer[\count($this->mpdf->textbuffer) - 1][0], \GFPDF_Vendor\Mpdf\Mpdf::OBJECT_IDENTIFIER)) {
                // not special content
        ...
        if (\count($this->mpdf->textbuffer) == 0 && $this->mpdf->lastblocklevelchange != 0) {
            ...
            $this->mpdf->newFlowingBlock($this->mpdf->blk[$this->mpdf->blklvl]['width'], $this->mpdf->lineheight, '', \false, $blockstate, \true, isset($this->mpdf->blk[$this->mpdf->blklvl]['direction']) ? $this->mpdf->blk[$this->mpdf->blklvl]['direction'] : 'ltr');
            $this->mpdf->finishFlowingBlock(\true);
        ...
        if ($this->mpdf->blk[$this->mpdf->blklvl]['float'] === 'R') {
            ...
            $this->mpdf->floatDivs[] = ['side' => 'R', 'startpage' => $this->mpdf->blk[$this->mpdf->blklvl]['startpage'], 'y0' => $this->mpdf->blk[$this->mpdf->blklvl]['float_start_y'], 'startpos' => $this->mpdf->blk[$this->mpdf->blklvl]['startpage'] * 1000 + $this->mpdf->blk[$this->mpdf->blklvl]['float_start_y'], 'endpage' => $this->mpdf->page, 'y1' => $this->mpdf->y, 'endpos' => $this->mpdf->page * 1000 + $this->mpdf->y, 'w' => $this->mpdf->blk[$this->mpdf->blklvl]['float_width'], 'blklvl' => $this->mpdf->blklvl, 'blockContext' => $this->mpdf->blk[$this->mpdf->blklvl - 1]['blockContext']];
        ...
        if ($this->mpdf->blk[$this->mpdf->blklvl]['float'] === 'L') {
            ...
            $this->mpdf->floatDivs[] = ['side' => 'L', 'startpage' => $this->mpdf->blk[$this->mpdf->blklvl]['startpage'], 'y0' => $this->mpdf->blk[$this->mpdf->blklvl]['float_start_y'], 'startpos' => $this->mpdf->blk[$this->mpdf->blklvl]['startpage'] * 1000 + $this->mpdf->blk[$this->mpdf->blklvl]['float_start_y'], 'endpage' => $this->mpdf->page, 'y1' => $this->mpdf->y, 'endpos' => $this->mpdf->page * 1000 + $this->mpdf->y, 'w' => $this->mpdf->blk[$this->mpdf->blklvl]['float_width'], 'blklvl' => $this->mpdf->blklvl, 'blockContext' => $this->mpdf->blk[$this->mpdf->blklvl - 1]['blockContext']];
            $this->mpdf->y = $this->mpdf->blk[$this->mpdf->blklvl]['float_start_y'];
            $this->mpdf->page = $this->mpdf->blk[$this->mpdf->blklvl]['startpage'];
            $this->mpdf->ResetMargins();
            $this->mpdf->pageoutput[$this->mpdf->page] = [];
        ...
        if ($this->mpdf->blk[$this->mpdf->blklvl]['keep_block_together']) {
            ...
            // avoid re-iterating; read in OpenTag()
        ...
        if (!$this->mpdf->tableLevel && $page_break_after) {
            ...
            $save_bflpc = $this->mpdf->InlineBDFctr;
            // mPDF 6
            // mPDF 6 pagebreaktype
            $startpage = $this->mpdf->page;
            $pagebreaktype = $this->mpdf->defaultPagebreakType;
            ...
            $this->mpdf->InlineBDFctr = $save_bflpc;
            // mPDF 6
