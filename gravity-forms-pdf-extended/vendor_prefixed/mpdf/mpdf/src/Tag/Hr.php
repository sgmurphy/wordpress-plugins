...
use GFPDF_Vendor\Mpdf\Utils\NumericString;
class Hr extends \GFPDF_Vendor\Mpdf\Tag\Tag
{
    public function open($attr, &$ahtml, &$ihtml)
    {
        ...
        if (isset($properties['HEIGHT'])) {
            $objattr['linewidth'] = $this->sizeConverter->convert($properties['HEIGHT'], $this->mpdf->blk[$this->mpdf->blklvl]['inner_width'], $this->mpdf->FontSize, \false);
        }
        /* -- TABLES -- */
        if ($this->mpdf->tableLevel) {
            $objattr['W-PERCENT'] = 100;
            if (isset($properties['WIDTH']) && \GFPDF_Vendor\Mpdf\Utils\NumericString::containsPercentChar($properties['WIDTH'])) {
                $properties['WIDTH'] = \GFPDF_Vendor\Mpdf\Utils\NumericString::removePercentChar($properties['WIDTH']);
                // make "90%" become simply "90"
                $objattr['W-PERCENT'] = $properties['WIDTH'];
            }
            if (isset($attr['WIDTH']) && \GFPDF_Vendor\Mpdf\Utils\NumericString::containsPercentChar($attr['WIDTH'])) {
                $attr['WIDTH'] = \GFPDF_Vendor\Mpdf\Utils\NumericString::removePercentChar($attr['WIDTH']);
                // make "90%" become simply "90"
