...
namespace GFPDF_Vendor\Mpdf\Image;

use GFPDF_Vendor\Mpdf\AssetFetcher;
...
use GFPDF_Vendor\Mpdf\Gif\Gif;
use GFPDF_Vendor\Mpdf\Language\LanguageToFontInterface;
use GFPDF_Vendor\Mpdf\Language\ScriptToLanguageInterface;
...
class ImageProcessor implements \GFPDF_Vendor\Psr\Log\LoggerAwareInterface
{
    ...
    public function __construct(\GFPDF_Vendor\Mpdf\Mpdf $mpdf, \GFPDF_Vendor\Mpdf\Otl $otl, \GFPDF_Vendor\Mpdf\CssManager $cssManager, \GFPDF_Vendor\Mpdf\SizeConverter $sizeConverter, \GFPDF_Vendor\Mpdf\Color\ColorConverter $colorConverter, \GFPDF_Vendor\Mpdf\Color\ColorModeConverter $colorModeConverter, \GFPDF_Vendor\Mpdf\Cache $cache, \GFPDF_Vendor\Mpdf\Language\LanguageToFontInterface $languageToFont, \GFPDF_Vendor\Mpdf\Language\ScriptToLanguageInterface $scriptToLanguage, \GFPDF_Vendor\Mpdf\AssetFetcher $assetFetcher, \GFPDF_Vendor\Psr\Log\LoggerInterface $logger)
    {
        $this->mpdf = $mpdf;
        $this->otl = $otl;
        ...
        $this->logger = $logger;
        $this->guesser = new \GFPDF_Vendor\Mpdf\Image\ImageTypeGuesser();
        $this->failedImages = [];
    }
    public function getImage(&$file, $firstTime = \true, $allowvector = \true, $orig_srcpath = \false, $interpolation = \false)
    {
        ...
        if (!$data) {
            ...
            } catch (\GFPDF_Vendor\Mpdf\Exception\AssetFetchingException $e) {
    ...
    private function gzCompress($data)
    {
        if (!\function_exists('gzcompress')) {
            throw new \GFPDF_Vendor\Mpdf\MpdfException('gzcompress is not available. install ext-zlib extension.');
        }
    ...
    private function imageError($file, $firstTime, $msg)
    {
        ...
        if ($firstTime && ($this->mpdf->showImageErrors || $this->mpdf->debug)) {
            throw new \GFPDF_Vendor\Mpdf\MpdfImageException(\sprintf('%s (%s)', $msg, \substr($file, 0, 256)));
        }
        $this->logger->warning(\sprintf('%s (%s)', $msg, $file), ['context' => \GFPDF_Vendor\Mpdf\Log\Context::IMAGES]);
    }
    /**
     * @since mPDF 5.7.4
     * @param string $url
    ...
    public function processJpg($data, $file, $firstTime, $interpolation)
    {
        ...
        if ($a[2] === 'DeviceCMYK' && ($this->mpdf->restrictColorSpace === 2 || $this->mpdf->PDFA && $this->mpdf->restrictColorSpace !== 3)) {
            ...
            if (!\function_exists('gd_info')) {
                throw new \GFPDF_Vendor\Mpdf\MpdfException(\sprintf('JPG image may not use CMYK color space (%s).', $file));
            }
    ...
    public function processPng($data, $file, $firstTime, $interpolation)
    {
        ...
        if ($firstTime && ($colspace === 'DeviceRGB' || $colspace === 'Indexed') && ($this->mpdf->PDFX || $this->mpdf->restrictColorSpace === 3)) {
            ...
            if ($pngalpha) {
                if ($this->mpdf->PDFA) {
                    throw new \GFPDF_Vendor\Mpdf\MpdfException(\sprintf('PDFA1-b does not permit images with alpha channel transparency (%s).', $file));
                }
    ...
    {
        $svg = new \GFPDF_Vendor\Mpdf\Image\Svg($this->mpdf, $this->otl, $this->cssManager, $this, $this->sizeConverter, $this->colorConverter, $this->languageToFont, $this->scriptToLanguage);
    ...
    public function processGif($data, $file, $firstTime, $interpolation)
    {
        ...
        $gif = new \GFPDF_Vendor\Mpdf\Gif\Gif();
        $h = 0;
        $w = 0;
        $gif->loadFile($data, 0);
        $nColors = 0;
    ...
    {
        if ($this->bmp === null) {
            $this->bmp = new \GFPDF_Vendor\Mpdf\Image\Bmp($this->mpdf);
        }
    ...
    {
        if ($this->wmf === null) {
            $this->wmf = new \GFPDF_Vendor\Mpdf\Image\Wmf($this->mpdf, $this->colorConverter);
        }
