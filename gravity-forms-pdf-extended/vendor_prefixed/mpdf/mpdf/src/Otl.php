...
use GFPDF_Vendor\Mpdf\Shaper\Indic;
use GFPDF_Vendor\Mpdf\Shaper\Myanmar;
use GFPDF_Vendor\Mpdf\Shaper\Sea;
use GFPDF_Vendor\Mpdf\Utils\UtfString;
class Otl
{
    use Strict;
    const _OTL_OLD_SPEC_COMPAT_1 = \true;
    ...
    var $debugOTL = \false;
    public function __construct(\GFPDF_Vendor\Mpdf\Mpdf $mpdf, \GFPDF_Vendor\Mpdf\Fonts\FontCache $fontCache)
    {
        $this->mpdf = $mpdf;
    ...
    function applyOTL($str, $useOTL)
    {
        ...
        foreach ($earr as $char) {
            $ucd_record = \GFPDF_Vendor\Mpdf\Ucdn::get_ucd_record($char);
            $sbl = $ucd_record[6];
            // Special case - Arabic End of Ayah
            if ($char == 1757) {
                $sbl = \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_ARABIC;
            }
        ...
        for ($sch = 0; $sch <= $subchunk; $sch++) {
            ...
            if (\GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_DEVANAGARI <= $scriptblock && $scriptblock <= \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_MALAYALAM) {
                $this->shaper = "I";
            } elseif ($scriptblock == \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_ARABIC || $scriptblock == \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_SYRIAC) {
                $this->shaper = "A";
            } elseif ($scriptblock == \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_NKO || $scriptblock == \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_MANDAIC) {
                $this->shaper = "A";
            } elseif ($scriptblock == \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_KHMER) {
                $this->shaper = "K";
            } elseif ($scriptblock == \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_THAI) {
                $this->shaper = "T";
            } elseif ($scriptblock == \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_LAO) {
                $this->shaper = "L";
            } elseif ($scriptblock == \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_SINHALA) {
                $this->shaper = "S";
            } elseif ($scriptblock == \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_MYANMAR) {
                $this->shaper = "M";
            } elseif ($scriptblock == \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_NEW_TAI_LUE) {
                $this->shaper = "E";
            } elseif ($scriptblock == \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_CHAM) {
                $this->shaper = "E";
            } elseif ($scriptblock == \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_TAI_THAM) {
                $this->shaper = "E";
            } else {
                $this->shaper = "";
            }
            // Get scripttag based on actual text script
            $scripttag = \GFPDF_Vendor\Mpdf\Ucdn::$uni_scriptblock[$scriptblock];
            $GSUBscriptTag = '';
            $GSUBlangsys = '';
            $GPOSscriptTag = '';
            $GPOSlangsys = '';
            ...
            if (!isset($this->GDEFdata[$this->fontkey]['GSUBGPOStables'])) {
                ...
                if (!$this->ttfOTLdata) {
                    throw new \GFPDF_Vendor\Mpdf\MpdfException('Can\'t open file ' . $this->fontCache->tempFilename($this->fontkey . '.GSUBGPOStables.dat'));
                }
            } else {
            ...
            } elseif ($this->mpdf->useTibetanLBR && $scriptblock == \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_TIBETAN) {
                // Sets $this->OTLdata[$i]['wordend']=true at possible end of word boundaries
            ...
            if ($useOTL & 0xff && $GSUBscriptTag && $GSUBlangsys && $GSUBFeatures) {
                ...
                if (!isset($this->GSUBdata[$this->GSUBfont])) {
                    ...
                    if ($this->fontCache->jsonHas($fontCacheFilename)) {
                        ...
                        $this->GSUBdata[$this->GSUBfont] = ['rtlSUB' => [], 'rphf' => [], 'rphf' => [], 'pref' => [], 'blwf' => [], 'pstf' => [], 'finals' => ''];
                    }
                }
                ...
                if ($this->shaper == 'A') {
                    ...
                    for ($ptr = 1; $ptr < \count($this->OTLdata); $ptr++) {
                        if ($this->OTLdata[$ptr]['general_category'] == \GFPDF_Vendor\Mpdf\Ucdn::UNICODE_GENERAL_CATEGORY_NON_SPACING_MARK && $this->OTLdata[$ptr - 1]['uni'] == 32) {
                            $this->OTLdata[$ptr - 1]['uni'] = 0xa0;
                            $this->OTLdata[$ptr - 1]['hex'] = '000A0';
                        }
                    }
                } elseif ($this->shaper == 'I' || $this->shaper == 'K' || $this->shaper == 'S') {
                    $this->restrictToSyllable = \true;
                    //-----------------------------------------------------------------------------------
                    ...
                    for ($ptr = 0; $ptr < \count($this->OTLdata); $ptr++) {
                        ...
                        $sub = \GFPDF_Vendor\Mpdf\Shaper\Indic::decompose_indic($char);
                        if ($sub) {
                            $newinfo = [];
                            for ($i = 0; $i < \count($sub); $i++) {
                                $newinfo[$i] = [];
                                $ucd_record = \GFPDF_Vendor\Mpdf\Ucdn::get_ucd_record($sub[$i]);
                        ...
                        if ($this->shaper == 'I') {
                            if ($char == 0x9af && isset($this->OTLdata[$ptr + 1]) && $this->OTLdata[$ptr + 1]['uni'] == 0x9bc) {
                                $sub = 0x9df;
                                $newinfo = [];
                                $newinfo[0] = [];
                                $ucd_record = \GFPDF_Vendor\Mpdf\Ucdn::get_ucd_record($sub);
                    ...
                    foreach ($this->OTLdata as $eid => $c) {
                        \GFPDF_Vendor\Mpdf\Shaper\Indic::set_indic_properties($this->OTLdata[$eid], $scriptblock);
                        // sets ['indic_category'] and ['indic_position']
                        //$c['general_category']
                        //$c['combining_class']
                        //$c['uni'] =  $char;
                        $indic_category_string .= \GFPDF_Vendor\Mpdf\Shaper\Indic::$indic_category_char[$this->OTLdata[$eid]['indic_category']];
                    }
                    $broken_syllables = \false;
                    if ($this->shaper == 'I') {
                        \GFPDF_Vendor\Mpdf\Shaper\Indic::set_syllables($this->OTLdata, $indic_category_string, $broken_syllables);
                    } elseif ($this->shaper == 'S') {
                        \GFPDF_Vendor\Mpdf\Shaper\Indic::set_syllables_sinhala($this->OTLdata, $indic_category_string, $broken_syllables);
                    } elseif ($this->shaper == 'K') {
                        \GFPDF_Vendor\Mpdf\Shaper\Indic::set_syllables_khmer($this->OTLdata, $indic_category_string, $broken_syllables);
                    }
                    $indic_category_string = '';
                    //-----------------------------------------------------------------------------------
                    ...
                    $indic_config = \GFPDF_Vendor\Mpdf\Shaper\Indic::$indic_configs[$scriptblock];
                    $dottedcircle = \false;
                    if ($broken_syllables) {
                        if ($this->mpdf->_charDefined($this->mpdf->fonts[$this->fontkey]['cw'], 0x25cc)) {
                            $dottedcircle = [];
                            $ucd_record = \GFPDF_Vendor\Mpdf\Ucdn::get_ucd_record(0x25cc);
                            ...
                            $dottedcircle[0]['indic_category'] = \GFPDF_Vendor\Mpdf\Shaper\Indic::OT_DOTTEDCIRCLE;
                            $dottedcircle[0]['indic_position'] = \GFPDF_Vendor\Mpdf\Shaper\Indic::POS_BASE_C;
                            $dottedcircle[0]['hex'] = '025CC';
                            // TEMPORARY *****
                        }
                    }
                    \GFPDF_Vendor\Mpdf\Shaper\Indic::initial_reordering($this->OTLdata, $this->GSUBdata[$this->GSUBfont], $broken_syllables, $indic_config, $scriptblock, $is_old_spec, $dottedcircle);
                    //-----------------------------------------------------------------------------------
                    ...
                    \GFPDF_Vendor\Mpdf\Shaper\Indic::final_reordering($this->OTLdata, $this->GSUBdata[$this->GSUBfont], $indic_config, $scriptblock, $is_old_spec);
                    //-----------------------------------------------------------------------------------
                    ...
                    foreach ($this->OTLdata as $eid => $c) {
                        \GFPDF_Vendor\Mpdf\Shaper\Myanmar::set_myanmar_properties($this->OTLdata[$eid]);
                        // sets ['myanmar_category'] and ['myanmar_position']
                        $myanmar_category_string .= \GFPDF_Vendor\Mpdf\Shaper\Myanmar::$myanmar_category_char[$this->OTLdata[$eid]['myanmar_category']];
                    }
                    $broken_syllables = \false;
                    \GFPDF_Vendor\Mpdf\Shaper\Myanmar::set_syllables($this->OTLdata, $myanmar_category_string, $broken_syllables);
                    $myanmar_category_string = '';
                    //-----------------------------------------------------------------------------------
                    ...
                    if ($broken_syllables) {
                        if ($this->mpdf->_charDefined($this->mpdf->fonts[$this->fontkey]['cw'], 0x25cc)) {
                            ...
                            $dottedcircle[0]['group'] = 'C';
                            $dottedcircle[0]['uni'] = 0x25cc;
                            $dottedcircle[0]['myanmar_category'] = \GFPDF_Vendor\Mpdf\Shaper\Myanmar::OT_DOTTEDCIRCLE;
                            $dottedcircle[0]['myanmar_position'] = \GFPDF_Vendor\Mpdf\Shaper\Myanmar::POS_BASE_C;
                            $dottedcircle[0]['hex'] = '025CC';
                        }
                    }
                    \GFPDF_Vendor\Mpdf\Shaper\Myanmar::reordering($this->OTLdata, $this->GSUBdata[$this->GSUBfont], $broken_syllables, $dottedcircle);
                    //-----------------------------------------------------------------------------------
                    ...
                    $sea_category_string = '';
                    foreach ($this->OTLdata as $eid => $c) {
                        \GFPDF_Vendor\Mpdf\Shaper\Sea::set_sea_properties($this->OTLdata[$eid], $scriptblock);
                        // sets ['sea_category'] and ['sea_position']
                        //$c['general_category']
                        //$c['combining_class']
                        //$c['uni'] =  $char;
                        $sea_category_string .= \GFPDF_Vendor\Mpdf\Shaper\Sea::$sea_category_char[$this->OTLdata[$eid]['sea_category']];
                    }
                    $broken_syllables = \false;
                    \GFPDF_Vendor\Mpdf\Shaper\Sea::set_syllables($this->OTLdata, $sea_category_string, $broken_syllables);
                    $sea_category_string = '';
                    //-----------------------------------------------------------------------------------
                    ...
                    if ($broken_syllables) {
                        if ($this->mpdf->_charDefined($this->mpdf->fonts[$this->fontkey]['cw'], 0x25cc)) {
                            ...
                            $dottedcircle[0]['group'] = 'C';
                            $dottedcircle[0]['uni'] = 0x25cc;
                            $dottedcircle[0]['sea_category'] = \GFPDF_Vendor\Mpdf\Shaper\Sea::OT_GB;
                            $dottedcircle[0]['sea_position'] = \GFPDF_Vendor\Mpdf\Shaper\Sea::POS_BASE_C;
                            $dottedcircle[0]['hex'] = '025CC';
                            // TEMPORARY *****
                        }
                    }
                    \GFPDF_Vendor\Mpdf\Shaper\Sea::initial_reordering($this->OTLdata, $this->GSUBdata[$this->GSUBfont], $broken_syllables, $scriptblock, $dottedcircle);
                    //-----------------------------------------------------------------------------------
                    ...
                    // e. Final Re-ordering
                    //-----------------------------------------------------------------------------------
                    \GFPDF_Vendor\Mpdf\Shaper\Sea::final_reordering($this->OTLdata, $this->GSUBdata[$this->GSUBfont], $scriptblock);
                    //-----------------------------------------------------------------------------------
                    ...
                    if ($this->shaper == 'T' || $this->shaper == 'L') {
                        for ($ptr = 0; $ptr < \count($this->OTLdata); $ptr++) {
                            ...
                            if (($char & ~0x80) == 0xe33) {
                                ...
                                $newinfo = [];
                                $ucd_record = \GFPDF_Vendor\Mpdf\Ucdn::get_ucd_record($sub[0]);
                                $newinfo[0]['general_category'] = $ucd_record[0];
                                $newinfo[0]['bidi_type'] = $ucd_record[2];
                                ...
                                while (isset($this->OTLdata[$ptr - 1 - $ntones]) && (($this->OTLdata[$ptr - 1 - $ntones]['uni'] & ~0x80) == 0xe31 || ($this->OTLdata[$ptr - 1 - $ntones]['uni'] & ~0x80) >= 0xe34 && ($this->OTLdata[$ptr - 1 - $ntones]['uni'] & ~0x80) <= 0xe37 || ($this->OTLdata[$ptr - 1 - $ntones]['uni'] & ~0x80) >= 0xe47 && ($this->OTLdata[$ptr - 1 - $ntones]['uni'] & ~0x80) <= 0xe4e)) {
                                    $ntones++;
                                }
                                $newinfo = [];
                                $ucd_record = \GFPDF_Vendor\Mpdf\Ucdn::get_ucd_record($sub[1]);
                                $newinfo[0]['general_category'] = $ucd_record[0];
                                $newinfo[0]['bidi_type'] = $ucd_record[2];
                    ...
                    if ($scriptblock == \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_TIBETAN) {
                        // =========================
                        // Reordering TIBETAN
                        // =========================
                        ...
                        for ($ptr = 0; $ptr < \count($this->OTLdata) - 1; $ptr++) {
                            if (\GFPDF_Vendor\Mpdf\Shaper\Indic::in_range($this->OTLdata[$ptr]['uni'], 0xf20, 0xf33) && $this->OTLdata[$ptr + 1]['uni'] == 0xf3f) {
                    ...
                    if (!($this->mpdf->textvar & \GFPDF_Vendor\Mpdf\Css\TextVars::FC_SMALLCAPS && $scriptblock == \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_LATIN && \strpos($useGSUBtags, 'smcp') === \false)) {
            ...
            //=======================================================
            if ($this->shaper == "K" || $this->shaper == "T" || $this->shaper == "L" || $scriptblock == \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_TIBETAN) {
                ...
                $newinfo[0] = ['general_category' => \GFPDF_Vendor\Mpdf\Ucdn::UNICODE_GENERAL_CATEGORY_FORMAT, 'bidi_type' => \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_BN, 'group' => 'S', 'uni' => 0x200b, 'hex' => '0200B'];
            ...
            if ($useOTL & 0xff && $GPOSscriptTag && $GPOSlangsys && $GPOSFeatures) {
                ...
                if ($scriptblock != \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_LATIN) {
                    $tags .= ' kern';
                }
                $omittags = '';
                $usetags = $tags;
        ...
        for ($sch = 0; $sch <= $subchunk; $sch++) {
            for ($i = 0; $i < \count($this->schOTLdata[$sch]); $i++) {
                ...
                $e .= \GFPDF_Vendor\Mpdf\Utils\UtfString::code2utf($this->schOTLdata[$sch][$i]['uni']);
                    ...
                    foreach ($this->GSUBLookups[$lu]['Subtables'] as $c => $subtable_offset) {
                        ...
                        if (isset($this->GSLuCoverage[$lu][$c][$currGID])) {
                            if (\strpos('rphf pref blwf half pstf cfar init', $usetag) !== \false) {
                                ...
                                switch ($usetag) {
                                    case 'rphf':
                                        $mask = 1 << \GFPDF_Vendor\Mpdf\Shaper\Indic::RPHF;
                                        break;
                                    case 'pref':
                                        $mask = 1 << \GFPDF_Vendor\Mpdf\Shaper\Indic::PREF;
                                        break;
                                    case 'blwf':
                                        $mask = 1 << \GFPDF_Vendor\Mpdf\Shaper\Indic::BLWF;
                                        break;
                                    case 'half':
                                        $mask = 1 << \GFPDF_Vendor\Mpdf\Shaper\Indic::HALF;
                                        break;
                                    case 'pstf':
                                        $mask = 1 << \GFPDF_Vendor\Mpdf\Shaper\Indic::PSTF;
                                        break;
                                    case 'cfar':
                                        $mask = 1 << \GFPDF_Vendor\Mpdf\Shaper\Indic::CFAR;
                                        break;
                                    case 'init':
                                        $mask = 1 << \GFPDF_Vendor\Mpdf\Shaper\Indic::INIT;
                                        break;
                                }
    ...
    function _applyGSUBsubtable($lookupID, $subtable, $ptr, $currGlyph, $currGID, $subtable_offset, $Type, $Flag, $MarkFilteringSet, $LuCoverage, $level, $currentTag, $is_old_spec, $tagInt)
    {
        ...
        if ($Type == 1) {
            ...
            if ($SubstFormat == 1) {
                ...
                throw new \GFPDF_Vendor\Mpdf\MpdfException("GSUB Lookup Type " . $Type . " Format " . $SubstFormat . " not TESTED YET.");
            }
        } elseif ($Type == 6) {
            //===========
            // Format 1:
            ...
            throw new \GFPDF_Vendor\Mpdf\MpdfException("GSUB Lookup Type " . $Type . " not supported.");
        }
    }
    ...
    private function get_arab_glyphs($char, $type, &$chars, $i, $scriptTag, $usetags)
    {
        ...
        if ($scriptTag == 'syrc' && $char == '00710') {
            ...
            if (isset($chars[$n]) && isset($this->arabLeftJoining[\hexdec($chars[$n])])) {
                ...
                if (isset($chars[$i + 1]) && \preg_match('/[\\x{0700}-\\x{0745}]/u', \GFPDF_Vendor\Mpdf\Utils\UtfString::code2utf(\hexdec($chars[$n]))) && \preg_match('/[\\x{0700}-\\x{0745}]/u', \GFPDF_Vendor\Mpdf\Utils\UtfString::code2utf(\hexdec($chars[$i + 1]))) && isset($this->arabGlyphs[$char][4])) {
                    $retk = 4;
                } elseif (!isset($chars[$i + 1]) || !\preg_match('/[\\x{0700}-\\x{0745}]/u', \GFPDF_Vendor\Mpdf\Utils\UtfString::code2utf(\hexdec($chars[$i + 1])))) {
        ...
        if (($type > 0 || $type === 0) && isset($this->arabGlyphs[$char][$type])) {
            $retk = $type;
        } elseif ($type == 3 && isset($this->arabGlyphs[$char][1])) {
            // if <medial> not defined, but <final>, return <final>
            $retk = 1;
        } elseif ($type == 2 && isset($this->arabGlyphs[$char][0])) {
    ...
    private function seaLineBreaking()
    {
        ...
        while ($ptr < \count($this->OTLdata) - 3) {
            ...
            if (\count($matches) == 1) {
                ...
                // Set end of word marker in OTLdata at matchpos
                $this->OTLdata[$matchpos]['wordend'] = \true;
                $ptr = $matchpos + 1;
    ...
    private function _applyGPOSsubtable($lookupID, $subtable, $ptr, $currGlyph, $currGID, $subtable_offset, $Type, $Flag, $MarkFilteringSet, $LuCoverage, $tag, $level, $is_old_spec)
    {
        ...
        if ($Type == 1) {
            ...
            if ($matchedpos !== \false) {
                ...
                $BaseRecord = ['AnchorX' => $x, 'AnchorY' => $y];
            ...
            //===========
            if ($PosFormat == 1) {
                throw new \GFPDF_Vendor\Mpdf\MpdfException("GPOS Lookup Type " . $Type . " Format " . $PosFormat . " not TESTED YET.");
            } elseif ($PosFormat == 2) {
                $CoverageTableOffset = $subtable_offset + $this->read_ushort();
                $InputClassDefOffset = $subtable_offset + $this->read_ushort();
                ...
                throw new \GFPDF_Vendor\Mpdf\MpdfException("GPOS Lookup Type " . $Type . ", Format " . $PosFormat . " not supported.");
            }
        } elseif ($Type == 8) {
            //===========
            // Format 1:
            ...
            throw new \GFPDF_Vendor\Mpdf\MpdfException("GPOS Lookup Type " . $Type . " not supported.");
        }
    }
    //////////////////////////////////////////////////////////////////////////////////
    ...
    private function _getGCOMignoreString($flag, $MarkFilteringSet)
    {
        ...
        if ($flag & 0x10) {
            throw new \GFPDF_Vendor\Mpdf\MpdfException("This font [" . $this->fontkey . "] contains MarkGlyphSets - Not tested yet");
            // Change also in ttfontsuni.php
            if ($MarkFilteringSet == '') {
                throw new \GFPDF_Vendor\Mpdf\MpdfException("This font [" . $this->fontkey . "] contains MarkGlyphSets - but MarkFilteringSet not set");
            }
    ...
    public function bidiSort($ta, $str, $dir, &$chunkOTLdata, $useGPOS)
    {
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if ($chunkOTLdata['char_data'][$i]['uni'] == 8235) {
            ...
            } elseif ($chunkOTLdata['char_data'][$i]['uni'] == 8234) {
                // LRE
            ...
            } elseif ($chunkOTLdata['char_data'][$i]['uni'] == 8238) {
                // RLO
                ...
                if ($next_level < 62) {
                    ...
                    $cel = $next_level;
                    $dos = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R;
                }
                ...
                if ($next_level < 62) {
                    ...
                    $cel = $next_level;
                    $dos = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L;
                }
            ...
            } elseif ($chunkOTLdata['char_data'][$i]['uni'] == 10) {
                // NEW LINE
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            ...
            $chardata[$i]['sor'] = \max($left, $level) % 2 ? \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R : \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L;
            $chardata[$i]['eor'] = \max($right, $level) % 2 ? \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R : \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L;
        }
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if ($chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_NSM) {
        ...
        $levcount = 0;
        for ($i = 0; $i < $numchars; ++$i) {
            if ($chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN) {
                $found = \false;
                for ($j = $levcount; $j >= 0; $j--) {
                    if ($chardata[$j]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_AL) {
                        $chardata[$i]['type'] = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_AN;
                        $found = \true;
                        break;
                    } elseif ($chardata[$j]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L || $chardata[$j]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R) {
                        $found = \true;
                        break;
                    }
                }
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if ($chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_AL) {
                $chardata[$i]['type'] = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R;
            }
        }
        ...
        for ($i = 1; $i < $numchars; ++$i) {
            if ($i + 1 < $numchars && $chardata[$i]['level'] == $chardata[$i + 1]['level'] && $chardata[$i]['level'] == $chardata[$i - 1]['level']) {
                if ($chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_ES && $chardata[$i - 1]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN && $chardata[$i + 1]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN) {
                    $chardata[$i]['type'] = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN;
                } elseif ($chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_CS && $chardata[$i - 1]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN && $chardata[$i + 1]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN) {
                    $chardata[$i]['type'] = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN;
                } elseif ($chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_CS && $chardata[$i - 1]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_AN && $chardata[$i + 1]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_AN) {
                    $chardata[$i]['type'] = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_AN;
                }
            }
        }
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if ($chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_ET) {
                if ($i > 0 && $chardata[$i - 1]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN && $chardata[$i]['level'] == $chardata[$i - 1]['level']) {
                    $chardata[$i]['type'] = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN;
                } else {
                    $j = $i + 1;
                    while ($j < $numchars && $chardata[$j]['level'] == $chardata[$i]['level']) {
                        if ($chardata[$j]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN) {
                            $chardata[$i]['type'] = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN;
                            break;
                        } elseif ($chardata[$j]['type'] != \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_ET) {
                            break;
                        }
                        ++$j;
                    }
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if ($chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_ET || $chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_ES || $chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_CS) {
                $chardata[$i]['type'] = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_ON;
            }
        }
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if ($chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN) {
                if ($i == 0) {
                    // Start of Level run
                    if ($chardata[$i]['sor'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L) {
                        $chardata[$i]['type'] = $chardata[$i]['sor'];
                    }
                } else {
                    for ($j = $i - 1; $j >= 0; $j--) {
                        if ($chardata[$j]['level'] != $chardata[$i]['level']) {
                            // Level run boundary
                            if ($chardata[$j + 1]['sor'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L) {
                                $chardata[$i]['type'] = $chardata[$j + 1]['sor'];
                            }
                            break;
                        } elseif ($chardata[$j]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L) {
                            $chardata[$i]['type'] = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L;
                            break;
                        } elseif ($chardata[$j]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R) {
                            break;
                        }
                    }
                }
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if ($chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_ON || $chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_WS) {
                $left = -1;
                // LEFT
                if ($i == 0) {
                    // first char
                    ...
                    // run boundary
                    $left = $chardata[$i]['sor'];
                } elseif ($chardata[$i - 1]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L) {
                    $left = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L;
                } elseif ($chardata[$i - 1]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R || $chardata[$i - 1]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN || $chardata[$i - 1]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_AN) {
                    $left = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R;
                }
                // RIGHT
                $right = -1;
                $j = $i;
                // move to the right of any following neutrals OR hit a run boundary
                while (($chardata[$j]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_ON || $chardata[$j]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_WS) && $j <= $numchars - 1) {
                    if ($j == $numchars - 1) {
                        // last char
                        ...
                        $right = $chardata[$j]['eor'];
                        break;
                    } elseif ($chardata[$j + 1]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L) {
                        $right = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L;
                        break;
                    } elseif ($chardata[$j + 1]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R || $chardata[$j + 1]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN || $chardata[$j + 1]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_AN) {
                        $right = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R;
                        break;
                    }
                    $j++;
                }
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if ($chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_ON || $chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_WS) {
                $chardata[$i]['type'] = $chardata[$i]['level'] % 2 ? \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R : \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L;
                $chardata[$i]['orig_type'] = $chardata[$i]['type'];
                // Need to store the original 'WS' for reference in L1 below
            }
        }
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            ...
            if ($odd) {
                if ($chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L || $chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_AN || $chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN) {
                    $chardata[$i]['level'] += 1;
                }
            } else {
                if ($chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R) {
                    $chardata[$i]['level'] += 1;
                } elseif ($chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_AN || $chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN) {
                    $chardata[$i]['level'] += 2;
                }
            }
        ...
        for ($i = $numchars - 1; $i > 0; $i--) {
            if ($chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_WS || isset($chardata[$i]['orig_type']) && $chardata[$i]['orig_type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_WS) {
        ...
        for ($j = $maxlevel; $j > 0; $j--) {
            ...
            for ($i = 0; $i < $numchars; ++$i) {
                if ($chardata[$i]['level'] >= $j) {
                    ...
                    if (isset(\GFPDF_Vendor\Mpdf\Ucdn::$mirror_pairs[$chardata[$i]['char']]) && $chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R) {
                        $chardata[$i]['char'] = \GFPDF_Vendor\Mpdf\Ucdn::$mirror_pairs[$chardata[$i]['char']];
                    }
                    $revarr[] = $chardata[$i];
                } else {
                    if ($onlevel) {
        ...
        foreach ($chardata as $cd) {
            $e .= \GFPDF_Vendor\Mpdf\Utils\UtfString::code2utf($cd['char']);
            $group .= $cd['group'];
            if ($useGPOS && \is_array($cd['GPOSinfo'])) {
                $GPOS[$cctr] = $cd['GPOSinfo'];
                $GPOS[$cctr]['wDir'] = $cd['level'] % 2 ? 'RTL' : 'LTR';
            }
            if ($cd['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L) {
                $rtl_content |= 1;
            } elseif ($cd['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R) {
                $rtl_content |= 2;
            }
            $cctr++;
        }
    ...
    public function bidiPrepare(&$para, $dir)
    {
        ...
        for ($nc = 0; $nc < $numchunks; $nc++) {
            ...
            for ($i = 0; $i < $numchars; ++$i) {
                if ($chunkOTLdata['char_data'][$i]['uni'] == 8235) {
                ...
                } elseif ($chunkOTLdata['char_data'][$i]['uni'] == 8294 || $chunkOTLdata['char_data'][$i]['uni'] == 8295 || $chunkOTLdata['char_data'][$i]['uni'] == 8296) {
                    // LRI // RLI // FSI
                    // X5a. With each RLI:
                    // X5b. With each LRI:
                    ...
                    if ($chunkOTLdata['char_data'][$i]['uni'] == 8296) {
                        ...
                        while (!($nc2 == $numchunks - 1 && $i2 == \count($para[$nc2][18]['char_data']) - 1)) {
                            ...
                            if ($para[$nc2][18]['char_data'][$i2]['bidi_class'] === \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L || $para[$nc2][18]['char_data'][$i2]['bidi_class'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_AL || $para[$nc2][18]['char_data'][$i2]['bidi_class'] === \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R) {
                        ...
                        if (!$fsi && $fsi !== 0) {
                            if ($pel == 1) {
                                $fsi = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R;
                            } else {
                                $fsi = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L;
                            }
                        }
                    }
                    if ($chunkOTLdata['char_data'][$i]['uni'] == 8294 || $fsi === \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L) {
                        // LRI or FSI-L
                        ...
                        $next_level = $cel + 2 - $cel % 2;
                    } elseif ($chunkOTLdata['char_data'][$i]['uni'] == 8295 || $fsi == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R || $fsi == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_AL) {
                        // RLI or FSI-R
                    ...
                    $controlchars = \true;
                } elseif ($chunkOTLdata['char_data'][$i]['uni'] == 8297) {
                    // PDI
                    ...
                    if ($dos != -1) {
                        ...
                        $chardir = $chunkOTLdata['char_data'][$i]['bidi_class'];
                        if ($chardir == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R || $chardir == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_AL) {
                            $strongrtl = \true;
                        }
                    }
                    $chunkOTLdata['char_data'][$i]['level'] = $cel;
            ...
            for ($nc = 0; $nc < $numchunks; $nc++) {
                ...
                for ($i = 0; $i < $numchars; ++$i) {
                    ...
                    if ($firstchar || $level != $prelevel) {
                        $chardata[$i]['sor'] = \max($prelevel, $level) % 2 ? \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R : \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L;
                    }
        ...
        for ($ir = 0; $ir <= $dictr; $ir++) {
            ...
            for ($nc = 0; $nc < $numchunks; $nc++) {
                ...
                for ($i = 0; $i < $numchars; ++$i) {
                    ...
                    if ($chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN && $laststrongtype == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_AL) {
                        $chardata[$i]['type'] = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_AN;
                    }
                    if ($chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L || $chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R || $chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_AL) {
        ...
        for ($nc = 0; $nc < $numchunks; $nc++) {
            ...
            $numchars = \count($chardata);
            for ($i = 0; $i < $numchars; ++$i) {
                if (isset($chardata[$i]['type']) && $chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_AL) {
                    $chardata[$i]['type'] = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R;
                }
            }
        }
            ...
            for ($nc = 0; $nc < $numchunks; $nc++) {
                ...
                for ($i = 0; $i < $numchars; ++$i) {
                    ...
                    if (!isset($chardata[$i]['sor']) && !isset($chardata[$i]['eor'])) {
                        if ($chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_ES && $prevtype == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN && $nexttype == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN) {
                            $chardata[$i]['type'] = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN;
                        } elseif ($chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_CS && $prevtype == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN && $nexttype == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN) {
                            $chardata[$i]['type'] = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN;
                        } elseif ($chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_CS && $prevtype == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_AN && $nexttype == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_AN) {
                            $chardata[$i]['type'] = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_AN;
                        }
                    }
                    $prevtype = $chardata[$i]['type'];
        ...
        for ($ir = 0; $ir <= $dictr; $ir++) {
            ...
            for ($nc = 0; $nc < $numchunks; $nc++) {
                ...
                for ($i = 0; $i < $numchars; ++$i) {
                    ...
                    if ($chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_ET) {
                        if ($prevtype == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN) {
                            $chardata[$i]['type'] = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN;
                            ...
                            while (!($nc2 == $numchunks - 1 && $i2 == \count($para[$nc2][18]['char_data']) - 1)) {
                                ...
                                if ($nexttype == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN) {
                                    $chardata[$i]['type'] = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN;
                                    break;
                                } elseif ($nexttype != \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_ET) {
                                    break;
                                }
                            }
                        }
        ...
        for ($nc = 0; $nc < $numchunks; $nc++) {
            ...
            $numchars = \count($chardata);
            for ($i = 0; $i < $numchars; ++$i) {
                if (isset($chardata[$i]['type']) && ($chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_ET || $chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_ES || $chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_CS)) {
                    $chardata[$i]['type'] = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_ON;
                }
            }
        }
        ...
        for ($ir = 0; $ir <= $dictr; $ir++) {
            ...
            for ($nc = 0; $nc < $numchunks; $nc++) {
                ...
                for ($i = 0; $i < $numchars; ++$i) {
                    ...
                    if (isset($chardata[$i]['type']) && $chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN && $laststrongtype == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L) {
                        $chardata[$i]['type'] = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L;
                    }
                    if (isset($chardata[$i]['type']) && ($chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L || $chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R || $chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_AL)) {
                        $laststrongtype = $chardata[$i]['type'];
                    }
                }
            }
            ...
            for ($nc = 0; $nc < $numchunks; $nc++) {
                ...
                for ($i = 0; $i < $numchars; ++$i) {
                    ...
                    if ($chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_ON || $chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_WS) {
                        $left = -1;
                        // LEFT
                        if ($laststrongtype == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R || $laststrongtype == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN || $laststrongtype == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_AN) {
                            $left = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R;
                        } elseif ($laststrongtype == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L) {
                            $left = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L;
                        }
                        // RIGHT
                        $right = -1;
                        ...
                        if (isset($chardata[$i]['eor'])) {
                            ...
                            while (!($nc2 == $numchunks - 1 && $i2 == \count($para[$nc2][18]['char_data']) - 1)) {
                                ...
                                $nexttype = $para[$nc2][18]['char_data'][$i2]['type'];
                                if ($nexttype == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R || $nexttype == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN || $nexttype == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_AN) {
                                    $right = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R;
                                    break;
                                } elseif ($nexttype == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L) {
                                    $right = \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L;
                                    break;
                    ...
                    } elseif ($chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L || $chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R || $chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_EN || $chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_AN) {
                        $laststrongtype = $chardata[$i]['type'];
                    }
                }
            }
        ...
        for ($nc = 0; $nc < $numchunks; $nc++) {
            ...
            $numchars = \count($chardata);
            for ($i = 0; $i < $numchars; ++$i) {
                if (isset($chardata[$i]['type']) && ($chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_ON || $chardata[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_WS)) {
                    $chardata[$i]['orig_type'] = $chardata[$i]['type'];
                    // Need to store the original 'WS' for reference in L1 below
                    $chardata[$i]['type'] = $chardata[$i]['level'] % 2 ? \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R : \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_L;
                }
    ...
    public function bidiReorder(&$chunkorder, &$content, &$cOTLdata, $blockdir)
    {
        ...
        for ($i = $numchars - 1; $i > 0; $i--) {
            if ($bidiData[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_WS || isset($bidiData[$i]['orig_type']) && $bidiData[$i]['orig_type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_WS) {
        ...
        for ($j = $maxlevel; $j > 0; $j--) {
            ...
            for ($i = 0; $i < $numchars; ++$i) {
                if ($bidiData[$i]['level'] >= $j) {
                    $onlevel = \true;
                    // L4. A character is depicted by a mirrored glyph if and only if (a) the resolved directionality of that character is R, and (b) the Bidi_Mirrored property value of that character is true.
                    if (isset(\GFPDF_Vendor\Mpdf\Ucdn::$mirror_pairs[$bidiData[$i]['uni']]) && $bidiData[$i]['type'] == \GFPDF_Vendor\Mpdf\Ucdn::BIDI_CLASS_R) {
                        $bidiData[$i]['uni'] = \GFPDF_Vendor\Mpdf\Ucdn::$mirror_pairs[$bidiData[$i]['uni']];
                    }
                    $revarr[] = $bidiData[$i];
                } else {
                    if ($onlevel) {
        ...
        foreach ($bidiData as $carac) {
            ...
            if ($carac['uni'] != 0xfffc) {
                ...
                $content[$nc] .= \GFPDF_Vendor\Mpdf\Utils\UtfString::code2utf($carac['uni']);
    ...
    private function _getOTLscriptTag($ScriptLang, $scripttag, $scriptblock, $shaper, $useOTL, $mode)
    {
        ...
        */
        ...
        } elseif ($scriptblock == \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_CYRILLIC) {
            if (!($useOTL & 0x2)) {
                return ['', \false];
            }
        } elseif ($scriptblock == \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_GREEK) {
            if (!($useOTL & 0x4)) {
                return ['', \false];
            }
        } elseif ($scriptblock >= \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_HIRAGANA && $scriptblock <= \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_YI) {
            if (!($useOTL & 0x8)) {
                return ['', \false];
            }
        } else {
    ...
    private function _getOTLLangTag($ietf, $available)
    {
        ...
        if ($lang != '' && isset(\GFPDF_Vendor\Mpdf\Ucdn::$ot_languages[$lang])) {
            $langsys = \GFPDF_Vendor\Mpdf\Ucdn::$ot_languages[$lang];
        } elseif ($lang != '' && $country != '' && isset(\GFPDF_Vendor\Mpdf\Ucdn::$ot_languages[$lang . '' . $country])) {
            $langsys = \GFPDF_Vendor\Mpdf\Ucdn::$ot_languages[$lang . '' . $country];
        } else {
            $langsys = "DFLT";
        }
