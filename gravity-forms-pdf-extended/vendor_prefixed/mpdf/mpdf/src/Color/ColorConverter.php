...
namespace GFPDF_Vendor\Mpdf\Color;
...
class ColorConverter
{
    ...
    private $cache;
    public function __construct(\GFPDF_Vendor\Mpdf\Mpdf $mpdf, \GFPDF_Vendor\Mpdf\Color\ColorModeConverter $colorModeConverter, \GFPDF_Vendor\Mpdf\Color\ColorSpaceRestrictor $colorSpaceRestrictor)
    {
        $this->mpdf = $mpdf;
    ...
    public function convert($color, array &$PDFAXwarnings = [])
    {
        ...
        if (isset(\GFPDF_Vendor\Mpdf\Color\NamedColors::$colors[$color])) {
            $color = \GFPDF_Vendor\Mpdf\Color\NamedColors::$colors[$color];
        }
    ...
    public function invert($c)
    {
        ...
        throw new \GFPDF_Vendor\Mpdf\MpdfException('Trying to invert non-RGB color');
    }
    /**
            ...
            case 'spot':
                ...
                if (!isset($this->mpdf->spotColors[$name])) {
                    if (isset($cores[5])) {
                        ...
                        throw new \GFPDF_Vendor\Mpdf\MpdfException(\sprintf('Undefined spot color "%s"', $name));
                    }
                }
    ...
    {
        if (!\is_string($color)) {
            throw new \GFPDF_Vendor\Mpdf\MpdfException('Invalid color input, binary color string expected');
        }
        if (\strlen($color) !== 6) {
            throw new \GFPDF_Vendor\Mpdf\MpdfException('Invalid color input, binary color string expected');
        }
