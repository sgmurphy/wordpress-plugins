...
use GFPDF_Vendor\Mpdf\Utils\PdfDate;
use GFPDF_Vendor\Psr\Log\LoggerInterface;
class MetadataWriter implements \GFPDF_Vendor\Psr\Log\LoggerAwareInterface
{
    use Strict;
     ...
     */
    private $protection;
    public function __construct(\GFPDF_Vendor\Mpdf\Mpdf $mpdf, \GFPDF_Vendor\Mpdf\Writer\BaseWriter $writer, \GFPDF_Vendor\Mpdf\Form $form, \GFPDF_Vendor\Mpdf\Pdf\Protection $protection, \GFPDF_Vendor\Psr\Log\LoggerInterface $logger)
    {
        $this->mpdf = $mpdf;
        $this->writer = $writer;
        $this->form = $form;
	  ...
	  <rdf:li>' . $this->mpdf->author . '</rdf:li>
        ...
        if ($this->mpdf->PDFX) {
            ...
            if (\strpos($this->mpdf->PDFAversion, '-') === \false) {
                throw new \GFPDF_Vendor\Mpdf\MpdfException(\sprintf('PDFA version (%s) is not valid. (Use: 1-B, 3-B, etc.)', $this->mpdf->PDFAversion));
            }
    ...
    public function writeInfo()
    {
        ...
        $now = \GFPDF_Vendor\Mpdf\Utils\PdfDate::format(\time());
    ...
    public function writeOutputIntent()
    {
        ...
        if ($this->mpdf->ICCProfile) {
            if (!\file_exists($this->mpdf->ICCProfile)) {
                throw new \GFPDF_Vendor\Mpdf\MpdfException(\sprintf('Unable to find ICC profile "%s"', $this->mpdf->ICCProfile));
            }
    ...
    public function writeAssociatedFiles()
    {
        if (!\function_exists('gzcompress')) {
            throw new \GFPDF_Vendor\Mpdf\MpdfException('ext-zlib is required for compression of associated files');
        }
        ...
        foreach ($this->mpdf->associatedFiles as $k => $file) {
            ...
            if (!$fileContent) {
                throw new \GFPDF_Vendor\Mpdf\MpdfException(\sprintf('Cannot access associated file - %s', $file['path']));
            }
            ...
            $this->writer->write('/Filter /FlateDecode');
            if (isset($file['path'])) {
                $this->writer->write('/Params <</ModDate ' . $this->writer->string('D:' . \GFPDF_Vendor\Mpdf\Utils\PdfDate::format(\filemtime($file['path']))) . ' >>');
            } else {
                $this->writer->write('/Params <</ModDate ' . $this->writer->string('D:' . \GFPDF_Vendor\Mpdf\Utils\PdfDate::format(\time())) . ' >>');
            }
            $this->writer->write('>>');
    ...
    public function writeAnnotations()
    {
        ...
        for ($n = 1; $n <= $nb; $n++) {
            if (isset($this->mpdf->PageLinks[$n]) || isset($this->mpdf->PageAnnots[$n]) || \count($this->form->forms) > 0) {
                $wPt = $this->mpdf->pageDim[$n]['w'] * \GFPDF_Vendor\Mpdf\Mpdf::SCALE;
                $hPt = $this->mpdf->pageDim[$n]['h'] * \GFPDF_Vendor\Mpdf\Mpdf::SCALE;
                // Links
                if (isset($this->mpdf->PageLinks[$n])) {
                    foreach ($this->mpdf->PageLinks[$n] as $key => $pl) {
                        ...
                        if (\strpos($pl[4], '@') === 0) {
                            ...
                            $htarg = $this->mpdf->pageDim[$p]['h'] * \GFPDF_Vendor\Mpdf\Mpdf::SCALE;
                            ...
                            if (isset($this->mpdf->pageDim[$l[0]]['h'])) {
                                $htarg = $this->mpdf->pageDim[$l[0]]['h'] * \GFPDF_Vendor\Mpdf\Mpdf::SCALE;
                            } else {
                                $htarg = $this->mpdf->h * \GFPDF_Vendor\Mpdf\Mpdf::SCALE;
                            }
                            // doesn't really matter
                            $annot .= \sprintf(' /Dest [%d 0 R /XYZ 0 %.3F null]>>', 1 + 2 * $l[0], $htarg - $l[1] * \GFPDF_Vendor\Mpdf\Mpdf::SCALE);
                        }
                ...
                if (isset($this->mpdf->PageAnnots[$n])) {
                    foreach ($this->mpdf->PageAnnots[$n] as $key => $pl) {
                        ...
                        if ($this->mpdf->annotMargin != 0 || $x == 0 || $x < 0) {
                            ...
                            $x = $wPt / \GFPDF_Vendor\Mpdf\Mpdf::SCALE - $this->mpdf->annotMargin;
                        }
                        $w = $h = 0;
                        $a = $x * \GFPDF_Vendor\Mpdf\Mpdf::SCALE;
                        $b = $hPt - $pl['y'] * \GFPDF_Vendor\Mpdf\Mpdf::SCALE;
                        $annot .= '<</Type /Annot ';
                        if ($fileAttachment) {
                        ...
                        if ($fileAttachment) {
                            ...
                            if (!$file) {
                                throw new \GFPDF_Vendor\Mpdf\MpdfException('mPDF Error: Cannot access file attachment - ' . $pl['opt']['file']);
                            }
                            ...
                            if (\is_array($pl['opt']['popup']) && isset($pl['opt']['popup'][0])) {
                                $x = $pl['opt']['popup'][0] * \GFPDF_Vendor\Mpdf\Mpdf::SCALE;
                            } else {
                                $x = $pl['x'] * \GFPDF_Vendor\Mpdf\Mpdf::SCALE;
                            }
                            if (\is_array($pl['opt']['popup']) && isset($pl['opt']['popup'][1])) {
                                $y = $hPt - $pl['opt']['popup'][1] * \GFPDF_Vendor\Mpdf\Mpdf::SCALE;
                            } else {
                                $y = $hPt - $pl['y'] * \GFPDF_Vendor\Mpdf\Mpdf::SCALE;
                            }
                            if (\is_array($pl['opt']['popup']) && isset($pl['opt']['popup'][2])) {
                                $w = $pl['opt']['popup'][2] * \GFPDF_Vendor\Mpdf\Mpdf::SCALE;
                            } else {
                                $w = 180;
                            }
                            if (\is_array($pl['opt']['popup']) && isset($pl['opt']['popup'][3])) {
                                $h = $pl['opt']['popup'][3] * \GFPDF_Vendor\Mpdf\Mpdf::SCALE;
                            } else {
                                $h = 120;
                            }
    ...
    {
        $return = \GFPDF_Vendor\Mpdf\Mpdf::VERSION;
