...
class TelegramBotHandler extends \GFPDF_Vendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(string $apiKey, string $channel, $level = \GFPDF_Vendor\Monolog\Logger::DEBUG, bool $bubble = \true, ?string $parseMode = null, ?bool $disableWebPagePreview = null, ?bool $disableNotification = null, bool $splitLongMessages = \false, bool $delayBetweenMessages = \false)
    {
    ...
    private function handleMessageLength(string $message) : array
    {
        ...
        if (!$this->splitLongMessages && \strlen($message) > self::MAX_MESSAGE_LENGTH) {
            return [\GFPDF_Vendor\Monolog\Utils::substr($message, 0, self::MAX_MESSAGE_LENGTH - \strlen($truncatedMarker)) . $truncatedMarker];
        }
