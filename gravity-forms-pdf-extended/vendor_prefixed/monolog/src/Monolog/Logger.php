 ...
 */
namespace GFPDF_Vendor\Monolog;
...
class Logger implements \Psr\Log\LoggerInterface, \GFPDF_Vendor\Monolog\ResettableInterface
{
     ...
     */
    public function pushHandler(\GFPDF_Vendor\Monolog\Handler\HandlerInterface $handler) : self
    {
     ...
     */
    public function popHandler() : \GFPDF_Vendor\Monolog\Handler\HandlerInterface
    {
        if (!$this->handlers) {
    ...
    {
        foreach ($this->handlers as $handler) {
            if ($handler instanceof \GFPDF_Vendor\Monolog\ResettableInterface) {
                $handler->reset();
            }
        }
        foreach ($this->processors as $processor) {
            if ($processor instanceof \GFPDF_Vendor\Monolog\ResettableInterface) {
                $processor->reset();
            }
        }
    }
