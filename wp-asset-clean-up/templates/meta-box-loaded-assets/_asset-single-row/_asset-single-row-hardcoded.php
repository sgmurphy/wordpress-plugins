...
// Probably most reliable to use to check the unloaded assets; it might be the only one used in future plugin versions
if ( ! $data['row']['is_load_exception'] && isset($data['current_unloaded_all'][$assetType]) && strpos($data['row']['class'], 'wpacu_not_load') === false && in_array($data['row']['obj']->handle, $data['current_unloaded_all'][$assetType])) {
...
<tr data-<?php echo $assetTypeS; ?>-handle-row="<?php echo htmlentities(esc_attr($data['row']['obj']->handle), ENT_QUOTES); ?>"
    ...
    <td style="position: relative;" data-wpacu-row-status="<?php echo esc_attr($dataRowStatusAttr); ?>">
        ...
        if (isset($data['row']['obj']->src) && trim($data['row']['obj']->src)) {
            // Source
            include dirname(__DIR__).'/_asset-single-row/_asset-single-row-hardcoded-source.php';
        ...
        <div class="wpacu_handle_row_expanded_area <?php if ($rowIsContracted) { echo 'wpacu_hide'; } ?>">
            <div class="wrap_bulk_unload_options">
                ...
                // Unload on this page
                include dirname(__DIR__).'/_asset-single-row/_asset-single-row-unload-per-page.php';

                // Unload site-wide (everywhere)
                include dirname(__DIR__).'/_asset-single-row/_asset-single-row-unload-site-wide.php';

                // Unload on all pages of [post] post type (if applicable)
                include dirname(__DIR__).'/_asset-single-row/_asset-single-row-unload-post-type.php';
                ...
                if (isset($data['post_type']) && $data['post_type'] !== 'attachment' && ! $data['row']['is_post_type_unloaded'] && ! empty($data['post_type_has_tax_assoc'])) {
                    // Unload on all pages where this [post] post type has a certain taxonomy set for it (e.g. a Tag or a Category) (if applicable)
                    include dirname(__DIR__).'/_asset-single-row/_asset-single-row-unload-post-type-taxonomy.php';
                }

                // Unload via RegEx (if site-wide is not already chosen)
                include dirname(__DIR__).'/_asset-single-row/_asset-single-row-unload-via-regex.php';
            ...
            if ( (! (isset($isBase64EncodedSrc) && $isBase64EncodedSrc)) && isset($data['row']['obj']->src) && trim($data['row']['obj']->src) ) {
                include dirname(__DIR__).'/_asset-single-row/_asset-single-row-loaded-rules.php';
            }

            // Handle Note
            include dirname(__DIR__).'/_asset-single-row/_asset-single-row-notes.php';
            ?>
        </div>
