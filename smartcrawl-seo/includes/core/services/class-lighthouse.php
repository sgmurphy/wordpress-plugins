...
class Lighthouse extends Service {
	...
	public function refresh_report() {
		...
		if ( ! $results ) {
			$this->set_error(
				'network-error',
				esc_html__( 'We were unable to connect to the API server.', 'smartcrawl-seo' )
			);
	...
	public function get_last_report( $device = 'desktop' ) {
		...
		if ( ! $device_report ) {
			$report->set_error(
				'unexpected-error',
				esc_html__( 'An unexpected error occurred', 'smartcrawl-seo' )
			);
		...
		foreach ( $recipients as $recipient ) {
			...
			if ( 'desktop' === $reporting_device || 'mobile' === $reporting_device ) {
				$subject = sprintf(
					...
					esc_html__( 'SEO Report for %1$s - Score %2$s', 'smartcrawl-seo' ),
					site_url(),
				...
				$subject = sprintf(
					/* translators: 1: Site url, 2: Score */
					esc_html__( 'SEO Report for %1$s - Desktop score %2$s / Mobile score %3$s', 'smartcrawl-seo' ),
					site_url(),
					$desktop_report->get_score(),
					$mobile_report->get_score()
				);
