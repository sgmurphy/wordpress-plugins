...
class Plugins extends Check {
	...
	private function print_common_description() {
		...
		<div class="wds-lh-section">
			...
			<p><?php esc_html_e( "Search engines often can't index content that relies on browser plugins, such as Java or Flash. That means browser plugin-based content doesn't show up in search results.", 'smartcrawl-seo' ); ?></p>
			<p><?php esc_html_e( "Also, most mobile devices don't support browser plugins, which creates frustrating experiences for mobile users.", 'smartcrawl-seo' ); ?></p>
		</div>
		<?php
	}
		...
		<div class="wds-lh-section">
			...
			Simple_Renderer::render(
				...
				array(
					...
					'message' => esc_html__( 'Document avoids browser plugins - Google is loving it!', 'smartcrawl-seo' ),
				)
			);
			?>
		</div>
	...
	private function format_failure_description() {
		...
		<div class="wds-lh-section">
			...
			Simple_Renderer::render(
				...
				array(
					...
					'message' => esc_html__( 'Document uses browser plugins - Search Engines can’t index browser plugins.', 'smartcrawl-seo' ),
				)
			);
			?>
		...
		<div class="wds-lh-section">
			<strong><?php esc_html_e( "Don't use browser plugins to display your content", 'smartcrawl-seo' ); ?></strong>
			<p>
				<?php
				echo \smartcrawl_format_link(
					/* translators: %s: Link to documentation */
					esc_html__( 'To convert browser plugin-based content to HTML, refer to guidance for that plugin. For example, MDN explains %s.', 'smartcrawl-seo' ),
					'https://developer.mozilla.org/en-US/docs/Plugins/Flash_to_HTML5/Video',
					esc_html__( 'how to convert Flash video to HTML5 video', 'smartcrawl-seo' ),
					'_blank'
				);
				?>
			</p>
	...
	public function parse_details( $raw_details ) {
		$table = new Table(
			array( esc_html__( 'Element source', 'smartcrawl-seo' ) ),
			$this->get_report()
		);
	...
	private function format_copy_description() {
		$parts = array_merge(
			array(
				...
				'',
				__( 'Failing Audit: Document uses browser plugins', 'smartcrawl-seo' ),
				'',
				__( 'Status: Document uses browser plugins - Search Engines can’t index browser plugins.', 'smartcrawl-seo' ),
				'',
			),
			...
			array(
				...
				__( "Search engines often can't index content that relies on browser plugins, such as Java or Flash. That means browser plugin-based content doesn't show up in search results.", 'smartcrawl-seo' ),
				__( "Also, most mobile devices don't support browser plugins, which creates frustrating experiences for mobile users.", 'smartcrawl-seo' ),
				'',
