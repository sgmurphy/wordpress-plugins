...
class Onpage extends Admin_Settings {
	...
	private function sanitize_and_include_char_lengths( $result, $input, $toggle_name, $min_field_name, $max_field_name, $default_min, $default_max ) {
		...
		if ( $custom_title_min_length > 0 ) {
			...
			add_settings_error( $this->option_name, 'min-limit-invalid', __( 'Min length invalid', 'smartcrawl-seo' ) );
		}
		...
		if ( $custom_title_max_length ) {
			...
			add_settings_error( $this->option_name, 'max-limit-invalid', __( 'Max length invalid', 'smartcrawl-seo' ) );
		}
	...
	public static function get_robots_options_for( $type, $include_subsequent_pages_option = true, $context = '' ) {
		$options = array(
			"meta_robots-noindex-{$type}"  => array(
				'label'            => sprintf( '%s %s', esc_html__( 'Index', 'smartcrawl-seo' ), $context ),
				'description'      => esc_html__( 'Disabling indexing means that this content will not be indexed and searchable in search engines.', 'smartcrawl-seo' ),
				'inverted'         => true,
				'html_description' => self::sitemap_notice( $type ),
			),
			"meta_robots-nofollow-{$type}" => array(
				'label'       => sprintf( '%s %s', esc_html__( 'Follow', 'smartcrawl-seo' ), $context ),
				'description' => esc_html__( 'Disabling following means search engines will not follow and crawl links it finds in this content.', 'smartcrawl-seo' ),
				'inverted'    => true,
			),
		);
		...
		if ( $include_subsequent_pages_option ) {
			$options[ 'meta_robots-' . $type . '-subsequent_pages' ] = array(
				'label'       => esc_html__( 'Apply to all pages except the first', 'smartcrawl-seo' ),
				'description' => esc_html__( 'If you select this option, the first page will be left alone, but the indexing settings will be applied to subsequent pages.', 'smartcrawl-seo' ),
			);
		}
	...
	private static function sitemap_notice( $type ) {
		...
		// translators: %s target.
			esc_html__( 'You might want to exclude this type from the %s as well.', 'smartcrawl-seo' ),
			Admin_Settings::admin_url( Settings::TAB_SITEMAP ),
			esc_html__( 'sitemap', 'smartcrawl-seo' )
		);
	...
	protected function get_other_types_options_context( $prefix = '' ) {
		...
		foreach ( $this->get_other_types_options( '' ) as $option ) {
			...
			if ( in_array( $option, array( 'category', 'post_tag' ), true ) ) {
				...
				$context = esc_html__( 'author archives', 'smartcrawl-seo' );
			} elseif ( 'date' === $option ) {
				$context = esc_html__( 'date archives', 'smartcrawl-seo' );
			}
	...
	public function get_title() {
		return __( 'Title & Meta', 'smartcrawl-seo' );
	}
	...
	public function options_page() {
		...
		$arguments = array(
			'meta_robots_main_blog_archive' => self::get_robots_options_for( 'main_blog_archive', true, esc_html__( 'this website', 'smartcrawl-seo' ) ),
		);
		...
		foreach ( $archive_post_types as $archive_post_type => $archive_post_type_label ) {
			...
			$pt_archive_context = sprintf( esc_html__( '%s archive', 'smartcrawl-seo' ), strtolower( $archive_post_type_label ) );
		...
		$arguments['meta_robots_bp_groups'] = self::get_robots_options_for(
			'bp_groups',
			false,
			esc_html__( 'BuddyPress groups', 'smartcrawl-seo' )
		);

		$arguments['meta_robots_bp_profile'] = self::get_robots_options_for(
			'bp_profile',
			false,
			esc_html__( 'BuddyPress profile', 'smartcrawl-seo' )
		);
	...
	private function static_frontpage_notice( $front_page ) {
		ob_start();
		esc_html_e( 'Your homepage is set to a static page, Homepage. You can edit your homepage meta from here, as well as in the WordPress editor for that page.', 'smartcrawl-seo' );
		if ( $front_page ) {
			?>
			<br/>
			<a
			...
			>
				<?php esc_html_e( 'Go To Homepage', 'smartcrawl-seo' ); ?></a>
			<?php
		}
	...
	public static function get_singular_macros( $post_type = '' ) {
		$singular_macros = array(
			'%%id%%'               => __( 'Post/page ID', 'smartcrawl-seo' ),
			'%%title%%'            => __( 'Title of the post/page', 'smartcrawl-seo' ),
			'%%excerpt%%'          => __( 'Post/page excerpt (or auto-generated if it does not exist)', 'smartcrawl-seo' ),
			'%%excerpt_only%%'     => __( 'Post/page excerpt (without auto-generation)', 'smartcrawl-seo' ),
			'%%modified%%'         => __( 'Post/page modified time', 'smartcrawl-seo' ),
			'%%date%%'             => __( 'Date of the post/page', 'smartcrawl-seo' ),
			'%%name%%'             => __( "Post/page author's 'nicename'", 'smartcrawl-seo' ),
			'%%userid%%'           => __( "Post/page author's userid", 'smartcrawl-seo' ),
			'%%user_description%%' => __( "Post/page author's description", 'smartcrawl-seo' ),
		);

		if ( empty( $post_type ) || 'attachment' === $post_type ) {
			$singular_macros['%%caption%%'] = __( 'Attachment caption', 'smartcrawl-seo' );
		}

		if ( empty( $post_type ) || 'post' === $post_type ) {
			$singular_macros['%%category%%'] = __( 'Post categories (comma separated)', 'smartcrawl-seo' );
			$singular_macros['%%tag%%']      = __( 'Current tag/tags', 'smartcrawl-seo' );
		}
	...
	public static function get_term_macros( $taxonomy = '' ) {
		$term_macros = array(
			'%%id%%'               => __( 'Term ID', 'smartcrawl-seo' ),
			'%%term_title%%'       => __( 'Term name', 'smartcrawl-seo' ),
			'%%term_description%%' => __( 'Term description', 'smartcrawl-seo' ),
		);

		if ( empty( $taxonomy ) || 'category' === $taxonomy ) {
			$term_macros['%%category%%']             = __( 'Category name', 'smartcrawl-seo' );
			$term_macros['%%category_description%%'] = __( 'Category description', 'smartcrawl-seo' );
		}

		if ( empty( $taxonomy ) || 'post_tag' === $taxonomy ) {
			$term_macros['%%tag%%']             = __( 'Tag name', 'smartcrawl-seo' );
			$term_macros['%%tag_description%%'] = __( 'Tag description', 'smartcrawl-seo' );
		}
	...
	public static function get_bp_profile_macros() {
		return array(
			'%%bp_user_username%%'  => __( 'BuddyPress username', 'smartcrawl-seo' ),
			'%%bp_user_full_name%%' => __( "BuddyPress user's full name", 'smartcrawl-seo' ),
		);
	}
	...
	public static function get_bp_group_macros() {
		return array(
			'%%bp_group_name%%'        => __( 'BuddyPress group name', 'smartcrawl-seo' ),
			'%%bp_group_description%%' => __( 'BuddyPress group description', 'smartcrawl-seo' ),
		);
	}
	...
	public static function get_pt_archive_macros() {
		return array(
			'%%pt_plural%%' => __( 'Post type label plural', 'smartcrawl-seo' ),
			'%%pt_single%%' => __( 'Post type label singular', 'smartcrawl-seo' ),
		);
	}
	...
	public static function get_search_macros() {
		return array(
			'%%searchphrase%%' => __( 'Current search phrase', 'smartcrawl-seo' ),
		);
	}
	...
	public static function get_author_macros() {
		return array(
			'%%name%%'             => __( "Author's 'nicename'", 'smartcrawl-seo' ),
			'%%userid%%'           => __( "Author's userid", 'smartcrawl-seo' ),
			'%%user_description%%' => __( "Author's description", 'smartcrawl-seo' ),
		);
	}
	...
	public static function get_date_macros() {
		return array(
			'%%date%%' => __( 'Date of the archive', 'smartcrawl-seo' ),
		);
	}
	...
	private static function get_search_archive_data() {
		$search_query = esc_html__( 'Example search query', 'smartcrawl-seo' );
