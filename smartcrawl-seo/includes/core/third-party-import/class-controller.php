...
class Controller extends Work_Unit {
	...
	private function do_import( $importer, $options = array() ) {
		...
		if ( ! $this->user_has_permission_to_import() ) {
			$result['message'] = __( "You don't have permission to perform this operation.", 'smartcrawl-seo' );
			wp_send_json_error( $result );
		}

		if ( ! $importer->data_exists() ) {
			$result['message'] = __( "We couldn't find any compatible data to import.", 'smartcrawl-seo' );
			wp_send_json_error( $result );
		}
