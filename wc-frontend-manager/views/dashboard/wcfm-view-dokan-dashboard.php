...
$query_from = apply_filters( 'wcfm_report_low_in_stock_query_from', $wpdb->prepare( "FROM {$wpdb->posts} as posts
	...
	AND postmeta.meta_key = '_stock' AND CAST(postmeta.meta_value AS SIGNED) > '%s'
", $stock, $nostock ), $stock, $nostock );
$lowinstock_count = absint( $wpdb->get_var( "SELECT COUNT( DISTINCT posts.ID ) {$query_from};" ) );

$query_from = apply_filters( 'wcfm_report_out_of_stock_query_from', $wpdb->prepare( "FROM {$wpdb->posts} as posts
	INNER JOIN {$wpdb->postmeta} AS postmeta ON posts.ID = postmeta.post_id
	INNER JOIN {$wpdb->postmeta} AS postmeta2 ON posts.ID = postmeta2.post_id
	WHERE 1=1
	AND posts.post_type IN ( 'product', 'product_variation' )
