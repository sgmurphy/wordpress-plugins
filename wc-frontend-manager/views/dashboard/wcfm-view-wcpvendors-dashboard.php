...
$vendor_product_ids_placeholders = implode(', ', array_fill(0, count($vendor_product_ids), '%d'));
...
$query['where']  .= $wpdb->prepare("AND order_item_meta_2.meta_value IN ( {$vendor_product_ids_placeholders} ) ", $vendor_product_ids);
...
$query_from = apply_filters( 'wcpv_report_low_in_stock_query_from', $wpdb->prepare("FROM {$wpdb->posts} as posts
...
$query_from = apply_filters( 'wcpv_report_out_of_stock_query_from', $wpdb->prepare("FROM {$wpdb->posts} as posts
	INNER JOIN {$wpdb->postmeta} AS postmeta ON posts.ID = postmeta.post_id
	INNER JOIN {$wpdb->postmeta} AS postmeta2 ON posts.ID = postmeta2.post_id
	WHERE 1=1
	AND posts.post_type IN ( 'product', 'product_variation' )
