...
class WCFM_Marketplace {
  ...
  function wcfmmp_orders_filter() {
	...
	function wcfmmp_wcb_include_bookings() {
		...
		$vendor_products_placeholders = implode(', ', array_fill(0, count($vendor_products), '%d'));

		$query = "SELECT ID FROM {$wpdb->posts} as posts
		INNER JOIN {$wpdb->postmeta} AS postmeta ON posts.ID = postmeta.post_id
		WHERE 1=1
		AND posts.post_type IN ( 'wc_booking' )
		AND postmeta.meta_key = '_booking_product_id' AND postmeta.meta_value in ({$vendor_products_placeholders})";

		$vendor_bookings = $wpdb->get_results($wpdb->prepare($query, $vendor_products));
  ...
  function wcfmmp_wcb_include_tych_bookings( ) {
	...
	$vendor_products_placeholders = implode(', ', array_fill(0, count($vendor_products), '%d'));
  	...
  	$query = "SELECT ID FROM {$wpdb->posts} as posts
							...
							AND postmeta.meta_key = '_bkap_product_id' AND postmeta.meta_value in ({$vendor_products_placeholders})";
  ...
  function wcfmmp_after_line_total( $item, $order ) {
  	...
  	if( $item->get_product_id() ) {
		...
		$sql = "
			...
			AND   item_id     = %d
			AND   `vendor_id` = %d";
		$order_line_due = $wpdb->get_results( $wpdb->prepare( $sql, $product_id, $variation_id, $order->get_id(), $item->get_id(), $this->vendor_id ) );
  		...
  		?>
			<td class="line_cost">
			  ...
			  if( apply_filters( 'wcfmmp_is_allow_shipping_from_order', true ) && !apply_filters( 'wcfmmp_is_allow_commission_on_shipping', false ) ) {
					...
					if ( $WCFMmp->wcfmmp_vendor->is_vendor_get_tax( $this->vendor_id ) && apply_filters( 'wcfmmp_is_allow_vendor_get_shipping_tax', true ) ) {
					...
					$sql = "
							...
							AND   order_id    = %d
							AND   `vendor_id` = %d";
					$order_line_shippings = $wpdb->get_results( $wpdb->prepare( $sql, $order->get_id(), $this->vendor_id ) );
  ...
  function wcfmmp_generate_csv_url( $url, $order_id ) {
	...
	function wcfmmp_report_out_of_stock_query_from($query_from, $stock) {
		...
		return $wpdb->prepare($query_from, $user_id, $stock);
	}
  ...
  function wcfmmp_reports_order_statuses( $order_status ) {
	...
	function wcfmmp_dashboard_status_widget_top_seller_query($query, $limit = 5) {
		...
		$query['fields']  = "SELECT SUM( quantity ) as qty, product_id
			FROM {$wpdb->prefix}wcfm_marketplace_orders";
		$query['where']   = $wpdb->prepare("WHERE vendor_id = %d ", $user_id);
		...
		$query['limits']  = $wpdb->prepare("LIMIT %d", $limit);
