...
class WCFM_WCPVendors {
  ...
  function wcpvendors_valid_line_items( $items, $order_id ) {
  	...
  	$sql = 'SELECT * FROM ' . WC_PRODUCT_VENDORS_COMMISSION_TABLE . ' AS commission';
		...
		$sql .= " AND `vendor_id` = %d";
		$sql .= " AND `order_id` = %d";
		$data_items = $wpdb->get_results( $wpdb->prepare( $sql, $vendor_id, $order_id ) );
  ...
  function wcpvendors_after_line_total( $item, $order ) {
  	...
  	if( $item->get_product_id() ) {
		...
		$sql = "
			...
			AND     order_id = %d
			AND     `vendor_id` = %d";
		$order_line_due = $wpdb->get_results( $wpdb->prepare( $sql, $product_id, $order->get_id(), $vendor_id ) );
  ...
  function wcpvendors_order_total_commission( $order_id ) {
  	...
  	SELECT SUM(total_commission_amount) as line_total,
       ...
       WHERE `vendor_id` = %d 
       AND order_id = %d";
    $order_due = $wpdb->get_results( $wpdb->prepare( $sql, $vendor_id, $order_id ) );
		?>
		<?php if( apply_filters( 'wcfm_is_allow_view_commission', true ) ) { ?>
			<tr>
