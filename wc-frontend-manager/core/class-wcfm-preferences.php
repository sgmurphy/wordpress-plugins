...
class WCFM_Preferences {
	...
	public function __construct() {
		...
		// Multi Store
		add_filter('wcfm_is_pref_multi_store', [&$this, 'wcfmpref_multi_store'], 750);
	}
	...
	// Multi Store Branch
	function wcfmpref_multi_store($is_pref) {
		$multi_store = (isset($this->wcfm_module_options['multi_store'])) ? $this->wcfm_module_options['multi_store'] : 'no';
		$is_ultimate_gte_6_7_0 = defined( 'WCFMu_VERSION') && version_compare( WCFMu_VERSION, '6.7.0', '>=' );
		if ($multi_store == 'yes' || !$is_ultimate_gte_6_7_0) {
			$is_pref = false;
		}

		return $is_pref;
	} //end wcfmpref_multi_store()
