	...
	public function processing() {
	  ...
	  if( isset( $wcfm_withdrawal_manage_form_data['withdrawals'] ) && !empty( $wcfm_withdrawal_manage_form_data['withdrawals'] ) ) {
        ...
        if ( ! $installed_version || version_compare( $installed_version, '2.8.2', '>' ) ) {
					...
					$balance_sql    = "SELECT * FROM `{$wpdb->prefix}dokan_vendor_balance` WHERE `trn_id`=%s AND `trn_type` = 'dokan_withdraw'";
					$balance_result = $wpdb->get_row( $wpdb->prepare( $balance_sql, $withdrawal_id ) );
				...
				$wpdb->query($wpdb->prepare(
					"UPDATE {$wpdb->dokan_withdraw} SET status = %d, note = %s WHERE user_id=%d AND id = %d",
					1,
					$withdraw_note,
					$user_id,
					$withdrawal_id
