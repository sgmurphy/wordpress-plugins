...
if (!class_exists('TVC_Ajax_File')) :
  ...
  class TVC_Ajax_File extends TVC_Ajax_Calls
  {
    ...
    {
      if ($this->safe_ajax_call(filter_input(INPUT_POST, 'CONVNonce', FILTER_SANITIZE_STRING), 'conv_checkMcc-nonce')) {
    ...
    {
      if (wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['sendmail_req_nonce'])), 'sendmail_req_nonce_val')) {
        $sendmail_message = $_POST["sendmail_message"];
        $emailmessage = "<ul><li><b>Name:</b> " . sanitize_text_field($sendmail_message['name']) . "</li><li><b>Contact No:</b> " . sanitize_text_field($sendmail_message['phone']) . "</li><li><b>Email:</b> " . sanitize_text_field($sendmail_message['email']) . "</li><li><b>Business Name:</b> " . sanitize_text_field($sendmail_message['businessName']) . "</li></ul>";
        $formdata = array();
    ...
    public function save_category_mapping()
    {
      if (is_admin() && wp_verify_nonce(filter_input(INPUT_POST, 'auto_product_sync_setting', FILTER_SANITIZE_STRING), 'auto_product_sync_setting-nonce')) {
        $data = is_array($_POST['ee_data']) ? $_POST['ee_data'] : $_POST['ee_data'];
    ...
    public function save_feed_data()
    {
      if ($this->safe_ajax_call(filter_input(INPUT_POST, 'conv_onboarding_nonce', FILTER_SANITIZE_STRING), 'conv_onboarding_nonce')) {
        ...
        if (isset($_POST['fb_catalog_id']) && $_POST['fb_catalog_id'] == 2) {
          $channel_id['fb_catalog_id'] = sanitize_text_field($_POST['fb_catalog_id']);
        }
            ...
            if ($_POST['inculdeExtraProduct'] != '') {
              ...
              if (!empty($p_map_attribute) && isset($p_map_attribute['items']) && !empty($p_map_attribute['items'])) {
                ...
                $data = [
                  ...
                  'is_on_facebook' => strpos(sanitize_text_field($_POST['channel_ids']), '2') !== false ? true : false,
                  'business_id' =>  strpos($_POST['channel_ids'], '2') !== false ? $ee_options['facebook_setting']['fb_business_id'] : '',
                  'catalog_id' =>  strpos($_POST['channel_ids'], '2') !== false ? $ee_options['facebook_setting']['fb_catalog_id'] : '',
              ...
              $feed_data = array(
                ...
                "fb_status" => strpos(sanitize_text_field($_POST['channel_ids']), '2') !== false ? esc_sql('In Progress') : null,
              );
              $TVC_Admin_DB_Helper->tvc_update_row("ee_product_feed", $feed_data, array("id" => sanitize_text_field($_POST['feedId'])));
