...
<div class="pmax-campaign add-pmax-campaign">
    <div class="mt24 whiteroundedbx dshreport-sec">
        ...
        <a href="<?php echo esc_url($this->site_url); ?>" class="btn-withborder"><img
                src="<?php echo esc_url(ENHANCAD_PLUGIN_URL."/admin/images/icon/left-angle-arrow.svg"); ?>"
                alt="back" /> <?php esc_html_e("Back to List","enhanced-e-commerce-for-woocommerce-store"); ?></a>
        <div class="tabs">
            ...
            <div id="tab1" class="tab active">
                <div class="campaignformwrp">
                    <form id="edit-pmax-campaign-form" method="post">
                        <div class="form-row campform-row">
                            ...
                            <input type="text" class="fromfiled" style="pointer-events: none; background: #f2f2f2;"
                                name="campaign_name" id="campaign_name"
                                value="<?php echo esc_attr($this->object_value($this->campaign, "name")); ?>"
                                placeholder="<?php echo esc_attr__("Enter Campaign Name","enhanced-e-commerce-for-woocommerce-store"); ?>"
                                required>
                        </div>
                        <div class="form-row campform-row">
                            <label><?php esc_html_e("Daily Budget","enhanced-e-commerce-for-woocommerce-store"); ?>
                                (<span
                                    class="ga_currency_symbols"><?php echo isset($this->currency_symbol) ? esc_html($this->currency_symbol):''; ?></span>)
                                *</label>
                            <input type="number" class="fromfiled" name="budget" id="budget"
                                value="<?php echo esc_attr($budget); ?>"
                                placeholder="<?php echo esc_attr__("Enter your budget","enhanced-e-commerce-for-woocommerce-store"); ?>"
                  ...
                  $target_roas = $this->object_value($maximizeconversionvalue, "targetRoas")*100; ?>
                            ...
                            <input type="number" name="target_roas" value="<?php echo esc_attr($target_roas); ?>"
                                class="fromfiled smtext"
                                placeholder="<?php echo esc_attr__("Add Number","enhanced-e-commerce-for-woocommerce-store"); ?>">
                        </div>
                        <div class="form-row campform-row">
                            <label><?php esc_html_e("Start Date","enhanced-e-commerce-for-woocommerce-store"); ?></label>
                            <input type="text" style="pointer-events: none; background: #f2f2f2;" name="start_date"
                                id="start_date"
                                value="<?php echo esc_attr($this->object_value($this->campaign, "startDate")); ?>"
                                class="fromfiled smtext datepicker">
                        </div>
                        <div class="form-row campform-row">
                            <label><?php esc_html_e("End Date","enhanced-e-commerce-for-woocommerce-store"); ?></label>
                            <input type="text" name="end_date" id="end_date"
                                value="<?php echo esc_attr($this->object_value($this->campaign, "endDate")); ?>"
                                class="fromfiled smtext datepicker">
                        </div>
                        <p class="label"><b>Status</b></p>
                        <div class="form-row form-row-grp campform-row">
                            <input type="radio"
                                <?php echo ($this->campaign->status == "ENABLED")?'checked="checked"':''; ?>
                                class="radio" value="ENABLED" name="status" id="cmp_active">
                            <label class="radio-label"
                                for="cmp_active"><?php esc_html_e("Enable","enhanced-e-commerce-for-woocommerce-store"); ?></label>
                            <input type="radio"
                                <?php echo ($this->campaign->status == "PAUSED")?'checked="checked"':''; ?>
                                class="radio" value="PAUSED" name="status" id="cmp_inactive">
                            <label class="radio-label"
                                for="cmp_inactive"><?php esc_html_e("Pause","enhanced-e-commerce-for-woocommerce-store"); ?></label>
                            <input type="radio"
                                <?php echo ($this->campaign->status == "REMOVED")?'checked="checked"':''; ?>
                                class="radio" value="REMOVED" name="status" id="cmp_removed">
                            <label class="radio-label"
                                for="cmp_removed"><?php esc_html_e("Remove","enhanced-e-commerce-for-woocommerce-store"); ?></label>
                        </div>
                        <div class="campfooterbtn">
                            <input type="hidden" name="customer_id"
                                value="<?php echo esc_attr($this->google_ads_id); ?>">
                            ...
                            <input type="hidden" name="resource_name"
                                value="<?php echo esc_attr($this->campaign->resourceName); ?>">
                            <input type="hidden" name="campaign_budget_resource_name"
                                value="<?php echo esc_attr($this->campaign->campaignBudget); ?>">
                            <button type="submit"
                                class="ppblubtn cretemrchntbtn"><?php esc_html_e("Save","enhanced-e-commerce-for-woocommerce-store"); ?></button>
                        </div>
                    </form>
                ...
                <a href="<?php echo esc_url($this->site_url); ?>" class="btn-withborder"><img
                        src="<?php echo esc_url(ENHANCAD_PLUGIN_URL."/admin/images/icon/left-angle-arrow.svg"); ?>"
                        alt="back" />
                    <?php esc_html_e("Back to List","enhanced-e-commerce-for-woocommerce-store"); ?></a>
            </div>
            <?php ?>
