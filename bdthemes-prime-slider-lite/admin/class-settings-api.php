	...
	class PrimeSlider_Settings_API {
		...
		function do_settings_sections( $page ) {
			...
			foreach ( (array) $wp_settings_sections[ $page ] as $section ) {
				...
				} elseif ( $section['id'] == 'prime_slider_license_settings' ) {
		...
		function callback_checkbox( $args ) {
			...
			if ( $plugin_name and $plugin_path ) {
				...
				if ( $this->_is_plugin_installed( $plugin_name, $plugin_path ) and is_plugin_active( $plugin_path ) ) {
					...
					$html .= sprintf( '<input type="checkbox" class="checkbox" id="bdt_ps_%1$s[%2$s]" name="%1$s[%2$s]" value="on" %3$s %4$s />', $args['section'], $args['id'], checked( $value, 'on', false ), ( 'pro' == $pro && $premium == false ) ? 'disabled="disabled"' : '' );
		...
		function callback_multicheck( $args ) {
			...
			foreach ( $args['options'] as $key => $label ) {
				...
				$html .= sprintf( '<label for="bdt_ps_%1$s[%2$s][%3$s]">', $args['section'], $args['id'], $key );
				$html .= sprintf( '<input type="checkbox" class="checkbox" id="bdt_ps_%1$s[%2$s][%3$s]" name="%1$s[%2$s][%3$s]" value="%3$s" %4$s />', $args['section'], $args['id'], $key, checked( $checked, $key, false ) );
				$html .= '<span class="switch"></span>';
		...
		function callback_radio( $args ) {
			...
			foreach ( $args['options'] as $key => $label ) {
				$html .= sprintf( '<label for="bdt_ps_%1$s[%2$s][%3$s]">', $args['section'], $args['id'], $key );
				$html .= sprintf( '<input type="radio" class="radio" id="bdt_ps_%1$s[%2$s][%3$s]" name="%1$s[%2$s]" value="%3$s" %4$s />', $args['section'], $args['id'], $key, checked( $value, $key, false ) );
				$html .= sprintf( '%1$s</label><br>', $label );
			}
