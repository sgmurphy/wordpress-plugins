...
class Status {
	...
	/*public $success = array('#0a9581', 'dashicons dashicons-yes-alt');
	public $warning = array('#f8b82d', "dashicons dashicons-warning");
	public $danger = array("#f65021", " dashiconsdashicons-dismiss");*/
...
//		woo_feed_delete_cache_data( "woo_feed_status_page_info" );
...
//			Cache::set( 'woo_feed_status_page_info', $status );
		}
	...
	private function product_per_batch( ) {
		$status = $this->success;
		$message =  Settings::get('per_batch');
		//TODO AI Message
		return [
	...
	private function product_query_type( ) {
		$status = $this->success;
		$message = Settings::get('product_query_type');
		if ( 'wc' == $message ) {
	...
	private function woo_feed_get_feed_file_list() {
		global $wpdb;
	...
	public function get_status_logs_error( $wp_status ){
		$status_text = "\n";

		foreach ( $wp_status as $key => $woo_feed_status ) {
			$label = isset( $woo_feed_status['label'] ) ? $woo_feed_status['label'] : '';
			$message = isset( $woo_feed_status['message'] ) ? $woo_feed_status['message'] : '';
