...
/**
 * Class representing the structure for Google.
 * Implements the StructureInterface for Google-related operations.
 */
...
class GoogleStructure implements StructureInterface {
	...
	/**
	 * Constructor for GoogleStructure.
	 *
	...
	public function get_xml_structure() {
		...
		if($this->exclude_identifier_exists()){
			if ( !\in_array( "identifier_exists", $mattributes ) ){
		...
		foreach ( $mattributes as $key => $attribute ) {
			...
			elseif ( \in_array( $attribute, $group['subscription_cost'], true ) && \count( $subscription ) < 2 ) {
				...
				$data[ $wrapper ][][ $attribute ] = $attribute_value;
	...
	public function get_csv_structure() {
		...
		foreach ( $mattributes as $key => $attribute ) {
			...
			if ( \strpos( $attribute, 'images_' ) !== false ) {
				...
				$data[][ $product_highlight_label ] = $attribute_value;
			} elseif ( \in_array( $attribute, $group['product_detail'], true ) ) {
				if ( $attribute === 'section_name' || $attribute === 'attribute_name' ) {
	...
	/**
	 * Exclude identifier exists
	 * @return bool
	 */
	public function exclude_identifier_exists(){
		$template = array('google_shopping_action','google_local','google_local_inventory');
		if( in_array( $this->config->get_feed_template(),$template ) ){
			return false;
		}
		return true;
	}
