...
class AutoFilter
{
    ...
    private function readDateRangeAutoFilter(SimpleXMLElement $filters, Column $column) : void
    {
        foreach ($filters->dateGroupItem as $dateGroupItem) {
            //    Operator is undefined, but always treated as EQUAL
            $column->createRule()->setRule(null, ['year' => (string) $dateGroupItem['year'], 'month' => (string) $dateGroupItem['month'], 'day' => (string) $dateGroupItem['day'], 'hour' => (string) $dateGroupItem['hour'], 'minute' => (string) $dateGroupItem['minute'], 'second' => (string) $dateGroupItem['second']], (string) $dateGroupItem['dateTimeGrouping'])->setRuleType(Rule::AUTOFILTER_RULETYPE_DATEGROUP);
        }
    }
    ...
    private function readDynamicAutoFilter(SimpleXMLElement $filterColumn, Column $column) : void
    {
        if ($filterColumn->dynamicFilter) {
            ...
            foreach ($filterColumn->dynamicFilter as $filterRule) {
                //    Operator is undefined, but always treated as EQUAL
                $column->createRule()->setRule(null, (string) $filterRule['val'], (string) $filterRule['type'])->setRuleType(Rule::AUTOFILTER_RULETYPE_DYNAMICFILTER);
    ...
    private function readTopTenAutoFilter(SimpleXMLElement $filterColumn, Column $column) : void
    {
        if ($filterColumn->top10) {
            ...
            foreach ($filterColumn->top10 as $filterRule) {
                $column->createRule()->setRule(isset($filterRule['percent']) && $filterRule['percent'] == 1 ? Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_PERCENT : Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_BY_VALUE, (string) $filterRule['val'], isset($filterRule['top']) && $filterRule['top'] == 1 ? Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_TOP : Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_BOTTOM)->setRuleType(Rule::AUTOFILTER_RULETYPE_TOPTENFILTER);
            }
        }
    }
}
