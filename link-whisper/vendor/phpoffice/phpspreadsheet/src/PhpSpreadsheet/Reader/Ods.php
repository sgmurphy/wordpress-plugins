...
use LWVendor\PhpOffice\PhpSpreadsheet\Reader\Ods\Properties as DocumentProperties;
...
class Ods extends BaseReader
{
    ...
    public function canRead($pFilename)
    {
        ...
        if ($zip->open($pFilename) === \true) {
            ...
            if ($stat && $stat['size'] <= 255) {
                ...
                $xml = \simplexml_load_string($this->securityScanner->scan($zip->getFromName('META-INF/manifest.xml')), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
    ...
    public function listWorksheetNames($pFilename)
    {
        ...
        $worksheetNames = [];
        $xml = new XMLReader();
        $xml->xml($this->securityScanner->scanFile('zip://' . \realpath($pFilename) . '#content.xml'), null, Settings::getLibXmlLoaderOptions());
    ...
    public function loadIntoExisting($pFilename, Spreadsheet $spreadsheet)
    {
        ...
        // Meta
        $xml = \simplexml_load_string($this->securityScanner->scan($zip->getFromName('meta.xml')), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
        if ($xml === \false) {
        ...
        $dom->loadXML($this->securityScanner->scan($zip->getFromName('content.xml')), Settings::getLibXmlLoaderOptions());
        $officeNs = $dom->lookupNamespaceUri('office');
        $tableNs = $dom->lookupNamespaceUri('table');
        $textNs = $dom->lookupNamespaceUri('text');
        $xlinkNs = $dom->lookupNamespaceUri('xlink');
        $spreadsheets = $dom->getElementsByTagNameNS($officeNs, 'body')->item(0)->getElementsByTagNameNS($officeNs, 'spreadsheet');
        foreach ($spreadsheets as $workbookData) {
            ...
            foreach ($tables as $worksheetDataSet) {
                ...
                if (isset($this->loadSheetsOnly) && $worksheetName && !\in_array($worksheetName, $this->loadSheetsOnly)) {
                    continue;
                }
                // Create sheet
                if ($worksheetID > 0) {
                                ...
                                if (\count($paragraphs) > 0) {
                                    ...
                                    switch ($type) {
                                        ...
                                        case 'date':
                                            ...
                                            $dataValue = Date::formattedPHPToExcel((int) $year, (int) $month, (int) $day, (int) $hour, (int) $minute, (int) $second);
                                        ...
                                        case 'time':
                                            ...
                                            $dataValue = Date::PHPToExcel(\strtotime('01-01-1970 ' . \implode(':', \sscanf($timeValue, 'PT%dH%dM%dS'))));
