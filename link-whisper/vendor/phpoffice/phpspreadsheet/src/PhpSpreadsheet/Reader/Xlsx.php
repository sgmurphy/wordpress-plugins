...
use LWVendor\PhpOffice\PhpSpreadsheet\Cell\Hyperlink;
use LWVendor\PhpOffice\PhpSpreadsheet\NamedRange;
use LWVendor\PhpOffice\PhpSpreadsheet\Reader\Security\XmlScanner;
use LWVendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\AutoFilter;
use LWVendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\Chart;
use LWVendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\ColumnAndRowAttributes;
use LWVendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\ConditionalStyles;
use LWVendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\DataValidations;
use LWVendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\Hyperlinks;
use LWVendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\PageSetup;
use LWVendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\Properties as PropertyReader;
use LWVendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\SheetViewOptions;
use LWVendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\SheetViews;
use LWVendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\Styles;
use LWVendor\PhpOffice\PhpSpreadsheet\ReferenceHelper;
use LWVendor\PhpOffice\PhpSpreadsheet\RichText\RichText;
use LWVendor\PhpOffice\PhpSpreadsheet\Settings;
use LWVendor\PhpOffice\PhpSpreadsheet\Shared\Date;
use LWVendor\PhpOffice\PhpSpreadsheet\Shared\Drawing;
use LWVendor\PhpOffice\PhpSpreadsheet\Shared\File;
use LWVendor\PhpOffice\PhpSpreadsheet\Shared\Font;
...
use LWVendor\PhpOffice\PhpSpreadsheet\Style\Borders;
use LWVendor\PhpOffice\PhpSpreadsheet\Style\Color;
use LWVendor\PhpOffice\PhpSpreadsheet\Style\NumberFormat;
use LWVendor\PhpOffice\PhpSpreadsheet\Style\Protection;
use LWVendor\PhpOffice\PhpSpreadsheet\Style\Style;
use LWVendor\PhpOffice\PhpSpreadsheet\Worksheet\HeaderFooterDrawing;
...
class Xlsx extends BaseReader
{
    ...
    public function listWorksheetNames($pFilename)
    {
        ...
        $rels = \simplexml_load_string($this->securityScanner->scan($this->getFromZipArchive($zip, '_rels/.rels')));
        foreach ($rels->Relationship as $rel) {
            switch ($rel['Type']) {
                case 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument':
                    //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main"
                    $xmlWorkbook = \simplexml_load_string($this->securityScanner->scan($this->getFromZipArchive($zip, "{$rel['Target']}")));
    ...
    public function listWorksheetInfo($pFilename)
    {
        ...
        foreach ($rels->Relationship as $rel) {
            if ($rel['Type'] == 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument') {
                ...
                //~ http://schemas.openxmlformats.org/package/2006/relationships"
                $relsWorkbook = \simplexml_load_string($this->securityScanner->scan($this->getFromZipArchive($zip, "{$dir}/_rels/" . \basename($rel['Target']) . '.rels')), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
                ...
                if ($xmlWorkbook->sheets) {
                    ...
                    foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                        ...
                        $xml = new XMLReader();
                        $xml->xml($this->securityScanner->scanFile('zip://' . File::realpath($pFilename) . '#' . "{$dir}/{$fileWorksheet}"), null, Settings::getLibXmlLoaderOptions());
    ...
    public function load($pFilename)
    {
        ...
        //~ http://schemas.openxmlformats.org/package/2006/relationships"
        $workbookBasename = $this->getWorkbookBaseName($zip);
        $wbRels = \simplexml_load_string($this->securityScanner->scan($this->getFromZipArchive($zip, "xl/_rels/{$workbookBasename}.rels")), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
        foreach ($wbRels->Relationship as $rel) {
            switch ($rel['Type']) {
                case 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/theme':
                    $themeOrderArray = ['lt1', 'dk1', 'lt2', 'dk2'];
                    $themeOrderAdditional = \count($themeOrderArray);
                    $xmlTheme = \simplexml_load_string($this->securityScanner->scan($this->getFromZipArchive($zip, "xl/{$rel['Target']}")), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
        ...
        foreach ($rels->Relationship as $rel) {
            switch ($rel['Type']) {
                ...
                case 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument':
                    ...
                    if ($xpath) {
                        //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main"
                        $xmlStrings = \simplexml_load_string($this->securityScanner->scan($this->getFromZipArchive($zip, "{$dir}/{$xpath['Target']}")), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
                    ...
                    //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main"
                    $xmlStyles = \simplexml_load_string($this->securityScanner->scan($this->getFromZipArchive($zip, "{$dir}/{$xpath['Target']}")), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
                    $styles = [];
                    $cellStyles = [];
                    $numFmts = null;
                    ...
                    if (!$this->readDataOnly && $xmlStyles) {
                        foreach ($xmlStyles->cellXfs->xf as $xf) {
                            ...
                            $style = (object) ['numFmt' => $numFmt, 'font' => $xmlStyles->fonts->font[(int) $xf['fontId']], 'fill' => $xmlStyles->fills->fill[(int) $xf['fillId']], 'border' => $xmlStyles->borders->border[(int) $xf['borderId']], 'alignment' => $xf->alignment, 'protection' => $xf->protection, 'quotePrefix' => $quotePrefix];
                            $styles[] = $style;
                        ...
                        foreach (isset($xmlStyles->cellStyleXfs->xf) ? $xmlStyles->cellStyleXfs->xf : [] as $xf) {
                            ...
                            $cellStyle = (object) ['numFmt' => $numFmt, 'font' => $xmlStyles->fonts->font[(int) $xf['fontId']], 'fill' => $xmlStyles->fills->fill[(int) $xf['fillId']], 'border' => $xmlStyles->borders->border[(int) $xf['borderId']], 'alignment' => $xf->alignment, 'protection' => $xf->protection, 'quotePrefix' => $quotePrefix];
                    ...
                    if ($xmlWorkbook->sheets) {
                        ...
                        foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                            ...
                            $fileWorksheet = $worksheets[(string) self::getArrayItem($eleSheet->attributes('http://schemas.openxmlformats.org/officeDocument/2006/relationships'), 'id')];
                            //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main"
                            $xmlSheet = \simplexml_load_string($this->securityScanner->scan($this->getFromZipArchive($zip, "{$dir}/{$fileWorksheet}")), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
                            $sharedFormulas = [];
                            ...
                            if (!$this->readDataOnly) {
                                ...
                                foreach ($vmlComments as $relName => $relPath) {
                                    ...
                                    $relPath = File::realpath(\dirname("{$dir}/{$fileWorksheet}") . '/' . $relPath);
                                    try {
                                        $vmlCommentsFile = \simplexml_load_string($this->securityScanner->scan($this->getFromZipArchive($zip, $relPath)), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
                                ...
                                if ($xmlSheet && $xmlSheet->legacyDrawingHF && !$this->readDataOnly) {
                                    if ($zip->locateName(\dirname("{$dir}/{$fileWorksheet}") . '/_rels/' . \basename($fileWorksheet) . '.rels')) {
                                        ...
                                        if ($vmlRelationship != '') {
                                            ...
                                            // Fetch VML document
                                            $vmlDrawing = \simplexml_load_string($this->securityScanner->scan($this->getFromZipArchive($zip, $vmlRelationship)), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
                            ...
                            if ($zip->locateName(\dirname("{$dir}/{$fileWorksheet}") . '/_rels/' . \basename($fileWorksheet) . '.rels')) {
                                ...
                                if ($xmlSheet->drawing && !$this->readDataOnly) {
                                            ...
                                            foreach ($relsDrawing->Relationship as $ele) {
                                                ...
                                                if ($ele['Type'] == 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/image') {
                                                    ...
                                                    if ($this->includeCharts) {
                                                        $charts[self::dirAdd($fileDrawing, $ele['Target'])] = ['id' => (string) $ele['Id'], 'sheet' => $docSheet->getTitle()];
                                                    }
                                                }
                                            }
                                        }
                                        $xmlDrawing = \simplexml_load_string($this->securityScanner->scan($this->getFromZipArchive($zip, $fileDrawing)), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
                                        ...
                                        if ($xmlDrawingChildren->oneCellAnchor) {
                                            foreach ($xmlDrawingChildren->oneCellAnchor as $oneCellAnchor) {
                                                if ($oneCellAnchor->pic->blipFill) {
                                                    ...
                                                    $objDrawing = new \LWVendor\PhpOffice\PhpSpreadsheet\Worksheet\Drawing();
                                                    $objDrawing->setName((string) self::getArrayItem($oneCellAnchor->pic->nvPicPr->cNvPr->attributes(), 'name'));
                                                    $objDrawing->setDescription((string) self::getArrayItem($oneCellAnchor->pic->nvPicPr->cNvPr->attributes(), 'descr'));
                                                    $objDrawing->setPath('zip://' . File::realpath($pFilename) . '#' . $images[(string) self::getArrayItem($blip->attributes('http://schemas.openxmlformats.org/officeDocument/2006/relationships'), 'embed')], \false);
                                    ...
                                    $xmlAltDrawing = \simplexml_load_string($this->securityScanner->scan($this->getFromZipArchive($zip, $fileDrawing)), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions())->children('http://schemas.openxmlformats.org/markup-compatibility/2006');
                                        ...
                                        switch ((string) $definedName['name']) {
                                            ...
                                            case '_xlnm.Print_Area':
                                                ...
                                                foreach ($rangeSets as $rangeSet) {
                                                    [$sheetName, $rangeSet] = Worksheet::extractSheetTitle($rangeSet, \true);
                        ...
                        if ($xmlWorkbook->definedNames) {
                            foreach ($xmlWorkbook->definedNames->definedName as $definedName) {
                                ...
                                if ((string) $definedName['localSheetId'] != '') {
                                            ...
                                            if ($mapSheetId[(int) $definedName['localSheetId']] !== null) {
                                                if (\strpos((string) $definedName, '!') !== \false) {
                                                    ...
                                                    if ($worksheet = $docSheet->getParent()->getSheetByName($range[0])) {
                                                        ...
                                                        $excel->addNamedRange(new NamedRange((string) $definedName['name'], $worksheet, $extractedRange, \true, $scope));
                                                    }
                                                }
                                            }
                                            break;
                                    ...
                                    if (\strpos((string) $definedName, '!') !== \false) {
                                        // Extract sheet name
                                        $extractedSheetName = Worksheet::extractSheetTitle((string) $definedName, \true);
                                        $extractedSheetName = \trim($extractedSheetName[0], "'");
                                        // Locate sheet
                                        $locatedSheet = $excel->getSheetByName($extractedSheetName);
                                        // Modify range
                                        [$worksheetName, $extractedRange] = Worksheet::extractSheetTitle($extractedRange, \true);
                                    }
                ...
                switch ($contentType['ContentType']) {
                    case 'application/vnd.openxmlformats-officedocument.drawingml.chart+xml':
                        if ($this->includeCharts) {
                            ...
                            $chartElements = \simplexml_load_string($this->securityScanner->scan($this->getFromZipArchive($zip, $chartEntryRef)), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
    ...
    private static function readStyle(Style $docStyle, $style) : void
    {
        ...
        if (isset($style->font)) {
            ...
            if (isset($style->font->u) && !isset($style->font->u['val'])) {
                $docStyle->getFont()->setUnderline(\LWVendor\PhpOffice\PhpSpreadsheet\Style\Font::UNDERLINE_SINGLE);
                ...
                foreach ($is->r as $run) {
                    if (!isset($run->rPr)) {
                        ...
                        if (isset($run->rPr->u) && !isset($run->rPr->u['val'])) {
                            $objText->getFont()->setUnderline(\LWVendor\PhpOffice\PhpSpreadsheet\Style\Font::UNDERLINE_SINGLE);
    ...
    private function readRibbon(Spreadsheet $excel, $customUITarget, $zip) : void
    {
        ...
        if ($dataRels) {
            ...
            $UIRels = \simplexml_load_string($this->securityScanner->scan($dataRels), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
            if (\false !== $UIRels) {
    ...
    private function readHyperLinkDrawing($objDrawing, $cellAnchor, $hyperlinks) : void
    {
        ...
        $hyperlink = new Hyperlink($hyperlinks[$hlinkId], (string) self::getArrayItem($cellAnchor->pic->nvPicPr->cNvPr->attributes(), 'name'));
