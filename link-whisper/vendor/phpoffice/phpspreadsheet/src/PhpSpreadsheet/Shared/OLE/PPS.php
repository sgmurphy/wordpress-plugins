...
namespace LWVendor\PhpOffice\PhpSpreadsheet\Shared\OLE;
...
class PPS
{
    ...
    public function _getPpsWk()
    {
        ...
        $ret .= \pack('v', \strlen($this->Name) + 2) . \pack('c', $this->Type) . \pack('c', 0x0) . \pack('V', $this->PrevPps) . \pack('V', $this->NextPps) . \pack('V', $this->DirPps) . "\x00\t\x02\x00" . "\x00\x00\x00\x00" . "\xc0\x00\x00\x00" . "\x00\x00\x00F" . "\x00\x00\x00\x00" . OLE::localDateToOLE($this->Time1st) . OLE::localDateToOLE($this->Time2nd) . \pack('V', isset($this->startBlock) ? $this->startBlock : 0) . \pack('V', $this->Size) . \pack('V', 0);
        // 128
        return $ret;
    }
    /**
