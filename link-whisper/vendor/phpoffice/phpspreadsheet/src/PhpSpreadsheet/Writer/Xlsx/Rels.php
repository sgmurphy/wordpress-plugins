...
class Rels extends WriterPart
{
    ...
    public function writeRelationships(Spreadsheet $spreadsheet)
    {
        ...
        if ($spreadsheet->hasRibbon()) {
            $this->writeRelationShip($objWriter, 5, 'http://schemas.microsoft.com/office/2006/relationships/ui/extensibility', $spreadsheet->getRibbonXMLData('target'));
        }
     ...
     * @return string XML Output
     */
    public function writeWorksheetRelationships(\LWVendor\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $pWorksheet, $pWorksheetId = 1, $includeCharts = \false)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
        ...
        if ($pWorksheet->getDrawingCollection()->count() > 0 || \count($charts) > 0 || $drawingOriginalIds) {
            ...
            $this->writeRelationship($objWriter, $rId, 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/drawing', $relPath);
        }
    ...
    private function writeUnparsedRelationship(\LWVendor\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $pWorksheet, XMLWriter $objWriter, $relationship, $type) : void
    {
        $unparsedLoadedData = $pWorksheet->getParent()->getUnparsedLoadedData();
     ...
     * @return string XML Output
     */
    public function writeDrawingRelationships(\LWVendor\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $pWorksheet, &$chartRef, $includeCharts = \false)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
        ...
        while ($iterator->valid()) {
            if ($iterator->current() instanceof \LWVendor\PhpOffice\PhpSpreadsheet\Worksheet\Drawing || $iterator->current() instanceof MemoryDrawing) {
                // Write relationship for image drawing
                /** @var \PhpOffice\PhpSpreadsheet\Worksheet\Drawing $drawing */
                $drawing = $iterator->current();
                $this->writeRelationship($objWriter, $i, 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/image', '../media/' . \str_replace(' ', '', $drawing->getIndexedFilename()));
     ...
     * @return string XML Output
     */
    public function writeHeaderFooterDrawingRelationships(\LWVendor\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $pWorksheet)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
    ...
    private function writeDrawingHyperLink($objWriter, $drawing, $i)
    {
        ...
        ++$i;
        $this->writeRelationship($objWriter, $i, 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink', $drawing->getHyperlink()->getUrl(), $drawing->getHyperlink()->getTypeHyperlink());
        return $i;
    }
}
