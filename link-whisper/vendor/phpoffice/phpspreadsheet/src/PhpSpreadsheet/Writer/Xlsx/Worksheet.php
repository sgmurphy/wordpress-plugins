...
use LWVendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column;
use LWVendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column\Rule;
use LWVendor\PhpOffice\PhpSpreadsheet\Worksheet\SheetView;
use LWVendor\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet as PhpspreadsheetWorksheet;
class Worksheet extends WriterPart
{
    ...
    {
        if ($conditional->getConditionType() == Conditional::CONDITION_CELLIS || $conditional->getConditionType() == Conditional::CONDITION_CONTAINSTEXT || $conditional->getConditionType() == Conditional::CONDITION_EXPRESSION) {
        ...
        foreach ($pSheet->getConditionalStylesCollection() as $cellCoordinate => $conditionalStyles) {
            foreach ($conditionalStyles as $conditional) {
                ...
                if ($conditional->getConditionType() != Conditional::CONDITION_NONE) {
                    ...
                    self::writeAttributeif($objWriter, ($conditional->getConditionType() == Conditional::CONDITION_CELLIS || $conditional->getConditionType() == Conditional::CONDITION_CONTAINSTEXT) && $conditional->getOperatorType() != Conditional::OPERATOR_NONE, 'operator', $conditional->getOperatorType());
    ...
    private function writeAutoFilter(XMLWriter $objWriter, PhpspreadsheetWorksheet $pSheet) : void
    {
        ...
        if (!empty($autoFilterRange)) {
            ...
            [$ws, $range[0]] = PhpspreadsheetWorksheet::extractSheetTitle($range[0], \true);
            $range = \implode(':', $range);
    ...
    private function writeSheetData(XMLWriter $objWriter, PhpspreadsheetWorksheet $pSheet, array $pStringTable) : void
    {
        ...
        while ($currentRow++ < $highestRow) {
            ...
            // Write current row?
            $writeCurrentRow = isset($cellsByRow[$currentRow]) || $rowDimension->getRowHeight() >= 0 || $rowDimension->getVisible() == \false || $rowDimension->getCollapsed() == \true || $rowDimension->getOutlineLevel() > 0 || $rowDimension->getXfIndex() !== null;
            if ($writeCurrentRow) {
                // Start a new row
