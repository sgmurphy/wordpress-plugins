    ...
    public function bindValue(Cell $cell, $value = null)
    {
        ...
        if ($dataType === DataType::TYPE_STRING && !$value instanceof RichText) {
            ...
            if (\preg_match('/^' . \preg_quote($currencyCode, '/') . ' *(\\d{1,3}(' . \preg_quote($thousandsSeparator, '/') . '\\d{3})*|(\\d+))(' . \preg_quote($decimalSeparator, '/') . '\\d{2})?$/', $value)) {
                ...
                $cell->setValueExplicit($value, DataType::TYPE_NUMERIC);
                // Set style
                $cell->getWorksheet()->getStyle($cell->getCoordinate())->getNumberFormat()->setFormatCode(\str_replace('$', $currencyCode, NumberFormat::FORMAT_CURRENCY_USD_SIMPLE));
                return \true;
                ...
                $cell->setValueExplicit($value, DataType::TYPE_NUMERIC);
                // Set style
                $cell->getWorksheet()->getStyle($cell->getCoordinate())->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_CURRENCY_USD_SIMPLE);
                return \true;
            }
