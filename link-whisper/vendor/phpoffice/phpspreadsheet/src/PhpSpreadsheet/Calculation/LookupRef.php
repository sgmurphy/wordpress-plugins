...
class LookupRef
{
    ...
    public static function COLUMN($cellAddress = null)
    {
        ...
        if (\is_array($cellAddress)) {
            ...
            [$sheet, $cellAddress] = Worksheet::extractSheetTitle($cellAddress, \true);
    ...
    public static function INDIRECT($cellAddress = null, ?Cell $pCell = null)
    {
        ...
        if (!\preg_match('/^' . Calculation::CALCULATION_REGEXP_CELLREF . '$/i', $cellAddress1, $matches) || $cellAddress2 !== null && !\preg_match('/^' . Calculation::CALCULATION_REGEXP_CELLREF . '$/i', $cellAddress2, $matches)) {
    ...
    public static function MATCH($lookupValue, $lookupArray, $matchType = 1)
    {
        ...
        foreach ($lookupArray as $i => $lookupArrayValue) {
            ...
            if (!\is_numeric($lookupArrayValue) && !\is_string($lookupArrayValue) && !\is_bool($lookupArrayValue) && $lookupArrayValue !== null) {
                return Functions::NA();
            }
    ...
    public static function VLOOKUP($lookup_value, $lookup_array, $index_number, $not_exact_match = \true)
    {
        ...
        foreach ($lookup_array as $rowKey => $rowData) {
            ...
            if (\is_numeric($lookup_value) && \is_numeric($rowData[$firstColumn]) && $rowData[$firstColumn] > $lookup_value || !\is_numeric($lookup_value) && !\is_numeric($rowData[$firstColumn]) && $firstLower > $lookupLower) {
                break;
            }
            // remember the last key, but only if datatypes match
            if (\is_numeric($lookup_value) && \is_numeric($rowData[$firstColumn]) || !\is_numeric($lookup_value) && !\is_numeric($rowData[$firstColumn])) {
                if ($not_exact_match) {
                    $rowNumber = $rowKey;
                    continue;
    ...
    public static function HLOOKUP($lookup_value, $lookup_array, $index_number, $not_exact_match = \true)
    {
        ...
        foreach ($lookup_array[$firstColumn] as $rowKey => $rowData) {
            ...
            if ($not_exact_match && ($bothNumeric && $rowData > $lookup_value || $bothNotNumeric && $rowDataLower > $lookupLower)) {
                break;
            }
