...
use LWVendor\PhpOffice\PhpSpreadsheet\Calculation\Engine\CyclicReferenceStack;
use LWVendor\PhpOffice\PhpSpreadsheet\Calculation\Engine\Logger;
use LWVendor\PhpOffice\PhpSpreadsheet\Calculation\Token\Stack;
...
class Calculation
{
     ...
     * @var array
     */
    private static $operators = ['+' => \true, '-' => \true, '*' => \true, '/' => \true, '^' => \true, '&' => \true, '%' => \false, '~' => \false, '>' => \true, '<' => \true, '=' => \true, '>=' => \true, '<=' => \true, '<>' => \true, '|' => \true, ':' => \true];
    /**
     * List of binary operators (those that expect two operands).
     *
     * @var array
     */
    private static $binaryOperators = ['+' => \true, '-' => \true, '*' => \true, '/' => \true, '^' => \true, '&' => \true, '>' => \true, '<' => \true, '=' => \true, '>=' => \true, '<=' => \true, '<>' => \true, '|' => \true, ':' => \true];
    /**
     ...
     * @return mixed
     */
    public function calculateCellValue(?Cell $pCell = null, $resetLog = \true)
    {
        if ($pCell === null) {
            return null;
        }
    ...
    public function _calculateFormulaValue($formula, $cellID = null, ?Cell $pCell = null)
    {
        ...
        if ($pCell !== null && $pCell->getStyle()->getQuotePrefix() === \true) {
    ...
    private function _parseFormula($formula, ?Cell $pCell = null)
    {
        ...
        while (\true) {
            ...
            if ($opCharacter == '-' && !$expectingOperator) {
                ...
                if (\is_array($d) && \preg_match('/^' . self::CALCULATION_REGEXP_FUNCTION . '$/i', $d['value'], $matches)) {
                ...
                if (!empty($pendingStoreKey) && $parenthesisDepthMap[$pendingStoreKey] == 0) {
                    // We must go to the IF next argument
                    if ($expectingConditionMap[$pendingStoreKey]) {
                        $expectingConditionMap[$pendingStoreKey] = \false;
                        $expectingThenMap[$pendingStoreKey] = \true;
                    } elseif ($expectingThenMap[$pendingStoreKey]) {
                        $expectingThenMap[$pendingStoreKey] = \false;
                        $expectingElseMap[$pendingStoreKey] = \true;
                ...
                if (\preg_match('/^' . self::CALCULATION_REGEXP_FUNCTION . '$/i', $val, $matches)) {
                    ...
                    if ($this->branchPruningEnabled && $valToUpper == 'IF(') {
                        ...
                        $expectingConditionMap[$pendingStoreKey] = \true;
            ...
            if ($formula[$index] == ' ') {
                ...
                if ($expectingOperator && (\preg_match('/^' . self::CALCULATION_REGEXP_CELLREF . '.*/Ui', \substr($formula, $index), $match) && $output[\count($output) - 1]['type'] == 'Cell Reference' || \preg_match('/^' . self::CALCULATION_REGEXP_NAMEDRANGE . '.*/Ui', \substr($formula, $index), $match) && ($output[\count($output) - 1]['type'] == 'Named Range' || $output[\count($output) - 1]['type'] == 'Value'))) {
                    while ($stack->count() > 0 && ($o2 = $stack->last()) && isset(self::$operators[$o2['value']]) && @(self::$operatorAssociativity[$opCharacter] ? self::$operatorPrecedence[$opCharacter] < self::$operatorPrecedence[$o2['value']] : self::$operatorPrecedence[$opCharacter] <= self::$operatorPrecedence[$o2['value']])) {
                        $output[] = $stack->pop();
                        //    Swap operands and higher precedence operators from the stack to the output
                    }
    ...
    private function processTokenStack($tokens, $cellID = null, ?Cell $pCell = null)
    {
        ...
        foreach ($tokens as $tokenData) {
            ...
            if ($this->branchPruningEnabled && isset($tokenData['onlyIf'])) {
                ...
                if (isset($storeValue) && (!$storeValueAsBool || Functions::isError($storeValue) || $storeValue === 'Pruned branch')) {
        ...
        if (\is_array($operand1) || \is_array($operand2)) {
            ...
            if (\is_array($operand1) && !\is_array($operand2)) {
                ...
                foreach ($operand1 as $x => $operandData) {
                    ...
                    $this->executeBinaryComparisonOperation($cellID, $operandData, $operand2[$x], $operation, $stack, \true);
                    $r = $stack->pop();
                    $result[$x] = $r['value'];
                }
            }
     ...
     */
    public function extractCellRange(&$pRange = 'A1', ?Worksheet $pSheet = null, $resetLog = \true)
    {
        // Return value
        $returnValue = [];
        if ($pSheet !== null) {
            ...
            if (\strpos($pRange, '!') !== \false) {
                [$pSheetName, $pRange] = Worksheet::extractSheetTitle($pRange, \true);
     ...
     * @return mixed Array of values in range if range contains more than one element. Otherwise, a single value is returned.
     */
    public function extractNamedRange(&$pRange = 'A1', ?Worksheet $pSheet = null, $resetLog = \true)
    {
        // Return value
        $returnValue = [];
        if ($pSheet !== null) {
