...
class ReferenceHelper
{
    ...
    protected function adjustPageBreaks(Worksheet $pSheet, $pBefore, $beforeColumnIndex, $pNumCols, $beforeRow, $pNumRows) : void
    {
        ...
        $pNumCols > 0 || $pNumRows > 0 ? \uksort($aBreaks, ['self', 'cellReverseSort']) : \uksort($aBreaks, ['self', 'cellSort']);
    ...
    protected function adjustHyperlinks($pSheet, $pBefore, $beforeColumnIndex, $pNumCols, $beforeRow, $pNumRows) : void
    {
        ...
        $pNumCols > 0 || $pNumRows > 0 ? \uksort($aHyperlinkCollection, ['self', 'cellReverseSort']) : \uksort($aHyperlinkCollection, ['self', 'cellSort']);
    ...
    protected function adjustDataValidations($pSheet, $pBefore, $beforeColumnIndex, $pNumCols, $beforeRow, $pNumRows) : void
    {
        ...
        $pNumCols > 0 || $pNumRows > 0 ? \uksort($aDataValidationCollection, ['self', 'cellReverseSort']) : \uksort($aDataValidationCollection, ['self', 'cellSort']);
    ...
    protected function adjustProtectedCells($pSheet, $pBefore, $beforeColumnIndex, $pNumCols, $beforeRow, $pNumRows) : void
    {
        ...
        $pNumCols > 0 || $pNumRows > 0 ? \uksort($aProtectedCells, ['self', 'cellReverseSort']) : \uksort($aProtectedCells, ['self', 'cellSort']);
        ...
        while ($coordinate = \array_pop($allCoordinates)) {
            ...
            if ($cellIndex >= $beforeColumnIndex && $cell->getRow() >= $beforeRow) {
                ...
                if ($cell->getDataType() == DataType::TYPE_FORMULA) {
                    ...
                    $pSheet->getCell($newCoordinate)->setValue($this->updateFormulaReferences($cell->getValue(), $pBefore, $pNumCols, $pNumRows, $pSheet->getTitle()));
                } else {
