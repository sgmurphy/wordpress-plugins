 ...
 *
 */
if ( ! class_exists( 'BEAF_Repeater' ) ) {
	class BEAF_Repeater extends BEAF_Fields {
			...
			<div id="tf-repeater-1" class="tf-repeater <?php echo esc_attr( $this->field['id'] ); ?>">
				...
				<div
					...
					<div class="tf-single-repeater tf-single-repeater-<?php echo esc_attr( $this->field['id'] ); ?>">
						...
						<input type="hidden" name="tf_current_field"
							value="<?php if ( isset( $this->field['id'] ) ) {
								echo esc_attr( $this->field['id'] );
							} ?>">
						...
						<div class="tf-repeater-header">
							...
							<span
								class="tf-repeater-title"><?php if ( isset( $this->field['label'] ) ) {
				...
				if ( empty( $this->field['drag_only'] ) || ! $this->field['drag_only'] ) {
					?>
					<div
						class="tf-repeater-add tf-repeater-add-<?php if ( isset( $this->field['id'] ) ) {
							echo esc_attr( $this->field['id'] );
						} ?>">
						...
						<span data-repeater-id="<?php if ( isset( $this->field['id'] ) ) {
							...
							if ( isset( $this->field['button_title'] ) && ! empty( $this->field['button_title'] ) ) {
								echo esc_attr( $this->field['button_title'] );
							} else {
