	...
	public function add_toolbar( $admin_bar ) {
		if ( is_user_logged_in() ) {
			...
			if ( in_array( 'administrator', $roles ) || in_array( 'editor', $roles ) ) {
				...
				$title = '<span><img src="' . esc_url( GUTENVERSE_URL . '/assets/icon/icon-logo-dashboard.svg' ) . '"/>' . esc_html__( 'Gutenverse', 'gutenverse' ) . '</span>';
	...
	public function setting_toolbar( $admin_bar, $parent = 'gutenverse' ) {
		...
		$admin_bar->add_menu(
			array(
				'id'     => 'form',
				'parent' => $parent,
				'title'  => esc_html__( 'Form Action', 'gutenverse' ),
		...
		$admin_bar->add_menu(
			array(
				'id'     => 'form-entries',
				'parent' => $parent,
				'title'  => esc_html__( 'Form Entries', 'gutenverse' ),
