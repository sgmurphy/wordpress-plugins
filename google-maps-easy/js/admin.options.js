		...
		$notices.each(function(){
			jQuery(this).find('.notice-dismiss').click(function(){
				...
				if(!$notice.data('stats-sent')) {
					...
					jQuery.sendFormGmp({
						data: {mod: 'supsystic_promo', action: 'addNoticeAction', '_wpnonce' : GMP_NONCE['gmp_nonce'], code: $notice.data('code'), choice: 'hide'}
					});
				}
			});
			jQuery(this).find('[data-statistic-code]').click(function(){
				...
				jQuery.sendFormGmp({
					data: {mod: 'supsystic_promo', action: 'addNoticeAction', '_wpnonce' : GMP_NONCE['gmp_nonce'], code: $notice.data('code'), choice: jQuery(this).data('statistic-code')}
				});
