		    ...
		    'checkbox' => __('Checkbox', 'wpf'),
				...
				<?php if ( $type === 'wpf_cat' || is_taxonomy_hierarchical( $type ) ) : ?>
                    ...
                    <div class="wpf_back_active_module_row">
                        ...
                        <div class="wpf_back_active_module_input">
                            <label>
                                <input id="wpf_<?php echo $type ?>[arch_ctx]" type="checkbox" name="[<?php echo $type ?>][arch_ctx]" value="1"<?php if ( ! empty( $module['arch_ctx'] ) ) : ?> checked="checked"<?php endif; ?> />
                                <?php _e('On taxonomy archive, show only children of current term', 'wpf') ?>
                            </label>
                        </div>
                    </div>
					<div class="wpf_back_active_module_row">
						<div class="wpf_back_active_module_label">
							<label for="wpf_<?php echo $type ?>[include]"><?php _e('Include Children ', 'wpf') ?>
								<small class="wpf_help_icon">
									<span><?php _e( '(?)', 'wpf' ); ?></span>
									<div class="wpf_help_content"><?php _e( 'If set to Yes, when performing a filter, products assigned to the child terms will also display. Please change this option only if you\'re sure that is not desired.', 'wpf' ); ?></div>
								</small>
							</label>
						</div>
                                        ...
                                        <div class="wpf_color_options_wrap">
                                            ...
                                            <label class="wpf_color_wrap wpf_background_image <?php if (!empty($module['image_bg_' . $cat->term_id])): ?> has-image <?php endif;?>" for="wpf_<?php echo $type ?>_<?php echo $cat->term_id ?>_image_bg">
                                                ...
                                                <div class="image-area">
                                                    <img class="preview-image-wraper" src="<?php echo empty( $module['image_bg_' . $cat->term_id] ) ? '' : esc_attr( $module['image_bg_' . $cat->term_id] ) ?>" alt="">
