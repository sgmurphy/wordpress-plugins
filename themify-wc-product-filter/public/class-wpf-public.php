...
class WPF_Public
{
	...
	public function register_assets() {
		...
		wp_register_script( $this->plugin_name, $plugin_url . 'js/wpf-public.js', array( 'jquery' ), $this->version, [ 'strategy' => 'defer', 'in_footer' => true ] );
		...
		if ( !empty( $get_products ) ) {
			...
			foreach ( $get_products as $p ) {
				if ( !empty( $p['meta_value'] ) ) {
					if ( $p['meta_value'] === 'instock' ) $instock[] = $p['post_id'];

					if ( $p['meta_value'] === 'outofstock' ) $outofstock[] = $p['post_id'];
				}
			}
			...
			if ( !empty( $stock ) ) {
				...
				if ( !empty( $get_products ) ) {
					foreach ( $get_products as $p ) {
						if ( in_array( $p->ID, $instock ) ) $n_instock[] = $p->post_parent;

						if ( in_array( $p->ID, $outofstock ) ) $n_outofstock[] = $p->post_parent;
					}
