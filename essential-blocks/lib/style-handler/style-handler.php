    ...
    final class EbStyleHandler
    {
        ...
        public function enqueue_frontend_assets()
        {
            ...
            if (in_array('gp-premium/gp-premium.php', apply_filters('active_plugins', get_option('active_plugins')))) {
                ...
                $post_ids = $this->gp_ids;

                foreach ($post_ids as $element) {
                    if (file_exists($this->eb_style_dir . $this->prefix . '-' . $element . '.min.css')) {
                        wp_enqueue_style(
                            'eb-block-style-' . $element,
                            $this->eb_style_url . $this->prefix . '-' . $element . '.min.css',
                            $deps,
            ...
            // var_dump(file_exists($this->eb_style_dir . $this->prefix . '-widget.min.css'));
            // if (file_exists($this->eb_style_dir . $this->prefix . '-widget.min.css')) {
            //     wp_enqueue_style('eb-widget-style', $this->eb_style_url . $this->prefix . '-widget.min.css', $deps, substr(md5(microtime(true)), 0, 10), 'all');
            // // }
