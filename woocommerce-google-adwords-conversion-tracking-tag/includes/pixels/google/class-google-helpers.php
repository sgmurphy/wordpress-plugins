...
class Google_Helpers {
	...
	public static function get_google_enhanced_conversion_data( $order ) {
		...
		if ($order->get_billing_email()) {
			...
			$email = self::prep_email_for_enhanced_conversions($email);
		...
		if (self::is_shipping_address_set($order)) {
	...
	private static function is_shipping_address_set( $order ) {
...
//            $product = new WC_Product($refund_item->get_product_id());
	 ...
	 * @since 1.42.5
	 */
	private static function prep_email_for_enhanced_conversions( $email ) {
