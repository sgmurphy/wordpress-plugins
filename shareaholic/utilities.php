...
class ShareaholicUtilities {
	...
	public static function accept_terms_of_service() {
		...
		if ( ! isset( $_POST['shareaholic_get_started_nonce'] ) ) {
			return;
		}
		// Verify that the nonce is valid.
		if ( ! wp_verify_nonce( $_POST['shareaholic_get_started_nonce'], 'shareaholic_get_started_nonce' ) ) {
			return;
		}
		...
		if ( is_array( $d ) ) {
			...
			*/
			return array_map(function ( $item ) {
				return self::object_to_array($item);
			}, $d);
	 ...
	 */
	public static function rocket_exclude_js( $excluded_external ) {
		if ( defined( 'WP_ROCKET_VERSION' ) ) {
			$excluded_external[] = 'cdn.shareaholic.net';
			$excluded_external[] = 'cdn.openshareweb.com';
	...
	public static function location_sync_status() {
		...
		if ( $api_key ) {
			...
			if ( is_array( $response ) && array_key_exists( 'body', $response ) ) {
				$response_code = wp_remote_retrieve_response_code( $response );
				if ( $response_code == '200' ) {
					return 200; // If all the location records sent are present with backend.
				}
				if ( $response_code == '409' ) {
					return 409; // If any of the location records sent are not present with backend.
				}
				if ( $response_code == '404' ) {
					return 404;
				}
