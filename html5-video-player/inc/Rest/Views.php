...
namespace H5VP\Rest;
use H5VP\Model\View;
class Views extends \WP_REST_Controller{
    ...
    function create_view($request){
        global $wpdb;
        $table_name = $wpdb->prefix.'h5vp_views';
        $args = $this->prepare($request);
        // return 'sldfksdlf';

        // $args = $this->getTitle($args);
        $view = new View();
        $insert = $view->createOrUpdate($args);

        if($insert){
            return new \WP_REST_Response([
                'status' => 201,
                'response' => 'View counted',
                'insert' => $insert
            ]);
        }else {
            return new \WP_REST_Response([
    ...
    public function get_view( $request){
        ...
        $id = isset($params['id']) ? sanitize_text_field($params['id']) : false;
        $table_name = $wpdb->prefix.'h5vp_views';
        
        if(!$id){
            return new \WP_Error( 'rest_forbidden', esc_html__( 'OMG you can not view private data.', 'h5vp' ), array( 'status' => 401 ) );
        }
        $video = $wpdb->get_row($wpdb->prepare("SELECT * FROM $table_name WHERE id='%s'", $id));
        if(!$video){
    ...
    public function update_view($request){
        ...
        $update = $wpdb->update($table_name, $params,
        [
            'id' => $info['id']
        ]);
    ...
    public function prepare($request){
        ...
        return [
            ...
            'video_id' => isset($request['video_id']) ? sanitize_text_field($request['video_id']) :'',
        ];
    }
