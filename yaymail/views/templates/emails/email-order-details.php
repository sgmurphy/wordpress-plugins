...
<div class="yaymail_builder_order" style="color: #7f54b3;font-size: 18px;font-weight: 700;">
	...
	if ( $sent_to_admin ) {
		...
		$after  = '</h2>';
		/* translators: %s: Order ID. */
		echo wp_kses_post( $before . sprintf( __( '[Order #%s]', 'woocommerce' ) . ' (<time datetime="%s">%s</time>)', $order->get_order_number(), $order->get_date_created() != null ? $order->get_date_created()->format( 'c' ) : '', wc_format_datetime( $order->get_date_created() ) ) . $after );
	}
	?>
</div>
<?php } ?>
...
<table class="yaymail_builder_table_items_content yaymail-list-order-item" cellspacing="0" cellpadding="6" border="1" style="border-collapse: separate; color: #636363; border: 1px solid #e5e5e5; font-family: Helvetica,Roboto,Arial,sans-serif;" width="100%">
	<thead>
		<tr style="word-break: normal">
			...
			<th class="td yaymail-title-item-price" scope="col" style="width: 30%;text-align:left;vertical-align: middle;padding: 12px;font-size: 14px;border: 1px solid;border-color: inherit;">
