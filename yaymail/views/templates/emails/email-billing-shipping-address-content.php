...
if ( ! empty( $order ) ) {
	$shipping_methods = ! empty( $order->get_shipping_methods() ) ? $order->get_shipping_methods() : array();
	foreach ( $shipping_methods as $shipping_method ) {
		if ( ! empty( $shipping_method->get_method_id() ) && str_contains( $shipping_method->get_method_id(), 'pickup' ) ) {
...
">
	<tr>
		...
		<?php } ?>
		<?php if ( ! empty( $shipping_address ) && ! $is_local_pickup ) { ?>
