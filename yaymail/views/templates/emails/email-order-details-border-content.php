...
$product_title         = $is_preview ? '{{product_title}}' : ( false != $order_item_title && isset( $order_item_title['product_title'] ) ? do_shortcode( $order_item_title['product_title'] ) : 'Product' );
$cost_title            = $is_preview ? '{{cost_title}}' : ( false != $order_item_title && isset( $order_item_title['cost_title'] ) ? do_shortcode( $order_item_title['cost_title'] ) : 'Cost' );
$quantity_title        = $is_preview ? '{{quantity_title}}' : ( false != $order_item_title && isset( $order_item_title['quantity_title'] ) ? do_shortcode( $order_item_title['quantity_title'] ) : 'Quantity' );
$price_title           = $is_preview ? '{{price_title}}' : ( false != $order_item_title && isset( $order_item_title['price_title'] ) ? do_shortcode( $order_item_title['price_title'] ) : 'Price' );
$subtoltal_title       = $is_preview ? '{{subtoltal_title}}' : ( false != $order_item_title && isset( $order_item_title['subtoltal_title'] ) ? do_shortcode( $order_item_title['subtoltal_title'] ) : 'Subtotal:' );
$discount_title        = $is_preview ? '{{discount_title}}' : ( false != $order_item_title && isset( $order_item_title['discount_title'] ) ? do_shortcode( $order_item_title['discount_title'] ) : 'Discount:' );
$shipping_title        = $is_preview ? '{{shipping_title}}' : ( false != $order_item_title && isset( $order_item_title['shipping_title'] ) ? do_shortcode( $order_item_title['shipping_title'] ) : 'Shipping:' );
$payment_method_title  = $is_preview ? '{{payment_method_title}}' : ( false != $order_item_title && isset( $order_item_title['payment_method_title'] ) ? do_shortcode( $order_item_title['payment_method_title'] ) : 'Payment method:' );
$total_title           = $is_preview ? '{{total_title}}' : ( false != $order_item_title && isset( $order_item_title['total_title'] ) ? do_shortcode( $order_item_title['total_title'] ) : 'Total:' );
$fully_refunded        = $is_preview ? '{{fully_refunded}}' : ( false != $order_item_title && isset( $order_item_title['fully_refunded'] ) ? do_shortcode( $order_item_title['fully_refunded'] ) : 'Order fully refunded.' );
$customer_note         = $is_preview ? '{{customer_note}}' : ( false != $order_item_title && isset( $order_item_title['customer_note'] ) ? do_shortcode( $order_item_title['customer_note'] ) : 'Note:' );
...
<thead class="yaymail_element_head_order_item">
	<tr style="word-break: normal">
		<th colspan="<?php echo wp_kses_post( apply_filters( 'yaymail_order_item_product_title_colspan', 1, $yaymail_template ) ); ?>" class="td yaymail_item_product_title" scope="col" style="text-align:<?php echo esc_attr( $text_align ); ?>;vertical-align: middle;padding: 12px;font-size: 14px;border-width: 1px;border-style: solid;<?php echo esc_attr( $borderColor ); ?>;">
			<?php
			if ( $is_preview ) {
				$cleaned_title = str_replace( array( '{{', '}}' ), '', $product_title );
				echo wp_kses( '<span v-html="' . $cleaned_title . '"></span>', $allowed_html_tags );
			} else {
		...
		<?php if ( $productItemCost ) { ?>
			<th class="td yaymail_item_price_per_item" colspan="<?php echo wp_kses_post( apply_filters( 'yaymail_order_item_cost_colspan', 1, $yaymail_template ) ); ?>" scope="col" style="text-align:<?php echo esc_attr( $text_align ); ?>;vertical-align: middle;padding: 12px;font-size: 14px;border-width: 1px;border-style: solid;<?php echo esc_attr( $borderColor ); ?>;">
				<?php
				if ( $is_preview ) {
					$cleaned_title = str_replace( array( '{{', '}}' ), '', $cost_title );
					echo wp_kses( '<span v-html="' . $cleaned_title . '"></span>', $allowed_html_tags );
				} else {
					echo esc_html( $cost_title );
				}
				?>
			</th>
		<?php } ?>
		<th colspan="<?php echo wp_kses_post( apply_filters( 'yaymail_order_item_quantity_colspan', 1, $yaymail_template ) ); ?>" class="td yaymail_item_quantity_title" scope="col" style="text-align:<?php echo esc_attr( $text_align ); ?>;vertical-align: middle;padding: 12px;font-size: 14px;border-width: 1px;border-style: solid;<?php echo esc_attr( $borderColor ); ?>;">
			<?php
			if ( $is_preview ) {
				$cleaned_title = str_replace( array( '{{', '}}' ), '', $quantity_title );
				echo wp_kses( '<span v-html="' . $cleaned_title . '"></span>', $allowed_html_tags );
			} else {
		...
		<th colspan="<?php echo wp_kses_post( apply_filters( 'yaymail_order_item_price_colspan', 1, $yaymail_template ) ); ?>" class="td yaymail_item_price_title" scope="col" style="width: 30%;text-align:<?php echo esc_attr( $text_align ); ?>;vertical-align: middle;padding: 12px;font-size: 14px;border-width: 1px;border-style: solid;<?php echo esc_attr( $borderColor ); ?>;">
			<?php
			if ( $is_preview ) {
				$cleaned_title = str_replace( array( '{{', '}}' ), '', $price_title );
				echo wp_kses( '<span v-html="' . $cleaned_title . '"></span>', $allowed_html_tags );
			} else {
...
<tbody class="yaymail_element_body_order_item">
	<?php
	$order_item_html = $this->ordetItemTables(
		$order,
		array(
	...
	if ( ! empty( $order_item_html ) ) {
		echo wp_kses_post( $order_item_html );
	}
...
<tfoot class="yaymail_element_foot_order_item">	
	...
	<tr class="<?php echo esc_attr( $tr_class ); ?>">
		<th class="td <?php echo esc_attr( $th_class ); ?>" scope="row" colspan="<?php echo esc_attr( $productItemCost ? 3 : 2 ); ?>" style="text-align:<?php echo esc_attr( $text_align ); ?>;vertical-align: middle;padding: 12px;font-size: 14px;border-width: 1px;border-style: solid;<?php echo esc_attr( $borderColor ); ?>; <?php echo esc_attr( ( 1 === $i ) ? 'border-top-width: 4px;' : '' ); ?>">
			<?php
			if ( array_key_exists( $key, $get_order_item_totals ) ) {
				if ( $is_preview ) {
					$cleaned_title = str_replace( array( '{{', '}}' ), '', $get_order_item_totals[ $key ] );
					$html          = '<span v-html="' . $cleaned_title . '"></span>';
				} else {
					$html = esc_html( $get_order_item_totals[ $key ] );
				}
				if ( 'refund_0' == $key ) {
					if ( __( 'Order fully refunded.', 'woocommerce' ) == $total['label'] ) {
	...
	if ( ! empty( $order->get_customer_note() ) ) {
		...
		$customer_note_content_colspan = ! empty( $productItemCost ) ? 2 : '';

		?>

		<tr class="yaymail_item_note_title_row">
			<th class="td" scope="row" colspan="<?php echo wp_kses_post( apply_filters( 'yaymail_item_note_title_colspan', 2, $yaymail_template ) ); ?>" style="text-align:<?php echo esc_attr( $text_align ); ?>;vertical-align: middle;padding: 12px;font-size: 14px;border-width: 1px;border-style: solid;<?php echo esc_attr( $borderColor ); ?>; <?php echo esc_attr( ( 1 === $i ) ? 'border-top-width: 4px;' : '' ); ?>">
				<?php esc_html_e( $get_order_item_totals['customer_note'], 'woocommerce' ); ?>
			</th>
			<th class="td" colspan="<?php echo wp_kses_post( apply_filters( 'yaymail_item_note_content_colspan', $customer_note_content_colspan, $yaymail_template ) ); ?>"  style="font-weight: normal;text-align:<?php echo esc_attr( $text_align ); ?>;vertical-align: middle;padding: 12px;font-size: 14px;border-width: 1px;border-style: solid;<?php echo esc_attr( $borderColor ); ?>; <?php echo esc_attr( ( 1 === $i ) ? 'border-top-width: 4px;' : '' ); ?>">
