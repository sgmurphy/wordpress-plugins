...
class MCProtectIpstore_V572 {
	...
	function __construct($storage_type = MCProtectIpstore_V572::STORAGE_TYPE_DB) {
		$this->storage_type = $storage_type;
		if ($this->storage_type == MCProtectIpstore_V572::STORAGE_TYPE_FS) {
			$this->storage = new MCProtectIpstoreFS_V572();
		} else {
			$this->storage = new MCProtectIpstoreDB_V572();
		}
	}

	public static function uninstall() {
		MCProtectIpstoreDB_V572::uninstall();
	}

	public function isLPIPBlacklisted($ip) {
		if ($this->storage_type == MCProtectIpstore_V572::STORAGE_TYPE_DB) {
