...
class MCProtectIpstore_V567 {
	...
	function __construct($storage_type = MCProtectIpstore_V567::STORAGE_TYPE_DB) {
		$this->storage_type = $storage_type;
		if ($this->storage_type == MCProtectIpstore_V567::STORAGE_TYPE_FS) {
			$this->storage = new MCProtectIpstoreFS_V567();
		} else {
			$this->storage = new MCProtectIpstoreDB_V567();
		}
	}

	public static function uninstall() {
		MCProtectIpstoreDB_V567::uninstall();
	}

	public function isLPIPBlacklisted($ip) {
		if ($this->storage_type == MCProtectIpstore_V567::STORAGE_TYPE_DB) {
