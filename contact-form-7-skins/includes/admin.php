...
class CF7_Skins_Admin {
	...
	function cf7skins_update( $cf7 ) {
		...
		// For new created form, set 'default' style
		if ( isset( $_POST['cf7s-style'] ) ) {
			if ( metadata_exists( 'post', $form_id, 'cf7s_style' ) ) { // edited form
				update_post_meta( $form_id, 'cf7s_style', sanitize_text_field( $_POST['cf7s-style'] ) );
			} else { // new form
				// Check if a styles is selected, or set 'default' if not selected.
				$style = empty( $_POST['cf7s-style'] ) ? 'default' : sanitize_text_field( $_POST['cf7s-style'] );
				update_post_meta( $form_id, 'cf7s_style', $style );
			}
		}
	...
	function admin_enqueue_scripts( $hook_suffix ) {
		...
		wp_localize_script( 'cf7s-admin', 'cf7s', array(
			...
			'l10n'		=> array(
				...
				'select_style'		=> __('Select to apply the Style to your form - is applied to your form once you Save.', 'contact-form-7-skins' ),
				'select_template'	=> __('Select to apply the Template to your form - appears in the form editing area, where you can edit your requirements.', 'contact-form-7-skins' ),
