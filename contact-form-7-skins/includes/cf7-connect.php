...
class CF7_Skins_CF7_Connect {
	...
	function __construct() {
		...
		add_filter( 'wpcf7_autop_or_not', array( $this, 'disable_wpcf7_autop' ), 1, 2 ); // @since 2.5.4
	}
	 ...
	 * CF7 uses this filter for sending mail, without autop enabled, 
	 * message will be sent inline.
	 * 
	 * @filter 'wpcf7_autop_or_not' at contact-form-7/includes/formatting.php
	 * and at includes/mail.php
	 * 
	 * @param $wpcf7_autop (boolean) use wpcf7_autop or not
	 * @param $options (array) see 'wpcf7_autop_or_not' filter $options
	 * 
	 * @return (bool) $wpcf7_autop
	 *
	 * @since 2.5.4
	 */
	function disable_wpcf7_autop( $wpcf7_autop, $options = false ) {

		if ( is_array( $options ) ) { // for CF7 5.8 or greater

			if ( 'mail' === $options['for'] ) { // enable autop to add new line in sent mail content
				return true;
			
			} elseif ( 'form' === $options['for'] ) { // disable autop in backend/frontend form
				return false;
			}

		} else { // for lower CF7 

			if ( is_admin() ) { // disable CF7 autop while editing/creating form
				return false;
			}
		}

		return $wpcf7_autop; // autop enabled by default
	}
