<div class="wrap about-wrap full-width-layout qlwrap">
  <form id="qlwapp_box_form" method="post" action="options.php">
	<table class="form-table">
		...
		<tr>
			<th scope="row"><?php esc_html_e( 'Images', 'wp-whatsapp-chat' ); ?></th>
			<td>
			<select name="lazy_load" class="qlwapp-select2">
				<option value="yes" <?php selected( $box['lazy_load'], 'yes' ); ?>><?php esc_html_e( 'Enable lazy load', 'wp-whatsapp-chat' ); ?></option>
				<option value="no" <?php selected( $box['lazy_load'], 'no' ); ?>><?php esc_html_e( 'Disable lazy load', 'wp-whatsapp-chat' ); ?></option>
			</select>
			</td>
		</tr>
		<tr>
