...
if ( ! class_exists( 'YITH_WCAN_Presets' ) ) {
	...
	class YITH_WCAN_Presets {
		...
		public function save_preset() {
			...
			if ( $preset_id ) {
				/**
				 * DO_ACTION: yith_wcan_save_preset
				 *
				 * Fires after preset saving procedure.
				 *
				 * @param int              $preset_id Current preset id.
				 * @param YITH_WCAN_Preset $preset    Current object.
				 */
		...
		public function save_preset_filter() {
			...
			$prev_filter_id = isset( $_POST['prev_filter_id'] ) ? (int) $_POST['prev_filter_id'] : false;
			...
			$preset->set_filter( $filter_id, $filter, $prev_filter_id );
			$preset->save();
