		...
		if ( $sanitized == $attr && $attr != "" ) {
			...
			switch( $validator ) {
				case "text":
					...
					break;
				case "htmltext":
					$new_value = esc_html(wp_strip_all_tags($value));
					break;
				case "integer":
...
/**
 * Get Debug Log Filepath.
 *
 * Returns the file path of debug log file.
 *
 * @since 4.24.2
 *
 * @redeclarable
 *
 * @return string The filepath of debug log file.
 */
function wfu_debug_log_filepath() {
	$a = func_get_args(); $a = WFU_FUNCTION_HOOK(__FUNCTION__, $a, $out); if (isset($out['vars'])) foreach($out['vars'] as $p => $v) $$p = $v; switch($a) { case 'R': return $out['output']; break; case 'D': die($out['output']); }
	return WP_CONTENT_DIR.'/debug_log.txt';
}
...
function wfu_debug_log($message) {
	$logpath = wfu_debug_log_filepath();
 ...
 * @since 4.24.2
 */
function wfu_activate_debug_log() {
	@ini_set('log_errors', 'On');
	@ini_set('display_errors', 'Off');
	@ini_set('error_log', wfu_debug_log_filepath());
	if ( isset($GLOBALS["WFU_GLOBALS"]["WFU_DEBUG"]) ) $GLOBALS["WFU_GLOBALS"]["WFU_DEBUG"][3] = "ON";
	$GLOBALS['wfu_debug-wfu_tf_LOG'] = "1";
}
...
function wfu_get_uploader_template($templatename = "") {
	...
	if ($templatename != "") {
		...
		$exists = false;
		// check first whether the templates exists in the external templates
		// folder
		$external = WFU_VAR("WFU_EXTERNALTEMPLATES_FOLDER");
		if ( trim($external) != "" ) {
			if ( substr($external, 0, 1) != '/' ) $external = '/'.$external;
			if ( substr($external, -1) != '/' ) $external .= '/';
			$filepath = WP_CONTENT_DIR.$external."uploader-$templatename.php";
			$exists = file_exists($filepath);
		}
		// else check the default plugin templates folder
		if ( !$exists ) {
