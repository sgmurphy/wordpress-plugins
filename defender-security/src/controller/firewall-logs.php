...
class Firewall_Logs extends Controller {
		...
		switch ( $data['action'] ) {
			...
			case 'delete':
				...
				/* translators: %s: IP Address(es) */
					__( 'IP %s has been deleted', 'defender-security' ),
					implode( ', ', $ips )
				);
				break;
			default:
	...
	public function export_as_csv(): void {
		...
		$headers = [
			...
			__( 'IP Status', 'defender-security' ),
			__( 'User Agent Status', 'defender-security' ),
		];
		fputcsv( $fp, $headers );
	...
	public function toggle_ua_to_list( Request $request ): Response {
		...
		if ( 'remove' === $action && $model->is_ua_in_list( $ua, $list ) ) {
			...
			return new Response(
				false,
				[ 'message' => __( 'Wrong result.', 'defender-security' ) ]
			);
		}
	...
	public function query_logs( Request $request ): Response {
		...
		if ( ! $v->validate() ) {
			return new Response( false, [ 'message' => __( 'Wrong start and end date.', 'defender-security' ) ] );
		}
