...
class Two_Factor extends Event {
	...
	public function verify_otp_login_time() {
		...
		if ( empty( $_POST['_wpnonce'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'verify_otp' ) ) {
			wp_die( __( 'Nonce verification failed.', 'defender-security' ) );
		}
		...
		if ( empty( $token ) || empty( $user_id ) || empty( $auth_method ) || empty( $password ) ) {
			wp_die( __( 'Missing parameter(s)', 'defender-security' ) );
		}

		$user = get_user_by( 'id', $user_id );
		// Spoofed data? E.g. a hidden field user is changed.
		if ( ! is_object( $user ) ) {
			wp_die( __( 'Invalid user.', 'defender-security' ) );
		}
		...
		if ( $result ) {
			...
			if ( true === $is_weak_password ) {
				...
				if ( isset( $_REQUEST['interim-login'] ) ) {
					...
					$params['message'] = '<p class="message">' . __( 'You have logged in successfully.', 'defender-security' ) . '</p>';
					$this->render_otp_screen( $params );
					exit;
				} else {
					// Usual success.
	...
	private function render_otp_screen( array $params = [] ) {
		...
		if ( 0 === count( $params['providers'] ) ) {
			// Since 3.5.0.
			$error_msg = __( 'No providers.', 'defender-security' );
	...
	public function verify_otp_for_enabling( Request $request ) {
		if ( is_user_logged_in() ) {
			...
			if ( ! $setup_key ) {
				return new Response(
					false,
					[ 'message' => __( 'The setup key is incorrect.', 'defender-security' ) ]
				);
			}
			$user_id = get_current_user_id();
	...
	public function export_strings(): array {
		...
		return [
			$settings->enabled ? __( 'Active', 'defender-security' ) : __( 'Inactive', 'defender-security' ),
		];
	}
	...
	public function generate_backup_codes(): Response {
		...
		return new Response(
			...
			[
				...
				'title' => sprintf(
				/* translators: %s: count */
					__( '2FA Backup Codes for %s:', 'defender-security' ),
					get_bloginfo( 'url' )
				),
				'button_text' => __( 'Get New Codes', 'defender-security' ),
				'description' => __( 'Each backup code can only be used to log in once.', 'defender-security' ),
			]
		);
	}
	...
	public function display_2fa_user_settings(): void {
		if ( ( ! is_admin() || defined( 'DOING_AJAX' ) || defined( 'DOING_CRON' ) ) ) {
			...
			echo '<button type="submit" class="button" name="save_def_2fa_user_settings" value="' . esc_attr__( 'Save changes', 'defender-security' ) . '">'
	...
	public function wp_defender_2fa_link_my_account( $items ): array {
		...
		return array_slice( $items, 0, $needed_place, true )
			+ [ $this->slug => __( '2FA', 'defender-security' ) ]
	...
	public function save_2fa_details() {
		...
		wc_add_notice( __( 'Two-Factor settings updated successfully.', 'defender-security' ) );
		// @since 3.2.0
