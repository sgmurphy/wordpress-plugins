...
class Upgrader {
	...
	private function upgrade_2_6_1(): void {
		...
		if ( ! empty( $configs ) ) {
			foreach ( $configs as $k => $config ) {
				if ( 0 === strcmp( $config['name'], __( 'Basic config', 'defender-security' ) ) ) {
					$config['name'] = __( 'Basic Config', 'defender-security' );
...
Your temporary password is {{passcode}}. To finish logging in, copy and paste the temporary password into the Password field on the login screen.';
	...
	private function replace_scan_email_subject_issue_found( string $subject ): string {
		$subject = str_replace(
			__( 'Scan of {SITE_URL} complete.', 'defender-security' ),
			__( 'Malware Scan of {SITE_URL} is complete.', 'defender-security' ),
			$subject
		);

		return str_replace(
			__( '{ISSUES_COUNT} issues found.', 'defender-security' ),
			__( '{ISSUES_COUNT} issue(s) found.', 'defender-security' ),
			$subject
		);
	}
	...
	private function replace_scan_email_content_issue_found_new( string $content ): string {
		$content = str_replace(
			__( '{ISSUES_COUNT} vulnerabilities were identified for {SITE_URL} during a Malware Scan.', 'defender-security' ),
			__( 'Malware Scan identified {ISSUES_COUNT} issue(s) on {SITE_URL}.', 'defender-security' ),
			$content
		);

		return str_replace(
			__( 'See details for each issue below.', 'defender-security' ),
			__( 'The identified issue(s) is/are listed below.', 'defender-security' ),
			$content
		);
	}
