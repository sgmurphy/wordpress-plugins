...
class Ezoic_Cdn extends Ezoic_Feature {
	 ...
	 * @param WP_Post|null $post WordPress post object (found with get_post if omitted).
	 *
	...
	public function modify_urls_for_gtranslate($urls) {
		...
		foreach ($urls as $original_url) {
			...
			foreach ($language_codes as $code) {
				if ($is_enterprise) {
					// For enterprise, the code becomes a subdomain
					$new_url = $url_parts['scheme'] . '://' . $code . '.' . $main_domain . $url_parts['path'] . $url_parts['query'] . $url_parts['fragment'];
				} else {
					// For Pro version, the language code is added to the path
			...
			// If enterprise, add the original URL since the language code doesn't affect the path.
	...
	public function parse_url_components($url) {
		...
		// Ensure '://' is only appended when the scheme is present
		$scheme_and_host = $scheme . '://' . $host;

		return array(
			'scheme' => $scheme,
			'host' => $host,
			'scheme_and_host' => $scheme_and_host, // Correctly combined scheme and host
