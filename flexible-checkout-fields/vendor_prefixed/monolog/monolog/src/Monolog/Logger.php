...
class Logger implements \Psr\Log\LoggerInterface, \FcfVendor\Monolog\ResettableInterface
{
     ...
     */
    public function pushHandler(\FcfVendor\Monolog\Handler\HandlerInterface $handler) : self
    {
     ...
     */
    public function popHandler() : \FcfVendor\Monolog\Handler\HandlerInterface
    {
        if (!$this->handlers) {
        ...
        try {
            ...
            foreach ($this->handlers as $handler) {
                if (null === $record) {
                    ...
                    $record = ['message' => $message, 'context' => $context, 'level' => $level, 'level_name' => $levelName, 'channel' => $this->name, 'datetime' => $datetime ?? new \FcfVendor\Monolog\DateTimeImmutable($this->microsecondTimestamps, $this->timezone), 'extra' => []];
                    try {
