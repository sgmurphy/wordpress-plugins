(function ($) {
    /****** DethemeKit Vertical Scroll Handler ******/
    var DethemeKitVerticalScrollHandler = function ($scope, $) {
        ...
        var $vScrollElem = $scope.find(".dethemekit-vscroll-wrap"),
            instance = null,
            vScrollSettings = $vScrollElem.data("settings");

        vScrollSettings.deviceType = deviceType;

        instance = new dethemekitVerticalScroll($vScrollElem, vScrollSettings);
        instance.init();

    };

    window.dethemekitVerticalScroll = function ($selector, settings) {
        var self = this,
            $window = $(window),
            isTouch = 'desktop' !== elementorFrontend.getCurrentDeviceMode(),
            $instance = $selector,
            checkTemps = $selector.find(".dethemekit-vscroll-sections-wrap")
                .length,
            $htmlBody = $("html, body"),
            $itemsList = $(".dethemekit-vscroll-dot-item", $instance),
            $menuItems = $(".dethemekit-vscroll-nav-item", $instance),
            defaultSettings = {
                speed: 700,
                offset: 0,
                fullSection: true
        ...
        self.init = function () {
            ...
            $itemsList.on("click.dethemekitVerticalScroll", self.onNavDotChange);
            $menuItems.on("click.dethemekitVerticalScroll", self.onNavDotChange);

            $itemsList.on("mouseenter.dethemekitVerticalScroll", self.onNavDotEnter);

            $itemsList.on("mouseleave.dethemekitVerticalScroll", self.onNavDotLeave);

            if ("desktop" === settings.deviceType) {
                $window.on("scroll.dethemekitVerticalScroll", self.onWheel);
            }

            $window.on("resize.dethemekitVerticalScroll orientationchange.dethemekitVerticalScroll", self.debounce(50, self.onResize));
            ...
            if (settings.animation) {
                $instance.find(".dethemekit-vscroll-dots").removeClass("elementor-invisible").addClass("animated " + settings.animation + " animated-" + settings.duration);
            }
        ...
        self.checkCurrentActive = function () {
            ...
            if (currentSection)
                $instance.find(".dethemekit-vscroll-dots").removeClass("dethemekit-vscroll-dots-hide");
        ...
        self.onTouchMove = function (e) {
            ...
            var $target = $(e.target),
                sectionSelector = checkTemps ? ".dethemekit-vscroll-temp" : ".elementor-top-section",
            ...
            $(".dethemekit-vscroll-tooltip").hide();
            ...
            if (sectionId && sections.hasOwnProperty(sectionId)) {
                ...
                if (newSectionId) {
                    ...
                    $instance.find(".dethemekit-vscroll-dots, .dethemekit-vscroll-nav-menu").removeClass("dethemekit-vscroll-dots-hide");
                    ...
                    if ("down" === direction) {
                        ...
                        if ($lastselector.offset().top + $lastselector.innerHeight() - $(document).scrollTop() > 600) {
                            ...
                            $instance.find(".dethemekit-vscroll-dots, .dethemekit-vscroll-nav-menu").addClass("dethemekit-vscroll-dots-hide");
        ...
        self.onNavDotEnter = function () {
            ...
            if (settings.tooltips) {
                $(
                    '<div class="dethemekit-vscroll-tooltip"><span>' +
        ...
        self.onNavDotLeave = function () {
            $(".dethemekit-vscroll-tooltip").fadeOut(200, function () {
                $(this).remove();
            });
        };
        ...
        self.onNavDotChange = function (event) {
            ...
            if (!isScrolling) {
                ...
                if ($this.hasClass("dethemekit-vscroll-nav-item")) {
        ...
        self.onKeyUp = function (event, direction) {
            ...
            var direction = direction || "up",
                nextItem = $(".dethemekit-vscroll-dot-item[data-menuanchor=" + currentSection + "]", $instance).next(),
                prevItem = $(".dethemekit-vscroll-dot-item[data-menuanchor=" + currentSection + "]", $instance).prev();
            ...
            if ("up" === direction) {
                if (prevItem[0]) {
                    prevItem.trigger("click.dethemekitVerticalScroll");
                }
            }

            if ("down" === direction) {
                if (nextItem[0]) {
                    nextItem.trigger("click.dethemekitVerticalScroll");
                }
            }
        };
        ...
        self.onWheel = function (event) {
            ...
            var $target = $(event.target),
                ...
                windowScrollTop = $window.scrollTop(),
                dotIndex = $(".dethemekit-vscroll-dot-item.active").index();
    ...
    $(window).on("elementor/frontend/init", function () {
        elementorFrontend.hooks.addAction(
            "frontend/element_ready/dethemekit-vscroll.default",
            DethemeKitVerticalScrollHandler
        );
    });
})(jQuery);