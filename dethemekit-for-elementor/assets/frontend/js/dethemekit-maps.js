jQuery(window).on("elementor/frontend/init", function () {
    ...
    elementorFrontend.hooks.addAction(
        "frontend/element_ready/dethemekit-addon-maps.default",
        function ($scope, $) {

            var mapElement = $scope.find(".dethemekit_maps_map_height");

            var mapSettings = mapElement.data("settings");

            var mapStyle = mapElement.data("style");

            var dethemekitMapMarkers = [];

            dethemekitMap = newMap(mapElement, mapSettings, mapStyle);
            ...
            function newMap(map, settings, mapStyle) {
                ...
                var markers = map.find(".dethemekit-pin");
            ...
            function add_marker(pin, map, autoOpen, hoverOpen, hoverClose) {
                ...
                dethemekitMapMarkers.push(marker);

                //Used with Carousel Custom Navigation option
                if (customID) {
                    google.maps.event.addListener(marker, "click", function () {
                        console.log(customID);
                        var $carouselWidget = $(".dethemekit-carousel-wrapper");
                            ...
                            $carouselWidget.map(function (index, item) {
                                ...
                                if (carouselSettings.navigation) {
                                    if (-1 != carouselSettings.navigation.indexOf("#" + customID)) {
                                        ...
                                        $(item).find(".dethemekit-carousel-inner").slick("slickGoTo", slideIndex);
                                    }
                                }
                            })
                ...
                if (
                    pin.find(".dethemekit-maps-info-title").html() ||
                    pin.find(".dethemekit-maps-info-desc").html()
                ) {
                    // create info window
                            ...
                            if ($carouselWidget.length) {
                                $carouselWidget.map(function (index, item) {
                                    ...
                                    if (carouselSettings.navigation) {
                                        if (-1 != carouselSettings.navigation.indexOf("#" + customID)) {
                                            var slideIndex = carouselSettings.navigation.indexOf("#" + customID);
                                            $carouselWidget.find(".dethemekit-carousel-inner").slick("slickGoTo", slideIndex);
                                        }
                                    }
                                })
            ...
            if (markerCluster) {
                var markerCluster = new MarkerClusterer(dethemekitMap, dethemekitMapMarkers, {
                    imagePath:
