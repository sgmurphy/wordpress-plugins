...
if ( ! class_exists( 'DethemeKit_Templates_Manager' ) ) {

	/**
     * DethemeKit Templates Manager.
     *
     ...
     * 
     */
	class DethemeKit_Templates_Manager {
        ...
        /**
         * DethemeKit_Templates_Manager constructor.
         *
		...
		public function __construct() {
            ...
            //Register AJAX hooks
			add_action( 'wp_ajax_dethemekit_get_templates', array( $this, 'get_templates' ) );
            add_action( 'wp_ajax_dethemekit_inner_template', array( $this, 'insert_inner_template' ) );
			...
			add_filter( 'dethemekit-templates-core/assets/editor/localize', array( $this, 'localize_tabs' ) );
		...
		public function register_sources() {
			...
			require DETHEMEKIT_ADDONS_PATH . 'includes/templates/sources/base.php';

            $namespace = str_replace( 'Classes', 'Sources' , __NAMESPACE__ );
            
			$sources = array(
				'dethemekit-api'   =>  $namespace . '\DethemeKit_Templates_Source_Api',
			);

			foreach ( $sources as $key => $class ) {
                
				require DETHEMEKIT_ADDONS_PATH . 'includes/templates/sources/' . $key . '.php';
		...
		public function get_template_tabs() {
			...
			$tabs = Templates\dethemekit_templates()->types->get_types_for_popup();
		...
		public function get_templates() {
			...
			// Verify nonce
			if (!isset($_GET['nonce']) || !wp_verify_nonce($_GET['nonce'], 'ajax-nonce')) {
				wp_send_json_error();
			}
         ...
         * 
		 */
		// public function dethemekit_get_template_data() {
