...
/**
 * DethemeKit_Template_Tags class defines all the query of options of select box
 *
...
class DethemeKit_Template_Tags {
	...
	public static function get_query_args() {
		...
		$post_args = array(
			'post_type'        => $post_type,
			'posts_per_page'   => empty( $settings['dethemekit_blog_number_of_posts'] ) ? 9999 : $settings['dethemekit_blog_number_of_posts'],
			'paged'            => $paged,
			'post_status'      => 'publish',
			'suppress_filters' => false,
		);

		$post_args['orderby']             = $settings['dethemekit_blog_order_by'];
		$post_args['order']               = $settings['dethemekit_blog_order'];
		$post_args['ignore_sticky_posts'] = 'yes' === $settings['ignore_sticky_posts'] ? 1 : 0;

		if ( ! empty( $settings['dethemekit_blog_posts_exclude'] ) ) {

			$post_args[ $settings['posts_filter_rule'] ] = $settings['dethemekit_blog_posts_exclude'];
		}

		if ( ! empty( $settings['dethemekit_blog_users'] ) ) {

			$post_args[ $settings['author_filter_rule'] ] = $settings['dethemekit_blog_users'];
		}
		...
		if ( 0 < $settings['dethemekit_blog_offset'] ) {
			 ...
			 */
			$post_args['offset_to_fix'] = $settings['dethemekit_blog_offset'];
		}
	...
	public static function get_paged() {
        ...
        // Verify nonce
        if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'pa-blog-widget-nonce')) {
            return 0;
        }
	...
	public static function get_post_excerpt_link( $read_more ) {
		...
		echo '<div class="dethemekit-blog-excerpt-link-wrap">';
			echo '<a href="' . esc_url( get_permalink() ) . '" class="dethemekit-blog-excerpt-link elementor-button">';
	...
	protected function get_post_thumbnail( $target ) {
		...
		$skin = $settings['dethemekit_blog_skin'];
	...
	protected function render_post_title( $link_target ) {
		...
		$this->add_render_attribute( 'title', 'class', 'dethemekit-blog-entry-title' );

		?>
		<<?php echo wp_kses_post( $settings['dethemekit_blog_title_tag'] . ' ' . $this->get_render_attribute_string( 'title' ) ); ?>>
			<a href="<?php the_permalink(); ?>" target="<?php echo esc_attr( $link_target ); ?>">
				<?php esc_html( the_title() ); ?>
			</a>
		</<?php echo wp_kses_post( $settings['dethemekit_blog_title_tag'] ); ?>>
		<?php
	}
	...
	protected function get_post_meta( $link_target ) {
		...
		$author_meta = $settings['dethemekit_blog_author_meta'];

		$data_meta = $settings['dethemekit_blog_date_meta'];

		$categories_meta = $settings['dethemekit_blog_categories_meta'];

		$comments_meta = $settings['dethemekit_blog_comments_meta'];
		...
		?>
		<div class="dethemekit-blog-entry-meta">
			<?php if ( 'yes' === $author_meta ) : ?>
				<div class="dethemekit-blog-post-author dethemekit-blog-meta-data">
			...
			<?php if ( 'yes' === $data_meta ) { ?>
				<span class="dethemekit-blog-meta-separator">•</span>
				<div class="dethemekit-blog-post-time dethemekit-blog-meta-data">
			...
			<?php if ( 'yes' === $categories_meta && ! in_array( $skin, array( 'side', 'banner' ), true ) ) : ?>
				<span class="dethemekit-blog-meta-separator">•</span>
				<div class="dethemekit-blog-post-categories dethemekit-blog-meta-data">
			...
			<?php if ( 'yes' === $comments_meta ) : ?>
				<span class="dethemekit-blog-meta-separator">•</span>
				<div class="dethemekit-blog-post-comments dethemekit-blog-meta-data">
	...
	protected function get_post_content() {
		...
		if ( 'yes' !== $settings['dethemekit_blog_excerpt'] || empty( $settings['dethemekit_blog_excerpt_length'] ) ) {
			return;
		}

		$src = $settings['content_source'];

		$excerpt_type = $settings['dethemekit_blog_excerpt_type'];
		$excerpt_text = $settings['dethemekit_blog_excerpt_text'];

		$length = $settings['dethemekit_blog_excerpt_length'];

		// Get post content.
		if ( 'excerpt' === $src ) :
			echo '<p class="dethemekit-blog-post-content">';
		endif;
	...
	public function get_post_layout() {
		...
		$post_tag = 'yes' === $settings['dethemekit_blog_article_tag_switcher'] ? 'article' : 'div';

		$image_effect = $settings['dethemekit_blog_hover_image_effect'];

		$post_effect = $settings['dethemekit_blog_hover_color_effect'];

		if ( 'yes' === $settings['dethemekit_blog_new_tab'] ) {
			$target = '_blank';
		} else {
			$target = '_self';
		}
		...
		$this->add_render_attribute( $tax_key, 'class', 'dethemekit-blog-post-outer-container' );
		...
		$this->add_render_attribute(
			...
			array(
				...
				'dethemekit-blog-skin-' . $skin,
			)
		);

		$thumb = ( ! has_post_thumbnail() || 'yes' !== $settings['show_featured_image'] ) ? 'empty-thumb' : '';

		if ( 'yes' === $settings['dethemekit_blog_cat_tabs'] && 'yes' !== $settings['dethemekit_blog_carousel'] ) {
		...
		<<?php echo wp_kses_post( $post_tag . ' ' . $this->get_render_attribute_string( $tax_key ) ); ?>>
			<div <?php echo wp_kses_post( $this->get_render_attribute_string( $wrap_key ) ); ?>>
				<?php if ( empty( $thumb ) ) : ?>
					<div class="dethemekit-blog-thumb-effect-wrapper">
						<div class="dethemekit-blog-thumbnail-container <?php echo esc_attr( 'dethemekit-blog-' . $image_effect . '-effect' ); ?>">
							<?php $this->get_post_thumbnail( $target ); ?>
						</div>
						<?php if ( in_array( $skin, array( 'modern', 'cards' ), true ) ) : ?>
							<div class="dethemekit-blog-effect-container <?php echo esc_attr( 'dethemekit-blog-' . $post_effect . '-effect' ); ?>">
								<a class="dethemekit-blog-post-link" href="<?php the_permalink(); ?>" target="<?php echo esc_attr( $target ); ?>"></a>
								<?php if ( 'squares' === $settings['dethemekit_blog_hover_color_effect'] ) { ?>
									<div class="dethemekit-blog-squares-square-container"></div>
								<?php } ?>
							</div>
						<?php else : ?>
							<div class="dethemekit-blog-thumbnail-overlay">
				...
				<?php endif; ?>
				<?php if ( 'cards' === $skin ) : ?>
					<?php if( 'yes' === $settings['dethemekit_blog_author_img_switcher'] ) : ?>
						<div class="dethemekit-blog-author-thumbnail"> 
				...
				<div <?php echo wp_kses_post( $this->get_render_attribute_string( $content_key ) ); ?>>
					<div class="dethemekit-blog-content-wrapper-inner">
						<div class="dethemekit-blog-inner-container">
							<div class="dethemekit-blog-entry-container">
								<?php if ( in_array( $skin, array( 'side', 'banner' ), true ) && 'yes' === $settings['dethemekit_blog_categories_meta'] ) { ?>
									<div class="dethemekit-blog-cats-container">
						...
						?>
						<?php if ( 'yes' === $settings['dethemekit_blog_tags_meta'] && has_tag() ) : ?>
							<div class="dethemekit-blog-post-tags-container">
	...
	public function render_pagination() {
		...
		$nav_links = paginate_links(
			array(
				...
				'prev_text' => sprintf( '« %s', $settings['dethemekit_blog_prev_text'] ),
				'next_text' => sprintf( '%s »', $settings['dethemekit_blog_next_text'] ),
				'type'      => 'array',
			)
		);
	...
	public static function get_current_product_swap_image() {
		...
		if ( $attachment_ids ) {
			...
			echo wp_kses_post( apply_filters( 'pa_woo_product_swap_image', wp_get_attachment_image( reset( $attachment_ids ), $image_size, false, array( 'class' => 'dethemekit-woo-product__on_hover' ) ) ) );
		}
	}
	...
	public static function get_current_product_gallery_images() {
		...
		if ( $attachment_ids ) {
			...
			foreach ( $attachment_ids as $index => $id ) {
				...
				echo wp_kses_post( apply_filters( 'pa_woo_product_gallery_image', wp_get_attachment_image( $id, $image_size, false, array( 'class' => 'dethemekit-woo-product__gallery_image' ) ) ) );
			}
		}
	}
	...
	public static function get_current_product_category() {
		if ( apply_filters( 'pa_woo_product_parent_category', true ) ) :
			?>
			<span class="dethemekit-woo-product-category">
				<?php
					global $product;
	...
	public static function get_product_excerpt() {
		...
		if ( has_excerpt() ) {
			echo '<div class="dethemekit-woo-product-desc">';
