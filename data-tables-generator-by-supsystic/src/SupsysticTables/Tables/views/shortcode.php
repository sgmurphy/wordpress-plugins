...
<div class="supsystic-tables-wrap">
	<table
		class="supsystic-table <?php print wp_kses_post(implode(' ', $table->settings['styling'])); ?>"
		id="supsystic-table-<?php print wp_kses_post($table->id); ?>"
		data-features="<?php print wp_kses_post($features); ?>"
		data-lang="<?php print wp_kses_post($table->settings['language']['file']); ?>"
		data-override="<?php print wp_kses_post(json_encode_escape($table->settings['language'])); ?>">

		<?php if (isset($table->settings['elements']['caption'])): ?>
			<caption><?php print wp_kses_post($table->title); ?></caption>
		<?php endif; ?>
			...
			<tr>
				<?php
					foreach ($table->rows[0]['cells'] as $cellIndex => $cell) {
						if ($header) {
							print wp_kses_post(makeCell($cell, 0, $cellIndex, 'th'));
						} else {
	...
	<div
		id="supsystic-table-<?php print wp_kses_post($table->id); ?>-css"
		style="display: none;">
			<?php print wp_kses_post($table->meta['css']); ?>
	</div>
</div>
<!-- /.supsystic-tables-wrap -->

<!-- Tables Generator by Supsystic -->
<!-- Version: <?php print wp_kses_post($environment->getConfig()->get('plugin_version')) ?> -->
<!-- Generation time: <?php print wp_kses_post((microtime(true) - $start_table)) ?> -->
<!-- http://supsystic.com/ -->
