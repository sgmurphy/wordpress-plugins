	...
	class YITH_WCBM_Admin {
		...
		public function admin_enqueue_scripts() {
			...
			if ( in_array( $screen_id, array( 'yith-wcbm-badge', 'edit-yith-wcbm-badge', 'product', 'yith-plugins_page_yith_wcbm_panel' ), true ) ) {
				...
				if ( 'product' === $screen_id ) {
					wp_enqueue_script( 'yith_wcbm_admin' );
				}
		...
		public function register_panel() {
			...
			$admin_tabs_free = array(
				...
				'settings' => array(
					'title'       => esc_html__( 'General Settings', 'yith-woocommerce-badges-management' ),
					'description' => _x( "Configure the plugin's general settings.", '[ADMIN] Tab description', 'yith-woocommerce-badges-management' ),
					'icon'        => 'settings',
				),
			);
