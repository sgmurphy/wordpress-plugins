...
class WPBC_AJX__Setup_Wizard__Ajax_Request {
	...
	// <editor-fold     defaultstate="collapsed"                        desc=" ///  R e q u e s t  /// "  >
		...
		static public function request_rules_structure(){
			...
			return  array(
				  'do_action'                           => array( 'validate' => array( 'none', 'save_calendar_skin', 'make_reset' ), 'default'  => 'none' )
				, 'setup_wizard__booking_skin'          => array( 'validate' => 's', 'default' => '/css/skins/green-01.css' )
			    , 'resource_id' 		                => array( 'validate' => 'd', 'default' => $default_resource_id )
	...
	// <editor-fold     defaultstate="collapsed"                        desc=" ///  A J A X  /// "  >
		...
		public function define_ajax_hook(){
			...
			add_action( 'wp_ajax_'		     . 'WPBC_AJX_SETUP', array( $this, 'ajax_' . 'WPBC_AJX_SETUP' ) );	    // Admin & Client (logged in usres)
		 ...
		 */
		public function ajax_WPBC_AJX_SETUP() {
			...
			$action_name    = 'wpbc_ajx_setup_wizard_ajx' . '_wpbcnonce';
			...
			if ( 'save_calendar_skin' == $request_params['do_action'] ) {
				...
				$is_updated = update_bk_option( 'booking_skin', $request_params['setup_wizard__booking_skin'] );
			...
			// <editor-fold     defaultstate="collapsed"                        desc="  Dates | Resources for calendar  "  >
				...
				$data_arr['season_setup'] = wpbc__sql__get_season_availability( array(
																				'resource_id' => $request_params['resource_id']
																			) );
				...
				$data_arr['popover_hints']['toolbar_text'] = '<span style="font-size: 1.05em;line-height: 1.8em;">'.
												   sprintf( __('%sSelect days%s in calendar then select %sAvailable%s / %sUnavailable%s status and click %sApply%s setup button.' ,'booking')
															, '<strong>', '&nbsp;</strong>'
															, '<strong>&nbsp;', '&nbsp;</strong>'
															, '<strong>&nbsp;', '&nbsp;</strong>'
															, '<strong>&nbsp;', '&nbsp;</strong>'
 ...
 */
if ( true ) {
	$ajx_setup_wizard_loading = new WPBC_AJX__Setup_Wizard__Ajax_Request;
	$ajx_setup_wizard_loading->define_ajax_hook();
}