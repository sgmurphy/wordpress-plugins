...
function wpbc_form_gen_free_fields_selection_rangetime( $params,  $visual_form_structure ){
	...
	foreach ( $visual_form_structure as $form_field_el ) {
		if (  ( isset( $form_field_el['name'] ) ) && ( 'rangetime' == $form_field_el['name'] )  ){
			...
			//$params['items'][0]['options']['rangetime']['disabled'] = true;
			//$params['items'][0]['options']['rangetime']['title'] =  __('Time Slots', 'booking') . ' (' . __('exist in form', 'booking')  . ')';
			if (! empty($params['items'][0]['options']['rangetime'])){

				$save = $params['items'][0]['options']['rangetime'];

				unset( $params['items'][0]['options']['rangetime'] );
				unset( $params['items'][0]['options']['optgroup_af_e'] );
				// 							 -> edit_rangetime
				$params['items'][0]['options']['edit_rangetime'] = $save;
				$params['items'][0]['options']['optgroup_af_e'] = array( 'optgroup' => true, 'close'  => true );
			}
		}
	}
    return $params;
