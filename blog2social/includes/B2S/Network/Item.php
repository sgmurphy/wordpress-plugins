...
class B2S_Network_Item {
    ...
    private function getPortaleHtml($networkId, $networkName, $mandantId, $mandantenData, $networkData, $maxNetworkAccount = false, $showAllAuths = false, $addonText = false) {
        ...
        if (in_array($networkId, $this->allowProfil)) {
            ...
            } else if (($networkId == 38 || $networkId == 42) && B2S_PLUGIN_USER_VERSION < 2) {
                $html .= '<button href="#" class="btn btn-' . esc_attr(str_replace(' ', '', strtolower($networkName))) . ' btn-sm b2s-network-auth-btn b2s-btn-disabled b2sProFeatureModalBtn" data-title="' . (($networkId == 42) ? esc_attr__('Expand Your Social Media Universe with HumHub!', 'blog2social') : esc_attr__('You want to connect a network profile?', 'blog2social')) . '" data-type="' . (($networkId == 42) ? 'network-humhub' : 'auth-network') . '">' . sprintf(esc_html__('Connect %s', 'blog2social'), esc_html($name)) . ' <span class="label label-success">' . esc_html__("PRO", "blog2social") . '</button>';
    ...
    public function getEditTemplateFormContent($networkId, $networkType, $schema) {
        ...
        if ($networkId == 2) {
            ...
            $content .= '<label for="b2s-twitter-thread[' . esc_attr($networkType) . ']"> ' . esc_html__('Use twitter threads for posts with more than 280 characters.', 'blog2social') . '</label>';
        }
        if ($networkId == 12) {
            $content .= '<div class="row">';
            $content .= '<div class="col-md-12">';
    ...
    private function networkPreview($networkId, $networkType, $schema) {
        ...
        switch ($networkId) {
            ...
            case '43':
                ...
                $preview .= '<div class="row">';
                $preview .= '<div class="col-sm-2">';
                $preview .= '<img class="b2s-edit-template-preview-profile-img-43" src="' . esc_url(plugins_url('/assets/images/b2s@64.png', B2S_PLUGIN_FILE)) . '">';
                $preview .= '</div>';
