...
if ( ! class_exists( 'CWG_Instock_Bot_Protection' ) ) {
	...
	class CWG_Instock_Bot_Protection {
		...
		public function __construct() {
			...
			add_action( 'cwginstock_register_settings', array( $this, 'add_settings_field' ), 990 );
		...
		public function enqueue_script() {
			if ( self::get_bot_protection_type() == 'recaptcha' ) {
				...
				if ( '1' == self::is_turnstile_enabled() ) {
					wp_enqueue_script( 'turnstile', 'https://challenges.cloudflare.com/turnstile/v0/api.js', array(), CWGINSTOCK_VERSION );
				}
			}
		}
		...
		public function bot_protection_through() {
			...
			<select name="cwginstocksettings[bot_protection_via]" class="cwg_bot_protection_via">
				<option value="google_recaptcha" <?php selected( isset( $options['bot_protection_via'] ) && $options['bot_protection_via'] === 'google_recaptcha' ); ?>>
					<?php esc_html_e( 'Google reCAPTCHA', 'back-in-stock-notifier-for-woocommerce' ); ?>
				</option>
				<option value="turnstile" <?php selected( isset( $options['bot_protection_via'] ) && $options['bot_protection_via'] === 'turnstile' ); ?>>
		...
		public function enable_turnstile() {
			$options = get_option( 'cwginstocksettings' );
			?>
			<input type='checkbox' class="cwg_instock_turnstile" name='cwginstocksettings[enable_turnstile]' <?php checked( isset( $options['enable_turnstile'] ) && $options['enable_turnstile'] == 1 ); ?> value="1" />
			<p><i>
