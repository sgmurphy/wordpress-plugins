...
if (!class_exists('CWG_Instock_Settings')) {
	...
	class CWG_Instock_Settings {
		...
		public function disabled_variation_settings_option() {
			...
			<p>
				<?php
				esc_html_e('Some themes disable variation out of stock by default and some by an option, when activate our plugin it overwrite
                theme configuration(disabled variation become selectable), so by enable this option our plugin settings will not
                overwrite theme configuration', 'back-in-stock-notifier-for-woocommerce');
				?>
			</p>
		...
		public function ignore_settings_for_wc_out_of_stock_visibility() {
			...
			<p>
				<?php
				esc_html_e('WooCommerce has an option to hide out of stock products from catalog(WooCommerce->Products->Inventory->Out of stock
                ...
                woocommerce out of stock visibility settings only for variable products', 'back-in-stock-notifier-for-woocommerce');
				?>
		...
		public function instock_mail_message_set_stock_quantity() {
			...
			<i>
				<p> 
					<?php
					esc_html_e('Using this option Instock Email trigger can be controllable, when you manage product stock by quantity. For Ex:
                    If you set 5 in this option, you have to update product stock more than or equal to 5 in product stock quantity
                    in order to trigger instock email', 'back-in-stock-notifier-for-woocommerce')
					?>
				</p>
			</i>
			<?php
		...
		public function enable_post_status_subscribed() {
			...
			<i><p><?php esc_html_e('Using this option prevents subscribers from having to repeatedly sign up for notifications when a product becomes available in limited quantities after going out of stock. For instance, if there are 100 subscribers signed up for notifications and only 10 units are expected to be restocked at a time, enabling this option ensures that those subscribers won\'t need to re-register for notifications each time the product is restocked', 'back-in-stock-notifier-for-woocommerce'); ?></p></i>
			<?php
		}
		...
		public function email_already_subscribed() {
			...
			<i>
				<p> <?php esc_html_e('Supported Shortcodes {product_name}, {only_product_name}', 'back-in-stock-notifier-for-woocommerce'); ?></p>
			</i>
			<?php
		}
