...
if (!class_exists('CWG_Copy_Mailer')) {
	...
	class CWG_Copy_Mailer {
		...
		public function copy_subscription_mail_subject() {
			...
			?>
			<input type='text' style='width: 400px;' name='cwginstock_imail_settings[copy_sub_subject]' value="<?php echo esc_attr($this->api->sanitize_text_field($copy_sub_subject)); ?>" />
			<?php
		}
		...
		public function copy_subscription_mail_message() {
			...
			?>
			<textarea rows="5" cols="50" name="cwginstock_imail_settings[copy_sub_message]"><?php echo esc_textarea($this->api->sanitize_textarea_field($copy_sub_message)); ?></textarea>
			<?php
		}
