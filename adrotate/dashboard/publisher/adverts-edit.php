...
if(!$ad_edit_id) {
	...
	if($edit_id == 0) {
	    $wpdb->insert($wpdb->prefix."adrotate", array('title' => '', 'bannercode' => '', 'thetime' => $now, 'updated' => $now, 'author' => $userdata->user_login, 'imagetype' => 'dropdown', 'image' => '', 'tracker' => 'N', 'show_everyone' => 'Y', 'desktop' => 'Y', 'mobile' => 'Y', 'tablet' => 'Y', 'os_ios' => 'Y', 'os_android' => 'Y', 'type' => 'empty', 'weight' => 6, 'autodelete' => 'N', 'budget' => 0, 'crate' => 0, 'irate' => 0, 'state_req' => 'N', 'cities' => serialize(array()), 'states' => serialize(array()), 'countries' => serialize(array())));
...
if($edit_banner) {
	...
	<form method="post" action="admin.php?page=adrotate">
		...
		<p class="submit">
			<input tabindex="16" type="submit" name="adrotate_advert_submit" class="button-primary" value="<?php _e("Save Advert", 'adrotate'); ?>" />
		...
		<p class="submit">
			<input tabindex="17" type="submit" name="adrotate_advert_submit" class="button-primary" value="<?php _e("Save Advert", 'adrotate'); ?>" />
			<a href="admin.php?page=adrotate&view=manage" class="button"><?php _e("Cancel", 'adrotate'); ?></a>
		</p>
		<?php } ?>
