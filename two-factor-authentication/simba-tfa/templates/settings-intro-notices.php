...
if (is_multisite()) {
	if (is_super_admin()) {
		...
		echo ' <a href="'.esc_url($simba_tfa->get_site_wide_administration_url()).'">'.__('The site-wide administration options are here.', 'two-factor-authentication').'</a>';
...
<p class="simba_tfa_verify_tfa_notice simba_tfa_intro_notice"><strong>
	...
	<?php if (current_user_can($simba_tfa->get_management_capability())) { ?>
		<a href="<?php echo esc_url($simba_tfa->get_faq_url()); ?>"><?php _e('You should also bookmark the FAQs, which explain how to de-activate the plugin even if you cannot log in.', 'two-factor-authentication');?></a>
	<?php } ?>
</p>
