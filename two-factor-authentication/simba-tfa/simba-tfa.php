...
class Simba_Two_Factor_Authentication_1 {
	...
	public function show_user_profile($user) {
		...
		printf('<a target="_blank" href="%s">%s</a>', $settings_url, __('Go here for your two factor authentication settings...', 'two-factor-authentication'));
	}
	...
	public function tfaVerifyCodeAndUser($user, $username, $password) {
		...
		if (is_wp_error($user)) {
			...
			if (is_wp_error($code_ok)) {
				...
				$ret =  new WP_Error('authentication_failed', '<strong>'.__('Error:', 'two-factor-authentication').'</strong> '.apply_filters('simba_tfa_message_code_incorrect', __('The one-time password (TFA code) you entered was incorrect.', 'two-factor-authentication') . $additional));
	...
	private function notify_incorrect_tfa_code_attempts($user_info, $user_email) {
		$subject = __('Incorrect TFA code attempts', 'two-factor-authentication');
		$email_msg = sprintf(__('There has been an incorrect TFA code entered for logging in to your account %s.', 'two-factor-authentication'), $user_info['username']) . "\n\n" .
			__('Attempts', 'two-factor-authentication') . "\n\n";
		foreach ($user_info['attempts'] as $index => $attempt) {
			$email_msg.= ($index+1) . '. ' . wp_date('F j, Y g:i a', $attempt['activity_time'], wp_timezone()) . ' ' . __('from', 'two-factor-authentication') . ' ' . $attempt['ip_address']. "\n";
		}
		$email_msg.= "\n" . __('If the above attempts were not by you then someone else has your password.', 'two-factor-authentication') . "\n" .
			__('TFA codes are checked only after the password has been successfully checked.', 'two-factor-authentication') . "\n\n" .
			__('Please change your password urgently.', 'two-factor-authentication') . "\n";
	...
	public function paint_enable_tfa_radios($user_id, $long_label = false, $style = 'show_current') {
		...
		if ('show_current' == $style) {
			$tfa_enabled_label .= ' '.sprintf(__('(Current code: %s)', 'two-factor-authentication'), $this->get_controller('totp')->current_otp_code($user_id));
	...
	public function include_template($path, $extract_these = array(), $return_instead_of_echo = false) {
		...
		if (!file_exists($template_file)) {
			...
			echo __('Error:', 'two-factor-authentication').' '.__('Template path not found:', 'two-factor-authentication')." (".htmlspecialchars($path).")";
		} else {
