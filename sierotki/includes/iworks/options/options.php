...
class iworks_options {
	...
	public function build_options( $option_group = 'index', $echo = true, $term_id = false ) {
		...
		if ( ! is_array( $options['options'] ) ) {
			...
			esc_html_e( 'An error occurred while getting the configuration.', 'sierotki' );
			echo '</strong></p></div>';
			return;
		}
		/**
		...
		if ( $options['show_submit_button'] ) {
			$content .= get_submit_button(
				esc_html__( 'Save Changes', 'sierotki' ),
				'primary',
				'submit_button'
			);
		}
...
<?php } ?>
	...
	private function location( $name, $value = '', $args = array() ) {
		...
		$i18n     = array(
			'country' => esc_html__( 'Country', 'sierotki' ),
			'city'    => esc_html__( 'City', 'sierotki' ),
			'street'  => esc_html__( 'Street', 'sierotki' ),
			'zip'     => esc_html__( 'ZIP code', 'sierotki' ),
		);
	...
	public function get_switch_button_data() {
		$data = array(
			'labels' => array(
				'off_label' => esc_html__( 'OFF', 'sierotki' ),
				'on_label'  => esc_html__( 'ON', 'sierotki' ),
			),
		);
		return $data;
	}
	...
	private function get_nonce_value() {
		...
		return new WP_Error( 'security', esc_html__( 'Failed Security Check', 'sierotki' ) );
	}
