...
test( 'Purchase Gift card product', async ( { page } ) => {
	...
	await expect( await page.getByText( /John Doe/i ) ).toBeVisible();
	await expect( await page.getByText( /emily@example.com/i ) ).toBeVisible();
	await expect( await page.getByText( /Emily Doe/i ) ).toBeVisible();
	await expect( await page.getByText( /Happy Birthday!/i ) ).toBeVisible();
