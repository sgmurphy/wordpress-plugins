...
class Secure_Copy_Content_Protection {
	...
	private function load_dependencies() {
		 ...
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-secure-copy-content-protection-data.php';
		...
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/partials/class-secure-copy-content-protection-extra-shortcode.php';
	...
	private function define_admin_hooks() {
		...
		$data_admin   = new Ays_Sccp_Data( $this->get_plugin_name(), $this->get_version() );
		...
		//User search ajax
		$this->loader->add_action( 'wp_ajax_ays_sccp_reports_user_search', $plugin_admin, 'ays_sccp_reports_user_search' );
        $this->loader->add_action( 'wp_ajax_nopriv_ays_sccp_reports_user_search', $plugin_admin, 'ays_sccp_reports_user_search' );

		$this->loader->add_action( 'in_admin_footer', $plugin_admin, 'sccp_admin_footer', 1 );
		
		$this->loader->add_action( 'admin_notices', $data_admin, 'ays_sccp_sale_baner', 1 );
	...
	private function define_public_hooks() {
		...
		$plugin_public_extra_shortcodes = new Ays_Sccp_Extra_Shortcodes_Public( $this->get_plugin_name(), $this->get_version() );
