...
/**
 * Account Block view for user details and limits
 *
 * Called from Overview Controller
 */
?>
<?php
$whole = $decimal = 0;
    ...
    <div class="sq_account_info_content col-12 p-0 m-0 border-0 shadow text-left" style="display: none">
        <div class="position-relative">
                ...
                <?php if (isset($view->checkin->subscription_email)) { ?>
                    ...
                    <div class="col py-0 px-1 m-0">
                        ...
                        <?php if (isset($view->checkin->subscription_paid) && isset($view->checkin->subscription_expires) && $view->checkin->subscription_paid && $view->checkin->subscription_expires) { ?>
                            <div class="small <?php echo esc_attr(((time() - strtotime($view->checkin->subscription_expires) > 0) ? 'text-danger' : '')) ?>" style="font-size: 12px; line-height: 20px;" ><?php echo sprintf(esc_html__("Due Date: %s", "squirrly-seo"),  esc_attr(gmdate('d M Y', strtotime($view->checkin->subscription_expires))) ); ?> <a href="<?php echo esc_url(SQ_Classes_RemoteController::getMySquirrlyLink('account')) ?>" class="bg-primary text-white px-1" target="_blank"><?php echo esc_html($view->checkin->product_name) ?></a></div>
                        <?php }elseif (isset($view->checkin->subscription_expires) && $view->checkin->subscription_expires) { ?>
                            <div class="small" style="font-size: 12px; line-height: 20px;" ><a href="<?php echo esc_url(SQ_Classes_RemoteController::getMySquirrlyLink('account')) ?>" class="bg-primary text-white px-1" target="_blank"><?php echo esc_html($view->checkin->product_name) ?></a></div>
                        <?php }elseif (isset($view->checkin->subscription_expires) && !$view->checkin->subscription_expires) { ?>
                            <div class="small" style="font-size: 12px; line-height: 20px;" ><?php echo esc_html__("Expires: never", "squirrly-seo") ; ?> <a href="<?php echo esc_url(SQ_Classes_RemoteController::getMySquirrlyLink('account')) ?>" class="bg-primary text-white px-1" target="_blank"><?php echo esc_html($view->checkin->product_name) ?></a></div>
                        <?php } ?>
                    </div>
                <?php } ?>
            </div>
                ...
                <ul class="p-0 m-0">
                    ...
                    <?php  if (isset($view->checkin->subscription_limits_reset) && $view->checkin->subscription_limits_reset <> '' && strtotime($view->checkin->subscription_limits_reset) > time()) {  ?>
                        <li class="m-0 p-0 py-2 text-center">
                            <?php echo esc_html__("Reset day", "squirrly-seo") ?>: <span class="col text-left p-0 font-weight-bold"><?php echo esc_html(gmdate(get_option('date_format'), strtotime($view->checkin->subscription_limits_reset))) ?></span>
                        </li>
                    <?php }?>
                </ul>
            </div>
            ...
            <div class="border-top py-2 px-3 mt-2 text-center">
                <div class="small"><a href="<?php echo esc_url(SQ_Classes_RemoteController::getMySquirrlyLink('dashboard')) ?>" target="_blank"><?php echo esc_html__("Want to hide this section from your customers?", "squirrly-seo") ?></a></div>
            </div>

            <div class="my-3 mx-4">
                <div class="row p-0 m-0 my-3">
                    <div class="col-1 m-0 p-0 mr-2"><i class="fa-solid fa-cloud-arrow-up"></i> </div>
                    <div class="col m-0 p-0"><a href="<?php echo esc_url(SQ_Classes_RemoteController::getMySquirrlyLink('plans')) ?>" target="_blank"><?php echo esc_html__("Upgrade your account", "squirrly-seo") ?></a></div>
                </div>
                <div class="row p-0 m-0 my-3">
                    <div class="col-1 m-0 p-0 mr-2"><i class="fa-solid fa-wallet"></i> </div>
                    <div class="col m-0 p-0"><a href="<?php echo esc_url(SQ_Classes_RemoteController::getMySquirrlyLink('account')) ?>" target="_blank"><?php echo esc_html__("Billing info", "squirrly-seo") ?></a></div>
                </div>
                <div class="row p-0 m-0 my-3">
                    <div class="col-1 m-0 p-0 mr-2"><i class="fa-solid fa-gear"></i> </div>
                    <div class="col m-0 p-0"><a href="<?php echo esc_url(SQ_Classes_RemoteController::getMySquirrlyLink('profile')) ?>" target="_blank"><?php echo esc_html__("Account settings", "squirrly-seo") ?></a></div>
                </div>
                <div class="row p-0 m-0 my-3">
                    <div class="col-1 m-0 p-0 mr-2"><i class="fa-solid fa-cloud"></i> </div>
                    <div class="col m-0 p-0"><a href="<?php echo esc_url(SQ_Classes_RemoteController::getMySquirrlyLink('dashboard')) ?>" target="_blank"><?php echo esc_html__("Squirrly Cloud", "squirrly-seo") ?></a></div>
                </div>
                <div class="row p-0 m-0 my-3">
                    <div class="col-1 m-0 p-0 mr-2"><i class="fa-solid fa-cloud"></i> </div>
