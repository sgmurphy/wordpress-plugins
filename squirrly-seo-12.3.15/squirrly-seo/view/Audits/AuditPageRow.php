...
<?php if ($view->auditpage->audit_error) { ?>
    <td>
        ...
        <div class="text-black-50">
            <em><?php echo sprintf(esc_html__("The way your WordPress site is currently hosted can affect the way Squirrly SEO operates in order to retrieve and process data about this page. %sIt’s important to do everything on your end to ensure that the audits can be generated by our system. %s Whitelist our crawler IP address (5.161.83.61) to allow our server to verify your page so that you’ll receive a full audit.", "squirrly-seo"), '<br /><br />', '<br /><br />') ?></em>
        </div>
        <button class="btn btn-sm btn-primary sq_previewurl font-weight-bold"  style="cursor: pointer" onclick="jQuery('#sq_previewurl_modal').attr('data-post_id', '<?php echo (int)$view->auditpage->user_post_id ?>').sq_inspectURL()" data-dismiss="modal"><?php echo esc_html__("Inspect URL", "squirrly-seo"); ?></button>
