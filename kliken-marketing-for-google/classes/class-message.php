...
class Message {
	...
	private static function show_message( $type = 'info', $message = '', $message_name = '' ) {
		...
		printf(
			...
			esc_attr( $type ),
			$data_attr, // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- WPCS: XSS ok.
			$message // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- WPCS: XSS ok.
		);
	}
