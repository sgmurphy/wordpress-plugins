			...
			if (utils.isArray(plugins)) {
				for (i = 0, n = plugins.length; i < n; i++) {
					if (typeof plugins[i] === 'string') {
						...
						self.plugins.settings[plugins[i].name] =
							plugins[i].options;
	...
	$.extend(Selectize.prototype, {
		...
		setup: function () {
			...
			$control_input = $('<input type="text" autocomplete="off" />')
				...
				.attr(
					'tabindex',
					$input.is(':disabled') ? '-1' : self.tabIndex
				);
			...
			switch (e.keyCode) {
				...
				case KEY_DOWN:
					if (!self.isOpen && self.hasOptions) {
						...
						var $next = self.getAdjacentOption(
							self.$activeOption,
							1
						);
						if ($next.length)
				...
				case KEY_UP:
					if (self.$activeOption) {
						self.ignoreHover = true;
						var $prev = self.getAdjacentOption(
							self.$activeOption,
							-1
						);
						if ($prev.length)
		...
		search: function (query) {
			...
			if (settings.hideSelected) {
				for (i = result.items.length - 1; i >= 0; i--) {
					if (
						self.items.indexOf(hash_key(result.items[i].id)) !== -1
					) {
		...
		updateOriginalInput: function (opts) {
			...
			if (self.tagType === TAG_SELECT) {
				...
				for (i = 0, n = self.items.length; i < n; i++) {
					label =
						self.options[self.items[i]][self.settings.labelField] ||
						'';
					options.push(
						'<option value="' +
		...
		render: function (templateName, data) {
			...
			// render markup
			html = $(
				self.settings.render[templateName].apply(this, [
					data,
					escape_html,
				])
			);
					...
					if (index >= 0 && index < this.items.length) {
						...
						if (this.deleteSelection(e)) {
							this.setTextboxValue(
								options.text.apply(this, [option])
							);
