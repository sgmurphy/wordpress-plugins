	...
	var modulaSettingsView = Backbone.View.extend({
		...
		chechParents: function (parent, parentval) {
			if (jQuery(parent).data('parent') && 1 == parentval) {
				if (
					1 == wp.Modula.Settings.get(jQuery(parent).data('parent'))
				) {
					parentval = this.chechParents(
						jQuery(
							'[data-container="' +
								jQuery(parent).data('parent') +
								'"]'
						),
						parentval
					);
		...
		initSliders: function () {
			if (this.sliders.length > 0) {
				this.sliders.each(function ($index, $slider) {
					var input = jQuery($slider)
							.parent()
							.find('.modula-ui-slider-input'),
						max = input.data('max'),
						min = input.data('min'),
		...
		initCustomCSS: function () {
			...
			if (undefined !== wp.codeEditor) {
				...
				if (this.customEditors.length > 0) {
					this.customEditors.each(
						function ($index, customEditorContainer) {
							var syntax = $(customEditorContainer).data(
									'syntax'
								),
								id =
									'#' +
									$(customEditorContainer)
										.find('.modula-custom-editor-field')
										.attr('id'),
							...
							if (undefined !== wp.codeEditor) {
								...
								$(customEditorContainer)
									.parents('.modula-tab-content')
									.on('modula-current-tab', function () {
