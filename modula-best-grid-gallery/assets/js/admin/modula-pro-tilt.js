...
(function (u, r) {
	...
	var u = {
			...
			a.forEach(function (e, d) {
				...
				return b.forEach(function (b) {
					...
					if (l) {
						...
						h.delay =
							(f.fnc(h.delay)
								? h.delay(g, d, a.length)
								: h.delay) / k.speed;
						h.duration =
							(f.fnc(h.duration)
								? h.duration(g, d, a.length)
								: h.duration) / k.speed;
						c.push(h);
					}
				});
		...
		Y = function (a, b) {
			var c = X(a, b);
			return R(c, ['name', 'from', 'to', 'delay', 'duration']).map(
				function (a) {
					var b = B(a[0]);
		...
		P = function (a, b) {
			...
			for (var e = 0; e < a.tweens.length; e++) {
				...
				for (
					var f = d.currentValue, m = 0;
					m < d.animatables.length;
					m++
				) {
			...
			if (c)
				for (e in (y ||
					(y =
						(w(document.body, 'transform') ? '' : '-webkit-') +
						'transform'),
				c))
