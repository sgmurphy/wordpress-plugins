	...
	var modulaUpgrader = {
		...
		init: function () {
			...
			$('#modula-custom-upgrade-v2').click(function (e) {
				...
				var galleriesIDS = jQuery(
					'.modula-gallery-to-upgrade:checkbox:checked'
				)
					.map(function () {
						return this.value;
					})
			...
			galleries_ids.forEach(function (gallery_id) {
				var opts = {
					...
					success: function (response) {
						...
						$('.modula-ajax-output').append(
							'<p>' + response.message + '</p>'
						);
						modulaUpgrader.updateProgress();

						// Remove one ajax from queue
						modulaUpgrader.ajaxStarted =
							modulaUpgrader.ajaxStarted - 1;
					},
				};
		...
		runAjaxs: function () {
			...
			while (
				modulaUpgrader.ajaxStarted < 5 &&
				modulaUpgrader.ajaxRequests.length > 0
			) {
		...
		updateProgress: function () {
			var width =
				modulaUpgrader.completed * (100 / modulaUpgrader.counts) + '%';
			$(
				'.modula-upgrader-progress-bar-container .modula-upgrader-progress-bar'
			).css('width', width);
