(function ($) {
	...
	var modulaImporter = {
		...
		init: function () {
			$('.modula-importer-wrapper input[type="submit"]').click(
				function (e) {
					e.preventDefault();
					modulaImporter.source = $(this)
						.parents('.modula-importer-wrapper')
						.attr('source');
					...
					// Disable input
					$(
						'#modula_importer_' + modulaImporter.source + ' :input'
					).prop('disabled', true);
		...
		processAjax: function (galleries_ids) {
			...
			galleries_ids.forEach(function (gallery_id) {
				...
				var id = gallery_id;
				var image_count = $('input[data-id="' + gallery_id + '"]').data(
					'image-count'
				);
				var opts_chunks = {};
				...
				$('span', $(status))
					...
					.html(modula_importer.importing);
				// For WP core galleries in case we have multiple galleries in same page
				if ('wp_core' == modulaImporter.source) {
					$gallery_title = $('input#wp_core-galleries-' + id)
						.next('a')
						.text();
				}
				...
				if ('wp_core' == modulaImporter.source) {
					id = JSON.parse(
						$(
							'#modula_importer_wp_core input[data-id=' +
								gallery_id +
								']'
						).val()
					);
				...
				if (!status.data('imported')) {
					modulaImporter.counts +=
						Math.floor(image_count / modulaImporter.importChunk) +
						1;
					...
					for (
						...
						opts_chunks = {
							...
							success: function (response) {
								modulaImporter.ajaxStarted =
									modulaImporter.ajaxStarted - 1;

								if (
									response &&
									'undefined' != typeof response.attachments
								) {
									$.merge(
										modulaImporter.attachments,
								...
								if (
									$('span.importing-status', $(status)).length
								) {
									$('span.importing-status', $(status)).html(
										'Imported:' +
											modulaImporter.attachments.length +
											' / ' +
											image_count
									);
								}

								modulaImporter.completed =
									modulaImporter.completed + 1;
								...
								if ('end_of_array' == response.end_of_array) {
									...
									var opts = {
										...
										success: function (response) {
											...
											modulaImporter.modulaGalleryIds[
												id
											] = response.modula_gallery_id;
					...
					var opts = {
						...
						success: function (response) {
							...
							modulaImporter.modulaGalleryIds[id] =
								response.modula_gallery_id;
		...
		runAjaxs: function () {
			var currentAjax;
			while (
				modulaImporter.ajaxStarted < 2 &&
				modulaImporter.ajaxRequests.length > 0
			) {
			...
			if (modulaImporter.ajaxRequests.length > 0) {
				...
				$('#modula_importer_' + modulaImporter.source + ' :input').prop(
					'disabled',
					false
				);
			}
	...
	$(document).ready(function () {
		...
		$('#modula_select_gallery_source').on('change', function () {
			...
			$.post(ajaxurl, data, function (response) {
				...
				$('#modula-' + targetID + '-importer')
					.find('.modula-found-galleries')
					.html(response);
				$('.modula-importer-row')
					.not($('#modula-' + targetID + '-importer'))
					.addClass('hide');
				if (
					'none' != targetID &&
					$('#modula-' + targetID + '-importer')
						.find('input[type="checkbox"]')
						.not('#select-all-' + targetID).length > 0
				) {
					...
					$(
						'#modula-' +
							targetID +
							'-importer .modula-importer-upsell-wrapper'
					).addClass('hide');
					$(
						'#modula-' +
							targetID +
							'-importer .select-all-checkbox,#modula-' +
							targetID +
							'-importer .select-all-checkbox-wrapper'
					).addClass('hide');
					$('#modula-' + targetID + '-importer')
