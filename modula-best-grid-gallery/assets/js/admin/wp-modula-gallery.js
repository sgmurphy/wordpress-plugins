...
(function ($, modula) {
	var modulaGalleryResizer = Backbone.Model.extend({
		...
		resizeItems: function () {
			...
			if (
				'undefined' != typeof wp.Modula.Items &&
				wp.Modula.Items.length > 0
			) {
			...
			if ('custom-grid' == modula.Settings.get('type')) {
				...
				wp.Modula.GalleryView.setPackaryOption(
					'columnWidth',
					this.get('size')
				);
				wp.Modula.GalleryView.setPackaryOption(
	...
	var modulaGalleryView = Backbone.View.extend({
		...
		initialize: function (args) {
			...
			// Grid
			this.gridView = new wp.Modula.previewer['helpergrid']({
				el: this.$el.find('#modula-grid'),
				galleryView: this,
			});
		},
		...
		checkGalleryType: function (type) {
			if ('creative-gallery' == type || 'grid' == type) {
				...
				this.container
					...
					.addClass('modula-custom-grid');
			} else {
				jQuery(document).trigger('modula_gallery_type_check', [
					this,
					this.backboneInstance,
					type,
				]);
			}
		},
	...
	var modulaGalleryGrid = Backbone.View.extend({
		...
		generateGrid: function () {
			...
			neededRows =
				Math.round(
					(containerHeight + gutter) / (columnWidth + gutter)
				) + 1;
			neededContainerHeight =
				neededRows * (columnWidth + gutter) - gutter;

			while (containerHeight < neededContainerHeight) {
				neededContainerHeight =
					neededContainerHeight - (columnWidth + gutter);
			}

			this.$el.height(neededContainerHeight);
			$('#modula-uploader-container').css(
				'min-height',
				minContainerHeight + 'px'
			);
			...
			if (neededRows > this.currentRows) {
				...
				this.$el.find('.modula-grid-item').css({
					width: columnWidth,
					height: columnWidth,
					'margin-right': gutter,
