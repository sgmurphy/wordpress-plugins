jQuery(document).ready(function ($) {
	...
	$(uninstall).on('click', function () {
		...
		formContainer.on(
			'click',
			'#modula-uninstall-submit-form',
			function (e) {
				formContainer.addClass('toggle-spinner');
				var selectedOptions = {
					delete_options: formContainer
						.parent()
						.find('#delete_options')
						.is(':checked')
						? 1
						: 0,
					delete_transients: formContainer
						.parent()
						.find('#delete_transients')
						.is(':checked')
						? 1
						: 0,
					delete_cpt: formContainer
						.parent()
						.find('#delete_cpt')
						.is(':checked')
						? 1
						: 0,
					delete_old_tables: formContainer
						.parent()
						.find('#delete_old_tables')
						.is(':checked')
						? 1
		...
		$('.modula-uninstall-form-wrapper, .close-uninstall-form').on(
			'click',
			function (e) {
				e.stopPropagation();
