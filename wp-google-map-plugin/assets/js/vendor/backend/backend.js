jQuery(document).ready(function($) {
	...
	$(document).on('click', '#doaction', function( e ) {
        ...
        if($('#bulk-action-selector-top').val() == 'delete'){
			...
			if(checked_chkboxes.length == 0){
				alert(wpgmp_local.no_record_for_bulk_delete);
                return;
			}else{
				var result = confirm(wpgmp_local.confirm_bulk_delete);
                 if (result) {
    ...
    $('.delete_fc_record').click( function (event) {
        ...
        if(record_type == 'location_id'){
            var result = confirm(wpgmp_local.confirm_location_delete);
        }else if(record_type == 'map_id'){
            var result = confirm(wpgmp_local.confirm_map_delete);
        }else if(record_type == 'group_map_id'){
            var result = confirm(wpgmp_local.confirm_category_delete);
        }else{
            var result = confirm(wpgmp_local.confirm_record_delete);
        }
        if (result) {
    ...
    $(".wpgmp_check_key").click(function() {
            ...
            if (data.status == 'OK') { 
                     $('.wpgmp_apitest').html("<div class='fc-msg fc-success'>Congratulation, the API key you created is perfect. You can now <a class='create_location_link' target='_blank' href='https://www.wpmapspro.com/docs/create-a-location-using-simple-method/'>start</a> creating maps!</div>");
                } else {
                     $('.wpgmp_apitest').html("<div class='fc-msg fc-danger'>"+data.error_message+"</div>");
                }
    ...
    $(".buy_premium .hide_buy_notice").click(function() {
		...
		var ajaxdata = { action: 'wpgmp_hide_buy_notice', ajaxnonce : wpgmp_local.ajaxnonce  }
    ...
    $(".sample_notice .hide_sample_notice").click(function () {
        ...
        var ajaxdata = { action: 'wpgmp_hide_sample_notice', ajaxnonce: wpgmp_local.ajaxnonce }
        jQuery.ajax({
            url: wpgmp_local.urlforajax,
            type: 'post',
            dataType: 'json',
