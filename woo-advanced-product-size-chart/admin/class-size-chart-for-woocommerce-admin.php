...
class SCFW_Size_Chart_For_Woocommerce_Admin {
    ...
    /**
     * Upgrade to pro limit
     * 
     * @since 1.0.0
     */
    function scfw_upgrade_to_pro_limit() {
        global $pagenow;
        $scfw_post_type = filter_input( INPUT_GET, 'post_type', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
        $scfw_post = filter_input( INPUT_GET, 'post', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
        if ( empty( $scfw_post_type ) ) {
            if ( 'post.php' === $pagenow && isset( $scfw_post ) ) {
                $post_id = intval( $scfw_post );
                $scfw_post_type = get_post_type( $post_id );
            }
        }
        if ( isset( $scfw_post_type ) && 'size-chart' === $scfw_post_type ) {
            $allSizeCharts = get_posts( array(
            ...
            $prod_chart_values = array();
            foreach ( $products_with_meta as $product_id ) {
                $prod_chart_value = get_post_meta( $product_id, 'prod-chart', true );
                // Remove square brackets and explode the string to get an array of values
                $prod_chart_values[] = explode( ',', trim( $prod_chart_value, '[]' ) );
            }
            // Flatten the $prod_chart_values array
            $flattened_prod_chart_values = array();
            foreach ( $prod_chart_values as $item ) {
                $flattened_prod_chart_values = array_merge( $flattened_prod_chart_values, $item );
            }
            // Merge $posts and $flattened_prod_chart_values and remove duplicates
            $farray = array_merge( $allSizeCharts, $flattened_prod_chart_values );
            $farray = array_unique( $farray );
            $farray = array_filter( $farray );
            sort( $farray );
            $posts_count = count( $farray );
            if ( $posts_count >= 10 ) {
                ?>
				<script type="text/javascript">
					jQuery(document).ready(function($) {
						var upgradeText = sizeChartScriptObject.upgradeText;
						$('.page-title-action').text(upgradeText);
						$('.page-title-action').attr('href', 'javascript:void(0)');
						$('.page-title-action').attr('id', 'scfw-upgrade-to-pro-limit');
					});
				</script>
				<div class="notice notice-warning is-dismissible">
					<p><?php 
                echo esc_html_e( 'Usage limit reached: 10 size charts utilized. Unlock unlimited access by upgrading.', 'size-chart-for-woocommerce' );
                ?></p>
				</div>
				<?php 
            }
            $mc = 1;
            $chart_arr = array();
            foreach ( $farray as $post ) {
                if ( $mc <= 10 ) {
                    $chart_arr[] = $post;
                }
                $mc++;
            }
            update_option( 'scfw_allowed_post', $chart_arr );
            ?>
			<div id="scfw-pro-popup" class="scfw-pro-popup" style="display:none;">
				<div class="popup-content">
					<img src="<?php 
            echo esc_url( plugin_dir_url( __FILE__ ) ) . '/images/upgrade-to-pro/rocket.png';
            ?>" class="rocket-img">
					<span class="close-button" id="closePopupButton">⛌</span>
					<div class="popup-inner-content">
						<div class="content-wrapper model_pop_up_pro-content pro-feature-content">
							<div class="text-section">
								<h2><?php 
            echo esc_html_e( 'Unlock Premium Features with a 50% Discount!', 'size-chart-for-woocommerce' );
            ?></h2>
								<p><?php 
            echo esc_html_e( 'Upgrade now to enhance your site\'s usability and user experience with our Size Chart feature!', 'size-chart-for-woocommerce' );
            ?></p>
								<ul>
									<li><?php 
            echo esc_html_e( 'Reduce returns by eliminating size-related issues.', 'size-chart-for-woocommerce' );
            ?></li>
									<li><?php 
            echo esc_html_e( 'Size Chart minimizes customer confusion about sizes and fittings', 'size-chart-for-woocommerce' );
            ?></li>
									<li><?php 
            echo esc_html_e( 'Customers can conveniently view size charts before making purchases.', 'size-chart-for-woocommerce' );
            ?></li>
								</ul>
								<a href="javascript:void(0);" id="scfw_premium_purchase" class="button button-primary upgrade-button"><?php 
            esc_html_e( 'Save Now & Upgrade', 'size-chart-for-woocommerce' );
            ?></a>
							</div>
						</div>
					</div>
    ...
    /**
     * Add new size chart post restriction
     * 
     * @since 1.0.0
     */
    function restrict_add_new_size_chart_post() {
        $scfw_post_type = filter_input( INPUT_GET, 'post_type', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
        if ( isset( $scfw_post_type ) && 'size-chart' === $scfw_post_type ) {
            $valid_size_charts = get_option( 'scfw_allowed_post', array() );
            if ( count( $valid_size_charts ) >= 10 ) {
                wp_die( esc_html__( 'Usage limit reached: 10 size charts utilized. Unlock unlimited access by upgrading.', 'size-chart-for-woocommerce' ), '', array(
                    'response' => 403,
                ) );
            }
        }
