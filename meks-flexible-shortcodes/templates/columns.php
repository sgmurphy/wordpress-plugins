<form id="mks_shortcode_columns">
	<table class="form-table">
	   ...
	   <tr>
		...
		<tr>
		 	<th><?php _e('Enable Bootstrap columns','meks-flexible-shortcodes'); ?></th>
			<td><input type="checkbox"  class="bootstrap-columns" name="bootstrap_columns" /></td> 
		</tr>
		<tr>
		 	<th></th>
			<td><small style="width: 200px; position:relative; top:-30px;" class="howto"><?php _e('Check this option if your theme uses Bootstrap CSS framework to apply its styling on columns','meks-flexible-shortcodes'); ?> </small></td>
		</tr>
...
<script type="text/javascript">
  ...
  (function($) {
    	$('#mks_shortcode_columns').submit(function(e) {
			...
			var is_bootstrap_columns = $(this).closest('#mks_shortcode_columns').find('input[name="bootstrap_columns"]').prop("checked");
			var bootstrap_columns_enabled = is_bootstrap_columns ? ' bootstrap="true"' : '';    			
			var content = '[mks_col'+ bootstrap_columns_enabled +']'+$(this).find('input[name="columns"]:checked').val()+'[/mks_col]';
