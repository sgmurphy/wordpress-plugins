...
class WCFMvm_Pay_For_Product {
	...
	function wcfmvm_pay_per_product_settings( $wcfm_options ) {
		...
		<div class="page_collapsible" id="membership_settings_form_pay_per_product_head">
			...
			<?php esc_html_e('Pay for Product', 'wc-multivendor-membership'); ?><span></span>
		</div>
		<div class="wcfm-container">
			<div id="membership_settings_form_pay_per_product_expander" class="wcfm-content">
			  <h2><?php esc_html_e('Pay for Product', 'wc-multivendor-membership'); ?></h2>
			...
			if( $pay_for_product_id ) {
				...
				<div class="wcfm_pay_for_product_container">
				  <div class="wcfm_pay_for_product_message">
				    <?php esc_html_e( 'Increase your product limit ..', 'wc-multivendor-membership' ); ?>
				  </div>
				  <div class="wcfm_pay_for_product_content">
				    ...
				    <div class="wcfm_pay_for_product_description">
							...
							<div class="wcfm_pay_for_product_button_container">
								<input class="wcfm_pay_for_product_button wcfm_submit_button button" type="button" value="<?php esc_html_e( "Buy Now", 'wc-multivendor-membership' ); ?>">
							</div>
  	...
  	if ( !current_user_can( 'manage_woocommerce' ) && !current_user_can( 'wcfm_vendor' ) && !current_user_can( 'seller' ) && !current_user_can( 'vendor' ) && !current_user_can( 'shop_staff' ) ) {
		...
		if( !empty( $wcfm_pay_for_product_settings ) ) {
			...
			if( $pay_for_product_id ) {
				...
				echo '{"status": true, "message": "' . esc_html__( 'Pay for Product succes!', 'wc-multivendor-membership' ) . '", "redirect": "' . esc_url(wc_get_checkout_url()) . '"}';
				die;
			}
		}
		echo '{"status": false, "message": "' . esc_html__( 'Pay for Product failed!', 'wc-multivendor-membership' ) . '"}';
		die;
	}
