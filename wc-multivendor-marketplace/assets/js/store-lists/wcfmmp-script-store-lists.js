    ...
    if ($('.wcfmmp-store-search-form').length > 0) {
        ...
        if ($('#wcfmmp_radius_addr').length > 0) {
            ...
            if (navigator.geolocation) {
                ...
                if (wcfmmp_store_list_options.is_geolocate) {
                    lat = $( '#wcfmmp_radius_lat' ).val();
                    lng = $( '#wcfmmp_radius_lng' ).val();
                    ...
                    navigator.geolocation.getCurrentPosition(function(position) {
                        ...
                        if (wcfm_maps.lib == 'google') {
                            ...
                            $.get('https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=' + position.coords.latitude + '&lon=' + position.coords.longitude, function(data) {
                                ...
                                $('input.search-input.wcfmmp-radius-addr').val(data.display_name);
                                $('#wcfmmp_radius_lat').val(position.coords.latitude);
                                $('#wcfmmp_radius_lng').val(position.coords.longitude);
                                refreshStoreList();
                            });
    ...
    if ($('.wcfmmp-store-list-map').length > 0) {
        ...
        function setMarkers(locations) {
            ...
            if (wcfm_maps.lib == 'google') {
                ...
                if (wcfmmp_store_list_options.is_cluster) {
                    ...
                    markerClusterer = new MarkerClusterer(store_list_map, markers, {
                        imagePath: imagePath
                    });
                }
                ...
                $.each(locations, function(i, beach) {
                    ...
                    var marker = L.marker([beach.lat, beach.lang], {
                        icon: customIcon
                    }).bindPopup(beach.info_window_content);
        ...
        if (wcfm_maps.lib == 'google') {
            ...
            if (searchControl && $('#wcfmmp_radius_addr').length > 0) {
                ...
                $wcfmmp_radius_addr = $('#wcfm_radius_filter_container').find('.search-input').addClass('wcfmmp-radius-addr').attr('id', 'wcfmmp_radius_addr').css('float', 'none').attr('placeholder', wcfmmp_store_list_options.search_location);
				...
				if( $lat && $lng ) {
					$.get('https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=' + $lat + '&lon=' + $lng, function(data) {
						$wcfmmp_radius_addr.val(data.display_name);
					});
				}
