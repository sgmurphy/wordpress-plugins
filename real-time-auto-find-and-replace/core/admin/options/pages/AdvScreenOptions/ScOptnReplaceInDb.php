...
class ScOptnReplaceInDb {
	...
	public static function arr_available_features() {
		?>
			...
			<ul>
				<li>
					<?php printf( __( '%1$s Where to Replace %2$s allows to specify where you want to search and replace. ', 'real-time-auto-find-and-replace' ), '<strong>', '</strong>' ); ?>
				</li>
				<li>
					<?php printf( __( '%1$s Dry Run %2$s allows to create a visual report of the items you have searched for or if its unchecked, the searched item will be replaced if its found. ', 'real-time-auto-find-and-replace' ), '<strong>', '</strong>' ); ?>
				</li>
			</ul>
