...
class Recommended_Plugins {
    ...
    function __construct( $args ) {
        ...
        $this->menu_label        =  !empty( $args['menu_label'] ) ? $args['menu_label'] : esc_html__( 'Recommendations', 'ht-slider' );
        $this->menu_capability   =  !empty( $args['menu_capability'] ) ? $args['menu_capability'] : 'manage_options';
        $this->menu_page_slug    =  !empty( $args['menu_page_slug'] ) ? $args['menu_page_slug'] : 'ht-slider_extensions';
        ...
        // Ajax Action
        add_action( 'wp_ajax_ht-slider_ajax_plugin_activation', [ $this, 'plugin_activation' ] );
    ...
    public function enqueue_assets( $hook_suffix ) {
        ...
        */
        ...
        $localize_vars['text_domain'] = sanitize_title_with_dashes( 'ht-slider' );
        $localize_vars['nonce'] = wp_create_nonce('ht_slider_rp_nonce');
        $localize_vars['buttontxt'] = array(
            'buynow'     => esc_html__( 'Buy Now', 'ht-slider' ),
            'preview'    => esc_html__( 'Preview', 'ht-slider' ),
            'installing' => esc_html__( 'Installing..', 'ht-slider' ),
            'activating' => esc_html__( 'Activating..', 'ht-slider' ),
            'active'     => esc_html__( 'Activated', 'ht-slider' ),
        );
                            ...
                            foreach( $plugins as $plugin ):
                                ...
                                if ( ! is_wp_error( $data ) ):
                                    ...
                                    if ( file_exists( WP_PLUGIN_DIR . '/' . $data['location'] ) && is_plugin_inactive( $data['location'] ) ) {
                                        ...
                                        $button_text    = esc_html__( 'Activate', 'ht-slider' );

                                    // Not Installed.
                                    } elseif ( ! file_exists( WP_PLUGIN_DIR . '/' . $data['location'] ) ) {

                                        $button_classes = 'button htrp-install-now';
                                        $button_text    = esc_html__( 'Install Now', 'ht-slider' );

                                    // Active.
                                    } else {
                                        $button_classes = 'button disabled';
                                        $button_text    = esc_html__( 'Activated', 'ht-slider' );
                                    }
                                    ...
                                    <div class="plugin-card htrp-plugin-<?php echo sanitize_html_class( $plugin['slug'] ); ?>">
                                        <div class="plugin-card-top">
                                            <div class="name column-name" style="margin-right: 0;">
                                                <h3>
                                                    <a href="<?php echo esc_url( $details_link ) ?>" target="<?php echo esc_attr( $target ) ?>" <?php echo esc_attr($modal_class); ?>>
                                            ...
                                            <div class="desc column-description" style="margin-right: 0;">
                                                ...
                                                <p class="authors">
                                                    <cite><?php echo esc_html__( 'By ', 'ht-slider' ); ?>
                                        ...
                                        <div class="plugin-card-bottom">
                                            <div class="column-updated">
                                                <?php
                                                    if (! file_exists( WP_PLUGIN_DIR . '/' . $data['location'] ) && $plugins_type == 'pro' ) {
                                                        echo '<a class="button button-primary" href="'.esc_url( $details_link ).'" target="'.esc_attr( $target ).'">'.esc_html__( 'Buy Now', 'ht-slider' ).'</a>';
                                                    }else{
                                                ?>
                                            ...
                                            <div class="column-downloaded">
                                                <a href="<?php echo esc_url( $details_link ) ?>" target="<?php echo esc_attr( $target ) ?>" <?php echo esc_attr($modal_class); ?>><?php echo esc_html__('More Details', 'ht-slider') ?></a>
    ...
    public function plugin_activation() {
        ...
        check_ajax_referer('ht_slider_rp_nonce', 'nonce');

        if ( ! current_user_can( 'install_plugins' ) || ! isset( $_POST['location'] ) || ! $_POST['location'] ) {
            wp_send_json_error(
                array(
                    'success' => false,
                    'message' => esc_html__( 'Plugin Not Found', 'ht-slider' ),
                )
            );
        }
        ...
        wp_send_json_success(
            array(
                'success' => true,
                'message' => esc_html__( 'Plugin Successfully Activated', 'ht-slider' ),
            )
        );
