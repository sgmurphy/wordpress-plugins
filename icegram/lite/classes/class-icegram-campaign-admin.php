	...
	Icegram::gallery_screen();
		...
		public function add_campaign_settings(){
			...
			<div class="ig-top-nav shadow fixed bg-white p-0 box-border">
				<h1 class="flex items-center">
					...
					$campaign_ctas .= '<div class="button button-secondary float-right campaign_preview">' . esc_html__( 'Preview', 'icegram' ) . '</div>';
								...
								if ( !empty( $messages ) ) {
									foreach ( $messages as $row => $message ) {
										...
										<tr class="message-row basic-message-fields" value="<?php echo esc_attr( $message_id ); ?>">
											<td>
												<p class="inline-block w-28 font-bold message_title"><?php echo esc_html__( 'Message name', 'icegram' ); ?></p>
												...
												<br/>
												<p class="message_seconds inline-block w-28 font-bold message_title"><?php echo esc_html__( 'Show After', 'icegram' ); ?>
												</p>
												<p class="message_seconds my-2 inline-block">
													...
													<input type="number" class="seconds-text form-input" name="<?php echo esc_attr( $icegram_message_meta_key ) .'['.esc_attr( $row ); ?>][time]" min="-1" value="<?php echo ( !empty( $message['time'] ) ) ? esc_attr( $message['time'] ) : 0; ?>" size="3" />
		...
		function icegram_add_campaign_target_rules( $campaign_id, $campaign_target_rules  ) {
			...
			function get_message_action_row() {
				...
				wp_send_json( array( 'id' => $message_id, 'message' => $message_meta, 'message_settings' => $message_settings ) );
			...
			function save_campaign_settings( $post_id, $post ) {
				...
				if( ! empty( $_POST['load_display_rules_from'] ) ) {
					$campaign_target_rules = get_post_meta( absint( $_POST['load_display_rules_from'] ), 'icegram_campaign_target_rules', true );

					if ( isset( $campaign_target_rules['page_id'] ) ) {
						update_post_meta( $post_id, 'icegram_campaign_target_pages', $campaign_target_rules['page_id'] );
					}
					if ( isset( $campaign_target_rules['exclude_page_id'] ) ) {
						update_post_meta( $post_id, 'icegram_campaign_target_pages', $campaign_target_rules['exclude_page_id'] );
					}
			...
			function add_campaign_action($actions, $post) {
				...
				$nonce = wp_create_nonce('ig_campaigns');
			
				if ( current_user_can( 'manage_options' )) {
					$actions['duplicate_campaign'] = '<a class="ig-duplicate-campaign"  href="post.php?campaign_id=' . $post->ID . '&action=duplicate-campaign&_wpnonce=' . $nonce . '" >' . __('Duplicate', 'icegram') . '</a>';
				}
			
				// Add the "Preview" action link for all users.
			...
			function duplicate_campaign() {
				...
				if ( ! empty( $nonce ) && wp_verify_nonce( $nonce, 'ig_campaigns' ) ) {
					if ( ! empty( $_REQUEST['action'] ) && $_REQUEST['action'] == 'duplicate-campaign' && ! empty( $_REQUEST['campaign_id'] ) ) {
						...
						if ( current_user_can( 'manage_options' )) {
							Icegram::duplicate( absint( $_REQUEST['campaign_id'] ) );
						}
					}
				}
			}

			public static function get_expiry_options_for_shown(){
				return array(  'current_session' => __('Current Session' ,'icegram'),
			...
			public static function get_expiry_options_for_clicked(){
				return array(  '+50 years' => __('Never' ,'icegram'),
