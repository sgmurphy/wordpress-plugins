...
function extendthemes_remote_notifications_handle( $data ) {
	...
	$message = str_replace( '&nbsp;', ' ', trim( $data['message'] ) );
	...
	<div class="extendthemes-remote-notification extendthemes-remote-notification-<?php echo esc_attr( $data['notification_id'] ); ?>">
		...
		<style>
			[data-cp-notification-name="extendthemes_remote_notification_one-page-express_<?php echo esc_attr( $data['notification_id'] ); ?>"] {
				background-image: url("<?php echo strip_tags( $data['background_image'] ); ?>");
				background-size: cover;
				background-repeat: no-repeat;
			}

			[data-cp-notification-name="extendthemes_remote_notification_one-page-express_<?php echo esc_attr( $data['notification_id'] ); ?>"] .message p:first-child {
				font-size: 20px;
			}

			<?php echo str_replace( '#ID', "[data-cp-notification-name=\"extendthemes_remote_notification_one-page-express_{$data['notification_id']}\"]", $data['custom_css'] ); ?>
		</style>
	</div>
	<?php
}
	...
	<div>
		<div>
			...
			<div style="float: right;width: 50%;text-align: right; padding-top: 0.63em;padding-right: 2em;">
				<?php if ( $props['coupon'] ) : ?>
					<p class="coupon-code"><?php echo esc_html( $props['coupon'] ); ?></p>
				<?php endif; ?>
...
return array(
	...
	array(
		...
		'active_callback' => 'cp_notification_winter_holiday_2017_active_callback',
		'props'           => array(
			'message' => "Summer Sale - <span style='color:red'>20% discount </span> for all <strong>One Page Express PRO</strong> licenses",
			'buttons' => array(
				array(
					'class' => 'primary main',
					'link'  => 'https://extendthemes.com/go/one-page-express-summer-offer#pricing',
					'label' => 'Get the offer',
				),
				array(
					'class' => 'primary',
					'label' => 'See PRO Features',
					'link'  => 'https://extendthemes.com/go/one-page-express-summer-offer#features-6',
				),
			),
		),
