...
function su_shortcode_lightbox( $atts = null, $content = null ) {
	...
	return '<span class="su-lightbox' . su_get_css_class( $atts ) . '" data-mfp-src="' . esc_attr(su_do_attribute( $atts['src'] )) . '" data-mfp-type="' . sanitize_key( $atts['type'] ) . '" data-mobile="' . sanitize_key( $atts['mobile'] ) . '">' . do_shortcode( $content ) . '</span>';
