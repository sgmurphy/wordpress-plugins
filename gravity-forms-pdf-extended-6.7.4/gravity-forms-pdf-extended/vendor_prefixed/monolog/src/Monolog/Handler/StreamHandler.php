...
class StreamHandler extends \GFPDF_Vendor\Monolog\Handler\AbstractProcessingHandler
{
    ...
    protected function write(array $record) : void
    {
        if (!\is_resource($this->stream)) {
            $url = $this->url;
            if (null === $url || '' === $url) {
                throw new \LogicException('Missing stream url, the stream can not be opened. This may be caused by a premature call to close().' . \GFPDF_Vendor\Monolog\Utils::getRecordMessageForException($record));
            }
            $this->createDir($url);
            ...
            if (!\is_resource($stream)) {
                $this->stream = null;
                throw new \UnexpectedValueException(\sprintf('The stream or file "%s" could not be opened in append mode: ' . $this->errorMessage, $url) . \GFPDF_Vendor\Monolog\Utils::getRecordMessageForException($record));
            }
        ...
        $stream = $this->stream;
        if (!\is_resource($stream)) {
            throw new \LogicException('No stream was opened yet' . \GFPDF_Vendor\Monolog\Utils::getRecordMessageForException($record));
        }
        if ($this->useLocking) {
