...
class Gradient
{
     ...
     */
    private $writer;
    public function __construct(\GFPDF_Vendor\Mpdf\Mpdf $mpdf, \GFPDF_Vendor\Mpdf\SizeConverter $sizeConverter, \GFPDF_Vendor\Mpdf\Color\ColorConverter $colorConverter, \GFPDF_Vendor\Mpdf\Writer\BaseWriter $writer)
    {
        $this->mpdf = $mpdf;
    ...
    {
        $s = ' q ';
        $s .= \sprintf(' %.3F %.3F %.3F %.3F re W n ', $x * \GFPDF_Vendor\Mpdf\Mpdf::SCALE, ($this->mpdf->h - $y) * \GFPDF_Vendor\Mpdf\Mpdf::SCALE, $w * \GFPDF_Vendor\Mpdf\Mpdf::SCALE, -$h * \GFPDF_Vendor\Mpdf\Mpdf::SCALE);
        $s .= \sprintf(' %.3F 0 0 %.3F %.3F %.3F cm ', $w * \GFPDF_Vendor\Mpdf\Mpdf::SCALE, $h * \GFPDF_Vendor\Mpdf\Mpdf::SCALE, $x * \GFPDF_Vendor\Mpdf\Mpdf::SCALE, ($this->mpdf->h - ($y + $h)) * \GFPDF_Vendor\Mpdf\Mpdf::SCALE);
        $n = \count($this->mpdf->gradients) + 1;
        $this->mpdf->gradients[$n]['type'] = 6;
        //coons patch mesh
        $this->mpdf->gradients[$n]['colorspace'] = $colspace;
        //coons patch mesh
        $bpcd = 65535;
        //16 BitsPerCoordinate
        $trans = \false;
    ...
    public function Gradient($x, $y, $w, $h, $type, $stops = [], $colorspace = 'RGB', $coords = '', $extend = '', $return = \false, $is_mask = \false)
    {
        ...
        if ($type == self::TYPE_LINEAR) {
            ...
            if ($coords[0] !== \false && $coords[1] !== \false && $coords[2] !== \false && $coords[3] !== \false) {
                ...
                if ($angle == 0 || $angle == 360) {
                    ...
                    if ($angle <= 90) {
                        if ($angle <= 45) {
                            $endy = \tan(\deg2rad($angle));
                        } else {
                            $endx = \tan(\deg2rad(90 - $angle));
                        }
                        $b = \atan2($endy * $bboxh, $endx * $bboxw);
                        $ny = 1 - $coords[1] - \tan($b) * (1 - $coords[0]);
                        $tx = \sin($b) * \cos($b) * $ny;
                        $ty = \cos($b) * \cos($b) * $ny;
                        $coords[2] = 1 + $tx;
                        $coords[3] = 1 - $ty;
                        ...
                        if ($angle <= 135) {
                            $endx = \tan(\deg2rad($angle - 90));
                        } else {
                            $endy = \tan(\deg2rad(180 - $angle));
                        }
                        $b = \atan2($endy * $bboxh, $endx * $bboxw);
                        $ny = 1 - $coords[1] - \tan($b) * $coords[0];
                        $tx = \sin($b) * \cos($b) * $ny;
                        $ty = \cos($b) * \cos($b) * $ny;
                        $coords[2] = -$tx;
                        $coords[3] = 1 - $ty;
                        ...
                        if ($angle <= 225) {
                            $endy = \tan(\deg2rad($angle - 180));
                        } else {
                            $endx = \tan(\deg2rad(270 - $angle));
                        }
                        $b = \atan2($endy * $bboxh, $endx * $bboxw);
                        $ny = $coords[1] - \tan($b) * $coords[0];
                        $tx = \sin($b) * \cos($b) * $ny;
                        $ty = \cos($b) * \cos($b) * $ny;
                        $coords[2] = -$tx;
                        $coords[3] = $ty;
                        ...
                        if ($angle <= 315) {
                            $endx = \tan(\deg2rad($angle - 270));
                        } else {
                            $endy = \tan(\deg2rad(360 - $angle));
                        }
                        $b = \atan2($endy * $bboxh, $endx * $bboxw);
                        $ny = $coords[1] - \tan($b) * (1 - $coords[0]);
                        $tx = \sin($b) * \cos($b) * $ny;
                        $ty = \cos($b) * \cos($b) * $ny;
                        $coords[2] = 1 + $tx;
                        $coords[3] = $ty;
                ...
                $coords = [0, 0, 1, 0];
                // default for original linear gradient (L2R)
            }
            ...
            if ($coords[0] !== \false && $coords[1] !== \false && $coords[2] !== \false && $coords[3] !== \false && $coords[4] !== \false) {
                ...
                // default for radial gradient (centred)
            }
        }
        $s = ' q';
        $s .= \sprintf(' %.3F %.3F %.3F %.3F re W n', $x * \GFPDF_Vendor\Mpdf\Mpdf::SCALE, ($this->mpdf->h - $y) * \GFPDF_Vendor\Mpdf\Mpdf::SCALE, $w * \GFPDF_Vendor\Mpdf\Mpdf::SCALE, -$h * \GFPDF_Vendor\Mpdf\Mpdf::SCALE) . "\n";
        $s .= \sprintf(' %.3F 0 0 %.3F %.3F %.3F cm', $usew * \GFPDF_Vendor\Mpdf\Mpdf::SCALE, $useh * \GFPDF_Vendor\Mpdf\Mpdf::SCALE, $usex * \GFPDF_Vendor\Mpdf\Mpdf::SCALE, ($this->mpdf->h - ($usey + $useh)) * \GFPDF_Vendor\Mpdf\Mpdf::SCALE) . "\n";
        $n = \count($this->mpdf->gradients) + 1;
        ...
        if (!\is_array($extend) || \count($extend) < 1) {
            $extend = ['true', 'true'];
            // These are supposed to be quoted - appear in PDF file as text
        }
    ...
    private function parseMozLinearGradient($m, $repeat)
    {
        ...
        for ($i = $startStops; $i < \count($bgr); $i++) {
            ...
            $g['stops'][] = $this->getStop($col, $el, \true);
        }
        return $g;
    }
    ...
    public function parseBackgroundGradient($bg)
    {
        ...
        $g = [];
        if ($count_bgr > 6) {
            if (\stripos($bgr[0], 'L') === 0 && $count_bgr === 7) {
                // linear
                $g['type'] = self::TYPE_LINEAR;
