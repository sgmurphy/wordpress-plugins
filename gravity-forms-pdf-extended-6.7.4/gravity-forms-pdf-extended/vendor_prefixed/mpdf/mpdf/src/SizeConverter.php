...
class SizeConverter implements \GFPDF_Vendor\Psr\Log\LoggerAwareInterface
{
     ...
     */
    private $mpdf;
    public function __construct($dpi, $defaultFontSize, \GFPDF_Vendor\Mpdf\Mpdf $mpdf, \GFPDF_Vendor\Psr\Log\LoggerInterface $logger)
    {
        $this->dpi = $dpi;
    ...
    public function convert($size = 5, $maxsize = 0, $fontsize = \false, $usefontsize = \true)
    {
        ...
        if (!$res) {
            // ignore definition
            $this->logger->warning(\sprintf('Invalid size representation "%s"', $size), ['context' => \GFPDF_Vendor\Mpdf\Log\Context::CSS_SIZE_CONVERSION]);
        }
        ...
        switch ($unit) {
            ...
            case 'cm':
                ...
                break;
            case 'pt':
                $size *= 1 / \GFPDF_Vendor\Mpdf\Mpdf::SCALE;
                break;
            case 'rem':
                $size *= $this->mpdf->default_font_size / \GFPDF_Vendor\Mpdf\Mpdf::SCALE;
                break;
            case '%':
