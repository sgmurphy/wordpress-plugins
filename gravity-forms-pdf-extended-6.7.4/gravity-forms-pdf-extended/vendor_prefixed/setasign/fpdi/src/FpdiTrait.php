...
use GFPDF_Vendor\setasign\Fpdi\PdfReader\PdfReader;
use GFPDF_Vendor\setasign\Fpdi\PdfReader\PdfReaderException;
use GFPDF_Vendor\setasign\FpdiPdfParser\PdfParser\PdfParser as FpdiPdfParser;
/**
 * The FpdiTrait
 *
...
trait FpdiTrait
{
     ...
     */
    protected function getPdfParserInstance(\GFPDF_Vendor\setasign\Fpdi\PdfParser\StreamReader $streamReader, array $parserParams = [])
    {
        ...
        /** @noinspection PhpUndefinedClassInspection */
        if (\class_exists(\GFPDF_Vendor\setasign\FpdiPdfParser\PdfParser\PdfParser::class)) {
            /** @noinspection PhpUndefinedClassInspection */
            return new \GFPDF_Vendor\setasign\FpdiPdfParser\PdfParser\PdfParser($streamReader, $parserParams);
        }
        return new \GFPDF_Vendor\setasign\Fpdi\PdfParser\PdfParser($streamReader);
    }
    /**
    ...
    protected function getPdfReaderId($file, array $parserParams = [])
    {
        ...
        if (\is_resource($file)) {
            $streamReader = new \GFPDF_Vendor\setasign\Fpdi\PdfParser\StreamReader($file);
        } elseif (\is_string($file)) {
            $streamReader = \GFPDF_Vendor\setasign\Fpdi\PdfParser\StreamReader::createByFile($file);
            $this->createdReaders[] = $id;
        } else {
            $streamReader = $file;
        }
        $reader = new \GFPDF_Vendor\setasign\Fpdi\PdfReader\PdfReader($this->getPdfParserInstance($streamReader, $parserParams));
        /** @noinspection OffsetOperationsInspection */
     ...
     * @see PageBoundaries
     */
    public function importPage($pageNumber, $box = \GFPDF_Vendor\setasign\Fpdi\PdfReader\PageBoundaries::CROP_BOX, $groupXObject = \true, $importExternalLinks = \false)
    {
        ...
        $box = \ltrim($box, '/');
        if (!\GFPDF_Vendor\setasign\Fpdi\PdfReader\PageBoundaries::isValidName($box)) {
        ...
        if ($bbox === \false) {
            throw new \GFPDF_Vendor\setasign\Fpdi\PdfReader\PdfReaderException(\sprintf("Page doesn't have a boundary box (%s).", $box), \GFPDF_Vendor\setasign\Fpdi\PdfReader\PdfReaderException::MISSING_DATA);
        }
        $dict = new \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfDictionary();
        $dict->value['Type'] = \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfName::create('XObject');
        $dict->value['Subtype'] = \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfName::create('Form');
        $dict->value['FormType'] = \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfNumeric::create(1);
        $dict->value['BBox'] = $bbox->toPdfArray();
        if ($groupXObject) {
            $this->setMinPdfVersion('1.4');
            $dict->value['Group'] = \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfDictionary::create(['Type' => \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfName::create('Group'), 'S' => \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfName::create('Transparency')]);
        }
        ...
        if ($a != 1 || $b != 0 || $c != 0 || $d != 1 || $e != 0 || $f != 0) {
            $dict->value['Matrix'] = \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfArray::create([\GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfNumeric::create($a), \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfNumeric::create($b), \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfNumeric::create($c), \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfNumeric::create($d), \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfNumeric::create($e), \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfNumeric::create($f)]);
        }
        // try to use the existing content stream
        $pageDict = $page->getPageDictionary();
        try {
            $contentsObject = \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfType::resolve(\GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfDictionary::get($pageDict, 'Contents'), $reader->getParser(), \true);
            $contents = \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfType::resolve($contentsObject, $reader->getParser());
            // just copy the stream reference if it is only a single stream
            if (($contentsIsStream = $contents instanceof \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfStream) || $contents instanceof \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfArray && \count($contents->value) === 1) {
                if ($contentsIsStream) {
                    /**
                    ...
                    $stream = \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfType::resolve($contents->value[0], $reader->getParser());
                }
                $filter = \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfDictionary::get($stream->value, 'Filter');
                if (!$filter instanceof \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfNull) {
                    $dict->value['Filter'] = $filter;
                }
                $length = \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfType::resolve(\GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfDictionary::get($stream->value, 'Length'), $reader->getParser());
                ...
                $dict->value['Length'] = \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfNumeric::create(\strlen($streamContent));
                if ($this->compress) {
                    $dict->value['Filter'] = \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfName::create('FlateDecode');
                }
                $stream = \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfStream::create($dict, $streamContent);
            }
            // Catch faulty pages and use an empty content stream
        } catch (\GFPDF_Vendor\setasign\Fpdi\FpdiException $e) {
            $dict->value['Length'] = \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfNumeric::create(0);
            $stream = \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfStream::create($dict, '');
        }
        $externalLinks = [];
    ...
    {
        if ($value instanceof \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfNumeric) {
            if (\is_int($value->value)) {
                $this->_put($value->value . ' ', \false);
            } else {
                $this->_put(\rtrim(\rtrim(\sprintf('%.5F', $value->value), '0'), '.') . ' ', \false);
            }
        } elseif ($value instanceof \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfName) {
            $this->_put('/' . $value->value . ' ', \false);
        } elseif ($value instanceof \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfString) {
            $this->_put('(' . $value->value . ')', \false);
        } elseif ($value instanceof \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfHexString) {
            $this->_put('<' . $value->value . '>', \false);
        } elseif ($value instanceof \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfBoolean) {
            $this->_put($value->value ? 'true ' : 'false ', \false);
        } elseif ($value instanceof \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfArray) {
            $this->_put('[', \false);
            foreach ($value->value as $entry) {
                $this->writePdfType($entry);
            }
            $this->_put(']');
        } elseif ($value instanceof \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfDictionary) {
            $this->_put('<<', \false);
            ...
            $this->_put('>>');
        } elseif ($value instanceof \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfToken) {
            $this->_put($value->value);
        } elseif ($value instanceof \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfNull) {
            ...
            $this->_put('endstream');
        } elseif ($value instanceof \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfIndirectObjectReference) {
            ...
            if (!($value->value instanceof \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfArray || $value->value instanceof \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfDictionary || $value->value instanceof \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfToken || $value->value instanceof \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfStream)) {
                $this->_put("\n", \false);
            }
            $this->_put('endobj');
        }
