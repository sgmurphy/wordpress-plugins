 ...
 */
class LineReader extends \GFPDF_Vendor\setasign\Fpdi\PdfParser\CrossReference\AbstractReader implements \GFPDF_Vendor\setasign\Fpdi\PdfParser\CrossReference\ReaderInterface
{
    /**
     * The object offsets.
     *
     ...
     * @throws CrossReferenceException
     */
    protected function extract(\GFPDF_Vendor\setasign\Fpdi\PdfParser\StreamReader $reader)
    {
        $bytesPerCycle = 100;
        ...
        if ($trailerPos === \false) {
            throw new \GFPDF_Vendor\setasign\Fpdi\PdfParser\CrossReference\CrossReferenceException('Unexpected end of cross reference. "trailer"-keyword not found.', \GFPDF_Vendor\setasign\Fpdi\PdfParser\CrossReference\CrossReferenceException::NO_TRAILER_FOUND);
        }
    ...
    protected function read($xrefContent)
    {
        ...
        if (\count($m[0]) === 0) {
            throw new \GFPDF_Vendor\setasign\Fpdi\PdfParser\CrossReference\CrossReferenceException('No data found in cross-reference.', \GFPDF_Vendor\setasign\Fpdi\PdfParser\CrossReference\CrossReferenceException::INVALID_DATA);
        }
        ...
        foreach ($lines as $line) {
            ...
            switch (\count($pieces)) {
                ...
                default:
                    throw new \GFPDF_Vendor\setasign\Fpdi\PdfParser\CrossReference\CrossReferenceException(\sprintf('Unexpected data in xref table (%s)', \implode(' ', $pieces)), \GFPDF_Vendor\setasign\Fpdi\PdfParser\CrossReference\CrossReferenceException::INVALID_DATA);
            }
        }
