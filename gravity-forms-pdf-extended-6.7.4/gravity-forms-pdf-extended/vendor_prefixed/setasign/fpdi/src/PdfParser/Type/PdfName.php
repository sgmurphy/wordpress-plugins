 ...
 */
class PdfName extends \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfType
{
    /**
     ...
     * @return self
     */
    public static function parse(\GFPDF_Vendor\setasign\Fpdi\PdfParser\Tokenizer $tokenizer, \GFPDF_Vendor\setasign\Fpdi\PdfParser\StreamReader $streamReader)
    {
        $v = new self();
        if (\strspn($streamReader->getByte(), "\0\t\n\f\r ()<>[]{}/%") === 0) {
    ...
    {
        return \GFPDF_Vendor\setasign\Fpdi\PdfParser\Type\PdfType::ensureType(self::class, $name, 'Name value expected.');
    }
}
