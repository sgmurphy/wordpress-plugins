...
class Attachment
{
    ...
    {
        if ($this->isPro()) {
            \register_rest_route(Service::LEGACY_NAMESPACE, '/attachments/(?P<id>\\d+)', ['methods' => 'PUT', 'callback' => [$this, 'updateItem'], 'permission_callback' => [$this, 'permission_callback']]);
        }
        \register_rest_route(Service::LEGACY_NAMESPACE, '/attachments/(?P<id>\\d+)/shortcutInfo', ['methods' => 'GET', 'callback' => [$this, 'routeShortcutInfo'], 'permission_callback' => [$this, 'permission_callback']]);
        \register_rest_route(Service::LEGACY_NAMESPACE, '/attachments/bulk/move', ['methods' => 'PUT', 'callback' => [$this, 'routeBulkMove'], 'permission_callback' => [$this, 'permission_callback']]);
    }
    /**
    ...
    public function routeShortcutInfo($request)
    {
        ...
        return new WP_REST_Response(['html' => CustomField::getInstance()->getShortcutInfoContainer($id)]);
    }
    /**
     * See API docs.
     *
