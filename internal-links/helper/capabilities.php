...
class Capabilities
{
    ...
    public static function getCustomRoles(){
        ...
        $minimum_capability_requirements = ["manage_options", "publish_pages", "publish_posts", "edit_posts"];
        ...
        foreach ( $custom_roles as $role => $details ) {
            if(!in_array($role, $default_roles)){
                ...
                foreach($minimum_capability_requirements as $capability){
                    if ( isset( $details['capabilities'][$capability] ) && $details['capabilities'][$capability] == 1 ) {
