...
class Tools extends AbstractMenuPage
{
    ...
    public function register()
    {
        ...
        wp_localize_script('ilj_tools', 'ilj_tools', array('ajax_url' => admin_url('admin-ajax.php'), 'nonce' => wp_create_nonce('ilj-tools'), 'translation' => self::getTranslation()));
        $this->addAssets(array('tipso' => ILJ_URL . 'admin/js/tipso.js', 'ilj_select2' => ILJ_URL . 'admin/js/select2.js', 'ilj_menu_settings' => ILJ_URL . 'admin/js/ilj_menu_settings.js', 'ilj_tools' => ILJ_URL . 'admin/js/ilj_tools.js'), array('tipso' => ILJ_URL . 'admin/css/tipso.css', 'ilj_menu_settings' => ILJ_URL . 'admin/css/ilj_menu_settings.css', 'ilj_ui' => ILJ_URL . 'admin/css/ilj_ui.css', 'ilj_grid' => ILJ_URL . 'admin/css/ilj_grid.css', 'ilj_select2' => ILJ_URL . 'admin/css/select2.css', 'ilj_tools' => ILJ_URL . 'admin/css/ilj_tools.css'));
    }
    /**
    ...
    public function render()
    {
        ...
        $this->renderPostbox(array('title' => __('File import', 'internal-links'), 'content' => $this->getFileImport(), 'class' => 'tools'));
        $this->renderPostbox(array('title' => __('File export', 'internal-links'), 'content' => $this->getFileExport(), 'class' => 'tools'));
        echo '		</div>';
        echo '		<div class="col-6">';
        ?>
			<div>
        ...
        $this->renderPostbox(array('title' => __('Import keyword configurations from WordPress', 'internal-links'), 'content' => $this->getKeywordImport(), 'class' => $keyword_import_postbox_class . ' ' . 'tools', 'before_headline' => $before_headline));
        echo '		</div>';
        echo '	</div>';
        echo '</div>';
    }
    ...
    protected function getKeywordImport()
    {
        ...
        $output .= $this->getKeywordImportSource(array(array('title' => __('Post titles', 'internal-links'), 'class' => 'title'), array('title' => __('Post tags', 'internal-links'), 'class' => 'tags')), self::ILJ_FILTER_MENUPAGE_TOOLS_KEYWORD_IMPORT_POST, self::ILJ_MENUPAGE_TOOLS_IMPORT_INTERN_POST);
        $output .= $button;
        $output .= '<span class="spinner"></span>';
        $output .= '<div class="clear"></div>';
        $output .= '</form>';
        ...
        $output .= '</div>';
        $output .= sprintf('<p><strong>%s</strong></p>', __('Sources', 'internal-links'));
        $output .= $this->getKeywordImportSource(array('title' => __('Term titles', 'internal-links'), 'class' => 'title'), self::ILJ_FILTER_MENUPAGE_TOOLS_KEYWORD_IMPORT_TERM, self::ILJ_MENUPAGE_TOOLS_IMPORT_INTERN_TERM);
        $button = sprintf('<button class="button ilj-import" disabled style="cursor:not-allowed;">%s</button>', __('Import', 'internal-links'));
        $output .= $button;
        $output .= '<span class="spinner"></span>';
        $output .= '<div class="clear"></div>';
