...
class SchedulerBatchSize extends AbstractOption {
	...
	public function getHint() {
		...
		. '<li><p class="description"><code>' . __('Small value', 'internal-links') . '</code>: ' . sprintf(__('For resource-limited servers, you should set a value close to 1, so the plugin uses less CPU and memory. Minimum value is %s', 'internal-links'), ActionSchedulerOptions::MINIMAL) . '.</p></li>'
		. '<li><p class="description"><code>' . __('Large value', 'internal-links') . '</code>: ' . sprintf(__('If you have a lot of RAM and CPU, you can set a value like 100. Maximum value is %s', 'internal-links'), ActionSchedulerOptions::MAXIMAL) . '</p></li>'
		. '</ul>';
	}
