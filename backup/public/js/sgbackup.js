...
SG_PING_AWAKE_REQUEST_FREQUENCY = 13000;
...
sgBackup.checkExtractCreation = function () {
    jQuery('#manualBackup .btn-success').attr('disabled', true);
    var sgRestoreCreationHandler = new sgRequestHandler('checkExtractCreation', {token: BG_BACKUP_STRINGS.nonce});
    sgRestoreCreationHandler.callback = function (response) {
...
sgBackup.initManualBackupTooltips = function () {
    jQuery('[for=cloud-ftp]').tooltip({
        container: jQuery('[for=cloud-ftp]').parent()
    });
    jQuery('[for=cloud-dropbox]').tooltip({
        container: jQuery('[for=cloud-dropbox]').parent()
    });
    jQuery('[for=cloud-gdrive]').tooltip({
        container: jQuery('[for=cloud-gdrive]').parent()
    });
    jQuery('[for=cloud-one-drive]').tooltip({
        container: jQuery('[for=cloud-one-drive]').parent()
    });
    jQuery('[for=cloud-p-cloud]').tooltip({
        container: jQuery('[for=cloud-p-cloud]').parent()
    });
    jQuery('[for=cloud-box]').tooltip({
        container: jQuery('[for=cloud-box]').parent()
    });
    jQuery('[for=cloud-amazon]').tooltip({
        container: jQuery('[for=cloud-amazon]').parent()
    });
    jQuery('[for=cloud-backup-guard]').tooltip({
        container: jQuery('[for=cloud-backup-guard]').parent()
    });
...
sgBackup.startExtract = function (bname) {
    sgBackup.startExtractAction(bname);
};
...
sgBackup.startRestoreAction = function (bname) {
    ...
    checkPHPVersionCompatibility.callback = function (response) {
        if (typeof response.error !== 'undefined') {
            ...
            var alert = sgBackup.alertGenerator(response.warning, 'alert-warning');
            jQuery('#sg-modal .modal-header').prepend(alert);
            return false;
        }
...
sgBackup.startExtractAction = function (bname) {
    sgBackup.showAjaxSpinner('#sg-content-wrapper');
    var extractHandler = new sgRequestHandler('extract', {bname: bname, type: 1, paths: 2});
    extractHandler.run();
    sgBackup.checkExtractCreation();
};
...
sgBackup.getActionProgress = function (actionId) {
    ...
    sgActionHandler.callback = function (response) {
        ...
        if (response) {
            ...
            //console.log(response);
            if (response.tree_procesing === true) {
                jQuery('.tree-line-data').show(0);
                jQuery('.tree-line-data span').empty().append(response.tree_files);
            } else {
                jQuery('.tree-line-data').hide(0);

                if (response.log_lines != null) {
                    jQuery('.log-line-data').show(0);
                    jQuery('.log-line-data').empty().append(response.log_lines);
                }
            }
...
sgBackup.pingAwake = function () {
    var sgActionHandler = new sgRequestHandler('awake_frontend', {token: BG_BACKUP_STRINGS.nonce});
    ...
    sgActionHandler.callback = function (response) {
        setTimeout(
            ...
            SG_PING_AWAKE_REQUEST_FREQUENCY
        );
    };
    sgActionHandler.run();
};
...
sgBackup.statusUpdate = function (tooltip, response, progressInPercents) {
    ...
    if (response.type === '1') {
        ...
        tooltipText = currentAction + ' is generating tree file';
        }
        ...
        let subType = parseInt(response.subtype);
        let cloudIcon = jQuery('.sg-status-' + response.type + response.subtype);

        if (subType === SG_STORAGE_FTP) {
            tooltipText = 'Uploading to FTP - ' + progressInPercents;
        } else if (subType === SG_STORAGE_DROPBOX) {
            tooltipText = 'Uploading to Dropbox - ' + progressInPercents;
        } else if (subType === SG_STORAGE_GOOGLE_DRIVE) {
            tooltipText = 'Uploading to Google Drive - ' + progressInPercents;
        } else if (subType === SG_STORAGE_AMAZON) {
            tooltipText = 'Uploading to Amazon S3 - ' + progressInPercents;
        } else if (subType === SG_STORAGE_ONE_DRIVE) {
            tooltipText = 'Uploading to OneDrive - ' + progressInPercents;
        } else if (subType === SG_STORAGE_P_CLOUD) {
            tooltipText = 'Uploading to pCloud - ' + progressInPercents;
        } else if (subType === SG_STORAGE_BOX) {
    ...
    tooltip.attr('data-bs-original-title', tooltipText);
};
