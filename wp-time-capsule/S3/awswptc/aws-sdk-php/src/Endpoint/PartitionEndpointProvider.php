<?php
namespace AwsWPTC\Endpoint;

use JmesPathWPTC\Env;
...
class PartitionEndpointProvider
{
    ...
    {
        if( (defined('DEFAULT_REPO') && DEFAULT_REPO == 'wasabi') || 
            (defined('FORCE_WASABI') && FORCE_WASABI) ){

            $data = \AwsWPTC\load_compiled_json(__DIR__ . '/../data/endpoints.wasabi.json');
        } elseif( (defined('DEFAULT_REPO') && DEFAULT_REPO == 'backblaze') || 
            (defined('FORCE_BACKBLAZE') && FORCE_BACKBLAZE) ){

            $data = \AwsWPTC\load_compiled_json(__DIR__ . '/../data/endpoints.backblaze.json');
        } else {
            
            $data = \AwsWPTC\load_compiled_json(__DIR__ . '/../data/endpoints.json');
        }


        $prefixData = \AwsWPTC\load_compiled_json(__DIR__ . '/../data/endpoints_prefix_history.json');
