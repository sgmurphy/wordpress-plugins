...
use AwsWPTC\Api\Validator;
use AwsWPTC\Api\ApiProvider;
use AwsWPTC\Api\Service;
use AwsWPTC\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware;
use AwsWPTC\ClientSideMonitoring\ApiCallMonitoringMiddleware;
use AwsWPTC\ClientSideMonitoring\Configuration;
use AwsWPTC\Credentials\Credentials;
use AwsWPTC\Credentials\CredentialsInterface;
use AwsWPTC\Endpoint\PartitionEndpointProvider;
use AwsWPTC\EndpointDiscovery\ConfigurationInterface;
use AwsWPTC\EndpointDiscovery\ConfigurationProvider;
use AwsWPTC\EndpointDiscovery\EndpointDiscoveryMiddleware;
use AwsWPTC\Exception\InvalidRegionException;
use AwsWPTC\Retry\ConfigurationInterface as RetryConfigInterface;
use AwsWPTC\Retry\ConfigurationProvider as RetryConfigProvider;
use AwsWPTC\Signature\SignatureProvider;
use AwsWPTC\Endpoint\EndpointProvider;
use AwsWPTC\Credentials\CredentialProvider;
...
class ClientResolver
{
    ...
    private static $defaultArgs = [
        'service' => [
            'type'     => 'value',
            'valid'    => ['string'],
            'doc'      => 'Name of the service to utilize. This value will be supplied by default when using one of the SDK clients (e.g., AwsWPTC\\S3\\S3Client).',
            'required' => true,
            'internal' => true
        ],
        'exception_class' => [
            ...
            'default'  => 'AwsWPTC\Exception\AwsException',
            'internal' => true
        ],
        'scheme' => [
            'type'     => 'value',
        ...
        'signature_provider' => [
            'type'    => 'value',
            'valid'   => ['callable'],
            'doc'     => 'A callable that accepts a signature version name (e.g., "v4"), a service name, and region, and  returns a SignatureInterface object or null. This provider is used to create signers utilized by the client. See AwsWPTC\\Signature\\SignatureProvider for a list of built-in providers',
        ...
        'endpoint_provider' => [
            ...
            'doc'      => 'An optional PHP callable that accepts a hash of options including a "service" and "region" key and returns NULL or a hash of endpoint data, of which the "endpoint" key is required. See AwsWPTC\\Endpoint\\EndpointProvider for a list of built-in providers.',
        ...
        'credentials' => [
            ...
            'doc'     => 'Specifies the credentials used to sign requests. Provide an AwsWPTC\Credentials\CredentialsInterface object, an associative array of "key", "secret", and an optional "token" key, `false` to use null credentials, or a callable credentials provider used to create credentials or return null. See AwsWPTC\\Credentials\\CredentialProvider for a list of built-in credentials providers. If no credentials are provided, the SDK will attempt to load them from the environment.',
        ...
        'endpoint_discovery' => [
            ...
            'doc'      => 'Specifies settings for endpoint discovery. Provide an instance of AwsWPTC\EndpointDiscovery\ConfigurationInterface, an instance AwsWPTC\CacheInterface, a callable that provides a promise for a Configuration object, or an associative array with the following keys: enabled: (bool) Set to true to enable endpoint discovery, false to explicitly disable it. Defaults to false; cache_limit: (int) The maximum number of keys in the endpoints cache. Defaults to 1000.',
        ...
        'stats' => [
            ...
            'valid' => ['bool', 'array'],
            'default' => false,
            'doc'   => 'Set to true to gather transfer statistics on requests sent. Alternatively, you can provide an associative array with the following keys: retries: (bool) Set to false to disable reporting on retries attempted; http: (bool) Set to true to enable collecting statistics from lower level HTTP adapters (e.g., values returned in GuzzleHttpWPTC\TransferStats). HTTP handlers must support an http_stats_receiver option for this to have an effect; timer: (bool) Set to true to enable a command timer that reports the total wall clock time spent on an operation in seconds.',
            'fn'    => [__CLASS__, '_apply_stats'],
        ],
        'retries' => [
            'type'    => 'value',
            'valid'   => ['int', RetryConfigInterface::class, CacheInterface::class, 'callable', 'array'],
            'doc'     => "Configures the retry mode and maximum number of allowed retries for a client (pass 0 to disable retries). Provide an integer for 'legacy' mode with the specified number of retries. Otherwise provide an instance of AwsWPTC\Retry\ConfigurationInterface, an instance of  AwsWPTC\CacheInterface, a callable function, or an array with the following keys: mode: (string) Set to 'legacy', 'standard' (uses retry quota management), or 'adapative' (an experimental mode that adds client-side rate limiting to standard mode); max_attempts: (int) The maximum number of attempts for a given request. ",
        ...
        'csm' => [
            'type'     => 'value',
            'valid'    => [\AwsWPTC\ClientSideMonitoring\ConfigurationInterface::class, 'callable', 'array', 'bool'],
            ...
            'default'  => [\AwsWPTC\ClientSideMonitoring\ConfigurationProvider::class, 'defaultProvider']
        ],
        'http' => [
            'type'    => 'value',
            'valid'   => ['array'],
        ...
        'handler' => [
            'type'     => 'value',
            'valid'    => ['callable'],
            'doc'      => 'A handler that accepts a command object, request object and returns a promise that is fulfilled with an AwsWPTC\ResultInterface object or rejected with an AwsWPTC\Exception\AwsException. A handler does not accept a next handler as it is terminal and expected to fulfill a command. If no handler is provided, a default Guzzle handler will be utilized.',
        ...
        'use_aws_shared_config_files' => [
            'type'      => 'value',
            'valid'     => ['bool'],
            'doc'       => 'Set to false to disable checking for shared aws config files usually located in \'~/.awswptc/config\' and \'~/.awswptc/credentials\'.  This will be ignored if you set the \'profile\' setting.',
            'default'   => true,
        ],
    ];
    ...
    public static function _apply_credentials($value, array &$args)
    {
        ...
        if ($value instanceof CredentialsInterface) {
            ...
            throw new IAE('Credentials must be an instance of '
                . 'AwsWPTC\Credentials\CredentialsInterface, an associative '
    ...
    public static function _apply_csm($value, array &$args, HandlerList $list)
    {
        if ($value === false) {
            $value = new Configuration(
                false,
                \AwsWPTC\ClientSideMonitoring\ConfigurationProvider::DEFAULT_HOST,
                \AwsWPTC\ClientSideMonitoring\ConfigurationProvider::DEFAULT_PORT,
                \AwsWPTC\ClientSideMonitoring\ConfigurationProvider::DEFAULT_CLIENT_ID
            );
            $args['csm'] = $value;
        }
