...
use GuzzleHttpWPTC\Promise\EachPromise;
...
class CommandPool implements PromisorInterface
{
            ...
            foreach ($commands as $key => $command) {
                if (!($command instanceof CommandInterface)) {
                    throw new \InvalidArgumentException('Each value yielded by '
                        . 'the iterator must be an AwsWPTC\CommandInterface.');
                }
                if ($before) {
                    $before($command, $key);
                }
    ...
    /**
     * @return \GuzzleHttpWPTC\Promise\PromiseInterface
     */
     ...
     *
     * @return array
     * @see \AwsWPTC\CommandPool::__construct for available configuration options.
     */
