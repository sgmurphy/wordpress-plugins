...
use GuzzleHttpWPTC\Promise\PromisorInterface;
use Iterator;
...
class Transfer implements PromisorInterface
{
     ...
     *   callback accepts a single argument: AwsWPTC\CommandInterface $command.
    ...
    private function createDownloadPromise()
    {
        ...
        return (new AwsWPTC\CommandPool($this->client, $commands, [
    ...
    private function createUploadPromise()
    {
        ...
        $files = \AwsWPTC\map($this->getUploadsIterator(), function ($file) {
    ...
    private function getUploadsIterator()
    {
        if (is_string($this->source)) {
            return AwsWPTC\filter(
                AwsWPTC\recursive_dir_iterator($this->sourceMetadata['path']),
    ...
    private function getDownloadsIterator()
    {
        if (is_string($this->source)) {
            ...
            $files = $this->client
                ...
                ->search('Contents[].Key');
            $files = AwsWPTC\map($files, function ($key) use ($listArgs) {
                return "s3://{$listArgs['Bucket']}/$key";
            });
            return AwsWPTC\filter($files, function ($key) {
