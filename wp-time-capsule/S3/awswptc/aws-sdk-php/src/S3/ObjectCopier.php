...
use AwsWPTC\Arn\S3\AccessPointArn;
use AwsWPTC\Exception\MultipartUploadException;
use AwsWPTC\Result;
...
class ObjectCopier implements PromisorInterface
{
    ...
    {
        return \GuzzleHttpWPTC\Promise\coroutine(function () {
    ...
    private function validateLocation(array $location)
    {
        if (empty($location['Bucket']) || empty($location['Key'])) {
            throw new \InvalidArgumentException('Locations provided to an'
                . ' AwsWPTC\S3\ObjectCopier must have a non-empty Bucket and Key');
        }
    }
