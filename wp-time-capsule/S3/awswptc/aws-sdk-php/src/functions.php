...
use GuzzleHttpWPTC\Client;
use PsrWPTC\Http\Message\RequestInterface;
use GuzzleHttpWPTC\ClientInterface;
...
function default_http_handler()
{
    ...
    if ($version === 6 || $version === 7) {
        return new \AwsWPTC\Handler\GuzzleV6\GuzzleHandler();
    }

    // If Guzzle 5 installed
    if ($version === 5) {
        return new \AwsWPTC\Handler\GuzzleV5\GuzzleHandler();
    }
...
function default_user_agent()
{
    ...
    // If Guzzle 6 or 7 installed
    if ($version === 6 || $version === 7) {
        return \GuzzleHttpWPTC\default_user_agent();
    }

    // If Guzzle 5 installed
    if ($version === 5) {
        return \GuzzleHttpWPTC\Client::getDefaultUserAgent();
    }
 ...
 *
 * @internal This function is internal and should not be used outside awswptc/aws-sdk-php.
 * @return int
...
function guzzle_major_version()
{
    ...
    if (defined('\GuzzleHttpWPTC\ClientInterface::VERSION')) {
    ...
    } elseif (defined('\GuzzleHttpWPTC\ClientInterface::MAJOR_VERSION')) {
