...
class WPTC_Backblaze{
	...
	public function is_bucket_exist(){
		wptc_log(func_get_args(), "--------is_bucket_exist-----backblaze---");
		try{
			...
			wptc_log($err_msg,'-----------$err_msg-----is_bucket_exist--wasabi---------');
			...
			if (stristr($err_msg, '403 Forbidden')) {
				return array('error' => 'Cannot authorize, check the credentials entered.' );
			}

			if (stristr($err_msg, 'Error executing')) {
				return array('error' => 'Cannot authorize, check the credentials entered.' );
			}
	...
	public function is_object_exist($key){
		try{
			...
			$headObject = $this->s3_client->headObject(array(
				'Bucket' => $this->bucket_name,
				'Key' => $key,
			));

			wptc_log($headObject,'-----------$headObject----------------');
			...
			wptc_log($err_msg,'-----------$err_msg-------headObject---------');
	...
	public function get_bucket_lifecycle(){
		wptc_log(func_get_args(), "--------get_bucket_lifecycle--wasabi------");
		...
		try{
			...
			wptc_log($err_msg,'-----------$err_msg-----get_bucket_lifecycle--wasabi---------');
	...
	public function upsert_site_life_cycle($revision_days = false){
		...
		$new_rules['LifecycleConfiguration'] = [];
		$new_rules['LifecycleConfiguration']['Rules']  = $existing_rules;
		...
		try{
			...
			wptc_log($err_msg,'-----------$err_msg------upsert_site_life_cycle--wasabi--------');
	...
	public function create_bucket(){
		...
		try{
			...
			$neat_err_msg = $e->getAwsErrorMessage();

			wptc_log($neat_err_msg,'-----------$neat_err_msg----------------');

			if(!empty($neat_err_msg)){
				
				return array('error' => $neat_err_msg );
			}
