...
using the ``mtdowlingwptc/jmespath.php`` package.
...
.. code-block:: php
    ...
    JmesPathWPTC\search($expression, $data);
    // Returns: [1, 2, 3]
...
The ``JmesPathWPTC\search`` function can be used in most cases when using the
...
.. code-block:: php
    ...
    $result = JmesPathWPTC\search($expression, $data);

    // or, if you require PSR-4 compliance.
    $result = JmesPathWPTC\Env::search($expression, $data);
...
AstRuntime is utilized by ``JmesPathWPTC\search()`` and ``JmesPathWPTC\Env::search()``
by default.
...
.. code-block:: php
    ...
    $runtime = new JmesPathWPTC\AstRuntime();
...
``JmesPathWPTC\CompilerRuntime`` provides the most performance for
...
.. code-block:: php
    ...
    $runtime = new JmesPathWPTC\CompilerRuntime('/path/to/compile/folder');
    $runtime('foo.bar', ['foo' => ['bar' => 'baz']]);
    // > 'baz'
...
You can utilize the CompilerRuntime in ``JmesPathWPTC\search()`` by setting
