...
- Support for `guzzlehttpwptc/psr7:^2.0` [#2878](https://github.com/guzzle/guzzle/pull/2878)
...
- `GuzzleHttpWPTC\MessageFormatterInterface`
...
* Updated the default User Agent to "guzzlehttpwptc/7" [#2654](https://github.com/guzzle/guzzle/pull/2654)
...
* Improvement: Added `GuzzleHttpWPTC\Exception\InvalidArgumentException` [#2163](https://github.com/guzzle/guzzle/pull/2163)
* Improvement: Added `GuzzleHttpWPTC\_current_time()` to use `hrtime()` if that function exists. [#2242](https://github.com/guzzle/guzzle/pull/2242)
...
* Improvement:  	Use `\GuzzleHttpWPTC\Promise\rejection_for` function instead of object init [#1827](https://github.com/guzzle/guzzle/pull/1827)
...
* Feature: added `GuzzleHttpWPTC\json_encode` and `GuzzleHttpWPTC\json_decode`.
...
* Added `GuzzleHttpWPTC\RequestOptions` to allow request options to be applied
  using constants.
* `$maxHandles` has been removed from CurlMultiHandler.
* `MultipartPostBody` is now part of the `guzzlehttpwptc/psr7` package.
...
* Marked `GuzzleHttpWPTC\Client::getDefaultUserAgent` as deprecated.
...
* Updated to use PSR-7
  ...
  * Removed the dependency on `guzzlehttpwptc/streams`. These stream abstractions
    are available in the `guzzlehttpwptc/psr7` package under the `GuzzleHttpWPTC\Psr7`
    namespace.
* Added middleware and handler system
  * Replaced the Guzzle event and subscriber system with a middleware system.
  * No longer depends on RingPHP, but rather places the HTTP handlers directly
    in Guzzle, operating on PSR-7 messages.
  * Retry logic is now encapsulated in `GuzzleHttpWPTC\Middleware::retry`, which
    means the `guzzlehttpwptc/retry-subscriber` is now obsolete.
  * Mocking responses is now handled using `GuzzleHttpWPTC\Handler\MockHandler`.
* Asynchronous responses
  * No longer supports the `future` request option to send an async request.
    Instead, use one of the `*Async` methods of a client (e.g., `requestAsync`,
    `getAsync`, etc.).
  * Utilizing `GuzzleHttpWPTC\Promise` instead of React's promise library to avoid
...
* Request options
  * POST and form updates
    ...
    * Removed the `GuzzleHttpWPTC\Post` namespace.
...
* Removed `GuzzleHttpWPTC\BatchResults`. Batched pool results are now returned as
  an array.
* Removed `GuzzleHttpWPTC\Query`. Query string handling is now handled using an
  associative array passed into the `query` request option. The query string
  is serialized using PHP's `http_build_query`. If you need more control, you
  can pass the query string in as a string.
* `GuzzleHttpWPTC\QueryParser` has been replaced with the
  `GuzzleHttpWPTC\Psr7\parse_query`.
...
* Added a `GuzzleHttpWPTC\Pool::send()` convenience method to match the docs.
...
* Added support for non-blocking responses based on `guzzlehttpwptc/guzzle-ring`.
* Added a public API for creating a default HTTP adapter.
* Updated the redirect plugin to be non-blocking so that redirects are sent
  concurrently. Other plugins like this can now be updated to be non-blocking.
* Added a "progress" event so that you can get upload and download progress
  events.
* Added `GuzzleHttpWPTC\Pool` which implements FutureInterface and transfers
  requests concurrently using a capped pool size as efficiently as possible.
* Added `hasListeners()` to EmitterInterface.
* Removed `GuzzleHttpWPTC\ClientInterface::sendAll` and marked
  `GuzzleHttpWPTC\Client::sendAll` as deprecated (it's still there, just not the
  recommended way).
...
* Removed the fluent interfaces (i.e., `return $this`) from requests,
  responses, `GuzzleHttpWPTC\Collection`, `GuzzleHttpWPTC\Url`,
  `GuzzleHttpWPTC\Query`, `GuzzleHttpWPTC\Post\PostBody`, and
  `GuzzleHttpWPTC\Cookie\SetCookie`. This blog post provides a good outline of
  why I did this: https://ocramius.github.io/blog/fluent-interfaces-are-evil/.
  This also makes the Guzzle message interfaces compatible with the current
  PSR-7 message proposal.
* Removed "functions.php", so that Guzzle is truly PSR-4 compliant. Except
  for the HTTP request functions from function.php, these functions are now
  implemented in `GuzzleHttpWPTC\Utils` using camelCase. `GuzzleHttpWPTC\json_decode`
  moved to `GuzzleHttpWPTC\Utils::jsonDecode`. `GuzzleHttpWPTC\get_path` moved to
  `GuzzleHttpWPTC\Utils::getPath`. `GuzzleHttpWPTC\set_path` moved to
  `GuzzleHttpWPTC\Utils::setPath`. `GuzzleHttpWPTC\batch` should now be
  `GuzzleHttpWPTC\Pool::batch`, which returns an `objectStorage`. Using functions.php
  caused problems for many users: they aren't PSR-4 compliant, require an
  explicit include, and needed an if-guard to ensure that the functions are not
  declared multiple times.
* Rewrote adapter layer.
    * Removing all classes from `GuzzleHttpWPTC\Adapter`, these are now
      implemented as callables that are stored in `GuzzleHttpWPTC\Ring\Client`.
    * Removed the concept of "parallel adapters". Sending requests serially or
      concurrently is now handled using a single adapter.
    * Moved `GuzzleHttpWPTC\Adapter\Transaction` to `GuzzleHttpWPTC\Transaction`. The
...
* `GuzzleHttpWPTC\Message\MessageFactory` no longer allows subclasses to add
...
* Updated to "GuzzleHttpWPTC\Streams" 3.0.
    * `GuzzleHttpWPTC\Stream\StreamInterface::getContents()` no longer accepts a
      `maxLen` parameter. This update makes the Guzzle streams project
      compatible with the current PSR-7 proposal.
    * `GuzzleHttpWPTC\Stream\Stream::__construct`,
      `GuzzleHttpWPTC\Stream\Stream::factory`, and
      `GuzzleHttpWPTC\Stream\Utils::create` no longer accept a size in the second
...
* Updating guzzlehttpwptc/streams dependency to ~2.1
...
* Added `ToArrayInterface` to `GuzzleHttpWPTC\Cookie\CookieJar`

## 4.1.0 - 2014-05-27

* Added a `json` request option to easily serialize JSON payloads.
* Added a `GuzzleHttpWPTC\json_decode()` wrapper to safely parse JSON.
* Added `setPort()` and `getPort()` to `GuzzleHttpWPTC\Message\RequestInterface`.
...
* Added `GuzzleHttpWPTC\batch()` as a convenience function for sending requests in
  parallel without needing to write asynchronous code.
* Restructured how events are added to `GuzzleHttpWPTC\ClientInterface::sendAll()`.
...
* Bug fix: SchemaFormatter now returns an integer when formatting to a Unix timestamp
  (see https://github.com/awswptc/aws-sdk-php/issues/147)
...
* [BC] Removing Guzzle\Common\NullObject.  Use https://github.com/mtdowlingwptc/NullObject if you need it.
