...
class Wptc_Backup_Before_Update_Hooks_Hanlder extends Wptc_Base_Hooks_Handler {
	...
	public function may_be_prevent_auto_update($is_update_required, $update_details = null, $dets2 = null, $dets3 = null) {
		...
		$is_auto_update = $this->backup_before_auto_update_settings->get_auto_update_settings();

		if(!$is_auto_update || $is_auto_update['status'] == 'no'){

			return $is_update_required;
		}

		if($update_details && !empty($update_details->plugin) && $update_details->plugin == 'a-fake-plugin/a-fake-plugin.php'){
			return true;
		}
		
		if($update_details && !empty($update_details->theme) && $update_details->theme == 'a-fake-theme'){
			return true;
		}
	...
	public function save_bbu_settings(){
		...
		$this->config->set_option('wptc_bbu_user_id', $current_user_id);

		wptc_log($current_user_id, '---------current_user_id-----------');
	...
	public function myplugin_auto_update_setting_html( $html, $plugin_file, $plugin_data ) {
		// if ( 'my-plugin/my-plugin.php' === $plugin_file ) {
		// }
		$html = __( 'Auto-updates are managed by WPTC.', 'wptc' );
	 
		return $html;
	}

	public function filter_plugins_auto_update_enabled( $data ) {
		$is_auto_update = $this->backup_before_auto_update_settings->get_auto_update_settings();
	...
	public function filter_themes_auto_update_enabled( $data ) {
		wptc_log($data, "--------prevent_themes_auto_update_enabled--------");

		return false;
	}

	public function automatic_updater_disabled( $data ) {
		wptc_log($data, "--------automatic_updater_disabled--------");
