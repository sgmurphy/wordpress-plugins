...
class WPTC_S3Facade {
	...
	public function init() {
		try {
			...
			wptc_log($e->getMessage(), '--------s3 facade exception------');
	...
	public function enable_versioning() {
		$is_s3_versioning_enabled = WPTC_Factory::get('config')->get_option('is_s3_versioning_enabled');
		if( !empty($is_s3_versioning_enabled) ){

			return ;
		}

		WPTC_Factory::get('config')->set_option('is_s3_versioning_enabled', true);
	...
	public function is_authorized($is_restore = false) {
		//Handling the auth of S3 during backup upload
	...
	public function validate_max_revision_limit($requested_revision_limit) {
		wptc_log('', '---trying to validate_max_revision_limit---');
