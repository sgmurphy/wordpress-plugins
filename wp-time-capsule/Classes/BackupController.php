...
class WPTC_BackupController {
	...
	public function set_low_priority_updates(){
		if(defined('WPTC_DISABLE_LOW_PRIORITY_UPDATES') && WPTC_DISABLE_LOW_PRIORITY_UPDATES){

			return false;
		}

		$disable_low_priority_updates = $this->config->get_option('disable_low_priority_updates');
		if(!empty($disable_low_priority_updates)){
		...
		if($result === false){
			$this->config->set_option('disable_low_priority_updates', true);
		}
	}
		...
		while ($is_queue) {
			...
			foreach ($Qfiles as $file_info) {
				...
				if (isset($cloud_response['too_many_requests'])) {
					...
					if(!empty($file_with_version['revision_id'])){
	...
	private function do_meta_data_backup_related_works(){
		if ( !$this->config->get_option('wptc_main_cycle_running') 
			 && !$this->config->get_option('starting_first_backup') 
			 && !$this->config->get_option('is_migration_backup') ) {
	...
	public function unlink_current_acc_and_backups() {
		...
		wptc_log('', '--------unlink_current_acc_and_backups--------');
	...
	public function clear_prev_repo_backup_files_record($reset_inc_exc = false) {
		...
		wptc_log('', '--------clear_prev_repo_backup_files_record--------');
