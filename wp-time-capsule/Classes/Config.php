...
class WPTC_Config {
	...
	public function get_backup_dir($only_path = false) {
		...
		if (!$only_path) {
			if(!defined('WPTC_TEMP_DIR_BASENAME')){
				define('WPTC_TEMP_DIR_BASENAME', 'tCapsule');
			}
	...
	public function get_sql_files_count_current_backup_id(){
		...
		global $wpdb;
		$sql_files_count = $wpdb->get_var("SELECT COUNT(file) FROM " . $wpdb->base_prefix . "wptc_processed_files WHERE backupID=$cur_backup_id AND file LIKE '%backup.sql%'");

		return $sql_files_count;
	}

	public function complete_backup(){
		$sql_files_count = $this->get_sql_files_count_current_backup_id();

		wptc_log($sql_files_count, "--------sql_files_count--------");

		if( !empty($sql_files_count) ){
			$this_time = time();
			$this->set_option('last_backup_success_complete_time', $this_time);
		}
