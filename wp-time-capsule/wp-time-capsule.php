...
if (is_php_version_compatible_for_s3_wptc()) {
	if (version_compare(PHP_VERSION, '7.2.5') >= 0) {
		...
		include_once ( WPTC_PLUGIN_DIR . 'S3_BWC/autoload.php' );
		include_once ( WPTC_PLUGIN_DIR . 'S3_BWC/s3WPTCWrapper.php' );
	
		include_once ( WPTC_CLASSES_DIR . 'S3FacadeBWC.php' );
		include_once ( WPTC_CLASSES_DIR . 'WasabiFacadeBWC.php' );
		include_once ( WPTC_CLASSES_DIR . 'BackblazeFacadeBWC.php' );
	}
}
...
function wptc_install() {
	...
	if (!empty($errors)) {
		...
		add_option('wptc-init-errors', implode('<br />', $errors), false, 'no');
		return ;
	}
...
function check_wptc_update() {
	...
	if (version_compare('1.22.6', $installed_wptc_version) > 0) {
		$time = time() + (24 * 60 * 60);
		$config->set_option('last_backup_success_complete_time', $time);
	}

	if (version_compare('1.22.8', $installed_wptc_version) > 0) {
...
function get_wasabi_authorize_url_wptc() {
	...
	$wasabi_access_key = base64_encode($wasabi_access_key);
	$wasabi_access_key = str_replace('=', '', $wasabi_access_key);
	$wasabi_secure_key = base64_encode($wasabi_secure_key);
	$wasabi_secure_key = str_replace('=', '', $wasabi_secure_key);
...
function get_backblaze_authorize_url_wptc() {
	...
	$backblaze_access_key = base64_encode($backblaze_access_key);
	$backblaze_access_key = str_replace('=', '', $backblaze_access_key);
	$backblaze_secure_key = base64_encode($backblaze_secure_key);
	$backblaze_secure_key = str_replace('=', '', $backblaze_secure_key);
...
function wptc_trigger_truncate_cron(){
	...
	if( $last_time && ($cur_time_adjusted - $last_time) > $timeThreshold ){
		...
		$wpdb->query("DROP TABLE IF EXISTS `" . $wpdb->base_prefix . "wptc_query_recorder`");
...
add_action('set_site_transient_update_core', 'wptc_check_set_site_transient_update_core');

function wptc_check_set_site_transient_update_core($value){
	update_option('_site_transient_update_core', $value);
}

function check_and_send_no_backup_email(){
	$last_backup_success_complete_time = WPTC_Factory::get('config')->get_option('last_backup_success_complete_time');

	// wptc_log($last_backup_success_complete_time, "--------last_backup_success_complete_time--------");

	if(empty($last_backup_success_complete_time)){
		return;
	}
	...
	$hours_72 = 72 * 60 * 60;
	if( $current_time - $last_backup_success_complete_time > ($hours_72) ){

		wptc_log($last_backup_success_complete_time, "--------no backup for hours--------");

		WPTC_Factory::get('config')->set_option('last_backup_success_complete_time', false);

		$email_data = array(
			'type' => 'backup_schedule_failed',
		);
