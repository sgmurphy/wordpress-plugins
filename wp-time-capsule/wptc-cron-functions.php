...
class Wptc_Init{
	...
	private function should_enable_admin_login(){
		...
		if (!$this->get_option('start_upgrade_process') 
			&& !$this->get_option('run_vulns_checker') 
			&& $this->request_data['type'] !== 'AUTO_UPDATE_CHECK') {
	...
	public function sanitize_get_variables_wptc() {
		...
		if( stripos($_GET['page'], 'wp-time-capsule') === false ){
		...
		foreach ($_GET as $key => $value) {
			if(!empty($_GET[$key])){
				$filter_input_val = filter_input( INPUT_GET, $key );

				if(empty($filter_input_val)){
					$filter_input_val = '';
				}

				$_GET[$key] = sanitize_text_field($filter_input_val);
			}
		}

		wptc_log($_GET, '--new--get----');
	}
	...
	public function parse_request(){
		...
		$this->sanitize_get_variables_wptc();
	...
	public function process(){
		...
		$this->force_backblaze_if_needed();
	...
	private function force_backblaze_if_needed()
	{
		if( !empty($this->bulk_setup_needed_post_data) 
			&& !empty($this->bulk_setup_needed_post_data['type']) 
			&& $this->bulk_setup_needed_post_data['type'] == 'INITIAL_SETUP' 
			&& !empty($this->bulk_setup_needed_post_data['cloud_creds']) 
			&& !empty($this->bulk_setup_needed_post_data['cloud_creds']['default_repo']) 
			&& $this->bulk_setup_needed_post_data['cloud_creds']['default_repo'] == 'backblaze' ){

			wptc_log('', "-----defining---FORCE_BLACKBLAZE--------");

			if(!defined('FORCE_BACKBLAZE')){
				define('FORCE_BACKBLAZE', true);
			}
		}
	}
		...
		if ( !empty($HTTP_RAW_POST_DATA_LOCAL) 
		    ...
		    if ($data){
		        ...
		        if(isset($request_data['params'])){ 
		            $request_data['params'] = wptc_iwp_mmb_filter_params($request_data['params']);
		        }
		        $signature  = base64_decode($request_data['signature']);
		        $iwp_action     = $request_data['iwp_action'];
		        $id             = $request_data['id'];

		        // wptc_log($request_data, "--------iwp request data--------");

		        if ($iwp_action == 'do_upgrade') {
		            $auth = wptc_authenticate_message($iwp_action . $id, $signature, $id);  

		            wptc_log($auth, "----iwp--plugin--auth--------");

		            if($auth){
		            	wptc_log($HTTP_RAW_POST_DATA_LOCAL, "--------IWP_JSON_PREFIX--coming------");
			
						wp_cookie_constants();
						wptc_login_as_admin();

						wptc_log('', "--------below iwp plugin request--------");
		            }
		        }
		    }
		}
	...
	private function perform_request_wptc($request_type, $offset = 0, $limit = 100){
		...
		check_and_send_no_backup_email();
