...
class WPTC_Dropbox_API {
	...
	public function refreshToken() {
		$dropbox_access_token = WPTC_Factory::get('config')->get_option('dropbox_access_token', true);
		$dropbox_access_token = json_decode($dropbox_access_token, true);

		wptc_log($dropbox_access_token, '--------refreshing dropbox access token--------');
		// wptc_log($dropbox_access_token, '--------$dropbox_access_token refreshToken dropbox api--------');

		$refresh_token = $dropbox_access_token['refresh_token'];
		...
		// wptc_log($response, '--------$response refreshToken--dropbox api------');

		if($response && $response['body'] && $response['body']->access_token){
			$new_access_token = $response['body']->access_token;
	
			wptc_log($new_access_token, '--------$new_access_token new_access_token--dropbox api------');
	
			$dropbox_access_token['access_token'] = $new_access_token;

			$altered_dropbox_access_token = json_encode($dropbox_access_token, true);
			WPTC_Factory::get('config')->set_option('dropbox_access_token', $altered_dropbox_access_token);
