...
function ask_backup_name_wptc(type){
	...
	if (type === 'bbu') {
		var data = {
			bbu_note_view:{
				type: 'message',note:'We will notify you once the backup and updates are completed. <br> <a href="https://docs.wptimecapsule.com/article/update-plugins-without-backup" target="blank">  Click here</a> if you want to update immediately without backup.',
			},
		};
...
function styling_thickbox_wptc(styleType) {
	...
	if (styleType == 'progress') {
		...
		jQuery('.everest_tab_active').removeClass('everest_tab_active');
