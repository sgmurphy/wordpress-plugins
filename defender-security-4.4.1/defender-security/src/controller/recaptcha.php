...
class Recaptcha extends Event {
	...
	public function add_scripts(): void {
		...
		$error_text = __( 'More than one reCAPTCHA has been found in the current form. Please remove all unnecessary reCAPTCHA fields to make it work properly.', 'defender-security' );
		$options = [
			'hl' => $this->language,
			'size' => $this->recaptcha_size,
			'version' => $this->recaptcha_type,
			'sitekey' => $this->public_key,
			'error' => sprintf( '<strong>%s</strong>:&nbsp;%s', __( 'Warning', 'defender-security' ), $error_text ),
	...
	public function validate_captcha_field_on_woo_login( WP_Error $errors ): WP_Error {
		...
		if ( ! $this->recaptcha_response( 'woo_login' ) ) {
			...
			$message = str_replace( __( '<strong>Error:</strong> ', 'defender-security' ), '', $this->service->error_message() );
	...
	public function save_settings( Request $request ): Response {
		...
		if ( $this->model->validate() ) {
			...
			return new Response(
				...
				array_merge(
					[
						'message' => __( 'Google reCAPTCHA settings saved successfully.', 'defender-security' ),
						'auto_close' => true,
					],
					$this->data_frontend()
				)
	...
	public function load_recaptcha_preview( Request $request ): Response {
		...
		$notice .= '<p>' . esc_html__( 'Save your API keys to load the reCAPTCHA preview.', 'defender-security' ) . '</p>';
		$notice .= '</div>';
		$notice .= '</div>';
		$notice .= '</div>';
		...
		*/
		if ( $is_active ) {
			if ( $this->service->enable_any_location( $this->is_woo_activated, $this->is_buddypress_activated ) ) {
				...
				$notice_text = sprintf(
				/* translators: %s: Type. */
					__( 'Google reCAPTCHA is currently active. %s type has been set successfully.', 'defender-security' ),
					$type
				);
			} elseif ( ! $this->is_woo_activated && ! $this->is_buddypress_activated && ! $model->enable_default_location() ) {
				$notice_type = 'warning';
				$notice_text = __( 'Google reCAPTCHA is currently inactive for all forms. You can deploy reCAPTCHA for specific forms in the <b>reCAPTCHA Locations</b> below.', 'defender-security' );
			} elseif (
				...
				$notice_type = 'warning';
				$notice_text = __( 'Google reCAPTCHA is currently inactive for all forms. You can deploy reCAPTCHA for specific forms in the <b>reCAPTCHA Locations</b>, <b>WooCommerce</b> or <b>BuddyPress</b> settings below.', 'defender-security' );
			}
		} else {
			// Inactive case.
			$notice_type = 'warning';
			$notice_text = __( 'Google reCAPTCHA is currently inactive. Enter your Site and Secret keys and save your settings to finish setup.', 'defender-security' );
		}
		 ...
		 */
		$ticket_text = __( 'If you see any errors in the preview, make sure the keys you’ve entered are valid, and you\'ve listed your domain name while generating the keys.', 'defender-security' );
	...
	public function export_strings(): array {
		return [ $this->model->is_active() ? __( 'Active', 'defender-security' ) : __( 'Inactive', 'defender-security' ) ];
	}
