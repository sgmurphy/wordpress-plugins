	...
	*/
	...
	public function handle_reset_check_password( \WP_Error $errors, $user ) {
		...
		if (
			...
			$message = wp_kses(
				__( 'This password has been used already. Please choose a different one.', 'defender-security' ),
				[ 'strong' => [] ]
			);
			$errors->add( 'defender_password_reset', $message );
	...
	public function toggle_reset( Request $request ) {
		...
		if ( isset( $data['expire_force'] ) && true === $data['expire_force'] ) {
			...
			$response = [
				'message' => __( 'Selected user roles are required to reset their password upon next login.', 'defender-security' ),
			];
		} else {
			$response['message'] = __( 'Passwords reset is no longer required.', 'defender-security' );
		}
		$this->model->import( $data );
		if ( $this->model->validate() ) {
			$this->model->save();
