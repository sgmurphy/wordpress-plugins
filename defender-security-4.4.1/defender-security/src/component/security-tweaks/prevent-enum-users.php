...
class Prevent_Enum_Users extends Component implements Security_Key_Const_Interface {
	...
	public function maybe_block( string $request ): string {
		if ( ! current_user_can( 'edit_others_posts' ) ) {
			...
			if (
				...
				$message = __( 'Sorry, you are not allowed to access this page', 'defender-security' );
				$title = __( 'Forbidden', 'defender-security' );
				$status_code = 403;
	...
	public function to_array(): array {
		return [
			'slug' => $this->slug,
			'title' => __( 'Prevent user enumeration', 'defender-security' ),
			'errorReason' => __( 'User enumeration is currently allowed.', 'defender-security' ),
			'successReason' => __( 'User enumeration is currently blocked, nice work!', 'defender-security' ),
			'misc' => [],
			...
			'bulk_title' => __( 'Prevent user enumeration', 'defender-security' )
		];
	}
