...
if (!class_exists('WPRWP2FA')) :

require_once dirname(__FILE__) . '/authenticator.php';
require_once dirname(__FILE__) . '/utils.php';

class WPRWP2FA {
	const FLAG_META_KEY = 'wpr_2fa_enabled';
	const SECRET_META_KEY = 'wpr_2fa_secret';

	public static $cipher_algo = 'aes-256-cbc';
	public static $wp_2fa_option = 'wprWp2faConf';
	private $settings;
	private $config;
	...
	public function authenticate($user, $username, $password) {
		...
		$has_2fa = get_user_meta($user->ID, WPRWP2FA::FLAG_META_KEY, true);
		...
		if ('1' === $has_2fa) {
			if (empty($_POST['twofa_code'])) {
				...
				$encoded_secret_info = get_user_meta($user->ID, WPRWP2FA::SECRET_META_KEY, true);
				...
				if (defined('SECURE_AUTH_KEY') && $is_secret_encrypted === true) {
					$decryption_result = WPRHelper::opensslDecrypt($secret, self::$cipher_algo, SECURE_AUTH_KEY);
				...
				if (is_string($submitted_code) && ctype_digit($submitted_code) &&
						true === WPRWP2FAAuthenticator::verifyCode($secret, $submitted_code)) {
