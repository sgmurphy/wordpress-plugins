...
class WPRProtectIpstore_V556 {
	...
	function __construct($storage_type = WPRProtectIpstore_V556::STORAGE_TYPE_DB) {
		$this->storage_type = $storage_type;
		if ($this->storage_type == WPRProtectIpstore_V556::STORAGE_TYPE_FS) {
			$this->storage = new WPRProtectIpstoreFS_V556();
		} else {
			$this->storage = new WPRProtectIpstoreDB_V556();
		}
	}

	public static function uninstall() {
		WPRProtectIpstoreDB_V556::uninstall();
	}

	public function isLPIPBlacklisted($ip) {
		if ($this->storage_type == WPRProtectIpstore_V556::STORAGE_TYPE_DB) {
