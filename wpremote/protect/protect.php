...
if (!class_exists('WPRProtect_V573')) :
...
class WPRProtect_V573 {
	...
	public static function init($mode) {
		...
		if ($mode == WPRProtect_V573::MODE_PREPEND) {
			$config_file = MCDATAPATH .  MCCONFKEY . '-' . 'mc.conf';
			$config = WPRProtectUtils_V573::parseFile($config_file);

			if (empty($config['time']) || !($config['time'] > time() - (48*3600)) ||
					!isset($config['mc_conf_version']) ||
					(WPRProtect_V573::CONF_VERSION !== $config['mc_conf_version'])) {
				return false;
			...
			$request = new WPRProtectRequest_V573($request_ip_header, $req_config);
			$fw_config = array_key_exists('fw', $config) ? $config['fw'] : array();

			WPRProtectFW_V573::getInstance($mode, $request, $fw_config, $brand_name)->init();
		} else {
			...
			WPRProtectFW_V573::getInstance($mode, $request, $fw_config, $brand_name)->init();
			WPRProtectLP_V573::getInstance($request, $lp_config, $brand_name)->init();
		}
	}
	...
	public static function uninstall() {
		...
		WPRProtectIpstore_V573::uninstall();
		WPRProtectFW_V573::uninstall();
		WPRProtectLP_V573::uninstall();

		WPRProtect_V573::removeWPPrepend();
		WPRProtect_V573::removePHPPrepend();
		WPRProtect_V573::removeMCData();
	...
	private static function removeWPPrepend() {
		...
		WPRProtectUtils_V573::fileRemovePattern($fname, $pattern);
	}

	private static function removePHPPrepend() {
		WPRProtect_V573::removeHtaccessPrepend();
		WPRProtect_V573::removeUseriniPrepend();
	}

	private static function removeHtaccessPrepend() {
		$pattern = "/# MalCare WAF(.|\n)*# END MalCare WAF/i";

		WPRProtectUtils_V573::fileRemovePattern(rtrim(ABSPATH, DIRECTORY_SEPARATOR) . "/.htaccess", $pattern, true);
	}

	private static function removeUseriniPrepend() {
		$pattern = "/; MalCare WAF(.|\n)*; END MalCare WAF/i";

		WPRProtectUtils_V573::fileRemovePattern(rtrim(ABSPATH, DIRECTORY_SEPARATOR) . "/.user.ini", $pattern, true);
	}
	...
	private static function removeMCData() {
		...
		WPRProtectUtils_V573::rrmdir($mc_data_dir);
	}
}
endif;