...
if (!class_exists("nxs_snapClassLI")) { class nxs_snapClassLI extends nxs_snapClassNT { 
  ...
  public function doAuth() { $ntInfo = $this->ntInfo; global $nxs_snapSetPgURL;     
    ...
    // V2 Auth
    if ( isset($_GET['code']) && $_GET['code']!='' && isset($_GET['state']) && substr($_GET['state'], 0, 7) == 'nxs-li-'){ $this->showAuthTop(); $at = sanitize_text_field($_GET['code']);  $ii = str_replace('nxs-li-','',$_GET['state']);
