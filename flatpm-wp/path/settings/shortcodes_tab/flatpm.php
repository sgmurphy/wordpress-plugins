		...
		<table>
			<tbody>
				...
				<tr>
					<td>{{increment}}</td>
					<td>
						<?php _e( 'Will output a number starting from 1, each call to this shortcode increases the value by one.', 'flatpm_l10n' ); ?><br><br>
						<code>
						{{increment<b style="color:#d87a87">_1</b>}} - <?php _e( 'To output several separate increments at once', 'flatpm_l10n' ); ?>
						</code>
					</td>
				</tr>
				<tr>
					<td>{{fpm-close-event}}</td>
					<td><?php _e( 'Displays the css class for closing popups and sticky side blocks. For example:', 'flatpm_l10n' ); ?> <code>&lt;button class=&quot;{{fpm-close-event}}&quot;&gt;&lt;/button&gt;</code></td>
				</tr>
