...
if( ! function_exists( 'flat_pm_init' ) ){
	function flat_pm_init(){
		...
		$flat_pm_personalization['disabled_helpers'] = isset( $flat_pm_personalization['disabled_helpers'] ) ? $flat_pm_personalization['disabled_helpers'] : 'false';
		?>
		<script>
		var $ = jQuery,
			...
			disabled_helpers_flat_pm = <?php echo esc_html( $flat_pm_personalization['disabled_helpers'] ); ?>;
		</script>
		<?php
	}
}
...
if( ! function_exists( 'flat_pm_ip2long_v6' ) ){
	function flat_pm_ip2long_v6( $ip ){
		$ip_n = inet_pton( $ip );
		$bin = '';
...
if( ! function_exists( 'flat_pm_doing_ajax' ) ){
	function flat_pm_doing_ajax() {
		if( isset( $_GET['fpm-ajax'] ) ){
			return true;
		}
...
if( ! function_exists( 'flat_pm_get_pro_text' ) ){
	function flat_pm_get_pro_text() {
		if( ! flat_do_some() ){
			...
			return __( '<p style="margin:0 0 0 3px">You have a <span class="fpm_pro_tag">PRO version</span> You are awesome!</p>', 'flatpm_l10n' );
		}
	}
}
...
add_shortcode( 'flat_pm', function( $args ){
	global $fpm_shortcode_blob, $fpm_the_content_in_process;

	$fpm_shortcode_blob = ( ! empty( $fpm_shortcode_blob ) ) ? $fpm_shortcode_blob : array();

	$fpm_shortcode_blob[ $args['id'] ] = true;

	$is_turned = get_post_meta( (int) $args['id'], 'turned', true ) === 'true';

	if( ! $is_turned ){
		return '';
	}

	$output = '<span style="display:none" class="fpm-short-' . esc_attr( $args['id'] ) . '"></span>';

	if( flat_pm_doing_ajax() && ! $fpm_the_content_in_process ){
		$output .= flat_pm_filter_content( $content = '', $mode = 'ajax', array( (int) $args['id'] ) );
	}
