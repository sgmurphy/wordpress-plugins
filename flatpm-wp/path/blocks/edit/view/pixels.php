...
<div class="collapsible-body">
	<div class="switch">
		<label>
			Off
			<input class="hidden" type="checkbox" name="view[pixels][enabled]" <?php if( $pixels['enabled'] === 'true') echo 'checked'; ?>>
	...
	<div class="row">
		...
		<div class="col s12">
			<div style="display:flex;column-gap:40px;flex-wrap:wrap">
				<div style="display:flex;gap:15px;flex-wrap:wrap;flex-direction:column">
					<label>
						<input class="with-gap" name="view[pixels][type]" type="radio" value="percent_once" <?php checked( $pixels['type'], 'percent_once' ); ?>>
						<span><?php _e( 'Through percentage (%) of screens', 'flatpm_l10n' ); ?></span>
					</label>
					<label>
						<input class="with-gap" name="view[pixels][type]" type="radio" value="percent_iterable" <?php checked( $pixels['type'], 'percent_iterable' ); ?>>
						<span><?php _e( 'Each percentage (%) of screens', 'flatpm_l10n' ); ?></span>
					</label>
					<label>
						<input class="with-gap" name="view[pixels][type]" type="radio" value="center" <?php checked( $pixels['type'], 'center' ); ?>>
				...
				<div style="display:flex;gap:15px;flex-wrap:wrap;flex-direction:column">
					<label>
						<input class="with-gap" name="view[pixels][type]" type="radio" value="px_once" <?php checked( $pixels['type'], 'px_once' ); ?>>
						<span><?php _e( 'Through N pixels', 'flatpm_l10n' ); ?></span>
					</label>
					<label>
						<input class="with-gap" name="view[pixels][type]" type="radio" value="px_iterable" <?php checked( $pixels['type'], 'px_iterable' ); ?>>
		...
		<div class="input-field col s12 l4 xl3">
			<input id="view_pixels_n" type="number" class="validate fpm_disabled" min="0"
				step=".1"
				name="view[pixels][n]"
				value="<?php echo esc_attr( $pixels['n'] ); ?>"
			>
			<label for="view_pixels_n">
				<i class="material-icons tooltipped"
					...
					<?php esc_attr_e( 'The spacing is defined in number of pixels (percentage or fixed spacing N).', 'flatpm_l10n' ); ?>
					<br>
					<br>
					<?php esc_attr_e( 'The percentage will be the screen height of the user\'s device and is measured in vh units.', 'flatpm_l10n' ); ?>
					</span>
					"
				>info_outline</i>
		...
		<div class="input-field col s12 l4 xl3">
			<input id="view_pixels_start_from" type="number" class="validate fpm_disabled" min="0"
				...
				value="<?php echo esc_attr( $pixels['start'] ); ?>"
			>
			<label for="view_pixels_start_from">
				<i class="material-icons tooltipped"
					data-position="bottom"
					data-tooltip="<?php esc_html_e( 'From which iteration to start', 'flatpm_l10n' ); ?>"
				>info_outline</i>
		...
		<div class="input-field col s12 l4 xl3">
			<input id="view_pixels_max" type="number" class="validate fpm_disabled" min="0"
				...
				value="<?php echo esc_attr( $pixels['max'] ); ?>"
			>
			<label for="view_pixels_max">
				<i class="material-icons tooltipped"
					data-position="bottom"
					data-tooltip="<?php esc_html_e( 'How many iterations there will be', 'flatpm_l10n' ); ?>"
				>info_outline</i>
		...
		<div class="input-field col s12 l4 xl3">
			<input id="view_pixels_m" type="number" class="validate fpm_disabled" min="0"
				step="1"
				name="view[pixels][m]"
				value="<?php echo esc_attr( $pixels['m'] ); ?>"
			>
			<label for="view_pixels_m">
				<i class="material-icons tooltipped"
					data-position="bottom"
					data-tooltip="<?php esc_html_e( 'If there are fewer characters between blocks than M, then the block will not be displayed.', 'flatpm_l10n' ); ?>"
				>info_outline</i>
		...
		<div class="input-field col s12 l8 xl6">
			<input id="view_pixels_exclude" type="text" class="fpm_disabled"
				...
				value="<?php echo esc_attr( $pixels['exclude'] ); ?>"
			>
			<label for="view_pixels_exclude">
				<i class="material-icons tooltipped"
					data-position="bottom"
					data-tooltip="<?php esc_html_e( 'Selectors to be ignored on inserts. This is useful if you don\'t want ads to appear in blockquotes, lists or tables, default is:', 'flatpm_l10n' ); ?> <code>table *, blockquote *, ul *, ol *, a *, p *</code>"
				>info_outline</i>
		...
		<div class="input-field col s12 l8 xl6 hidden">
			<input id="view_pixels_xpath" type="text"
				name="view[pixels][xpath]"
				value="<?php if( isset( $pixels['xpath'] ) ) echo esc_attr( $pixels['xpath'] ); ?>"
			>
