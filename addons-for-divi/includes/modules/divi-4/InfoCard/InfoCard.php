<?php
class DTQ_Card extends Divi_Torque_Lite_Module
{
	...
	public function init()
	{
		...
		$this->slug       = 'ba_card';
		$this->vb_support = 'on';
		$this->folder_name = 'divi_torque_lite';
	...
	public function render_figure()
	{
		...
		$process_font_icon = esc_attr(et_pb_process_font_icon($overlay_icon));
		dtq_inject_fa_icons($overlay_icon);

		// Swapped Image Schema.
		$swapped_img = $this->get_swapped_img_schema('photo');

		// Lightbox Class.
		$lightbox = 'on' === $use_lightbox ? 'dtq-lightbox' : '';

		// If Icon is not used and photo is set.
		if ('on' !== $use_icon && !empty($photo)) {
			return sprintf(
				'<div class="dtq-figure dtq-card-figure">
                    %1$s
                    <div class="dtq-overlay">
						<i class="dtq-overlay-icon">%4$s</i>
					</div>
