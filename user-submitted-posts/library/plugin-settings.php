...
function usp_validate_options($input) {
	...
	$allowedposttags = apply_filters('usp_allowed_tags', $allowedposttags, $allowed_atts);
	...
	if (!isset($input['custom_checkbox_req'])) $input['custom_checkbox_req'] = null;
	$input['custom_checkbox_req'] = ($input['custom_checkbox_req'] == 1 ? 1 : 0);
	...
	if (usp_get_current_screen_id() === 'settings_page_user-submitted-posts') {
		...
		if (!usp_check_date_expired() && !usp_dismiss_notice_check()) {
			...
			<div class="notice notice-success">
				<p>
					<strong><?php esc_html_e('Go Pro!', 'usp'); ?></strong> 
					<?php esc_html_e('Save 30% on our', 'usp'); ?> 
					...
					<?php esc_html_e('Apply code', 'usp'); ?> <code>PLANET24</code> <?php esc_html_e('at checkout. Sale ends 5/25/24.', 'usp'); ?> 
...
function usp_check_date_expired() {
	...
	$expires = apply_filters('usp_check_date_expired', '2024-05-25');
