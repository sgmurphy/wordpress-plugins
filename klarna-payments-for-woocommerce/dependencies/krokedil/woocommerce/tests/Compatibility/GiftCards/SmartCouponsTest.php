...
namespace KrokedilKlarnaPaymentsDeps\Krokedil\WooCommerce\Tests\Compatibility\GiftCards;

use KrokedilKlarnaPaymentsDeps\Krokedil\WooCommerce\Compatibility\Giftcards\SmartCoupons;
...
class SmartCouponsTest extends TestCase
{
    ...
    public function setUp() : void
    {
        ...
        $this->mock_package = $this->getMockBuilder('KrokedilKlarnaPaymentsDeps\\Krokedil\\WooCommerce\\KrokedilWooCommerce')->disableOriginalConstructor()->getMock();
        $this->mock_package->method('config')->willReturn(array('slug' => 'test-slug', 'price_format' => 'minor'));
        \WP_Mock::userFunction('wp_parse_args', array('return' => array('slug' => 'test-slug', 'price_format' => 'minor')));
        \WP_Mock::userFunction('wc_format_decimal', array('return' => function ($price) {
            return $price;
        }));
    }
    /**
    ...
    {
        $mock_wc = $this->getMockBuilder('stdClass')->setMethods(array('cart'))->getMock();
        return $mock_wc;
    }
    /**
    ...
    {
        $mock_cart = $this->getMockBuilder('WC_Cart')->setMethods(array('get_coupons', 'get_coupon_discount_amount'))->getMock();
        ...
        $mock_cart->expects($this->once())->method('get_coupons')->willReturn(array('coupon1' => $mock_coupon_1, 'coupon2' => $mock_coupon_2));
        $mock_cart->expects($this->exactly(2))->method('get_coupon_discount_amount')->withConsecutive(['coupon1'], ['coupon2'])->willReturnOnConsecutiveCalls(100, 200);
        return $mock_cart;
    }
    /**
    ...
    {
        $mock_coupon = $this->getMockBuilder('WC_Coupon')->setMethods(array('get_discount_type'))->getMock();
        $mock_coupon->expects($this->atLeastOnce())->method('get_discount_type')->willReturn($discount_type);
        return $mock_coupon;
    }
    /**
    ...
    {
        // Arrange.
        $mock_order = $this->getMockBuilder('WC_Order')->setMethods(array('get_items'))->getMock();
        ...
        $mock_order->expects($this->once())->method('get_items')->with('coupon')->willReturn(array($mock_coupon_1, $mock_coupon_2));
        return $mock_order;
    }
    /**
    ...
    {
        $mock_coupon = $this->getMockBuilder('WC_Order_Item_Coupon')->setMethods(array('meta_exists', 'get_meta', 'coupon_data', 'get_code', 'get_discount'))->getMock();
        $mock_coupon->expects($this->once())->method('meta_exists')->with('coupon_data')->willReturn(\true);
        $mock_coupon->expects($this->once())->method('get_meta')->with('coupon_data')->willReturn(array('discount_type' => $discount_type));
        $mock_coupon->expects($this->once())->method('get_code')->willReturn($code);
        $mock_coupon->expects($this->once())->method('get_discount')->willReturn($discount);
        return $mock_coupon;
    }
    /**
    ...
    public function test_get_cart_giftcards_can_get_giftcards()
    {
        ...
        \WP_Mock::userFunction('WC', array('return' => $mock_wc));
        // Act
